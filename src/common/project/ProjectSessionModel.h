...
namespace xjw::common::project
{
    ...
    public:
        ...
        bool runIfCurrent(quint64 generation, const std::function<void()>& commit);
...
public:
    bool isValid() const;
    bool commitUpdated(QString* errorMsg = nullptr, bool* archiveCommitted = nullptr) const;
    bool commitOriginal(QString* errorMsg = nullptr, bool* archiveCommitted = nullptr) const;
...
public:
    using ProjectOpenPreflight = std::function<bool(const QString&, QString*)>;
    ...
    {
        return _activeChunkId;
    }
    QString activeChunkName() const
    {
        return _activeChunkName;
    }
    int activeChunkDirectory() const
    {
        return _activeChunkDirectory;
    }
    QJsonArray chunks() const;
    bool hasProject() const
    {
        return !_projectPath.isEmpty();
    }
    bool isDirty() const
    {
        return _isDirty;
    ...
    // === Chunk 管理 ===
    bool createChunk(const QString& name, QString* createdChunkId = nullptr, QString* errorMsg = nullptr);
    bool renameChunk(const QString& chunkId, const QString& name, QString* errorMsg = nullptr);
    bool removeChunk(const QString& chunkId, QString* errorMsg = nullptr);
    bool switchChunk(const QString& chunkId, QString* errorMsg = nullptr);
    ...
    QJsonObject coreFilesMeta() const
    {
        return _filesManager.coreData();
    }
    // 更新运行时元数据
    ...
    QJsonObject projectUiState() const
    {
        return _projectUiState;
    }
    void updateProjectUiState(const QJsonObject& state, bool markDirty = true);
    ...
    ProjectResourceCleanupPersistence prepareResourceCleanupPersistence(const QJsonObject& updatedMetadata);
    bool finalizeResourceCleanupPersistence(const ProjectResourceCleanupPersistence& persistence,
    ...
    bool upsertResultRecordByIndex(const QString& arrayKey,
                                   ...
                                   int replaceIndex,
                                   bool markDirty = true);
    bool replaceResultRecordWithLatest(const QString& arrayKey, const QJsonObject& record, bool markDirty = true);
    // 打包外部资源到项目内部
...
signals:
    ...
    void portableProjectExportCompleted(bool success, const QString& outputZipPath, const QString& errorMessage);
    void projectClosed();
    void chunkListChanged(const QJsonArray& chunks, const QString& activeChunkId);
    void activeChunkChanged(const QString& chunkId, const QString& chunkName, int chunkDirectory);
