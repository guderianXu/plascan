...
namespace xjw::common::project
{
    namespace
    {
        ...
        QString canonicalPath(const QString& path)
        {
            ...
            if (trimmed.isEmpty() || trimmed.startsWith(QStringLiteral("plascan:///")))
            {
    ...
    bool ProjectSession::create(const QString& projectPath, const QString& projectName, QString* errorMessage)
    {
        close();
        if (errorMessage)
        {
        ...
        const QString absolutePath = QDir::cleanPath(QFileInfo(projectPath).absoluteFilePath());
        const QString dataDirectory = ProjectPackageLayout::dataDirectory(absolutePath);
        if (QFileInfo::exists(absolutePath) || QFileInfo::exists(dataDirectory))
        {
            setError(errorMessage, QStringLiteral("项目文件或同名 .files 目录已存在: %1").arg(absolutePath));
            return false;
        }
        if (!QDir().mkpath(dataDirectory))
        {
            setError(errorMessage, QStringLiteral("无法创建项目数据目录: %1").arg(dataDirectory));
            return false;
        }
        ...
        const QString chunkDirectory = ProjectPackageLayout::chunkDirectory(absolutePath, chunk.directory);
        if (!QDir().mkpath(chunkDirectory))
        {
            setError(errorMessage, QStringLiteral("无法创建默认 Chunk 目录: %1").arg(chunkDirectory));
            cleanup();
            return false;
        }
        QJsonObject config{
        ...
        if (!PlascanArchive::createArchive(
                ProjectPackageLayout::chunkArchivePath(absolutePath, chunk.directory),
                {qMakePair(QString::fromLatin1(PortableProjectFormat::DocumentEntry), compactJson(chunkDocument))},
                &error) ||
            !PlascanArchive::createArchive(
                ProjectPackageLayout::metadataArchivePath(absolutePath),
                {qMakePair(QString::fromLatin1(PortableProjectFormat::DocumentEntry), compactJson(projectDocument))},
                &error) ||
            !ProjectPackageLayout::writeDescriptor(absolutePath, &error))
        {
            cleanup();
            setError(errorMessage, QStringLiteral("创建 4.0 Chunk 工程失败: %1").arg(error));
            return false;
        }
    ...
    bool ProjectSession::open(const QString& projectPath, QString* errorMessage)
    {
        close();
        if (errorMessage)
        {
        ...
        if (!_lock->acquire(_projectPath, errorMessage) || !chunkStore.ensureLayout(errorMessage))
        {
            close();
            return false;
        }
    ...
    bool ProjectSession::openOrCreate(const QString& projectPath, const QString& projectName, QString* errorMessage)
    {
        const QString absolutePath = QDir::cleanPath(QFileInfo(projectPath).absoluteFilePath());
        if (QFileInfo::exists(absolutePath) || QFileInfo::exists(ProjectPackageLayout::dataDirectory(absolutePath)))
        {
    ...
    bool ProjectSession::selectChunk(const QString& chunkId, const QString& chunkName, QString* errorMessage)
    {
        if (!isOpen())
        {
        ...
        else if (!chunkName.trimmed().isEmpty())
        {
            for (const ProjectChunkRecord& candidate : index.chunks())
            {
                if (candidate.name == chunkName.trimmed())
                {
                    ...
                    {
                        setError(errorMessage, QStringLiteral("Chunk 名称不唯一: %1").arg(chunkName));
                        return false;
                    }
                    selected = candidate;
                }
        ...
        if (selected.id.isEmpty())
        {
            setError(errorMessage, QStringLiteral("找不到指定 Chunk: %1").arg(chunkId.isEmpty() ? chunkName : chunkId));
            return false;
        }
    ...
    QJsonObject ProjectSession::mergedMetadata() const
    {
        ...
        for (auto it = _projectResults.constBegin(); it != _projectResults.constEnd(); ++it)
        {
    ...
    bool ProjectSession::mergeImages(const QJsonArray& images, QString* errorMessage)
    {
        if (!isOpen())
        {
            setError(errorMessage, QStringLiteral("项目会话未打开"));
        ...
        for (const QJsonValue& value : images)
        {
            ...
            {
                setError(errorMessage, QStringLiteral("影像不存在: %1").arg(path));
                return false;
            }
            incoming[QStringLiteral("path")] = path;
            if (!incoming.contains(QStringLiteral("type")))
            {
                incoming[QStringLiteral("type")] = QStringLiteral("external");
            }
            ...
            QString imageId = incoming.value(QStringLiteral("image_uuid")).toString().trimmed();
            if (imageId.isEmpty() || usedIds.contains(imageId))
            {
                do
                {
    ...
    void ProjectSession::appendResult(const QString& arrayKey, const QJsonObject& record)
    {
    ...
    void ProjectSession::upsertResultByPath(const QString& arrayKey, const QString& pathKey, const QJsonObject& record)
    {
        const QString targetPath = canonicalPath(record.value(pathKey).toString());
        const QString targetName = QFileInfo(targetPath).fileName();
        QJsonArray records;
        for (const QJsonValue& value : _projectResults.value(arrayKey).toArray())
        {
            const QString existingPath = canonicalPath(value.toObject().value(pathKey).toString());
            if (existingPath == targetPath ||
                (!targetName.isEmpty() && QFileInfo(existingPath).fileName() == targetName))
            {
                continue;
            }
    ...
    bool ProjectSession::save(QString* errorMessage)
    {
        ...
        if (!chunkStore.writeChunkSections(
                ...
                {qMakePair(QString::fromLatin1(PortableProjectFormat::ProjectFilesSection), portableFiles),
                 qMakePair(QString::fromLatin1(PortableProjectFormat::ProjectResultsSection), portableResults),
                 qMakePair(QString::fromLatin1(PortableProjectFormat::ProjectConfigSection), _projectConfig),
        ...
        return ProjectPackageLayout::pruneEmptyOptionalDirectories(_projectPath, _activeChunk.directory, errorMessage);
    }
    ...
    bool ProjectSession::load(QString* errorMessage)
    {
        ...
        _projectId = projectDocument.value(QStringLiteral("project_id")).toString();
        ...
        if (!chunkStore.readChunkDocument(_activeChunk.directory, &chunkDocument, errorMessage))
        {
            return false;
        }
        _projectFiles = chunkDocument.value(QString::fromLatin1(PortableProjectFormat::ProjectFilesSection)).toObject();
        _projectResults =
