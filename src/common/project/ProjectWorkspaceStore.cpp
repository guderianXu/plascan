...
namespace
{
    ...
    QString fileSha256(const QString& path, QString* errorMessage)
    {
        ...
        {
            setError(errorMessage, QStringLiteral("无法读取项目资源 %1: %2").arg(path, file.errorString()));
            return {};
        }
        ...
        while (!file.atEnd())
        {
            ...
            if (count < 0)
            {
                setError(errorMessage, QStringLiteral("计算项目资源校验值失败: %1").arg(path));
                return {};
            }
            if (count > 0)
            {
    ...
    QString diagnosticPathToken(const QString& path)
    {
        ...
        const QString label = info.fileName().isEmpty() ? QStringLiteral("path") : info.fileName();
        return QStringLiteral("plascan-diagnostic:///%1/%2").arg(stableToken(info.absoluteFilePath()), label);
    }
    ...
    bool copyFileAtomically(const QString& source, const QString& destination, QString* errorMessage)
    {
        ...
        if (!QDir().mkpath(QFileInfo(destination).absolutePath()))
        {
            setError(errorMessage,
                     QStringLiteral("无法创建工程资源目录: %1").arg(QFileInfo(destination).absolutePath()));
            return false;
        }
        ...
        {
            setError(errorMessage, QStringLiteral("无法读取待封装资源 %1: %2").arg(source, input.errorString()));
            return false;
        }
        QSaveFile output(destination);
        if (!output.open(QIODevice::WriteOnly))
        {
            setError(errorMessage, QStringLiteral("无法写入工程资源 %1: %2").arg(destination, output.errorString()));
            return false;
        }
        ...
        while (!input.atEnd())
        {
            ...
            if (count < 0 || output.write(buffer.constData(), count) != count)
            {
                ...
                setError(errorMessage, QStringLiteral("复制工程资源失败: %1").arg(source));
                return false;
            }
        }
        if (!output.commit())
        {
            setError(errorMessage, QStringLiteral("提交工程资源失败: %1").arg(destination));
            return false;
        }
        return true;
    }
    ...
    bool copyTree(const QString& source, const QString& destination, QString* errorMessage)
    {
        ...
        {
            return copyFileAtomically(sourceInfo.absoluteFilePath(), destination, errorMessage);
        }
        if (!sourceInfo.isDir())
        {
            setError(errorMessage, QStringLiteral("待封装路径不存在: %1").arg(source));
            return false;
        }
        if (!QDir().mkpath(destination))
        {
            setError(errorMessage, QStringLiteral("无法创建工程资源目录: %1").arg(destination));
            return false;
        }

        QDirIterator iterator(
            sourceInfo.absoluteFilePath(), QDir::Files | QDir::NoDotAndDotDot, QDirIterator::Subdirectories);
        ...
        while (iterator.hasNext())
        {
            ...
            if (!copyFileAtomically(sourceFile, QDir(destination).filePath(relative), errorMessage))
            {
                return false;
            }
        }
    ...
    QString chunkRelativePath(const QString& entryPath)
    {
        ...
        {
            return entryPath.mid(static_cast<int>(qstrlen(ChunkEntryPrefix)));
        }
        return {};
    }
    ...
    {
        const QString relative = QDir::fromNativeSeparators(QDir(runtimeRoot).relativeFilePath(path));
        if (relative.startsWith(QStringLiteral("../")) || relative == QStringLiteral(".."))
        {
            return {};
        }
        return PortableProjectFormat::resourceUriForEntry(QStringLiteral("chunk/%1").arg(relative));
    }

    QString projectUri(const QString& runtimeRoot, const QString& dataRoot, const QString& path)
    {
        ...
        {
            const QString relative = QDir::fromNativeSeparators(QDir(dataRoot).relativeFilePath(path));
                              ...
                              QString* errorMessage)
    {
        ...
        if ((!info.isFile() && !info.isDir()) ||
            QDir::cleanPath(info.absoluteFilePath()) == QDir::cleanPath(QFileInfo(projectPath).absoluteFilePath()))
        {
            return {};
        }
                             ...
                             bool* success)
    {
        ...
        if (value.isArray())
        {
            ...
            {
                result.append(portableValue(item,
                                            runtimeRoot,
                                            dataRoot,
                                            projectPath,
                                            referencedEntries,
        ...
        if (value.isObject())
        {
            ...
            const bool externalImage = object.value(QStringLiteral("type")).toString() == QStringLiteral("external");
            for (auto it = object.constBegin(); it != object.constEnd(); ++it)
            {
                // 普通工程只保存影像源引用。可移植副本由 ProjectExporter
                // 显式复制并改写，不能让通用资源收集器隐式复制外部影像。
                if (externalImage && it.key() == QStringLiteral("path"))
                {
                    result.insert(it.key(), it.value());
                    continue;
                }
                result.insert(it.key(),
                              portableValue(it.value(),
                                            runtimeRoot,
                                            dataRoot,
                                            projectPath,
                                            referencedEntries,
        ...
        const QString existingEntry = PortableProjectFormat::entryPathFromResourceUri(text);
        if (!existingEntry.isEmpty())
        {
            if (isChunkEntry(existingEntry))
            {
                const QString normalizedEntry = QStringLiteral("chunk/%1").arg(chunkRelativePath(existingEntry));
                referencedEntries->insert(normalizedEntry);
                return PortableProjectFormat::resourceUriForEntry(normalizedEntry);
            }
        ...
        if (!QFileInfo(candidate).isAbsolute() &&
            (candidate.startsWith(QStringLiteral("assets/")) || candidate.startsWith(QStringLiteral("assets\\"))))
        {
        ...
        if (QFileInfo(candidate).isAbsolute())
        {
            const QString uri = projectUri(runtimeRoot, dataRoot, candidate);
            if (!uri.isEmpty())
            {
                const QString entry = PortableProjectFormat::entryPathFromResourceUri(uri);
        ...
        if (uri.isEmpty())
        {
            setError(errorMessage, QStringLiteral("无法生成工程资源 URI: %1").arg(staged));
            *success = false;
            return value;
        }
        referencedEntries->insert(PortableProjectFormat::entryPathFromResourceUri(uri));
        return uri;
    }
                                 ...
                                 bool* success)
    {
        ...
        if (value.isArray())
        {
            ...
            for (const QJsonValue& item : value.toArray())
            {
                result.append(materializedValue(item, runtimeRoot, dataRoot, errorMessage, success));
            }
            return result;
        }
        if (value.isObject())
        {
            ...
            for (auto it = object.constBegin(); it != object.constEnd(); ++it)
            {
                result.insert(it.key(), materializedValue(it.value(), runtimeRoot, dataRoot, errorMessage, success));
            }
            return result;
        }
        if (!value.isString())
        ...
        const QString entry = PortableProjectFormat::entryPathFromResourceUri(value.toString());
        QString path;
...
ProjectWorkspaceStore::ProjectWorkspaceStore(const QString& projectPath, int chunkDirectory)
    : _projectPath(QDir::cleanPath(QFileInfo(projectPath).absoluteFilePath())), _chunkDirectory(chunkDirectory)
{
}
...
QString ProjectWorkspaceStore::runtimeRoot(QString* errorMessage) const
{
    ...
    if (!found)
    {
        setError(errorMessage, QStringLiteral("项目中不存在 Chunk 数字目录: %1").arg(_chunkDirectory));
        return {};
    }
    return ProjectPackageLayout::chunkDirectory(_projectPath, _chunkDirectory);
}

bool ProjectWorkspaceStore::loadResourceIndex(ProjectResourceIndex* index, QString* errorMessage) const
{
    if (!index)
    {
    ...
    const bool loaded = _chunkDirectory > 0
                            ? chunkStore.readChunkDocument(_chunkDirectory, &chunkDocument, errorMessage)
                            : chunkStore.readDefaultChunkDocument(&chunkDocument, errorMessage);
    if (!loaded)
    {
        return false;
    }
    ...
    QString indexError;
    const ProjectResourceIndex parsed = ProjectResourceIndex::fromJson(
        chunkDocument.value(QString::fromLatin1(PortableProjectFormat::ResourceIndexSection)).toObject(), &indexError);
...
bool ProjectWorkspaceStore::initializeRuntime(QString* runtimeRootOut, QString* errorMessage) const
{
    if (errorMessage)
    {
        errorMessage->clear();
    ...
    if (root.isEmpty() || !QDir().mkpath(root))
    {
        if (errorMessage && errorMessage->isEmpty())
        {
            *errorMessage = QStringLiteral("无法创建项目运行工作区: %1").arg(root);
        }
        return false;
    }
    ...
    const QString dataRoot = ProjectPackageLayout::dataDirectory(_projectPath);
    for (const ProjectResourceRef& resource : index.resources())
    {
        if (resource.kind != QStringLiteral("chunk") && resource.kind != QStringLiteral("workspace") &&
            resource.kind != QStringLiteral("shared_image"))
        {
            continue;
        }
        const bool shared = resource.entryPath.startsWith(QStringLiteral("shared/"));
        if (!shared && !isChunkEntry(resource.entryPath))
        {
            setError(errorMessage, QStringLiteral("工程资源路径无效: %1").arg(resource.entryPath));
            return false;
        }
        const QString destination =
            PortableProjectFormat::resolveEntryPath(shared ? dataRoot : root,
                                                    shared ? resource.entryPath : chunkRelativePath(resource.entryPath),
                                                    errorMessage);
        if (destination.isEmpty())
        {
            return false;
        }
        bool validExisting = QFileInfo(destination).isFile() && QFileInfo(destination).size() == resource.size;
        const qint64 modifiedMs = QFileInfo(destination).lastModified().toMSecsSinceEpoch();
        const bool unchanged = resource.modifiedMs > 0 && resource.modifiedMs == modifiedMs;
        if (validExisting && !unchanged && !resource.sha256.isEmpty())
        {
            QString hashError;
            validExisting = fileSha256(destination, &hashError) == resource.sha256;
        }
        if (!validExisting)
        {
            setError(errorMessage, QStringLiteral("工程资源缺失或校验失败: %1").arg(resource.entryPath));
            return false;
        }
    }
                                                ...
                                                QString* errorMessage) const
{
    ...
    {
        setError(errorMessage, QStringLiteral("待打包资源不存在: %1").arg(sourcePath));
        return false;
    }
...
bool ProjectWorkspaceStore::prepareSplitMetadata(QJsonObject* core,
                                                 QJsonObject* results,
                                                 ...
                                                 QString* errorMessage) const
{
    ...
    {
        setError(errorMessage, QStringLiteral("项目核心元数据或资源索引输出参数为空"));
        return false;
    }
    ...
    if (!QDir().mkpath(root))
    {
        setError(errorMessage, QStringLiteral("无法创建当前 Chunk 目录: %1").arg(root));
        return false;
    }
    ...
    bool success = true;
    *core = portableValue(
                *core, root, dataRoot, _projectPath, &referencedEntries, &stagedExternalPaths, errorMessage, &success)
                .toObject();
    if (results)
    {
        *results = PortableProjectFormat::normalizeProjectResults(*results);
        *results = portableValue(*results,
                                 root,
                                 dataRoot,
                                 _projectPath,
                                 &referencedEntries,
    ...
    {
        const bool shared = entryPath.startsWith(QStringLiteral("shared/"));
        const QString relative = shared ? entryPath : chunkRelativePath(entryPath);
        const QString path = PortableProjectFormat::resolveEntryPath(shared ? dataRoot : root, relative, errorMessage);
        const QFileInfo info(path);
        ...
        QString checksum;
        if (!old.id.isEmpty() && old.size == info.size() && old.modifiedMs == modifiedMs && !old.sha256.isEmpty())
        {
            checksum = old.sha256;
        }
        else
        ...
        resource.id = QStringLiteral("%1-%2").arg(shared ? QStringLiteral("shared-image") : QStringLiteral("chunk"),
                                                  stableToken(entryPath));
        resource.kind = shared ? QStringLiteral("shared_image") : QStringLiteral("chunk");
...
bool ProjectWorkspaceStore::validateProjectLayout(QString* errorMessage) const
{
    return ensureProjectManifest(errorMessage);
}

bool ProjectWorkspaceStore::materializeMetadata(QJsonObject* metadata, QString* errorMessage) const
{
    if (!metadata)
    {
    ...
    bool success = true;
    const QString dataRoot = ProjectPackageLayout::dataDirectory(_projectPath);
    *metadata = materializedValue(*metadata, root, dataRoot, errorMessage, &success).toObject();
    return success;
}

bool ProjectWorkspaceStore::materializeMetadataForRecovery(QJsonObject* metadata, QString* errorMessage) const
{
    if (!metadata)
    {
        setError(errorMessage, QStringLiteral("待解析项目元数据为空"));
    ...
    bool success = true;
    *metadata =
        materializedValue(*metadata, root, ProjectPackageLayout::dataDirectory(_projectPath), errorMessage, &success)
            .toObject();
    return success;
}
