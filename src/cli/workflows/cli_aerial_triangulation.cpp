...
namespace
{
    ...
    QJsonArray pairDiagnosticsToJson(const std::vector<xjw::matchphotos::MatchPhotosMatchRecord>& records)
    {
        QJsonArray array;
        for (const xjw::matchphotos::MatchPhotosMatchRecord& record : records)
        {
            ...
            object[QStringLiteral("algorithm_version")] = static_cast<qint64>(record.algorithmVersion);
    ...
    QJsonArray imageMatchFilesToJson(const std::vector<xjw::matchphotos::MatchPhotosImageMatchRecord>& records)
    {
        QJsonArray array;
        for (const xjw::matchphotos::MatchPhotosImageMatchRecord& record : records)
        {
            ...
            object[QStringLiteral("neighbors")] = QJsonArray::fromStringList(record.neighborImagePaths);
            object[QStringLiteral("neighbor_variant_count")] = record.neighborVariantCount;
            object[QStringLiteral("settings")] = record.settings;
            array.append(object);
        }
        return array;
    ...
    QJsonObject tiePointResultToJson(const xjw::matchphotos::MatchPhotosResult& tiePoints)
    {
        ...
        object[QStringLiteral("rejected_track_conflict_components")] = tiePoints.rejectedTrackConflictComponents;
        object[QStringLiteral("candidate_pair_count")] = static_cast<int>(tiePoints.pairSelection.candidates.size());
        object[QStringLiteral("pair_diagnostics")] = pairDiagnosticsToJson(tiePoints.matches);
        object[QStringLiteral("image_match_files")] = imageMatchFilesToJson(tiePoints.imageMatchFiles);
        object[QStringLiteral("track_summary")] = tiePoints.trackSummary;
        return object;
    }

    QJsonObject
    reconstructionResultToJson(const xjw::aerial_triangulation::AerialTriangulationReconstructionResult& result)
    {
        QJsonObject object;
    ...
    QJsonObject pipelineInputToJson(const xjw::aerial_triangulation::PreparedAerialTriangulationInput& input)
    {
        QJsonObject object;
        ...
        object[QStringLiteral("max_tracks_per_image")] = input.maxTracksPerImage;
        object[QStringLiteral("max_tracks_per_grid_cell")] = input.maxTracksPerGridCell;
        object[QStringLiteral("track_thinning_grid_columns")] = input.trackThinningGridColumns;
        object[QStringLiteral("track_thinning_grid_rows")] = input.trackThinningGridRows;
        return object;
    }
    ...
    QJsonObject tiePointOptionsToJson(const xjw::matchphotos::MatchPhotosOptions& options)
    {
        ...
        object[QStringLiteral("guided_image_matching")] =
            xjw::matchphotos::guidedMatchingEnabled(options.guidedMatchingMode);
                              ...
                              double elapsedMs)
    {
        ...
        report[QStringLiteral("tie_point_match_count")] = static_cast<int>(result.tiePointResult.matches.size());
        ...
        report[QStringLiteral("reconstruction_result")] = reconstructionResultToJson(result.reconstructionResult);
...
int main(int argc, char* argv[])
{
    ...
    app.add_flag("--reference-preselection",
                 ...
                 "启用参考预选；source/estimated 需要相机文件，sequence 可无相机");
    ...
    app.add_flag(
        "--no-reuse-existing-matches", noReuseExistingMatches, "删除当前匹配和连接点缓存，重新提取、匹配并整理连接点");
    ...
    app.add_flag(
        "--no-auto-generate-missing-matches", noAutoGenerateMissingMatches, "只复用已有逐影像匹配分片和连接点");
    ...
    const QString requestedCameraExportDir =
        exportCameraDirArg.empty() ? QString()
    ...
    const QString requestedAssetsDir =
        assetsDirArg.empty() ? QString() : xjw::cli::cleanAbsolutePath(xjw::cli::fromStdString(assetsDirArg));
    ...
    if (hasInitialImage1 && (initialImageId1 == initialImageId2 || initialImageId1 >= static_cast<int>(items.size()) ||
                             initialImageId2 >= static_cast<int>(items.size())))
    {
        std::fprintf(stderr, "错误: 初始像对索引必须互不相同，且位于 [0, %d]。\n", static_cast<int>(items.size()) - 1);
        return cli::EXIT_ARG_ERR;
    }
    ...
    {
        std::fprintf(stderr, "工程打开/创建失败: %s\n", qUtf8Printable(errorMessage));
        return cli::EXIT_IO_ERR;
    }
    ...
    if (!projectSession.selectChunk(
            xjw::cli::fromStdString(chunkIdArg), xjw::cli::fromStdString(chunkNameArg), &errorMessage))
    {
        std::fprintf(stderr, "Chunk 选择失败: %s\n", qUtf8Printable(errorMessage));
        return cli::EXIT_IO_ERR;
    }
    const QString assetsDir = requestedAssetsDir.isEmpty()
                                  ? QDir(projectSession.activeChunkRoot()).filePath(QStringLiteral("assets"))
                                  : requestedAssetsDir;
    const QString reconstructionDir =
        QDir(projectSession.activeChunkRoot()).filePath(QStringLiteral("reconstruction/sparse"));
    ...
    options.matchDir = matchDirArg.empty() ? QDir(options.assetsDir).filePath(QStringLiteral("image_matches"))
    ...
    options.lomaRTensorRtPackagePath =
        lomaRPackageArg.empty() ? QString() : xjw::cli::cleanAbsolutePath(xjw::cli::fromStdString(lomaRPackageArg));
    ...
    const QString reportPath =
        QDir(projectSession.activeChunkRoot()).filePath(QStringLiteral("reports/aerial_triangulation_cli_report.json"));
    ...
    if (dryRunConfig)
    {
        ...
        QJsonObject report = makeDryRunReport(config, outputDir, options.images.size(), options.cameraPaths.size());
        report[QStringLiteral("camera_export_requested")] = !requestedCameraExportDir.isEmpty();
        ...
        {
            std::fprintf(stderr, "配置报告已生成，但 Chunk 写回失败: %s\n", qUtf8Printable(errorMessage));
            return cli::EXIT_IO_ERR;
        }
    ...
    if (!projectSession.mergeImages(xjw::cli::inputItemsToJson(items), &errorMessage) ||
        !projectSession.save(&errorMessage))
    {
        std::fprintf(stderr, "工程影像初始化失败: %s\n", qUtf8Printable(errorMessage));
        return cli::EXIT_IO_ERR;
    }
    options.projectMeta = projectSession.mergedMetadata();
    ...
    {
        QJsonObject files{{QStringLiteral("sparse_cloud_xyz"), reconstruction.sparseCloudPath}};
        const QJsonObject extraFiles = reconstruction.resultRecordExtra.value(QStringLiteral("files")).toObject();
    ...
    if (result.reconstructionResult.success && !requestedCameraExportDir.isEmpty() && !cameraExportPerformed)
    {
