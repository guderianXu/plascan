...
void writeJson(FILE *stream, const QJsonObject &object)
{
    const QByteArray json = QJsonDocument(object).toJson(QJsonDocument::Indented);
    std::fwrite(json.constData(), 1, static_cast<std::size_t>(json.size()), stream);
}
...
int main(int argc, char *argv[])
{
    ...
    app.add_option("--dense-cloud", dense_cloud, "深度图流程可复用的内部融合点云 PLY");
    ...
    request.progress = [](const QString &stage, int percent)
    {
        ...
        std::fwrite(message.constData(), 1, static_cast<std::size_t>(message.size()), stdout);
        std::fflush(stdout);
    };
