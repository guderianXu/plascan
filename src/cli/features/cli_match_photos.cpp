...
namespace
{
    ...
    QJsonObject algorithmPlanToJson(const xjw::matchphotos::MatchPhotosAlgorithmPlan& plan)
    {
        ...
        object[QStringLiteral("guided_matching_mode")] =
            xjw::matchphotos::guidedMatchingModeName(plan.guidedMatchingMode);
                           ...
                           double elapsedMs)
    {
        ...
        report[QStringLiteral("image_match_file_count")] = static_cast<int>(result.imageMatchFiles.size());
...
int main(int argc, char* argv[])
{
    ...
    app.add_option("--loma-r-keypoint-budget", lomaRKeypointBudget, "LoMa-R 静态特征档位: 0(自动), 1024, 2048, 3840");
    ...
    app.add_option("--sift-ratio", siftMaximumRatio, "SIFT 最近邻/次近邻距离比上限（0-1）");
    app.add_option("--sift-adaptive-ratio-min", siftMinimumAdaptiveRatio, "SIFT 自适应 ratio 收紧下限（0-1）");
    app.add_flag("--no-adaptive-sift-ratio", noAdaptiveSiftRatio, "禁用按当前像对分布自适应收紧 SIFT ratio");
    app.add_option("--geometry-min-inlier-ratio", geometryMinInlierRatio, "几何质量门最低内点率");
    app.add_option("--geometry-min-grid-coverage", geometryMinGridCoverage, "几何质量门最低空间网格覆盖率");
    ...
    app.add_option(
        "--cuda-parallel-pairs", cudaParallelPairs, "GPU 并行像对数；0 按 SIFT/LightGlue/LoMa-R 独立显存模型自动选择");
    app.add_option("--feature-prefetch-depth", featurePrefetchDepth, "特征影像预取深度和流水线预算，范围 1..4");
    app.add_flag("--no-reuse-matches", noReuseMatches, "不复用兼容的逐影像匹配分片，强制重新提取并匹配");
    app.add_flag("--plan-only", planOnly, "只解析参数和规划候选对，不提取/匹配/构建连接点");
    app.add_flag("--force", force, "允许输出目录非空");

    CLI11_PARSE(app, argc, argv);

    if (lomaRKeypointBudget != 0 && lomaRKeypointBudget != 1024 && lomaRKeypointBudget != 2048 &&
        lomaRKeypointBudget != 3840)
    {
        std::fprintf(stderr, "错误: --loma-r-keypoint-budget 只接受 0、1024、2048 或 3840\n");
    ...
    options.cudaParallelPairs = std::max(0, cudaParallelPairs);
    options.featurePrefetchDepth = std::clamp(featurePrefetchDepth, 1, 4);
    ...
    options.maxTiePointsPerGridCell =
        options.maxTiePointsPerImage > 0 ? std::max(1, options.maxTiePointsPerImage / options.tiePointGridColumns) : 0;
    ...
    options.guidedUseReferenceCameraPoses =
        options.guidedMatchingMode != xjw::matchphotos::GuidedMatchingMode::Disabled && referencePreselection;
    options.siftMaximumRatio = std::clamp(siftMaximumRatio, 0.0f, 1.0f);
    options.siftMinimumAdaptiveRatio = std::clamp(siftMinimumAdaptiveRatio, 0.0f, options.siftMaximumRatio);
    ...
    const QString assetDir = QDir(projectSession.activeChunkRoot()).filePath(QStringLiteral("assets"));
    ...
    const QString reportPath =
        QDir(projectSession.activeChunkRoot()).filePath(QStringLiteral("reports/match_photos_report.json"));
    ...
    reportRecord[QStringLiteral("kind")] = QStringLiteral("match_photos_cli");
    ...
    {
        std::fprintf(stderr, "连接点结果已生成，但 Chunk 写回失败: %s\n", qUtf8Printable(errorMessage));
        return cli::EXIT_IO_ERR;
    }
