...
namespace xjw::aerial_triangulation
{
    namespace
    {
        ...
        BatchFocalPrior resolveBatchFocalPrior(const QStringList& imagePaths)
        {
            ...
            for (int sample = 0; sample < inspectedCount; ++sample)
            {
                ...
                const auto prior = estimateCameraIntrinsicPrior(*metadata, size.width(), size.height());
            ...
            {
                if (prior.make.trimmed().toLower() != make || prior.model.trimmed().toLower() != model)
                {
                    return {false, 0.0, {}, {}, {}, static_cast<int>(priors.size()), inspectedCount};
                }
            ...
            const double medianScale = 0.5 * (scales[(scales.size() - 1) / 2] + scales[scales.size() / 2]);
            return {
                true,
                medianScale,
                priors.front().source,
        ...
        AerialBlockGeometry evaluateAerialBlockGeometry(const SfmReconstruction& reconstruction)
        {
            ...
            const double opticalAxisConcentration =
            ...
            const double trace =
                std::max(0.0, eigenvalues[0]) + std::max(0.0, eigenvalues[1]) + std::max(0.0, eigenvalues[2]);
            const double planarityRatio = trace > 1.0e-12 ? std::max(0.0, eigenvalues[0]) / trace : 1.0;
        ...
        bool hasAbsoluteGeometryConstraint(bool hasCompleteKnownPosePrior, const QJsonObject& diagnostics)
        {
        ...
        bool hasAppliedAdaptiveCameraModelRefinement(const SfmAttemptExecutionResult& execution)
        {
            return execution.result.success &&
                   execution.result.sfmDiagnostics.value(QStringLiteral("ba_adaptive_camera_model_fitting_evaluated"))
                       .toBool(false) &&
                   execution.result.sfmDiagnostics.value(QStringLiteral("ba_adaptive_camera_model_fitting_applied"))
                       .toBool(false);
        }

        /// 从一次完整执行提取仅供焦距搜索排序的轻量指标。
        AdaptiveFocalCandidate candidateFromExecution(double focalScale, const SfmAttemptExecutionResult& execution)
        {
            return {
                focalScale,
        ...
        QJsonObject sparseQualityFromExecution(const PreparedAerialTriangulationInput& input,
                                               const SfmAttemptExecutionResult& execution)
        {
            ...
            return QualityReportWriter::buildSparseQualitySummary(input, *execution.reconstruction, execution.result);
        }
         ...
         */
        SfmCandidateSummary summaryFromExecution(int candidateIndex,
                                                 double focalScale,
                                                 const SfmAttemptExecutionResult& execution,
                                                 bool sequenceLoopClosure)
        {
            const QJsonArray pair =
                execution.result.sfmDiagnostics.value(QStringLiteral("selected_initial_pair")).toArray();
            const ImageId imageA = pair.size() >= 2 ? static_cast<ImageId>(pair.at(0).toInt(-1)) : kInvalidImageId;
            const ImageId imageB = pair.size() >= 2 ? static_cast<ImageId>(pair.at(1).toInt(-1)) : kInvalidImageId;
            ...
            const QJsonObject sparseQuality =
                execution.result.sfmDiagnostics.value(QStringLiteral("sparse_quality")).toObject();
            ...
            const QJsonObject angleSummary = sparseQuality.value(QStringLiteral("triangulation_angle")).toObject();
            summary.hasNetworkQuality = pointCount > 0 && angleSummary.value(QStringLiteral("count")).toInt() > 0;
            summary.medianTriangulationAngleDeg = angleSummary.value(QStringLiteral("p50")).toDouble();
            summary.twoViewTrackRatio =
                pointCount > 0 ? static_cast<double>(twoViewTrackCount) / static_cast<double>(pointCount) : 1.0;
            summary.observationGridCoverage = sparseQuality.value(QStringLiteral("observation_grid_coverage"))
                                                  .toObject()
                                                  .value(QStringLiteral("mean"))
                                                  .toDouble();
            if (execution.reconstruction)
            {
                const AerialBlockGeometry geometry = evaluateAerialBlockGeometry(*execution.reconstruction);
            ...
            if (sequenceLoopClosure && execution.reconstruction)
            {
                ...
                std::vector<ImageId> image_ids = execution.reconstruction->registeredImageIds();
                std::sort(image_ids.begin(), image_ids.end());
                if (image_ids.size() >= 6 && image_ids.size() == execution.reconstruction->numImages())
                {
                    ...
                    for (std::size_t index = 0; index < image_ids.size(); ++index)
                    {
                        ...
                        if (!execution.reconstruction->hasCamera(current) || !execution.reconstruction->hasCamera(next))
                        {
                            valid = false;
                            break;
                        }
                    ...
                    if (valid)
                    {
                        ...
                        const double median = 0.5 * (ordered[(ordered.size() - 1) / 2] + ordered[ordered.size() / 2]);
                        ...
                        const double median_deviation =
                            0.5 * (deviations[(deviations.size() - 1) / 2] + deviations[deviations.size() / 2]);
                        ...
                        summary.sequenceAdjacentDistanceMaximumRatio =
                            summary.hasClosedSequenceGeometry ? ordered.back() / median : 0.0;
                        summary.sequenceAdjacentDistanceMadRatio =
                            summary.hasClosedSequenceGeometry ? median_deviation / median : 0.0;
                    }
                }
            }
            return summary;
        ...
        QJsonObject candidateToJson(const AdaptiveFocalCandidate& candidate, const SfmCandidateSummary* summary)
        {
            QJsonObject object{
            ...
            {
                object.insert(QStringLiteral("median_triangulation_angle_deg"), summary->medianTriangulationAngleDeg);
                object.insert(QStringLiteral("two_view_track_ratio"), summary->twoViewTrackRatio);
                object.insert(QStringLiteral("observation_grid_coverage"), summary->observationGridCoverage);
            }
            ...
            {
                object.insert(QStringLiteral("optical_axis_concentration"), summary->opticalAxisConcentration);
                object.insert(QStringLiteral("camera_center_planarity_ratio"), summary->cameraCenterPlanarityRatio);
            }
            return object;
        }
         ...
         */
        FocalSearchBatchResult runFocalSearchBatch(const AerialTriangulationPipeline::AttemptRunner& attemptRunner,
                                                   ...
                                                   int progressSpan)
        {
                ...
                workers.emplace_back(
                    [&, workerIndex]()
                    {
                        while (true)
                        {
                            ...
                            try
                            {
                                batch.executions[static_cast<std::size_t>(scaleIndex)] = attemptRunner(candidateInput);
                            }
                            catch (const std::exception& error)
                            {
                                SfmAttemptExecutionResult failed;
                                failed.result.success = false;
                                failed.result.errorMessage = QStringLiteral("焦距候选 %1 执行异常: %2")
                            ...
                            catch (...)
                            {
                                SfmAttemptExecutionResult failed;
                                failed.result.success = false;
                                failed.result.errorMessage = QStringLiteral("焦距候选 %1 执行时发生未知异常")
                                                                 .arg(candidateInput.estimatedFocalScale);
                                failed.result.summary = failed.result.errorMessage;
                                batch.executions[static_cast<std::size_t>(scaleIndex)] = std::move(failed);
                            }
            ...
            while (completedCandidateCount.load() < candidateCount)
            {
                if (progressFn)
                {
                    ...
                    progressFn(QStringLiteral("%1：已完成 %2/%3，运行中 %4/%5，CPU线程预算 %6，候选内部最高 %7%")
                                   .arg(stageName)
            ...
            for (const SfmAttemptExecutionResult& candidate : batch.executions)
            {
                ...
                registeredMaximum = std::max(registeredMaximum, candidate.result.numRegisteredImages);
            }
            Logger::instance()->infof("[SFM] focal_probe_summary stage=%s candidates=%d success=%d failed=%d "
            ...
            if (progressFn)
            {
                progressFn(QStringLiteral("%1：已完成 %2/%2 个候选").arg(stageName).arg(candidateCount),
    ...
    bool AerialTriangulationPipeline::shouldFlagAerialDomingRisk(bool geometryValid,
                                                                 double opticalAxisConcentration,
                                                                 double cameraCenterNormalSpanRmsRatio,
                                                                 bool hasAbsoluteGeometryConstraint)
    {
        return geometryValid && !hasAbsoluteGeometryConstraint && std::isfinite(opticalAxisConcentration) &&
               opticalAxisConcentration >= 0.90 && std::isfinite(cameraCenterNormalSpanRmsRatio) &&
               cameraCenterNormalSpanRmsRatio >= 0.025;
    }

    AerialTriangulationPipeline::AerialTriangulationPipeline(AttemptRunner attemptRunner, ResultWriter resultWriter)
    ...
    AerialTriangulationPipeline::run(const PreparedAerialTriangulationInput& input) const
    {
        ...
        const bool adaptiveCameraModelFittingScheduled =
            shouldRunAdaptiveCameraModelRefinement(
                input.adaptiveCameraModelFitting, hasProjectOrExternalPrior, metadataPrior.valid) &&
        ...
        const bool needsFocalInitializationSearch = input.images.size() >= 3 && !hasProjectOrExternalPrior &&
                                                    !hasCompleteKnownPosePrior && !metadataPrior.valid;
        const int focalProbeLimit =
            needsFocalInitializationSearch ? focalProbeRegistrationLimit(input.images.size()) : 0;
        ...
        qint64 preparedTiePointDirectEdgeCount = 0;
        qint64 preparedTiePointClosureEdgeCount = 0;
        bool preparedTiePointUsesRawDirectEdges = false;
        if (_usesProductionAttemptRunner)
        {
            if (originalProgress)
            {
            ...
            preparedTiePointClosureEdgeCount = static_cast<qint64>(sharedGraph->synthesizedClosureEdgeCount);
            preparedTiePointUsesRawDirectEdges = sharedGraph->usesRawDirectEdges;
            if (!preparedTiePointUsesRawDirectEdges)
            {
                Logger::instance()->warn("[SFM] tie-point graph uses legacy v1 closure edges; rebuild tie points to "
                                         "enable raw direct-edge filtering");
            }
        ...
        attemptInput.progressFn = [originalProgress, needsFocalInitializationSearch](const QString& stage, int percent)
        {
            if (originalProgress)
            {
        ...
        if (needsFocalInitializationSearch)
        {
            ...
            const auto appendCandidate = [&](double focalScale, SfmAttemptExecutionResult candidateExecution)
            {
                const int candidateIndex = focalCandidates.size();
                const QJsonObject sparseQuality = sparseQualityFromExecution(focalQualityInput, candidateExecution);
            ...
            for (double focalScale : adaptiveFocalCoarseScaleCandidates())
            {
                const bool alreadyQueued =
                    std::any_of(evaluatedScales.cbegin(),
            ...
            const auto updateWorkerDiagnostics = [&](const SfmWorkerBudget& budget)
            {
                ...
                focalSearchMaxThreadsPerWorker =
                    std::max(focalSearchMaxThreadsPerWorker,
                             budget.threadsPerWorker + (budget.workersWithExtraThread > 0 ? 1 : 0));
            };
            const auto appendBatch = [&](const std::vector<double>& scales, FocalSearchBatchResult batch)
            {
                updateWorkerDiagnostics(batch.workerBudget);
                for (int scaleIndex = 0; scaleIndex < static_cast<int>(scales.size()); ++scaleIndex)
                {
                    appendCandidate(scales[static_cast<std::size_t>(scaleIndex)],
            ...
            appendBatch(evaluatedScales,
                        runFocalSearchBatch(_attemptRunner,
                                            attemptInput,
                                            evaluatedScales,
                                            focalProbeLimit,
                                            originalProgress,
            ...
            const int evaluationTarget = focalProbeLimit > 0 ? focalProbeLimit : static_cast<int>(input.images.size());
            const int bestCoarseIndex = rankedCoarse.empty() ? -1 : rankedCoarse.front().candidateIndex;
            const bool coarsePassesSparseQualityGate =
                bestCoarseIndex >= 0 && bestCoarseIndex < static_cast<int>(candidateExecutions.size()) &&
                candidateExecutions[bestCoarseIndex]
                    .result.sfmDiagnostics.value(QStringLiteral("sparse_quality"))
                    .toObject()
                    .value(QStringLiteral("quality_gate"))
                    .toObject()
                    .value(QStringLiteral("acceptable_for_mvs"))
                    .toBool(false);
            const bool coarseHasProductionCloud = !rankedCoarse.empty() && rankedCoarse.front().success &&
                                                  shouldStopAdaptiveFocalReplay(evaluationTarget,
            ...
            {
                secondStageScales = adaptiveFocalRefinementScaleCandidates(rankedCoarse, evaluatedScales, 2);
            ...
            else
            {
                ...
                for (const double scale : adaptiveFocalScaleCandidates())
                {
                    const bool alreadyEvaluated =
                        std::any_of(evaluatedScales.cbegin(),
                                    evaluatedScales.cend(),
                                    [scale](double value) { return std::abs(value - scale) <= kFocalScaleTolerance; });
                    if (!alreadyEvaluated)
                    {
            ...
            {
                appendBatch(secondStageScales,
                            runFocalSearchBatch(_attemptRunner,
                                                attemptInput,
                                                secondStageScales,
                                                focalProbeLimit,
                                                originalProgress,
                                                focalSearchExhaustiveFallback ? QStringLiteral("焦距全尺度兜底搜索")
                                                                              : QStringLiteral("最佳焦距邻域细化"),
                                                55,
                                                25));
                evaluatedScales.insert(evaluatedScales.end(), secondStageScales.cbegin(), secondStageScales.cend());
            }
            if (input.cancelFlag && input.cancelFlag->load())
            {
                return cancelledResult();
            ...
            if (focalProbeLimit > 0)
            {
                ...
                fullInput.progressFn = [originalProgress](const QString& stage, int percent)
                {
                    if (originalProgress)
                    {
                        originalProgress(QStringLiteral("使用最佳焦距执行完整空三: %1").arg(stage),
                                         80 + std::clamp(percent, 0, 100) * 10 / 100);
                    }
                };
                execution = _attemptRunner(fullInput);
                const int probeCoverage = candidateSummaries[static_cast<std::size_t>(selectedIndex)].registeredImages;
            ...
            else if (adaptiveCameraModelFittingScheduled)
            {
                ...
                refinementInput.progressFn = [originalProgress](const QString& stage, int percent)
                {
                    if (originalProgress)
                    {
                        originalProgress(QStringLiteral("使用最佳焦距细化内参: %1").arg(stage),
                                         80 + std::clamp(percent, 0, 100) * 10 / 100);
                    }
                };
                ...
                if (!sparseQuality.isEmpty())
                {
                    refinedExecution.result.sfmDiagnostics.insert(QStringLiteral("sparse_quality"), sparseQuality);
                }
                ...
                SfmCandidateSummary refinementSummary =
                    summaryFromExecution(1, selectedFocalScale, refinedExecution, input.sequenceLoopClosure);
        ...
        const bool adaptiveCameraModelFittingEvaluated =
            ...
            execution.result.sfmDiagnostics.value(QStringLiteral("ba_adaptive_camera_model_fitting_evaluated"))
                .toBool(false);
        const QString adaptiveCameraModel =
            execution.result.sfmDiagnostics.value(QStringLiteral("ba_adaptive_camera_model")).toString();
        const bool adaptiveCameraModelFittingEffective =
            adaptiveCameraModelFittingEvaluated &&
            (adaptiveCameraModel.isEmpty() || adaptiveCameraModel != QStringLiteral("fixed"));
        ...
        {
            adaptiveCameraModelFittingSkipReason = QStringLiteral("no_observable_parameters");
        }
        else if (!adaptiveCameraModelFittingApplied)
        {
            adaptiveCameraModelFittingSkipReason = QStringLiteral("not_applied");
        }
        execution.result.sfmDiagnostics.insert(QStringLiteral("adaptive_focal_search"), focalCandidates.size() > 1);
        execution.result.sfmDiagnostics.insert(QStringLiteral("focal_initialization_search"),
                                               focalCandidates.size() > 1);
        execution.result.sfmDiagnostics.insert(QStringLiteral("adaptive_camera_model_fitting"),
                                               adaptiveCameraModelFittingRequested);
        execution.result.sfmDiagnostics.insert(QStringLiteral("adaptive_camera_model_fitting_requested"),
                                               adaptiveCameraModelFittingRequested);
        execution.result.sfmDiagnostics.insert(QStringLiteral("adaptive_camera_model_fitting_scheduled"),
                                               adaptiveCameraModelFittingScheduled);
        execution.result.sfmDiagnostics.insert(QStringLiteral("adaptive_camera_model_fitting_effective"),
                                               adaptiveCameraModelFittingEffective);
        execution.result.sfmDiagnostics.insert(QStringLiteral("adaptive_camera_model_fitting_applied"),
                                               adaptiveCameraModelFittingApplied);
        execution.result.sfmDiagnostics.insert(QStringLiteral("adaptive_camera_model_fitting_skip_reason"),
        ...
        execution.result.sfmDiagnostics.insert(QStringLiteral("focal_probe_full_replay"), focalProbeLimit > 0);
        execution.result.sfmDiagnostics.insert(QStringLiteral("focal_search_worker_count"), focalSearchWorkerCount);
        execution.result.sfmDiagnostics.insert(QStringLiteral("focal_search_thread_budget"), focalSearchThreadBudget);
        execution.result.sfmDiagnostics.insert(QStringLiteral("focal_search_min_threads_per_worker"),
                                               focalSearchMinThreadsPerWorker);
        execution.result.sfmDiagnostics.insert(QStringLiteral("focal_search_max_threads_per_worker"),
                                               focalSearchMaxThreadsPerWorker);
        execution.result.sfmDiagnostics.insert(QStringLiteral("focal_search_coarse_candidate_count"),
                                               focalSearchCoarseCandidateCount);
        execution.result.sfmDiagnostics.insert(QStringLiteral("focal_search_refinement_candidate_count"),
                                               focalSearchRefinementCandidateCount);
        execution.result.sfmDiagnostics.insert(QStringLiteral("focal_search_fallback_candidate_count"),
        ...
        execution.result.sfmDiagnostics.insert(QStringLiteral("tie_point_graph_track_count"),
                                               preparedTiePointTrackCount);
        execution.result.sfmDiagnostics.insert(QStringLiteral("tie_point_graph_pair_count"), preparedTiePointPairCount);
        execution.result.sfmDiagnostics.insert(QStringLiteral("tie_point_graph_direct_edge_count"),
                                               preparedTiePointDirectEdgeCount);
        execution.result.sfmDiagnostics.insert(QStringLiteral("tie_point_graph_synthesized_closure_edge_count"),
                                               preparedTiePointClosureEdgeCount);
        execution.result.sfmDiagnostics.insert(QStringLiteral("tie_point_graph_uses_raw_direct_edges"),
                                               preparedTiePointUsesRawDirectEdges);
        ...
        execution.result.sfmDiagnostics.insert(QStringLiteral("image_metadata_focal_prior"), metadataPriorJson);
        ...
        if (execution.reconstruction)
        {
            ...
            const bool absoluteGeometryConstraint =
                hasAbsoluteGeometryConstraint(hasCompleteKnownPosePrior, execution.result.sfmDiagnostics);
            const bool domingRisk = shouldFlagAerialDomingRisk(geometry.valid,
            ...
            QJsonObject aerialGeometry{
                ...
                {QStringLiteral("camera_center_normal_span_rms_ratio"), geometry.cameraCenterNormalSpanRmsRatio},
            };
            ...
            aerialGeometry.insert(QStringLiteral("doming_assessment"),
                                  domingRisk
            ...
            aerialGeometry.insert(QStringLiteral("absolute_geometry_constraint"), absoluteGeometryConstraint);
            execution.result.sfmDiagnostics.insert(QStringLiteral("aerial_block_geometry"), aerialGeometry);
        }
        ...
        QString selfCalibrationStatus =
            hasCompleteKnownPosePrior
                ...
                : (hasProjectOrExternalPrior ? QStringLiteral("trusted_prior") : QStringLiteral("fixed_intrinsics"));
        bool selfCalibrationRequiresReview =
            execution.result.sfmDiagnostics.value(QStringLiteral("aerial_block_geometry"))
                .toObject()
                .value(QStringLiteral("doming_risk"))
                .toBool(false);
