...
namespace xjw::aerial_triangulation
{
    ...
    public:
        ...
        using PipelineRunner =
            std::function<AerialTriangulationReconstructionResult(const PreparedAerialTriangulationInput& input)>;

        /// 连接点任务注入点；为空时使用 MatchPhotosTask。
        using TiePointRunner = std::function<matchphotos::MatchPhotosResult(
            const matchphotos::MatchPhotosOptions& options, const matchphotos::MatchPhotosContext& context)>;

        /**
         * @brief 解析实际生效配置，并仅读取连接点缓存文件头，不展开轨迹或运行算法。
         *
         * 关键语义：resetAlignment 只决定是否复用工程相机位姿；
         * reuseExistingMatches 才决定是否保留匹配/连接点缓存。
         */
        static AerialTriangulationResolvedConfig resolveConfig(const AerialTriangulationOptions& options);

        /// 只读取连接点文件头中的真实持久化配额，不展开大型轨迹数组。
        static std::optional<int> storedTiePointLimit(const QString& tiePointPath);
