...
namespace xjw::aerial_triangulation
{
    ...
    struct AerialTriangulationOptions
    {
        ...
        // Metashape 风格常用参数。
        QString quality = QStringLiteral("high"); ///< 特征匹配精度：highest/high/medium/low/lowest。
        ...
        int tiepointLimit = 4000;  ///< 每幅影像最终连接点配额，按参考默认值做网格均匀化。
        ...
        // 算法、设备和缓存位置。
        QString matchingAlgorithmId = QStringLiteral("plamatch_hct"); ///< 统一算法注册标识。
        ...
        int featureMaxImageDim = 0; ///< 独立显式最长边上限；0 不覆盖 Accuracy 的原生尺度。
        ...
        int tiePointGridColumns = 8;                   ///< 已知位姿专用旧轨迹构建器的网格列数。
        int tiePointGridRows = 8;                      ///< 已知位姿专用旧轨迹构建器的网格行数。
        int maxTiePointsPerGridCell = 0;               ///< 已知位姿专用单网格上限；0 自动计算。
        float stationaryTiePointMaxPixelMotion = 1.0f; ///< 旧配置兼容值；参考静止点规则不读取。
        ...
        std::function<void(const QString& img0, const QString& img1, const QString& matchPath, int numMatches)>
            pairMatchedFn;
    };
