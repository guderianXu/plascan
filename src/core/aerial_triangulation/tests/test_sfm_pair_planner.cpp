...
#include "reconstruction/SfmMatchDiagnostics.h"
...
TEST(SfmPairPlannerTest, LargeKnownCameraSequenceUsesSlidingWindow)
{
    ...
    const xjw::aerial_triangulation::SfmPairPlan plan =
        xjw::aerial_triangulation::planSfmMatchPairs(imagePaths(25), cameraPaths(25), options);
    ...
    EXPECT_TRUE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(3))));
    EXPECT_FALSE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(4))));
}
...
TEST(SfmPairPlannerTest, SequenceLoopClosureDoesNotBoostExistingWindowPairs)
{
    xjw::aerial_triangulation::SfmPairPlannerOptions options;
    options.autoRestrictKnownCameraPairs = true;
    options.knownCameraPairWindow = 8;
    options.knownCameraAllPairsMaxImages = 2;
    options.knownCameraSpatialNeighborCount = 0;
    options.knownCameraSequenceLoopClosure = true;

    const xjw::aerial_triangulation::SfmPairPlan plan =
        xjw::aerial_triangulation::planSfmMatchPairs(imagePaths(16), QStringList(), options);
    ...
    EXPECT_TRUE(edgeIt->sourceTypes.contains(QStringLiteral("sequence_window")));
    EXPECT_FALSE(edgeIt->sourceTypes.contains(QStringLiteral("sequence_loop")))
        << "闭环补边只能补跨过序列首尾的配对，不能给普通窗口内的远距离配对重复加分。";
    EXPECT_GT(nearIt->priorityScore, edgeIt->priorityScore);
}

TEST(SfmPairPlannerTest, SequencePreselectionWithoutCameraReferenceStillRestrictsLargeProject)
{
    xjw::aerial_triangulation::SfmPairPlannerOptions options;
    options.autoRestrictKnownCameraPairs = true;
    options.knownCameraPairWindow = 2;
    ...
    const xjw::aerial_triangulation::SfmPairPlan plan =
        xjw::aerial_triangulation::planSfmMatchPairs(imagePaths(25), QStringList(), options);
    ...
    EXPECT_EQ(plan.allPairCount, 300);
    EXPECT_EQ(plan.allowedPairKeys.size(), 50);

    const QSet<QString> keys(plan.allowedPairKeys.begin(), plan.allowedPairKeys.end());
    EXPECT_TRUE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(2))));
    EXPECT_FALSE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(3))));
    EXPECT_TRUE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(24), imagePath(0))));
}
...
TEST(SfmPairPlannerTest, KnownCameraCentersAddSpatialNeighborsOutsideSequenceWindow)
{
    ...
    const QSet<QString> keys(plan.allowedPairKeys.begin(), plan.allowedPairKeys.end());
    EXPECT_TRUE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(20))));
    EXPECT_TRUE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(3))));
    EXPECT_FALSE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(4))));
}
...
TEST(SfmPairPlannerTest, KnownCameraCentersWithoutCameraFilesStillRestrictLargeProject)
{
    ...
    const xjw::aerial_triangulation::SfmPairPlan plan =
        xjw::aerial_triangulation::planSfmMatchPairs(imagePaths(40), QStringList(), options);
    ...
    const QSet<QString> keys(plan.allowedPairKeys.begin(), plan.allowedPairKeys.end());
    EXPECT_TRUE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(30))));
    EXPECT_TRUE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(3))));
    EXPECT_FALSE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(4))));
}
...
TEST(SfmPairPlannerTest, KnownCameraOverlapPairsTakePriorityOverCenterNeighbors)
{
    ...
    const QSet<QString> keys(plan.allowedPairKeys.begin(), plan.allowedPairKeys.end());
    EXPECT_TRUE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(1))));
    EXPECT_TRUE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(3))));
    EXPECT_TRUE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(10), imagePath(20))));
    EXPECT_FALSE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(20))));
    EXPECT_FALSE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(4))));
}
...
TEST(SfmPairPlannerTest, DenseKnownCameraOverlapPairsFallbackToBoundedNeighbors)
{
    ...
    EXPECT_TRUE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(20))));
    EXPECT_TRUE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(3))));
    EXPECT_FALSE(keys.contains(xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(0), imagePath(24))));
}
...
TEST(SfmPairPlannerTest, SmallKnownCameraSetKeepsAllPairs)
{
    ...
    const xjw::aerial_triangulation::SfmPairPlan plan =
        xjw::aerial_triangulation::planSfmMatchPairs(imagePaths(12), cameraPaths(12), options);
...
TEST(SfmPairPlannerTest, ExplicitPairRestrictionIsPreserved)
{
    ...
    options.knownCameraPairWindow = 3;
    options.knownCameraAllPairsMaxImages = 20;
    options.allowedPairs = {xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(2), imagePath(9))};
    ...
    EXPECT_EQ(plan.allowedPairKeys.front(), xjw::aerial_triangulation::canonicalSfmPairKey(imagePath(2), imagePath(9)));
}
...
TEST(SfmPairPlannerTest, MissingCameraPathsKeepAllPairs)
{
    ...
    const xjw::aerial_triangulation::SfmPairPlan plan =
        xjw::aerial_triangulation::planSfmMatchPairs(imagePaths(25), cameras, options);
...
TEST(SfmMatchDiagnosticsTest, SeparatesCandidateGraphFromActualMatchGraph)
{
    ...
    const xjw::aerial_triangulation::SfmMatchDiagnostics diagnostics =
        xjw::aerial_triangulation::analyzeSfmMatchDiagnostics(imageIds, pairs);
...
TEST(SfmGuidedMatchPlannerTest, PrioritizesRegisteredWeakOverlapPairsForEpipolarRematching)
{
    ...
    xjw::aerial_triangulation::SfmGuidedMatchPlannerOptions options;
    options.minSeedMatches = 80;
    options.maxHealthyMatches = 60;
    options.maxCandidates = 8;
    options.registeredImageIds = {0, 1, 2, 3};

    const xjw::aerial_triangulation::SfmGuidedMatchPlan plan =
        xjw::aerial_triangulation::planSfmGuidedMatching(imageIds, pairs, options);
