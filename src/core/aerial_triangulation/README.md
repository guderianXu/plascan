...
- 无完整相机先验时，焦距尺度表示 `焦距像素 / 影像最长边像素`。Pipeline 始终评估
  ...
  的广域候选，即使低焦距基准已注册全部影像，也不会跳过窄视场检查。粗搜索最多并行执行 4 个候选，
  并把总线程预算均分给 worker；界面进度表示所有候选的整体完成比例。
...
- BA 后端使用 `Auto`：小型固定焦距问题保留 CPU/OpenMP，共享焦距使用 Ceres 联合优化，大问题达到相机和观测阈值后
  可选择 Ceres CUDA。当前 native CUDA 只具备三维点优化能力，需要相机或焦距优化时不会被 Auto 选择。
