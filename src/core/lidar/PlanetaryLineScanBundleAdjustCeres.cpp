...
bool evaluateImageObservation(const PlanetaryLineScanCamera &camera,
                              ...
                              double *residuals)
{
    ...
    if (!camera.projectAtObservedLine(
            ground, observation.linePixels,
            PlanetaryLineScanCamera::PixelConvention::CsmPixelCenter,
            &projection, lineScanPoseBias(parameters)))
    {
        return false;
    }
    residuals[0] = (projection.sample - observation.samplePixels) / imageSigmaPixels;
...
public:
    ...
    {
        if (!evaluateImageObservation(*_camera, _observation, parameters[0], parameters[1],
...
private:
                            ...
                            double *jacobian) const
    {
        ...
        for (int column = 0; column < blockSize; ++column)
        {
            ...
            if (!evaluateImageObservation(*_camera, _observation,
                                          plusCamera.data(), plusPoint.data(),
                                          _imageSigmaPixels, plusResiduals) ||
                !evaluateImageObservation(*_camera, _observation,
                                          minusCamera.data(), minusPoint.data(),
    ...
    for (const LineScanImageObservation &observation : workingSet.imageObservations)
    {
        ...
        if (!evaluateImageObservation(
                *workingSet.cameraModels[observation.cameraIndex], observation,
                ...
                workingSet.tiePoints[observation.pointIndex].data(), 1.0, residuals))
        {
