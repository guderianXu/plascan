...
namespace xjw
{
    namespace mvs
    {
        ...
        struct StereoFusionConfig
        {
            ...
            /// 融合主体仍在 CPU；显式 CUDA/OpenCL 仅加速流式首帧反投影。
            /// Auto 保持 CPU，避免为一次反投影额外分配全分辨率中间缓冲。
            DenseCloudComputeBackend computeBackend = DenseCloudComputeBackend::Auto;
        ...
        public:
            ...
            const DenseCloudExecutionReport& unprojectionExecutionReport() const noexcept
            {
                return _unprojectionExecutionReport;
            }

        private:
            StereoFusionConfig _config;
            DenseCloudExecutionReport _unprojectionExecutionReport;
            std::string _unprojectionError;
            ...
            /// 预计算每帧的投影几何
            void prepareGeometry(const std::vector<FusionFrameInput>& frames, std::vector<FrameGeometry>& geom);
            ...
            bool isPixelEligible(const FusionFrameInput& frame, const FrameGeometry& geometry, int row, int col);
            void resetRejectionStats();

            /// 两视图 minNumPixels<=1 场景的并行快速融合路径
            bool fuseTwoViewSingleObservationFast(const std::vector<FusionFrameInput>& frames,
                                                  const std::vector<FrameGeometry>& geom,
                                                  const std::function<cv::Mat(int)>& colorProvider,
                                                  std::vector<FusedPoint>& fusedPoints,
            ...
            bool fuseFirstFrameObservationsFast(const std::vector<FusionFrameInput>& frames,
                                                const std::vector<FrameGeometry>& geom,
                                                const std::function<cv::Mat(int)>& colorProvider,
                                                std::vector<FusedPoint>& fusedPoints,
                                                MvsProgressCallback progressCb);
