...
namespace xjw::mvs
{
    ...
    public:
        using Loader = std::function<bool(int, const std::atomic_bool*, MvsImageFrame*, std::string*)>;
        ...
        MvsImageCache(std::size_t frameCount, std::size_t capacity, Loader loader);
        ~MvsImageCache();

        MvsImageCache(const MvsImageCache&) = delete;
        MvsImageCache& operator=(const MvsImageCache&) = delete;

        ImageLease acquire(int frameIndex, const std::atomic_bool* cancelFlag, std::string* errorMessage = nullptr);

        bool preloadAll(int workerCount, const std::atomic_bool* cancelFlag, std::string* errorMessage = nullptr);
