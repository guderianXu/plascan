...
namespace xjw
{
    namespace mvs
    {
        namespace
        {
            ...
            DepthPyramidLevelConfig makeLevel(const PatchMatchConfig& base_config, int level, int downsample_factor)
            {
        ...
        cv::Size depthPyramidWorkingSize(int image_width, int image_height, int downsample_factor)
        {
            const int factor = std::max(1, downsample_factor);
            return cv::Size(std::max(1, image_width / factor), std::max(1, image_height / factor));
        }
        ...
        DepthPyramidConfig
        makeDepthPyramidConfig(const PatchMatchConfig& base_config, int image_width, int image_height)
        {
            ...
            const int final_factor = requested_final_factor == 2 && short_side <= kMaximumNativeHighQualityShortSide
                                         ? 1
                                         : std::min(requested_final_factor, maximum_final_factor);
            const int maximum_factor = std::max(1, short_side / kMinimumLevelShortSide);
            const std::array<int, 3> desired_factors = {final_factor * 4, final_factor * 2, final_factor};
            ...
            for (size_t index = 0; index < desired_factors.size(); ++index)
            {
                ...
                if (!active_levels.empty() && active_levels.back().patchMatch.downsampleFactor <= factor)
                {
                    continue;
                }
            ...
            {
                result.levels[index] = index < active_levels.size() ? active_levels[index] : active_levels.back();
            }

            if (result.activeLevelCount < 3)
            {
                std::ostringstream message;
                message << "image short side " << short_side << " cannot keep three pyramid levels above "
                                              ...
                                              const cv::Size& depth_grid_size)
        {
            ...
            return raster_camera.scaledIntrinsics(static_cast<double>(depth_grid_size.width) / raster_size.width,
                                                    ...
                                                    const cv::Size& depth_grid_size) noexcept
        {
            ...
            if (raster_size.width <= 0 || raster_size.height <= 0 || depth_grid_size.width <= 0 ||
                depth_grid_size.height <= 0)
            {
                return result;
            }

            result.scaleX = static_cast<double>(depth_grid_size.width) / static_cast<double>(raster_size.width);
            result.scaleY = static_cast<double>(depth_grid_size.height) / static_cast<double>(raster_size.height);
            result.areaScale = result.scaleX * result.scaleY;
            result.linearScale = std::sqrt(result.areaScale);
            return result;
        }

        int scaleDepthPixelRadius(int raster_radius, const DepthPixelDomainScale& scale) noexcept
        {
            return std::max(
                0, static_cast<int>(std::lround(static_cast<double>(std::max(0, raster_radius)) * scale.linearScale)));
        }

        float scaleDepthPixelDistance(float raster_distance, const DepthPixelDomainScale& scale) noexcept
        {
            if (!std::isfinite(raster_distance) || raster_distance <= 0.0f)
            {
                return 0.0f;
            }
            return static_cast<float>(static_cast<double>(raster_distance) * scale.linearScale);
        }

        float scaleDepthNearestSampleDistance(float raster_distance, const DepthPixelDomainScale& scale) noexcept
        {
            const float scaled_distance = scaleDepthPixelDistance(raster_distance, scale);
            if (scaled_distance <= 0.0f || !scale.usesReducedGrid())
            {
                return scaled_distance;
            }
            constexpr float kHalfPixelDiagonal = 0.70710678f;
            return std::max(scaled_distance, kHalfPixelDiagonal);
        }

        int scaleDepthPixelArea(int raster_area, const DepthPixelDomainScale& scale) noexcept
        {
            if (raster_area <= 0)
            {
                return 0;
            }
            return std::max(1, static_cast<int>(std::lround(static_cast<double>(raster_area) * scale.areaScale)));
        }

        int scaleDepthLocalOutlierKernel(int raster_kernel_size, const DepthPixelDomainScale& scale) noexcept
        {
