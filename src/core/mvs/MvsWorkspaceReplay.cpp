...
namespace xjw::mvs
{
    namespace
    {
        ...
        QString maskPathForImage(const QString& maskDirectory, const QString& imagePath)
        {
            ...
            return QDir(maskDirectory).filePath(QFileInfo(imagePath).completeBaseName() + QStringLiteral("_mask.png"));
        }
        ...
        QString resolveManifestPath(const QString& manifestPath, const QString& storedPath)
        {
            ...
            return QDir::cleanPath(canonicalPath.isEmpty() ? pathInfo.absoluteFilePath() : canonicalPath);
        }
                            ...
                            QString* errorMessage)
    {
        ...
        for (const MvsDepthFrameRecord& record : manifest.frames())
        {
            ...
            if (!recordsByIndex.emplace(record.refIndex, record).second)
            {
                if (errorMessage)
                {
                    *errorMessage = QStringLiteral("MVS manifest 含重复 ref_index：%1").arg(record.refIndex);
                }
                return false;
            }
        }
        ...
        for (const auto& [index, record] : recordsByIndex)
        {
            if (index != expectedIndex)
            {
                if (errorMessage)
                {
                    *errorMessage = QStringLiteral("MVS manifest 的 ref_index 不连续：期望 %1，实际 %2")
                                        .arg(expectedIndex)
                                        .arg(index);
                }
                views->clear();
            ...
            const QString source_image_path = resolveManifestPath(manifestPath, record.refImage);
            const bool has_prepared_image = !record.preparedImage.trimmed().isEmpty();
            const bool has_prepared_mask = !record.preparedValidMaskPath.trimmed().isEmpty();
            const bool has_prepared_camera = !record.preparedCameraModel.isEmpty();
            if (has_prepared_image != has_prepared_mask || has_prepared_image != has_prepared_camera)
            {
                if (errorMessage)
                {
                    *errorMessage = QStringLiteral("MVS manifest 第 %1 帧的 prepared_image、prepared "
            ...
            const QString raster_path =
                resolveManifestPath(manifestPath, has_prepared_image ? record.preparedImage : record.refImage);
            if (!QFileInfo::exists(raster_path))
            {
                if (errorMessage)
                {
                    *errorMessage =
                        QStringLiteral("MVS replay 工作栅格不存在：%1").arg(QDir::toNativeSeparators(raster_path));
                }
                views->clear();
                return false;
            }
            ...
            if (replayImageIdentities.contains(imageIdentity))
            {
                if (errorMessage)
                {
                    *errorMessage = QStringLiteral("MVS manifest 含重复 ref_image：第 %1 帧与已有帧指向同一影像 %2")
                                        .arg(index)
            ...
            if (replayRasterIdentities.contains(raster_identity))
            {
                if (errorMessage)
                {
                    *errorMessage = QStringLiteral("MVS manifest 第 %1 帧与已有帧指向同一 prepared raster：%2")
                                        .arg(index)
            ...
            if (image.empty())
            {
                ...
                {
                    *errorMessage =
                        QStringLiteral("无法读取 MVS replay 影像：%1").arg(QDir::toNativeSeparators(raster_path));
                }
                views->clear();
                return false;
            }
            ...
            {
                const cv::Mat prepared_mask = xjw::common::io::readImage(prepared_mask_path, cv::IMREAD_GRAYSCALE);
                if (prepared_mask.empty())
                {
                    if (errorMessage)
                    {
                        *errorMessage = QStringLiteral("MVS replay prepared 有效蒙版不存在：%1")
                                            .arg(QDir::toNativeSeparators(prepared_mask_path));
                    }
                    views->clear();
                    return false;
                }
                if (prepared_mask.size() != image.size())
                {
                    if (errorMessage)
                    {
                        *errorMessage = QStringLiteral("MVS replay prepared 有效蒙版与工作栅格尺寸不一致：%1")
                                            .arg(QDir::toNativeSeparators(prepared_mask_path));
                    }
                    views->clear();
                    return false;
                ...
                view.preparedValidMaskSource = record.maskSource.toUtf8().toStdString();
            }
            else
            {
                const QString mask_path = maskPathForImage(maskDirectory, source_image_path);
                if (!mask_path.isEmpty() && !QFileInfo::exists(mask_path))
                {
                    if (errorMessage)
                    {
                        *errorMessage =
                            QStringLiteral("MVS replay 蒙版不存在：%1").arg(QDir::toNativeSeparators(mask_path));
                    }
                    views->clear();
                    return false;
                }
    ...
    bool loadMvsPairAuditReport(const QString& reportPath,
                                ...
                                QString* errorMessage)
    {
        ...
        for (const QJsonValue& value : pairs)
        {
            ...
            const QString storedImageA = pair.value(QStringLiteral("image_a")).toString().trimmed();
            const QString storedImageB = pair.value(QStringLiteral("image_b")).toString().trimmed();
            ...
            quality.totalMatches = std::max(0, pair.value(QStringLiteral("total_matches")).toInt());
            quality.geometricInliers = std::max(0, pair.value(QStringLiteral("geometric_inliers")).toInt());
            quality.geometricCoverage =
                static_cast<float>(std::clamp(pair.value(QStringLiteral("coverage_score")).toDouble(0.0), 0.0, 1.0));
            const QString status = pair.value(QStringLiteral("status")).toString().trimmed().toLower();
            ...
            quality.verificationReason = xjw::common::io::toUtf8Path(pair.value(QStringLiteral("reason")).toString());
        ...
        if (qualities->empty())
        {
            ...
            {
                *errorMessage =
                    QStringLiteral("MVS pair audit 报告不含有效像对：%1").arg(QDir::toNativeSeparators(reportPath));
            }
            return false;
        }
        if (summary)
