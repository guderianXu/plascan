...
namespace xjw
{
    namespace mvs
    {
        ...
        using DenseCloudComputeBackend = PatchMatchBackend;

        const char* denseCloudComputeBackendId(DenseCloudComputeBackend backend) noexcept;

        struct DenseCloudExecutionReport
        {
            DenseCloudComputeBackend requestedBackend = DenseCloudComputeBackend::Auto;
            DenseCloudComputeBackend actualBackend = DenseCloudComputeBackend::Cpu;
            int deviceIndex = -1;
        ...
        {
            bool useGPU = true; ///< 兼容开关；false 强制 CPU，true 时由 computeBackend 决定
        ...
        public:
            ...
            /// 可选：PLY 格式保存
            static bool
            savePLY(const std::string& path, const std::vector<DensePoint>& cloud, std::string* errorMsg = nullptr);

            /// 体素下采样，保留并平均 RGB 颜色属性
            static std::vector<DensePoint>
            voxelDownsample(const std::vector<DensePoint>& cloud,
                            float voxelSize,
            ...
            /// @return 过滤后的干净点云
            static std::vector<DensePoint>
            statisticalOutlierRemoval(const std::vector<DensePoint>& cloud,
                                      int kNeighbors = 30,
                                      float stdRatio = 1.5f,
                                      plapoint::ProcessingDevice processingDevice = plapoint::ProcessingDevice::Auto,
                                      plapoint::ProcessingReport* processingReport = nullptr);
            ...
            /// @return 过滤后的干净点云
            static std::vector<DensePoint>
            radiusOutlierRemoval(const std::vector<DensePoint>& cloud,
                                 float radius,
                                 int minNeighbors = 6,
                                 plapoint::ProcessingDevice processingDevice = plapoint::ProcessingDevice::Auto,
                                 plapoint::ProcessingReport* processingReport = nullptr);
