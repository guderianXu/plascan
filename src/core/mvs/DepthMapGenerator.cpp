...
namespace xjw
{
    namespace mvs
    {
        ...
        namespace
        {
            ...
            {
                return scaleDepthPixelArea(kFullRasterMinimumSmallHoleArea,
            ...
            {
                const GeometryEvidenceMaps discrete_evidence =
                    makeGeometryEvidenceMaps(depth,
                                             consistent_votes,
                ...
                else
                {
                    ...
                    for (int y = 0; y < depth.rows; ++y)
                    {
                        ...
                        for (int x = 0; x < depth.cols; ++x)
                        {
                            ...
                            {
                                conflict_row[x] =
                                    static_cast<float>(contradicted_row[x]) / static_cast<float>(observed);
                            }
                        }
                    }
                }
                ...
                options.textureRadiusPixels =
                    std::max(1, scaleDepthPixelRadius(options.textureRadiusPixels, pixel_scale));
                ...
                options.minimumComponentArea =
                    std::max(1, scaleDepthPixelArea(options.minimumComponentArea, pixel_scale));
                                                   ...
                                                   bool effective_native_final_grid)
            {
                ...
                const int quantized_boundary_edge_radius =
                    scaleDepthPixelRadius(kFullRasterBoundaryEdgeRadiusPixels, scale);
                const int boundary_edge_radius = fusion_config.enableBoundaryAwareRetention &&
                ...
                QJsonObject boundary_edge = configuredEffectiveInteger(
                    kFullRasterBoundaryEdgeRadiusPixels, boundary_retention_active ? boundary_edge_radius : 0);
                ...
                QJsonObject boundary_protection =
                    configuredEffectiveInteger(fusion_config.boundaryProtectionRadiusPixels,
                ...
                if (boundary_retention_active && scale.usesReducedGrid() &&
                    fusion_config.boundaryProtectionRadiusPixels > 0 && boundary_protection_radius == 1)
                {
                ...
                if (!boundary_retention_active)
                {
                    boundary_protection.insert(QStringLiteral("disabled_reason"),
...
#pragma omp parallel for schedule(static)
            ...
            MvsSourcePairQualityLookup
            buildMvsSourcePairQualityLookup(const std::vector<MvsSourcePairQuality>& qualities)
            {
            ...
            public:
                ...
                ProducerReservation reserveProducer(const std::atomic<bool>* cancelFlag = nullptr)
                {
                    const auto wait_start = std::chrono::steady_clock::now();
                    {
                        ...
                        while (!_stopping && !(cancelFlag && cancelFlag->load()) &&
                             ...
                             QString stageLabel)
                {
                    ...
                    if (!enqueue_exception.isEmpty())
                    {
                        ...
                        LOG_ERROR(QStringLiteral("[MVS] 深度帧 %1 保存任务入队异常：%2")
                                      .arg(frameIndex)
                ...
                bool waitUntilIdle(const std::atomic<bool>* cancelFlag = nullptr)
                {
                    ...
                    { return _tasks.empty() && _activeTasks == 0 && _producerReservations == 0; };
                    while (!idle())
                    {
            ...
            int resolvedTotalCpuThreadBudget(const DepthGenConfig& config)
            {
                ...
                const int requested_budget =
                    config.totalCpuThreadBudget > 0 ? config.totalCpuThreadBudget : derived_budget;
            ...
            {
                const bool tryCuda =
                    (config.backend == PatchMatchBackend::Cuda || config.backend == PatchMatchBackend::Auto) &&
                ...
                for (int attempt = 0; attempt < kMaxCudaAttempts; ++attempt)
                {
                    ...
                    PatchMatchConfig nextConfig =
                        DepthMapGenerator::nextCudaRetryPatchMatchConfig(attemptConfig, refGray.cols, refGray.rows);
                ...
                if (!config.cudaFallbackToCpu)
                {
                    ...
                    {
                        *errorMsg =
                            lastCudaError.empty() ? std::string("CUDA PatchMatch retries exhausted") : lastCudaError;
                    }
                    return false;
                }
                ...
                parallelForRows(
                    ...
                    [&](int row)
                    {
                        ...
                        float* adaptive_squared_row = accumulate_adaptive_evidence
                                                          ? adaptiveEvidence->squaredPositiveSupport.ptr<float>(row)
                                                          : nullptr;
                            ...
                            for (const DepthConsistencySourceInput& source : sources)
                            {
                                ...
                                if (accumulate_adaptive_evidence)
                                {
                                        ...
                                        result.sourcePixel.y < source.confidence.rows)
                                    {
                                        ...
                                        observation.reliabilityWeight *= std::isfinite(source_confidence)
                                ...
                                case DepthConsistencyEvidence::Consistent:
                                    ...
                                    if (source.sourceOrdinal >= 0 && source.sourceOrdinal < 16)
                                    {
                                        source_mask_row[column] = static_cast<std::uint16_t>(
                                            ...
                                            (static_cast<std::uint16_t>(1U) << source.sourceOrdinal));
                                    }
                                                  ...
                                                  cv::Mat* anchoredInterpolationMask = nullptr)
            {
                if (sceneProfile != MvsSceneProfile::OrbitalObject || !result.supportRegionMask ||
                    result.supportRegionMask->empty() || !result.crossViewRepairedMask ||
                ...
                cv::Mat* interpolation_mask =
                    anchoredInterpolationMask ? anchoredInterpolationMask : &local_interpolation_mask;
                                                         ...
                                                         const DiscreteGeometryCoreSummary& discrete_summary)
            {
                ...
                quality_input.adaptiveGeometryEvidenceAvailable =
                    ...
                    std::isfinite(adaptive_summary.effectiveViewCountMean) &&
                    adaptive_summary.effectiveViewCountMean >= 0.0f &&
            ...
            public:
                              ...
                              std::string* error_message) override
                {
                    const std::string stage_label =
                        "Level " + std::to_string(request.levelConfig.level) + " PatchMatch";
                    cv::Mat confidence;
                    const cv::Mat* hint =
                        request.prior && !request.prior->center.empty() ? &request.prior->center : nullptr;
                    const cv::Mat* radius =
                        request.prior && !request.prior->radius.empty() ? &request.prior->radius : nullptr;
                                          ...
                                          int requested_repair_source_count)
        {
            ...
            for (int reference_index = 0; reference_index < view_count; ++reference_index)
            {
                ...
                {
                    plan.geometrySourceViewIndices = planMvsRepairSourceViews(plan.consistencySourceIndices,
                                                                              source_eligibility,
                                                                              reference_index,
        ...
        cv::Mat DepthMapGenerator::projectMaskToValidMask(const cv::Mat& projectMask, cv::Size targetSize)
        {
                                                                 ...
                                                                 float* retainedRatio)
        {
            ...
            if (gray.empty() || projectValidMask.empty() || gray.type() != CV_8UC1 ||
                projectValidMask.type() != CV_8UC1 || gray.size() != projectValidMask.size())
            {
         ...
         * 与 `setConfig()` 注入。
         */
        DepthMapGenerator::DepthMapGenerator(QObject* parent) : QObject(parent)
        {
        ...
        bool DepthMapGenerator::publishTerminalDepthCheckpoints()
        {
            ...
            QString publication_error;
            {
                ...
                for (const MvsDepthFrameRecord& checkpoint : records)
                {
                    ...
                    {
                        publication_error =
                            QStringLiteral("帧 %1 的初始 MVS checkpoint 缺少可验证的终态工件，已拒绝发布")
            ...
            if (!publication_error.isEmpty())
            {
                ...
                emit errorOccurred(publication_error);
                return false;
            }
            for (const TerminalPublication& publication : publications)
            {
                emit depthMapSaved(publication.frame.depthPng,
                                   publication.frame.gridWidth,
                                   publication.frame.gridHeight,
                                   publication.frame.refImage);
                emit depthMapArtifactSaved(publication.artifact);
            }
            return true;
        }

        bool DepthMapGenerator::ensurePreparedRasterArtifact(int frameIndex,
            ...
            for (int refIdx = 0; refIdx < NV; ++refIdx)
            {
                ...
                if (_effectiveSceneProfile == MvsSceneProfile::OrbitalObject)
                {
                    scene_maximum_angle = adaptiveMvsSourceMaximumAngleDeg(
                        _effectiveSceneProfile, desiredSourceCount, legacyCandidateAngles);
                ...
                if (_config.evaluateCompleteVisibilityCandidatePool)
                {
                    const std::size_t legacy_candidate_count = std::min(
                        candidates.size(), static_cast<std::size_t>(std::max(0, legacyEvaluatedCandidateCount)));
                    const std::vector<MvsSourceCandidate> legacy_candidates(
                        candidates.cbegin(), candidates.cbegin() + legacy_candidate_count);
                ...
                _frameCaches[static_cast<size_t>(refIdx)].completePoolChangedLegacyPlan =
                    complete_pool_changed_legacy_plan;
                ...
                if (sourcePlan.sourceViewShortfall > 0)
                {
                    ...
                    {
                        verification_failed = verification_failed || rejected.candidate.verificationStatus ==
                                                                         MvsSourceVerificationStatus::Failed;
                        verification_missing =
                            verification_missing ||
                            rejected.candidate.verificationStatus == MvsSourceVerificationStatus::MissingStatistics;
        ...
        std::vector<size_t> DepthMapGenerator::visibleSparsePointIndicesForFrame(int refIdx,
        ...
        bool DepthMapGenerator::loadMvsImageFrame(int frameIndex,
                ...
                if (!projectMask.empty())
                {
                    ...
                    if (_effectiveSceneProfile == MvsSceneProfile::OrbitalObject)
                    {
                        frame->validMask = refineOrbitalProjectValidMask(
                            frame->gray, frame->validMask, &contentRefined, &retainedRatio);
                    }
        ...
        bool DepthMapGenerator::initializeImageProvider(const MvsPipelineMemoryPolicyDecision& decision,
                                                        QString* errorMessage)
        {
            if (decision.imageStrategy == MvsImageCacheStrategy::Insufficient || decision.imageCacheCapacity == 0)
            {
                if (errorMessage)
                {
                    ...
                    *errorMessage = QStringLiteral("MVS 内存不足: required=%1 GiB available=%2 GiB strategy=%3。"
                                        ...
                                        .arg(decision.estimate.visibility.saturated ? QStringLiteral("true")
            ...
            _imageCache = std::make_unique<MvsImageCache>(
                ...
                [this](
                    int frameIndex, const std::atomic_bool* cancelFlag, MvsImageFrame* frame, std::string* loaderError)
        ...
        MvsImageCache::ImageLease DepthMapGenerator::acquireImageFrame(int frameIndex, std::string* errorMessage)
        {
            if (!_imageCache)
            {
                if (errorMessage)
        ...
        void DepthMapGenerator::start()
        {
            ...
            _depthFrames.clear();
            _backgroundFuture = QtConcurrent::run([this]() { runInBackground(); });
        }
                                                   ...
                                                   const std::vector<int>& sourceIndices) const
        {
            ...
            if (visiblePointIndices.size() < 5 && minSourceViews > 0)
            {
                ...
                LOG_DEBUG("[MVS][帧 %d][深度范围] 共视稀疏点不足，回退参考帧可见点 (%zu)",
                          refIdx,
        ...
        bool DepthMapGenerator::estimateDepthRangeFromVisiblePoints(int refIdx,
                                                                    ...
                                                                    float& zFar) const
        {
            ...
            if (depths.size() < 5)
            {
                ...
                if (maxBaseline > 1e-3f)
                {
                    ...
                    LOG_DEBUG(
                        "[MVS][深度范围] 回退全局基线: baseline=%.4f zNear=%.4f zFar=%.4f", maxBaseline, zNear, zFar);
                }
                else
                {
                    // 实在无法估计
        ...
        // =============================================================================
        cv::Mat DepthMapGenerator::buildHintDepth(int refIdx, int W, int H, const std::vector<int>& sourceIndices) const
        {
            const int minSourceViews = sourceIndices.empty() ? 0 : 1;
            std::vector<size_t> visiblePointIndices =
                visibleSparsePointIndicesForFrame(refIdx, sourceIndices, minSourceViews);
            if (visiblePointIndices.empty() && minSourceViews > 0)
            {
                visiblePointIndices = visibleSparsePointIndicesForFrame(refIdx, {}, 0);
                LOG_DEBUG(
                    "[MVS][帧 %d][稀疏引导] 共视点为空，回退参考帧可见点 (%zu)", refIdx, visiblePointIndices.size());
            }

            return buildHintDepthFromVisiblePoints(refIdx, W, H, visiblePointIndices);
        }

        cv::Mat DepthMapGenerator::buildHintDepthFromVisiblePoints(int refIdx,
                                                                   int W,
                                                                   int H,
        ...
        cv::Mat DepthMapGenerator::buildHintDepthForCamera(int refIdx,
        ...
        DepthMapGenerator::collectProjectedSparseDepthSamples(const SparseCloud& sparse,
                                                              const FramePinholeCamera& camera,
                                                              int imageWidth,
                                                              int imageHeight,
                                                              const std::vector<size_t>& visiblePointIndices)
        {
            ...
            const size_t quantileSampleStride =
                visiblePointIndices.size() > kMaxProjectedDepthQuantileSamples
            ...
            for (size_t pointIndex : visiblePointIndices)
            {
                ...
                double depth = 0.0;
                if (!camera.projectWorldPointWithDepth(world, pixel, depth) || !std::isfinite(depth) ||
                    pixel[0] < 0.0 || pixel[0] >= static_cast<double>(imageWidth) || pixel[1] < 0.0 ||
            ...
            if (depthQuantileSamples.size() >= 4)
            {
                ...
                auto q3It =
                    depthQuantileSamples.begin() + static_cast<std::ptrdiff_t>(depthQuantileSamples.size() * 3 / 4);
            ...
            int refIdx, int W, int H, const std::vector<ProjectedSparseDepthSample>& samples)
        {
            ...
            const int radius =
                maxDim < 512 ? std::clamp(maxDim / 8, 8, 48)
        ...
        cv::Mat DepthMapGenerator::buildSparseSupportMaskForCamera(int refIdx,
                                                                   const FramePinholeCamera& camera,
                                                                   int W,
                                                                   int H,
                                                                   const std::vector<size_t>& visiblePointIndices) const
        {
            if (W <= 0 || H <= 0 || refIdx < 0 || refIdx >= static_cast<int>(_views.size()) ||
                _sparse.points.size() < 20)
            {
                return cv::Mat();
            }
...
#pragma omp parallel for schedule(static)
        ...
        int DepthMapGenerator::removeSmallDepthComponents(
                                                                           ...
                                                                           const cv::Size& rasterPixelDomainSize)
        {
            ...
            const int local_outlier_kernel =
                scaleDepthLocalOutlierKernel(config.localDepthOutlierKernelSize, pixel_scale);
            ...
            if (hasConfidence)
            {
                ...
                if (confThresh > 0.0f)
                {
                    ...
                    if (config.enableBoundaryAwareRetention && boundary_edge_radius > 0)
                    {
                        ...
                        cv::erode(valid_before,
                                  ...
                                  cv::getStructuringElement(
                                      cv::MORPH_ELLIPSE,
                                      cv::Size(2 * boundary_edge_radius + 1, 2 * boundary_edge_radius + 1)));
                    ...
                    const bool hasIndependentGeometryConfidence =
                        evidence && evidence->geometricConfidence.type() == CV_32FC1 &&
...
#pragma omp parallel for schedule(static) reduction(+ : lowConfidenceCandidateCount,                                   \
                        ...
                        for (int u = 0; u < depthMap.cols; ++u)
                        {
                            if (depthRow[u] > 0.0f && confRow[u] < confThresh)
                            {
                                ...
                                const bool retainForBoundary =
                                    ...
                                    confRow[u] >= config.boundaryMinimumConfidence && hasGeometryEvidence &&
            ...
            const std::vector<cv::Mat>* frozenDepthMaps)
        {
...

            {
                    ...
                    !_frameCaches[static_cast<size_t>(refIdx)].sourceViewScores.empty())
                {
                    ...
                    for (int sourceIndex : sourceIndices)
                    {
                        ...
                        details << it->viewIndex << "(tracks=" << it->sharedTracks
            ...
            {
                result.maskSource =
                    referenceLease->validMaskSource.empty() ? "technical" : referenceLease->validMaskSource;
            }
            else
            {
            ...
            if (useRectified && !depthMap.empty())
            {
                ...
                if (!photometricSourceMask.empty())
                {
                    photometricSourceMask =
                        mvs::EpipolarRectifier::unrectifyNearest(photometricSourceMask, rectPair, W, H);
                }
            ...
            const bool sparse_geometry_allows_targeted_recovery = permitsTargetedGapRecovery(
                _effectiveSceneProfile, _effectiveDepthFilterMode, pre_recovery_sparse_residual);
                ...
                !useRectified && srcGrays.size() >= 2 && sparse_geometry_allows_targeted_recovery)
            {
                ...
                recovery_options.minimumCandidateConfidence =
                    std::clamp(config.targetedGapRecoveryConfidence, 0.0f, 1.0f);
                ...
                if (srcGrays.size() >= 4 && config.targetedGapRecoveryHypothesisCount >= 2)
                {
                    ...
                    recovery_options.missingPriorRadiusRatio =
                        std::max(recovery_options.missingPriorRadiusRatio,
                                 recovery_options.maximumConsensusPriorRelativeDifference);
                }
                ...
                if (recovery_target.valid)
                {
                    ...
                    {
                        targeted_gap_stats =
                            mergeMultiHypothesisTargetedDepthGapCandidates(depthMap,
                                                                           confMap,
                                                                           candidate_depths,
            ...
            {
                const int previewOutliers =
                    removeLocalDepthOutliers(depthMap,
                                             confMap,
                                             output_local_kernel,
            ...
            markDepthLossReason(
                missing_reason_map, before_output_filter, depthMap, DepthMissingReason::LocalDepthOutlier);
            ...
            if (!photometricSourceMask.empty())
            {
                ...
                {
                    cv::resize(
                        photometricSourceMask, photometricSourceMask, depthMap.size(), 0.0, 0.0, cv::INTER_NEAREST);
                }
            ...
            result.normalMap =
                normalMap.empty() ? QSharedPointer<cv::Mat>() : QSharedPointer<cv::Mat>::create(normalMap);
            result.supportCount =
        ...
        void DepthMapGenerator::crossCheckDepthConsistency()
        {
            ...
            for (int i = 0; i < NV; ++i)
            {
                ...
                for (int source_ordinal = 0; source_ordinal < static_cast<int>(repairSources.size()); ++source_ordinal)
                {
                    ...
                    emit progressChanged(QStringLiteral("多视一致性：帧 %1/%2，源视图 %3/%4")
                                             .arg(i + 1)
                                             .arg(NV)
                ...
                emit progressChanged(QStringLiteral("多视一致性：帧 %1/%2，选择主深度层").arg(i + 1).arg(NV),
                ...
                emit progressChanged(QStringLiteral("多视一致性：帧 %1/%2，修复内部缺口").arg(i + 1).arg(NV),
                                     static_cast<float>(i) / static_cast<float>(NV));
                ...
                cv::Mat restoration_mask =
                    _depthFrames[i].supportRegionMask && !_depthFrames[i].supportRegionMask->empty()
                        ? *_depthFrames[i].supportRegionMask
                        : cv::Mat(depthI.size(), CV_8UC1, cv::Scalar(255));
                ...
                updateDepthProvenance(
                    ...
                    *_depthFrames[i].crossViewRepairedMask,
                    anchored_interpolation_mask);
                const GeometryEvidenceMaps geometry_evidence =
                    makeGeometryEvidenceMaps(depthI,
                                             consistent_votes,
                                             geometry_source_mask,
                                             source_inverse_depth_sum,
                                             source_inverse_depth_squared_sum);
                ...
                _depthFrames[i].depthCompleteness.consistencyPublicationFallbackApplied =
                ...
                captureStageSnapshot(i,
                                     ...
                                     restoration_confidence,
                                     depthI > 0.0f);
                emit progressChanged(QStringLiteral("多视一致性：已处理 %1/%2，单帧耗时 %3 秒")
                                         .arg(i + 1)
                                         .arg(NV)
        ...
        void DepthMapGenerator::applyLearnedDepthCandidatesAfterConsistency()
        {
            ...
            {
                LOG_WARN(QStringLiteral("[MVS][学习候选] 候选目录不存在，已跳过：%1")
            ...
            for (int frame_index = 0; frame_index < static_cast<int>(_depthFrames.size()); ++frame_index)
            {
                ...
                {
                    frame.confidence =
                        QSharedPointer<cv::Mat>::create(frame.depthMap->size(), CV_32FC1, cv::Scalar(0.0f));
                }
                cv::Mat accepted_mask;
                const LearnedDepthCandidateGateStats stats =
                    gateLearnedDepthCandidate(*frame.depthMap,
                                              *frame.confidence,
                                              candidate_depth,
                                              candidate_confidence,
        ...
        void DepthMapGenerator::recoverResidualDepthAfterConsistency()
        {
            ...
            parallelForRows(
                ...
                [&](int frame_index)
                {
                    ...
                    std::string referenceImageError;
                    MvsImageCache::ImageLease referenceImageLease =
                        acquireImageFrame(frame_index, &referenceImageError);
                    if (!referenceImageLease)
                    {
                    ...
                    cv::minMaxLoc(
                        target.hintDepth, &minimum_hint, &maximum_hint, nullptr, nullptr, target.residualMask);
                    ...
                    for (const std::vector<int>& source_group : source_groups)
                    {
                        ...
                        PatchMatchConfig patch_match =
                            patchMatchConfigForRecordedWorker(_config.patchMatch, frame.device);
                        ...
                        {
                            LOG_WARN(
                                "[MVS][帧 %d][残余重估] PatchMatch 组失败: %s", frame_index, error_message.c_str());
                        }
                    }
                    ...
                    evidence_outputs.rerankMaps =
                        reliability_guided_local_pass ? frame.geometryRerankMaps.data() : nullptr;
            ...
            for (int frame_index = 0; frame_index < view_count; ++frame_index)
            {
                ...
                *frame.confidence = std::move(recovery.confidence);
                ...
                if (frame.inverseDepthMean && frame.inverseDepthMean->size() == frame.depthMap->size())
                {
                    ...
                    cv::divide(recovery.sourceInverseDepthSum,
                               source_count_float,
        ...
        bool DepthMapGenerator::crossCheckDepthConsistencyStreaming()
        {
            ...
            const uint64_t cache_budget = std::max<uint64_t>(largest_frame_bytes * 2ull,
            ...
            for (int frame_index = 0; frame_index < view_count; ++frame_index)
            {
                ...
                const uint64_t consistency_batch_budget = std::max<uint64_t>(
                    ...
                    cache_budget > reference->byteSize() ? cache_budget - static_cast<uint64_t>(reference->byteSize())
                                                         : estimated_source_bytes);
                const float maximum_round_trip_error_pixels = scaleDepthPixelDistance(
                     ...
                     ++source_ordinal)
                {
                    ...
                    emit progressChanged(QStringLiteral("流式多视一致性：帧 %1/%2，源视图 %3/%4")
                                             .arg(frame_index + 1)
                                             .arg(view_count)
                                             .arg(source_ordinal + 1)
                    ...
                    if (is_consistency_source)
                    {
                        ...
                        input.reliabilityWeight =
                            sourceGeometryReliabilityWeight(_depthFrames[frame_index], source_index);
                        input.sourceOrdinal = source_ordinal;
                        const DepthPixelDomainScale source_pixel_scale =
                ...
                else if (_config.enableDepthLayerReliabilityGuidedCorrection)
                {
                    ...
                    guided_depth.setTo(0.0f, repair_mask == 0);
                    cv::Mat guided_confidence =
                        reference->confidence.empty() ? cv::Mat() : reference->confidence.clone();
                    cv::Mat guided_geometry_source_mask = geometry_source_mask.clone();
                    cv::Mat guided_inverse_sum = source_inverse_depth_sum.clone();
                    cv::Mat guided_inverse_squared_sum = source_inverse_depth_squared_sum.clone();
                    cv::Mat guided_votes = consistent_votes.clone();
                ...
                {
                    native_interpolation_anchor_eligibility_mask =
                        cv::Mat(filtered_depth.size(), CV_8UC1, cv::Scalar(0));
                    if (depth_layer_reliability.result.validInputs &&
                        depth_layer_reliability.result.classMap.type() == CV_8UC1 &&
                ...
                if (_config.enablePostConsistencyResidualReestimation &&
                    ...
                    referenceImageLease)
                {
                    emit progressChanged(QStringLiteral("流式多视一致性：帧 %1/%2，残余缺口局部重估")
                                             .arg(frame_index + 1)
                                             .arg(view_count),
                                         static_cast<float>(completed_frames) /
                                             static_cast<float>(std::max(1, view_count)));
                    ...
                    if (residual_target.valid && residual_source_images.size() >= 4)
                    {
                        ...
                        for (const std::vector<int>& source_group : source_groups)
                        {
                            ...
                            for (const int source_ordinal : source_group)
                            {
                                group_images.push_back(
                                    residual_source_images[static_cast<std::size_t>(source_ordinal)]);
                                group_cameras.push_back(
                                    residual_source_cameras[static_cast<std::size_t>(source_ordinal)]);
                        ...
                        if (candidate_depths.size() == 2)
                        {
                            ...
                            evidence_outputs.rerankMaps =
                                reliability_guided_local_pass ? &geometry_rerank_maps : nullptr;
                            ...
                            if (residual_stats.recoveredPixelCount > 0)
                            {
                                if (!reliability_guided_local_pass)
                                {
                                    ...
                                    recovered_inverse_depth_sum.copyTo(source_inverse_depth_sum,
                                                                       residual_reestimated_mask);
                ...
                emit progressChanged(
                    QStringLiteral("流式多视一致性：帧 %1/%2，深度后处理").arg(frame_index + 1).arg(view_count),
                    static_cast<float>(completed_frames) / static_cast<float>(std::max(1, view_count)));
                ...
                postprocess_evidence =
                    updateDepthEvidenceConfidence(_depthFrames[frame_index],
                                                  filtered_depth,
                                                  filtered_confidence,
                ...
                _depthFrames[frame_index].depthPostprocess =
                    postprocessFusionDepthMap(filtered_depth,
                                              filtered_confidence,
                                              fusion_config,
                                              frame_index,
                                              view_count,
                ...
                const GeometryEvidenceMaps geometry_evidence =
                    makeGeometryEvidenceMaps(filtered_depth,
                                             ...
                                             source_inverse_depth_sum,
                                             source_inverse_depth_squared_sum);
                const cv::Mat discrete_support_region = _depthFrames[frame_index].supportRegionMask
                                                            ? *_depthFrames[frame_index].supportRegionMask
                                                            : cv::Mat();
                const DiscreteGeometryCoreSummary discrete_summary =
                ...
                emit progressChanged(
                    QStringLiteral("流式多视一致性：帧 %1/%2，写入结果").arg(frame_index + 1).arg(view_count),
                    static_cast<float>(completed_frames) / static_cast<float>(std::max(1, view_count)));
                ...
                const QString filtered_adaptive_support_weight_path =
                    generate_adaptive_evidence
                        ? storage_dir.filePath(
                              QStringLiteral("depth_%1_adaptive_geometry_support_weight_consistency.bin")
                                  .arg(frame_index))
                        : QString();
                ...
                const QString filtered_adaptive_conflict_ratio_path =
                    generate_adaptive_evidence
                        ? storage_dir.filePath(
                              QStringLiteral("depth_%1_adaptive_geometry_conflict_ratio_consistency.bin")
                                  .arg(frame_index))
                        : QString();
                const QString filtered_cross_view_repaired_mask_path = storage_dir.filePath(
                    QStringLiteral("depth_%1_cross_view_repaired_consistency.bin").arg(frame_index));
                const QString filtered_residual_reestimated_mask_path = storage_dir.filePath(
                    QStringLiteral("depth_%1_residual_reestimated_consistency.bin").arg(frame_index));
                ...
                const xjw::common::OperationResult inverse_spread_write_result =
                    xjw::core::project::writeDepthMatStorage(filtered_inverse_depth_spread_path,
                                                             geometry_evidence.inverseDepthRelativeSpread);
                if (!source_mask_write_result.ok || !inverse_mean_write_result.ok || !inverse_spread_write_result.ok)
                {
                    ...
                    const QString message =
                        !source_mask_write_result.ok
                            ? source_mask_write_result.errorMessage
                            : (!inverse_mean_write_result.ok ? inverse_mean_write_result.errorMessage
                ...
                auto write_optional_pixel_map =
                    [](const QString& path, const QSharedPointer<cv::Mat>& matrix) -> xjw::common::OperationResult
                {
                ...
                emit progressChanged(QStringLiteral("多视一致性：已处理 %1/%2").arg(completed_frames).arg(view_count),
                                     ratio);
            ...
            for (const PendingDepthReplacement& replacement : pending_replacements)
            {
                ...
                if (!replacement.filteredConfidencePath.isEmpty())
                {
                        ...
                        !QFile::rename(replacement.originalConfidencePath, confidence_backup))
                    {
                        remove_pending_files();
                        emit errorOccurred(
                            QStringLiteral("无法备份原始置信度缓存：%1").arg(replacement.originalConfidencePath));
                        return false;
                    }
                ...
                {
                    remove_pending_files();
                    emit errorOccurred(QStringLiteral("无法备份原始深度缓存：%1").arg(replacement.originalPath));
                    return false;
                }
                if (!QFile::rename(replacement.filteredPath, replacement.originalPath))
                {
                    ...
                    remove_pending_files();
                    emit errorOccurred(QStringLiteral("无法提交一致性过滤深度缓存：%1").arg(replacement.originalPath));
                    return false;
                }
                QFile::remove(backup_path);
                    ...
                    filtered_depth.size() != replacement.expectedDepthSize)
                {
                    ...
                    const QString detail =
                        !load_result.ok ? load_result.errorMessage : QStringLiteral("类型或尺寸无效");
                ...
                else
                {
                        ...
                        photometric_source_mask.size() != filtered_depth.size())
                    {
                        ...
                        emit errorOccurred(
                            QStringLiteral("流式一致性无法恢复帧 %1 的 PatchMatch 光度来源掩码：%2（%3）")
                                .arg(replacement.frameIndex)
                    ...
                    if (replacement.targetedGapRecoveredMaskExpected)
                    {
                        ...
                        {
                            remove_pending_files();
                            emit errorOccurred(QStringLiteral("流式一致性提交帧 %1 时无法恢复权威定向恢复掩码：%2")
                                                   .arg(replacement.frameIndex)
                                                   .arg(targeted_recovered_path));
                            return false;
                        }
                    ...
                    const xjw::common::OperationResult provenance_result = load_optional_pixel_map(
                        replacement.filteredDepthProvenancePath, artifact_result.depthProvenance);
                    const xjw::common::OperationResult missing_reason_result = load_optional_pixel_map(
                        replacement.filteredMissingReasonPath, artifact_result.missingReasonMap);
                    if (!repaired_mask_result.ok || !residual_mask_result.ok || !provenance_result.ok ||
                        !missing_reason_result.ok)
                    {
                        const QString message =
                    ...
                    const xjw::common::OperationResult geometry_support_result =
                        xjw::core::project::loadDepthMatStorage(replacement.filteredGeometrySupportPath,
                                                                &geometry_support);
                    ...
                    if (!replacement.filteredAdaptiveSupportWeightPath.isEmpty())
                    {
                        ...
                        const xjw::common::OperationResult adaptive_view_result =
                            xjw::core::project::loadDepthMatStorage(replacement.filteredAdaptiveEffectiveViewCountPath,
                    ...
                    if (!saveDepthFrameArtifacts(
                            replacement.frameIndex, artifact_result, QStringLiteral("一致性过滤后")))
                    {
                        remove_pending_files();
                        return false;
                    }
        ...
        {
            if (frameIndex < 0 || frameIndex >= static_cast<int>(_views.size()) || !result.success ||
                !result.depthMap || result.depthMap->empty())
            {
                return true;
            }
            ...
            const GeometrySourceOrdinalContract geometry_source_contract = validateGeometrySourceOrdinalContract(
                result.geometrySourceMask && !result.geometrySourceMask->empty() ? *result.geometrySourceMask
                                                                                 : cv::Mat(),
            ...
            if (saveRawDepth && result.confidence && !result.confidence->empty())
            {
                ...
                {
                    LOG_WARN(
                        QStringLiteral("[MVS] 保存%1置信图失败: %2").arg(stageLabel, QString::fromStdString(saveErr)));
                }
                else
                {
            ...
            if (saveRawDepth && !supportMask.empty())
            {
                ...
                {
                    LOG_WARN(
                        QStringLiteral("[MVS] 保存%1深度缺失原因诊断失败: frame=%2").arg(stageLabel).arg(frameIndex));
                }
            }
            ...
            {
                geometrySourceMaskSaved = save_geometry_evidence(result.geometrySourceMask,
                                                                 CV_16UC1,
            ...
            if (saveRawDepth && !targeted_gap_recovered_mask_expected)
            {
                ...
                {
                    const QString message = QStringLiteral("帧 %1 当前无定向恢复证据，但无法移除旧掩码：%2")
                                                .arg(frameIndex)
                                                .arg(stale_path);
                    LOG_ERROR(QStringLiteral("[MVS] %1").arg(message));
                    markManifestFrameFailed(frameIndex, message);
            ...
            if (saveRawDepth && result.residualReestimatedMask && !result.residualReestimatedMask->empty())
            {
                ...
                if (recovered_mask.size() == result.depthMap->size())
                {
                    ...
                    residualReestimatedMaskSaved =
                        xjw::common::io::writeImage(residualReestimatedMaskPath, recovered_mask);
                }
            }
            ...
            if (_config.saveIntermediatePyramidLevels && saveRawDepth)
            {
                for (const DepthLevelResult& level : result.intermediatePyramidLevels)
                {
                    ...
                    const std::string level_prefix = raw_directory + "/depth_" + std::to_string(frameIndex) +
                                                     "_level_" + std::to_string(level.level);
            ...
            if (previewSaved && rawSaved && savePreviewPng)
            {
                ...
                for (auto it = adaptiveEvidenceDiagnostics.constBegin(); it != adaptiveEvidenceDiagnostics.constEnd();
                     ++it)
                {
                    ...
                    if (level == 1)
                    {
                        ...
                        level_object.insert(QStringLiteral("targeted_gap_recovered_mask_path"),
                                            targetedGapRecoveredMaskSaved
                                                ...
                                                : QString());
                        level_object.insert(QStringLiteral("residual_reestimated_mask_path"),
                                            residualReestimatedMaskSaved
                                                ? QString::fromStdString(residualReestimatedMaskPath)
                                                : QString());
                        level_object.insert(QStringLiteral("depth_provenance_path"),
                                            depthProvenanceSaved ? QString::fromStdString(depthProvenancePath)
                                                                 : QString());
                ...
                const QString acceptance =
                    QString::fromLatin1(depthFrameAcceptanceId(result.qualityDecision.acceptance));
                ...
                artifact[QStringLiteral("valid_mask_path")] =
                    maskSaved ? QString::fromStdString(validMaskPath) : QString();
                ...
                if (final_artifacts)
                {
                    emit depthMapSaved(QString::fromStdString(pngPath),
        ...
        void DepthMapGenerator::captureStageSnapshot(int frameIndex,
        ...
        void DepthMapGenerator::runDepthPoseRefinementCandidateStage(bool residentDepthFrames)
        {
            ...
            if (residentDepthFrames)
            {
                ...
                for (int frame_index = 0; frame_index < static_cast<int>(_depthFrames.size()); ++frame_index)
                {
                    ...
                    frame.adaptiveSupportWeight = depth_frame.adaptiveGeometrySupportWeight
                    ...
                    frame.adaptiveEffectiveViewCount = depth_frame.adaptiveGeometryEffectiveViewCount
                                                           ...
                                                           : cv::Mat();
                    frame.adaptiveConflictRatio = depth_frame.adaptiveGeometryConflictRatio
...

            {
                ...
                for (const DepthPoseRefinementCandidate& candidate : stage.candidates)
                {
                    ...
                    const QJsonObject derived_camera = frame.derivedCameraModel.isValid()
            ...
            LOG_INFO(QStringLiteral("[MVS] 深度约束位姿细化候选完成: accepted=%1/%2 mode=candidate_only; "
                                    "项目相机与本轮深度均未修改")
        ...
        void detail::runDepthMapBackgroundTaskWithExceptionBoundary(
            const std::function<void()>& task, const std::function<void(const QString&)>& failureHandler)
        {
            try
            {
                task();
        ...
        void DepthMapGenerator::runInBackground()
        {
            detail::runDepthMapBackgroundTaskWithExceptionBoundary([this]() { runInBackgroundImpl(); },
                                                                   [this](const QString& message)
                                                                   {
                                                                       ...
                                                                       LOG_ERROR(
                                                                           QStringLiteral("[MVS] %1").arg(message));
                                                                       emit errorOccurred(message);
                                                                       emitFinishedOnce(false);
                                                                   });
        }
        ...
        void DepthMapGenerator::runInBackgroundImpl()
        {
            ...
            for (const OpenClDeviceInfo& device : detectedOpenClDevices)
            {
                ...
                {
                    LOG_DEBUG(
                        QStringLiteral("[MVS] 跳过与已选 CUDA 设备重复的 OpenCL 接口: index=%1 device=%2 identity=%3")
                            .arg(device.index)
            ...
            const bool heterogeneousAuto = configuredBackend == PatchMatchBackend::Auto && automaticAcceleration &&
                                           cudaAvailable && openClAvailable;
            ...
            emit progressChanged(QStringLiteral("读取 %1 张影像头部并规划内存...").arg(NV), 0.0f);
            ...
            if (_config.runFusion)
            {
                const QString error = QStringLiteral(
                    "scene-wide recovered 深度与融合必须分阶段执行；请先保存深度图，再调用现有流式融合阶段");
                LOG_ERROR(QStringLiteral("[MVS] %1").arg(error));
                emit errorOccurred(error);
                emitFinishedOnce(false);
                return;
            ...
            const auto cuda_worker = std::find_if(physicalAcceleratorWorkers.begin(),
            ...
            emit progressChanged(QStringLiteral("执行 recovered scene-wide CUDA PatchMatch 与三层投票..."), 0.05f);
            ...
            for (RecoveredDepthFrame& recovered_frame : recovered_result.frames)
            {
                ...
                frame.depthCompleteness.finalMetrics =
                    analyzeDepthCompleteness(*frame.depthMap, *frame.supportRegionMask);
                ...
                emit depthMapReady(_depthFrames[static_cast<std::size_t>(frame_index)]);
                emit progressChanged(QStringLiteral("recovered 深度图：帧 %1/%2").arg(frame_index + 1).arg(NV),
            ...
            emit progressChanged(QStringLiteral("完成"), 1.0f);
