...
namespace xjw::mvs
{
    ...
    namespace
    {
        ...
        static_assert(sizeof(StageDepthMatHeader) == 40, "Stage snapshots must use the stable fast-depth layout");

        QString stageKey(int reference_index, MvsStageSnapshotStage stage)
        {
            return QStringLiteral("%1:%2").arg(reference_index).arg(static_cast<int>(stage));
        }

        int stageOrder(const QJsonObject& record)
        {
            const QString stage = record.value(QStringLiteral("stage")).toString();
            if (stage == QStringLiteral("patchmatch_output"))
                return 0;
            if (stage == QStringLiteral("cross_view_consistency"))
                return 1;
            if (stage == QStringLiteral("confidence_postprocess"))
                return 2;
            if (stage == QStringLiteral("final_admission"))
                return 3;
            return 4;
        }
        ...
        bool writeMatrixAtomically(const QString& path, const cv::Mat& matrix, QString* error_message)
        {
            if (matrix.empty())
            {
                if (error_message)
                    *error_message = QStringLiteral("矩阵为空");
                return false;
            }
        ...
        cv::Size boundedSize(const cv::Size& input, int maximum_long_edge)
        {
            ...
            const double scale = static_cast<double>(maximum_long_edge) / static_cast<double>(long_edge);
        ...
        QJsonObject artifactToJson(const QString& path, const cv::Mat& matrix)
        {
            ...
            return QJsonObject{{QStringLiteral("path"), info.absoluteFilePath()},
    ...
    MvsStageSnapshotRecorder::MvsStageSnapshotRecorder(const DepthGenConfig& config, int view_count)
        : _directory(QString::fromStdString(config.stageSnapshotDirectory)),
          _maximumLongEdge(config.stageSnapshotMaximumLongEdge), _budgetBytes(config.stageSnapshotBudgetBytes)
    {
        for (int reference_index : config.stageSnapshotReferenceIndices)
        {
            ...
            {
                _errors.append(QStringLiteral("reference_index_out_of_range:%1").arg(reference_index));
            }
        }
        if (_selectedReferences.empty())
            return;
        ...
        {
            _initializationError = QStringLiteral("阶段快照目录不可用或 manifest 已存在：%1").arg(_directory);
            return;
        }
        _enabled = true;
        QString error;
    ...
    void MvsStageSnapshotRecorder::capture(int reference_index,
                                           ...
                                           const cv::Mat& valid_mask) noexcept
    {
        try
        {
            ...
            if (!_enabled || !_selectedReferences.contains(reference_index) || _recordedStageKeys.contains(key))
            {
                return;
            }
            _recordedStageKeys.insert(key);

            QJsonObject record{{QStringLiteral("ref_index"), reference_index},
                               {QStringLiteral("stage"), mvsStageSnapshotStageId(stage)},
                               {QStringLiteral("boundary"), boundary.isEmpty() ? boundaryForStage(stage) : boundary}};
            if (depth.empty() || confidence.empty() || depth.type() != CV_32FC1 || confidence.type() != CV_32FC1 ||
                confidence.size() != depth.size())
            {
                record.insert(QStringLiteral("status"), QStringLiteral("unavailable"));
                record.insert(QStringLiteral("reason"), QStringLiteral("missing_or_incompatible_depth_confidence"));
            ...
            if (result.depthLayerReliabilityClass && result.depthLayerReliabilityClass->type() == CV_8UC1 &&
                result.depthLayerReliabilityClass->size() == depth.size())
            {
                snapshot_reliability = resizedNearest(*result.depthLayerReliabilityClass, snapshot_size);
            }
            cv::Mat snapshot_photometric_confidence;
            if (result.photometricConfidence && result.photometricConfidence->type() == CV_32FC1 &&
                result.photometricConfidence->size() == depth.size())
            {
                snapshot_photometric_confidence = resizedNearest(*result.photometricConfidence, snapshot_size);
            }
            cv::Mat snapshot_geometric_confidence;
            if (result.geometricConfidence && result.geometricConfidence->type() == CV_32FC1 &&
                result.geometricConfidence->size() == depth.size())
            {
                snapshot_geometric_confidence = resizedNearest(*result.geometricConfidence, snapshot_size);
            }
            cv::Mat snapshot_geometry_rerank;
            if (stage == MvsStageSnapshotStage::CrossViewConsistency && result.geometryRerankMaps &&
                result.geometryRerankMaps->compatible(depth.size()))
            {
                cv::Mat source_count;
                cv::Mat baseline_count;
                cv::Mat decision_action;
                result.geometryRerankMaps->supportingSourceCount.convertTo(source_count, CV_32FC1);
                result.geometryRerankMaps->baselineSectorCount.convertTo(baseline_count, CV_32FC1);
                result.geometryRerankMaps->decisionAction.convertTo(decision_action, CV_32FC1);
                cv::Mat geometry_rerank;
                cv::merge(std::vector<cv::Mat>{result.geometryRerankMaps->nativeCost,
                                               ...
                                               decision_action},
                          geometry_rerank);
                snapshot_geometry_rerank = resizedNearest(geometry_rerank, snapshot_size);
            }
            ...
            const std::uint64_t required_bytes =
                ...
                (snapshot_photometric_confidence.empty()
                     ...
                     : sizeof(StageDepthMatHeader) +
                           static_cast<std::uint64_t>(snapshot_photometric_confidence.total() *
                                                      snapshot_photometric_confidence.elemSize())) +
                (snapshot_geometric_confidence.empty()
                     ? 0ull
                     : sizeof(StageDepthMatHeader) +
                           static_cast<std::uint64_t>(snapshot_geometric_confidence.total() *
                                                      snapshot_geometric_confidence.elemSize())) +
                (snapshot_reliability.empty()
                     ? 0ull
                     : sizeof(StageDepthMatHeader) +
                           static_cast<std::uint64_t>(snapshot_reliability.total() * snapshot_reliability.elemSize())) +
                (snapshot_geometry_rerank.empty()
                     ? 0ull
                     : sizeof(StageDepthMatHeader) + static_cast<std::uint64_t>(snapshot_geometry_rerank.total() *
            ...
            const QString frame_directory =
                QDir(_directory).filePath(QStringLiteral("ref_%1").arg(reference_index, 4, 10, QLatin1Char('0')));
            ...
            const QString photometric_confidence_path = prefix + QStringLiteral("_photometric_confidence.bin");
            const QString geometric_confidence_path = prefix + QStringLiteral("_geometric_confidence.bin");
            const QString mask_path = prefix + QStringLiteral("_valid_mask.bin");
            const QString reliability_path = prefix + QStringLiteral("_depth_layer_reliability.bin");
            const QString geometry_rerank_path = prefix + QStringLiteral("_geometry_rerank.bin");
            QString write_error;
            const bool depth_ok = writeMatrixAtomically(depth_path, snapshot_depth, &write_error);
            const bool confidence_ok =
                depth_ok && writeMatrixAtomically(confidence_path, snapshot_confidence, &write_error);
            const bool mask_ok = confidence_ok && writeMatrixAtomically(mask_path, snapshot_mask, &write_error);
            const bool photometric_confidence_ok =
                mask_ok &&
                (snapshot_photometric_confidence.empty() ||
                 writeMatrixAtomically(photometric_confidence_path, snapshot_photometric_confidence, &write_error));
            const bool geometric_confidence_ok =
                photometric_confidence_ok &&
                (snapshot_geometric_confidence.empty() ||
                 writeMatrixAtomically(geometric_confidence_path, snapshot_geometric_confidence, &write_error));
            const bool reliability_ok = geometric_confidence_ok &&
                                        (snapshot_reliability.empty() ||
                                         writeMatrixAtomically(reliability_path, snapshot_reliability, &write_error));
            const bool geometry_rerank_ok =
                reliability_ok && (snapshot_geometry_rerank.empty() ||
                                   writeMatrixAtomically(geometry_rerank_path, snapshot_geometry_rerank, &write_error));
            if (!depth_ok || !confidence_ok || !mask_ok || !photometric_confidence_ok || !geometric_confidence_ok ||
            ...
            record.insert(QStringLiteral("confidence"), artifactToJson(confidence_path, snapshot_confidence));
            if (!snapshot_photometric_confidence.empty())
            {
                record.insert(QStringLiteral("photometric_confidence"),
                              artifactToJson(photometric_confidence_path, snapshot_photometric_confidence));
            }
            if (!snapshot_geometric_confidence.empty())
            {
                record.insert(QStringLiteral("geometric_confidence"),
                              artifactToJson(geometric_confidence_path, snapshot_geometric_confidence));
            }
            record.insert(QStringLiteral("valid_mask"), artifactToJson(mask_path, snapshot_mask));
            if (!snapshot_reliability.empty())
            {
                record.insert(QStringLiteral("depth_layer_reliability"),
                              artifactToJson(reliability_path, snapshot_reliability));
            }
            if (!snapshot_geometry_rerank.empty())
            {
                QJsonObject artifact = artifactToJson(geometry_rerank_path, snapshot_geometry_rerank);
                artifact.insert(QStringLiteral("channel_order"),
                                QJsonArray{QStringLiteral("native_cost"),
        ...
        {
            std::lock_guard<std::mutex> lock(_mutex);
            _errors.append(QStringLiteral("capture_exception:%1").arg(QString::fromUtf8(error.what())));
            QString ignored;
    ...
    bool MvsStageSnapshotRecorder::writeManifestLocked(QString* error_message)
    {
        ...
        std::sort(records.begin(),
                  ...
                  [](const QJsonObject& left, const QJsonObject& right)
                  {
                      ...
                      return left_ref != right_ref ? left_ref < right_ref : stageOrder(left) < stageOrder(right);
                  });
        ...
        for (int reference_index : _selectedReferences)
            selected_array.append(reference_index);
        QJsonArray stage_array;
        for (int stage = 0; stage < 4; ++stage)
        {
            stage_array.append(mvsStageSnapshotStageId(static_cast<MvsStageSnapshotStage>(stage)));
        }
        const QJsonObject manifest{{QStringLiteral("schema"), QStringLiteral("plascan_mvs_stage_snapshots_v1")},
        ...
        file.setDirectWriteFallback(false);
        if (!file.open(QIODevice::WriteOnly) ||
            file.write(QJsonDocument(manifest).toJson(QJsonDocument::Indented)) < 0 || !file.commit())
        {
            if (error_message)
            {
                *error_message = QStringLiteral("无法原子写入阶段快照 manifest：%1").arg(_manifestPath);
            }
            return false;
        }
        return true;
        ...
        try
        {
            ...
            for (int reference_index : _selectedReferences)
            {
                for (int stage_index = 0; stage_index < 4; ++stage_index)
                {
                    ...
                    const QString key = stageKey(reference_index, stage);
                    if (_recordedStageKeys.contains(key))
                        continue;
                    _recordedStageKeys.insert(key);
                    _records.push_back(QJsonObject{{QStringLiteral("ref_index"), reference_index},
                                                   {QStringLiteral("stage"), mvsStageSnapshotStageId(stage)},
