...
{
    return CpuNormal{invK[0] * static_cast<float>(col) + invK[1], invK[2] * static_cast<float>(row) + invK[3], 1.f};
}
...
CpuNormal cpuRotateNormal(const CpuNormal& normal, float a1, float a2, float a3)
{
    ...
    CpuNormal out{R[0] * normal[0] + R[1] * normal[1] + R[2] * normal[2],
...
inline float cpuPropagateDepth(
    float depth, const CpuNormal& normal, float row1, float col1, float row2, float col2, const float invK[4])
{
    const CpuNormal X1{depth * (invK[0] * col1 + invK[1]), depth * (invK[2] * row1 + invK[3]), depth};
    const CpuNormal ray2{invK[0] * col2 + invK[1], invK[2] * row2 + invK[3], 1.f};
...
CpuNormal
cpuLocalSurfaceNormal(const cv::Mat& depth_map, int row, int col, const float invK[4], const CpuNormal& fallback)
{
    if (row <= 0 || row + 1 >= depth_map.rows || col <= 0 || col + 1 >= depth_map.cols)
    {
        return fallback;
    }
    ...
    if (!(left_depth > 0.0f) || !(right_depth > 0.0f) || !(top_depth > 0.0f) || !(bottom_depth > 0.0f))
    {
        return fallback;
    }
    auto point = [invK](int point_row, int point_col, float point_depth)
    {
        return CpuNormal{point_depth * (invK[0] * static_cast<float>(point_col) + invK[1]),
    ...
    const CpuNormal horizontal{right[0] - left[0], right[1] - left[1], right[2] - left[2]};
    const CpuNormal vertical{bottom[0] - top[0], bottom[1] - top[1], bottom[2] - top[2]};
    CpuNormal normal{horizontal[1] * vertical[2] - horizontal[2] * vertical[1],
...
inline float cpuBilinear(const cv::Mat& image, float u, float v)
{
    ...
    return (1.f - fy) * ((1.f - fx) * v00 + fx * v10) + fy * ((1.f - fx) * v01 + fx * v11);
}
...
inline bool cpuBilinearMaskValid(const cv::Mat& mask, float u, float v)
{
    ...
    if (u < 0.f || v < 0.f || u >= static_cast<float>(mask.cols - 1) || v >= static_cast<float>(mask.rows - 1))
    {
        return false;
    }
    ...
    return mask.at<std::uint8_t>(row, column) != 0 && mask.at<std::uint8_t>(row, column + 1) != 0 &&
           mask.at<std::uint8_t>(row + 1, column) != 0 && mask.at<std::uint8_t>(row + 1, column + 1) != 0;
}

bool cpuReferenceGradient(const cv::Mat& image, const cv::Mat& mask, int x, int y, float* gradientX, float* gradientY)
{
    if (!gradientX || !gradientY || x <= 0 || y <= 0 || x + 1 >= image.cols || y + 1 >= image.rows)
    {
        return false;
    }
    if (!mask.empty() && (mask.at<std::uint8_t>(y, x - 1) == 0 || mask.at<std::uint8_t>(y, x + 1) == 0 ||
                          mask.at<std::uint8_t>(y - 1, x) == 0 || mask.at<std::uint8_t>(y + 1, x) == 0))
    {
        return false;
    }
...
bool cpuSourceGradient(const cv::Mat& image, const cv::Mat& mask, float x, float y, float* gradientX, float* gradientY)
{
    if (!gradientX || !gradientY || !cpuBilinearMaskValid(mask, x - 1.0f, y) ||
        !cpuBilinearMaskValid(mask, x + 1.0f, y) || !cpuBilinearMaskValid(mask, x, y - 1.0f) ||
...
void cpuComposeHomography(
    int row, int col, float depth, const CpuNormal& normal, const float* srcData, const float invK[4], float H[9])
{
    ...
    float dist = depth * (normal[0] * (invK[0] * static_cast<float>(col) + invK[1]) +
                          normal[1] * (invK[2] * static_cast<float>(row) + invK[3]) + normal[2]);
    ...
    H[8] = R[8] + invK[1] * (R[6] + iN0 * T[2]) + invK[3] * (R[7] + iN1 * T[2]) + iN2 * T[2];
}
                              ...
                              float minimumMaskedSupportRatio)
{
    ...
    const float center_z = H[6] * static_cast<float>(refCol) + H[7] * static_cast<float>(refRow) + H[8];
    const float center_u =
        std::fabs(center_z) > 1.0e-6f
            ? (H[0] * static_cast<float>(refCol) + H[1] * static_cast<float>(refRow) + H[2]) / center_z
            : -1.0f;
    const float center_v =
        std::fabs(center_z) > 1.0e-6f
            ? (H[3] * static_cast<float>(refCol) + H[4] * static_cast<float>(refRow) + H[5]) / center_z
            : -1.0f;
    const bool center_valid = center_u >= 0.0f && center_v >= 0.0f && cpuBilinearMaskValid(srcMask, center_u, center_v);
    const float reference_center = refImage.at<float>(refRow, refCol);
    const float source_center = center_valid ? cpuBilinear(srcImage, center_u, center_v) : -1.0f;
    ...
    for (int dv = -patchHalf; dv <= patchHalf; ++dv)
    {
        for (int du = -patchHalf; du <= patchHalf; ++du)
        {
            ...
            const bool gradient_valid =
                cpuReferenceGradient(refImage, refMask, pu, pv, &reference_gradient_x, &reference_gradient_y) &&
                cpuSourceGradient(srcImage, srcMask, srcU, srcV, &source_gradient_x, &source_gradient_y);
            accumulator.addGradientCandidate(
                gradient_valid, reference_gradient_x, reference_gradient_y, source_gradient_x, source_gradient_y);
            accumulator.addCensusCandidate(
                center_valid && source_center >= 0.0f, refValue - reference_center, srcValue - source_center);
        }
    }
...
float cpuRoundTripReprojectionError(
    int col, int row, float depth, const cv::Mat& source_depth, const float source_data[16], const float invK[4])
{
    ...
    const float z = depth;
    const float source_x = source_data[4] * x + source_data[5] * y + source_data[6] * z + source_data[13];
    const float source_y = source_data[7] * x + source_data[8] * y + source_data[9] * z + source_data[14];
    const float source_z = source_data[10] * x + source_data[11] * y + source_data[12] * z + source_data[15];
    if (!(source_z > 0.0f))
    {
        return -1.0f;
    }
    ...
    if (source_u < 0.0f || source_v < 0.0f || source_u > static_cast<float>(source_depth.cols - 1) ||
    ...
    const float round_trip_x =
        source_data[4] * translated_x + source_data[7] * translated_y + source_data[10] * translated_z;
    const float round_trip_y =
        source_data[5] * translated_x + source_data[8] * translated_y + source_data[11] * translated_z;
    const float round_trip_z =
        source_data[6] * translated_x + source_data[9] * translated_y + source_data[12] * translated_z;
                                 ...
                                 const PatchMatchConfig& config)
{
    ...
    const int source_count = std::min(static_cast<int>(srcImages.size()), kMaxPatchMatchSourceViews);
    for (int srcIndex = 0; srcIndex < source_count; ++srcIndex)
    {
        const float* srcData = srcDatas.data() + srcIndex * 16;
        const float ncc = cpuComputeHomographyNcc(col,
                                                  row,
                                                  depth,
                                                  normal,
                                                  refImage,
    ...
    if (!sourceDepths.empty() && config.geometricGuidanceWeight > 0.0f && config.geometricGuidanceMaxErrorPixels > 0.0f)
    {
        ...
        for (int source_index = 0; source_index < source_count; ++source_index)
        {
            ...
            const float error = cpuRoundTripReprojectionError(col,
                                                              row,
                                                              depth,
            ...
            normalized_error_sum += std::min(error / config.geometricGuidanceMaxErrorPixels, 1.0f);
            ++geometric_support;
        }
        if (geometric_support > 0)
        {
            result.objective +=
                config.geometricGuidanceWeight * normalized_error_sum / static_cast<float>(geometric_support);
        }
    }
    return result;
}
...
void cpuParallelForLinesUnchecked(int lineCount, int workerCount, const std::atomic<bool>* cancelFlag, Fn&& fn)
{
    if (lineCount <= 0)
    {
        return;
    ...
    xjw::common::concurrency::runWorkerGroup(static_cast<std::size_t>(workerCount),
...
template <typename Fn>
bool cpuParallelForLines(
    int lineCount, int workerCount, const std::atomic<bool>* cancelFlag, std::string* errorMsg, Fn&& fn)
{
    try
    {
        cpuParallelForLinesUnchecked(lineCount, workerCount, cancelFlag, std::forward<Fn>(fn));
        if (cancelFlag && cancelFlag->load(std::memory_order_relaxed))
        {
            if (errorMsg)
            {
                                           ...
                                           PatchMatchAuxiliaryOutput* auxiliaryOutput)
{
    ...
    cv::Mat refScaled;
    cv::resize(refGray, refScaled, cv::Size(std::max(1, refW / ds), std::max(1, refH / ds)), 0, 0, cv::INTER_AREA);
    ...
    if (!hintScaled.empty() && hintRadius && !hintRadius->empty())
    {
        ...
        {
            cv::resize(*hintRadius, hintRadiusScaled, cv::Size(W, H), 0, 0, cv::INTER_NEAREST);
        }
    }
    ...
    auto depthBounds = [&](int row, int column)
    {
        ...
        if (!hintScaled.empty())
        {
            ...
            {
                float radius = !hintRadiusScaled.empty() ? hintRadiusScaled.at<float>(row, column) : 0.0f;
                if (!std::isfinite(radius) || radius <= 0.0f)
                {
                    radius = std::max(center * kDefaultPatchMatchHintRadiusRatio, (zFar - zNear) * 0.01f);
                }
    ...
    const float invK[4] = {1.f / refCamS.focalX,
    ...
    {
        const cv::Mat depth_snapshot = config.enableAsymmetricPropagation ? depthS.clone() : cv::Mat();
        return cpuParallelForLines(W, cpuThreadCount, config.cancelFlag,
            errorMsg,
            [&](int col)
            {
                ...
                for (int row = rowStart; row != rowEnd; row += rowStep)
                {
                    ...
                    if (row != rowStart)
                    {
                        propDepth = cpuPropagateDepth(prevDepth,
                                                      prevNormal,
                                                      static_cast<float>(prevRow),
                    ...
                    float randDepth = clampDepth(row, col, cpuPerturbDepth(perturbation, boundedCurrDepth, rng));
                    CpuNormal randNormal =
                        cpuPerturbNormal(row, col, perturbation * static_cast<float>(M_PI), currNormal, invK, rng);

                    const std::array<float, 5> dep{boundedCurrDepth, propDepth, randDepth, boundedCurrDepth, randDepth};
                    const std::array<CpuNormal, 5> nor{{currNormal, prevNormal, randNormal, randNormal, currNormal}};

                    const float cached_ncc = confS.at<float>(idxRow, col);
                    float bestCost = cached_ncc >= 0.0f ? objectiveS.at<float>(idxRow, col) : 2.0f;
                    float bestNcc = std::max(0.0f, cached_ncc);
                    std::uint32_t bestMask = static_cast<std::uint32_t>(sourceMaskS.at<std::int32_t>(idxRow, col));
                    const std::uint32_t neighborMask =
                        bestMask | static_cast<std::uint32_t>(sourceMaskS.at<std::int32_t>(prevRow, col));
                    int bestIndex = 0;
                    ...
                    for (int hi = first_hypothesis; hi < 5; ++hi)
                    {
                        ...
                        {
                            duplicate = dep[hi] == dep[previous] && nor[hi][0] == nor[previous][0] &&
                                        nor[hi][1] == nor[previous][1] && nor[hi][2] == nor[previous][2];
                            if (duplicate)
                            {
                                break;
                            }
                        ...
                        const CpuHypothesisCost cost = cpuEvalHypCost(col,
                                                                      row,
                                                                      dep[hi],
                                                                      nor[hi],
                                                                      refF,
                    ...
                    if (config.enableAsymmetricPropagation)
                    {
                        ...
                        for (const int distance : propagation_distances)
                        {
                            ...
                            const cv::Vec3f candidate_vector = normalS.at<cv::Vec3f>(candidate_row, col);
                            const CpuNormal candidate_normal{
                                candidate_vector[0], candidate_vector[1], candidate_vector[2]};
                            const float propagated_depth =
                                clampDepth(row,
                                           col,
                            ...
                            const std::uint32_t candidate_mask =
                                neighborMask |
                                static_cast<std::uint32_t>(sourceMaskS.at<std::int32_t>(candidate_row, col));
                            const CpuHypothesisCost candidate_cost =
                                cpuEvalHypCost(col,
                                               row,
                                               propagated_depth,
                                               candidate_normal,
                        ...
                        const CpuNormal local_normal =
                            cpuLocalSurfaceNormal(depth_snapshot, row, col, invK, chosenNormal);
                        const CpuHypothesisCost local_cost = cpuEvalHypCost(col,
                                                                            row,
                                                                            chosenDepth,
                                                                            local_normal,
                                                                            refF,
                    ...
                    normalS.at<cv::Vec3f>(idxRow, col) = cv::Vec3f(chosenNormal[0], chosenNormal[1], chosenNormal[2]);
                    confS.at<float>(idxRow, col) = bestNcc;
                    objectiveS.at<float>(idxRow, col) = bestCost;
                    sourceMaskS.at<std::int32_t>(idxRow, col) = static_cast<std::int32_t>(bestMask);
    ...
    {
        const cv::Mat depth_snapshot = config.enableAsymmetricPropagation ? depthS.clone() : cv::Mat();
        return cpuParallelForLines(H, cpuThreadCount, config.cancelFlag,
            errorMsg,
            [&](int row)
            {
                ...
                for (int col = colStart; col != colEnd; col += colStep)
                {
                    ...
                    float bestCost = cached_ncc >= 0.0f ? objectiveS.at<float>(row, col) : 2.0f;
                    float bestNcc = std::max(0.0f, cached_ncc);
                    std::uint32_t bestMask = static_cast<std::uint32_t>(sourceMaskS.at<std::int32_t>(row, col));
                    const std::uint32_t neighborMask =
                        bestMask | static_cast<std::uint32_t>(sourceMaskS.at<std::int32_t>(row, prevCol));
                    int bestIndex = 0;
                    const int first_hypothesis = cached_ncc >= 0.0f ? 1 : 0;
                    for (int hi = first_hypothesis; hi < 5; ++hi)
                    {
                    ...
                    if (config.enableAsymmetricPropagation)
                    {
                        ...
                        for (const int distance : propagation_distances)
                        {
                            ...
                            const cv::Vec3f candidate_vector = normalS.at<cv::Vec3f>(row, candidate_col);
                            const CpuNormal candidate_normal{
                                candidate_vector[0], candidate_vector[1], candidate_vector[2]};
                            const float propagated_depth =
                                clampDepth(row,
                            ...
                            const std::uint32_t candidate_mask =
                                neighborMask |
                                static_cast<std::uint32_t>(sourceMaskS.at<std::int32_t>(row, candidate_col));
                            const CpuHypothesisCost candidate_cost =
                                cpuEvalHypCost(col,
                                               row,
                                               propagated_depth,
                    ...
                    normalS.at<cv::Vec3f>(row, col) = cv::Vec3f(chosenNormal[0], chosenNormal[1], chosenNormal[2]);
                    confS.at<float>(row, col) = bestNcc;
                    objectiveS.at<float>(row, col) = bestCost;
                    sourceMaskS.at<std::int32_t>(row, col) = static_cast<std::int32_t>(bestMask);
    ...
    const int propagation_iterations =
        patchMatchPropagationIterationCount(config.numIterations);
    const int scheduled_passes = patchMatchScheduledPropagationPassCount(
        config.numIterations, config.enableFinalPropagationPass);
    float perturbation = 0.35f;
    for (int iter = 0; iter < scheduled_passes; ++iter)
    {
        if (config.cancelFlag && config.cancelFlag->load(std::memory_order_relaxed))
        {
            if (errorMsg)
        ...
        const bool final_propagation = iter >= propagation_iterations;
        const float pass_perturbation = final_propagation ? 0.0f : perturbation;
        const unsigned long long baseSeed =
            static_cast<unsigned long long>(iter + 1) * 999983ULL;
        if (!runColumnSweep(true, baseSeed, pass_perturbation) ||
            !runColumnSweep(false, baseSeed + 111111ULL, pass_perturbation) ||
            !runRowSweep(true, baseSeed + 222222ULL, pass_perturbation) ||
            !runRowSweep(false, baseSeed + 333333ULL, pass_perturbation))
        {
            return false;
        }
        if (!final_propagation)
        {
        ...
        !cpuParallelForLines(
            ...
            [&](int row)
            {
                ...
                for (int col = 0; col < W; ++col)
                {
                    ...
                    const CpuNormal normal{normal_row[col][0], normal_row[col][1], normal_row[col][2]};
                    float competing_ncc = 0.0f;
                    const float relative_step = config.photometricUniquenessRelativeDepthStep;
                    const float lower_depth = std::max(zNear, depth * (1.0f - relative_step));
                    const float upper_depth = std::min(zFar, depth * (1.0f + relative_step));
                    const float minimum_distinct_depth = std::max(1e-6f, depth * relative_step * 0.25f);
                    if (depth - lower_depth >= minimum_distinct_depth)
                    {
                        const CpuHypothesisCost cost =
                            cpuEvalHypCost(col,
                                           row,
                                           lower_depth,
                                           normal,
                                           ...
                                           config);
                        competing_ncc = std::max(competing_ncc, cost.photometricNcc);
                    }
                    ...
                    confidence_row[col] = best_ncc * photometricUniquenessConfidenceScale(
                                                         best_ncc,
                                                         competing_ncc,
    ...
    LOG_DEBUG("[MVS][PatchMatch][CPU] size=%dx%d ds=%d threads=%d valid_rows=%d", W, H, ds, cpuThreadCount, H);

    postprocessPatchMatchDepth(depthS, config, refF);
    ...
    if (ds > 1 && !config.returnNativeResolution)
    {
        ...
        cv::resize(sourceMaskS, sourceMaskFull, cv::Size(refW, refH), 0, 0, cv::INTER_NEAREST);
    }
