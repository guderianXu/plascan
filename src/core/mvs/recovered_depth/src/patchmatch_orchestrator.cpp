...
namespace metmodel
{
    namespace
    {
        ...
        public:
            ...
            RecoveredCudaModuleSessionScope(const RecoveredCudaModuleSessionScope&) = delete;
            RecoveredCudaModuleSessionScope& operator=(const RecoveredCudaModuleSessionScope&) = delete;
            ...
            bool close(RecoveredCudaModuleSessionStats& stats, std::string& error)
            {
                if (!active_)
                {
            ...
            ~RecoveredCudaModuleSessionScope()
            {
                ...
                (void)end_recovered_cuda_module_session(ignored_stats, ignored_error);
            }
        ...
        std::uint64_t saturating_add_bytes(std::uint64_t left, std::uint64_t right)
        {
        ...
        template <class T> std::uint64_t vector_capacity_bytes(const std::vector<T>& values)
        {
            constexpr std::uint64_t maximum = std::numeric_limits<std::uint64_t>::max();
            if (values.capacity() > maximum / sizeof(T))
                return maximum;
            return static_cast<std::uint64_t>(values.capacity()) * sizeof(T);
        }

        std::uint64_t prepared_camera_dynamic_bytes(const RecoveredPatchMatchPreparedCamera& camera)
        {
            std::uint64_t bytes = vector_capacity_bytes(camera.image_levels);
            for (const auto& level : camera.image_levels)
            {
                bytes = saturating_add_bytes(bytes, vector_capacity_bytes(level.data.image));
                bytes = saturating_add_bytes(bytes, vector_capacity_bytes(level.data.rejection_mask));
            }
            return bytes;
        }

        std::uint64_t neighbor_resources_dynamic_bytes(const RecoveredPatchMatchNeighborResources& resources)
        {
            std::uint64_t bytes = vector_capacity_bytes(resources.resource_groups);
            for (const auto& group : resources.resource_groups)
            {
                bytes = saturating_add_bytes(bytes, vector_capacity_bytes(group.image_offsets));
                bytes = saturating_add_bytes(bytes, vector_capacity_bytes(group.image));
                bytes = saturating_add_bytes(bytes, vector_capacity_bytes(group.level_offsets));
            ...
            for (const auto& neighbor : resources.ranked_neighbors)
            {
                ...
                bytes = saturating_add_bytes(bytes, vector_capacity_bytes(neighbor.texture_source));
                bytes = saturating_add_bytes(bytes, vector_capacity_bytes(neighbor.texture_write_mask));
                bytes = saturating_add_bytes(bytes, vector_capacity_bytes(neighbor.texture_copy_regions));
            }
            return bytes;
        }

        std::uint64_t host_preparation_dynamic_bytes(const RecoveredPatchMatchHostPreparation& preparation)
        {
            std::uint64_t bytes = prepared_camera_dynamic_bytes(preparation.reference);
            bytes = saturating_add_bytes(bytes, vector_capacity_bytes(preparation.ranked_neighbor_camera_indices));
            bytes = saturating_add_bytes(bytes, vector_capacity_bytes(preparation.ranked_neighbors));
            for (const auto& neighbor : preparation.ranked_neighbors)
            {
                bytes = saturating_add_bytes(bytes, prepared_camera_dynamic_bytes(neighbor));
            }
            return saturating_add_bytes(bytes, neighbor_resources_dynamic_bytes(preparation.neighbor_resources));
        }

        std::uint64_t prepared_camera_cache_dynamic_bytes(const std::vector<RecoveredPatchMatchPreparedCamera>& cameras)
        {
            std::uint64_t bytes = vector_capacity_bytes(cameras);
            for (const auto& camera : cameras)
            {
                bytes = saturating_add_bytes(bytes, prepared_camera_dynamic_bytes(camera));
            }
            return bytes;
        }
                                                      ...
                                                      bool capture_diagnostic_checkpoints)
    {
        try
        {
            ...
            {
                local_atlas_state = make_recovered_patchmatch_cost_atlas_state(preparation.neighbor_resources);
                camera_atlas_state = &local_atlas_state;
            }
            const std::size_t width = (camera.width_original + downscale - 1U) / downscale;
            const std::size_t height = (camera.height_original + downscale - 1U) / downscale;
            const std::size_t pixels = width * height;
            const std::size_t finest_width =
                (camera.width_original + preparation.target_downscale - 1U) / preparation.target_downscale;
            const std::size_t finest_height =
                (camera.height_original + preparation.target_downscale - 1U) / preparation.target_downscale;
            const std::size_t finest_pixels = finest_width * finest_height;
            const std::uint32_t estimated_downscale = preparation.target_downscale * 2U;
            const std::size_t estimated_width =
                (camera.width_original + estimated_downscale - 1U) / estimated_downscale;
            const std::size_t estimated_height =
                (camera.height_original + estimated_downscale - 1U) / estimated_downscale;
            const std::size_t estimated_pixels = estimated_width * estimated_height;
            if (pixels == 0U || pixels > capacity || bilateral_image.size() != pixels)
            {
            ...
            const std::size_t neighbor_capacity = preparation.neighbor_resources.ranked_neighbors.size();
            const std::size_t inlier_groups = (neighbor_capacity + 7U) / 8U;
            state.neighbor_cost.assign(neighbor_capacity * hypotheses * capacity, 0.0F);
            state.average_cost.assign(hypotheses * capacity, 0.0F);
            state.auxiliary.assign(inlier_groups * hypotheses * capacity, std::uint8_t{0});
            state.neighbor_inlier_masks.assign(inlier_groups * finest_pixels, std::uint8_t{0});
            ...
            const auto reference_image = make_recovered_patchmatch_reference_image_allocation(
                preparation.reference, preparation.target_downscale, downscale);
            std::uint64_t next_producer_handoff = 0U;
            bool initial_cost_scratch_all_zero = true;
            const bool run_final_refinement = downscale <= preparation.target_downscale * 4U;

            const auto cost_and_wta = [&](PatchMatchCandidateOutput candidates,
                                          ...
                                          bool materialize_cost_output)
            {
                auto binding = make_recovered_patchmatch_cost_binding(
                    preparation, *camera_atlas_state, downscale, iteration, all_neighbors_state, false);
                ...
                cost_input.deviation_threshold_multiplier = binding.deviation_threshold_multiplier;
                ...
                cost_input.neighbor_texture_height = binding.neighbor_texture_height;
                cost_input.neighbor_texture_view = binding.initial_neighbor_texture_view;
                ...
                cost_input.neighbor_cost_capacity = binding.neighbor_cost_capacity;
                ...
                cost_input.camera_resource_generation = binding.camera_resource_generation;
                cost_input.cuda_workspace_handoff = cost_input.candidates.cuda_workspace_handoff;
                cost_input.initial_cuda_cost_scratch_materialized = state.cuda_cost_scratch_materialized;
                cost_input.initial_cuda_cost_scratch_all_zero = initial_cost_scratch_all_zero;
                cost_input.defer_cuda_host_output = !materialize_cost_output;
                PatchMatchCostOutput cost_output;
                if (!run_recovered_patchmatch_cost_cuda_movable(std::move(cost_input), cost_output, error))
                    return false;
                ...
                wta.cuda_workspace_handoff = cost_output.cuda_workspace_handoff;
                wta.cuda_cost_output_materialized = cost_output.cuda_host_output_materialized;
                wta.defer_cuda_host_output = !materialize_cost_output && cost_output.cuda_workspace_handoff != 0U;
                PatchMatchWtaOutput wta_output;
                if (!run_recovered_patchmatch_wta_cuda_source_movable(std::move(wta), wta_output, error))
                    return false;
                ...
                copy_inliers.neighbor_count = static_cast<std::uint32_t>(neighbor_capacity);
                copy_inliers.temporary_inlier_masks = std::move(cost_output.auxiliary);
                copy_inliers.winner = std::move(wta_output.winner);
                copy_inliers.initial_neighbor_inlier_masks = std::move(state.neighbor_inlier_masks);
                ...
                copy_inliers.cuda_workspace_handoff = wta_output.cuda_workspace_handoff;
                copy_inliers.cuda_temporary_inlier_masks_materialized = cost_output.cuda_host_output_materialized;
                copy_inliers.cuda_winner_materialized = wta_output.cuda_host_output_materialized;
                copy_inliers.cuda_initial_neighbor_inlier_masks_materialized = state.cuda_inlier_masks_materialized;
                copy_inliers.defer_cuda_host_output =
                    !materialize_cost_output && wta_output.cuda_workspace_handoff != 0U;
                PatchMatchCopyInlierMasksOutput copied_inliers;
                if (!run_recovered_patchmatch_copy_inlier_masks_cuda_source_movable(
                        std::move(copy_inliers), copied_inliers, error))
                    return false;
                cost_output.auxiliary = std::move(copy_inliers.temporary_inlier_masks);
                wta_output.winner = std::move(copy_inliers.winner);
                next_producer_handoff = copied_inliers.cuda_workspace_handoff;
                ...
                state.cuda_cost_scratch_materialized = cost_output.cuda_host_output_materialized;
                state.cuda_main_state_materialized = wta_output.cuda_host_output_materialized;
                state.neighbor_inlier_masks = std::move(copied_inliers.neighbor_inlier_masks);
                state.cuda_inlier_masks_materialized = copied_inliers.cuda_host_output_materialized;
                return true;
            };
            ...
            for (std::uint32_t iteration = 0U; iteration < 6U; ++iteration)
            {
                ...
                refinement.deviation_threshold_multiplier = preparation.deviation_threshold_multiplier;
                ...
                refinement.initial_candidate_depth = std::move(state.candidates.depth);
                refinement.initial_candidate_normal = std::move(state.candidates.normal);
                refinement.iteration = iteration;
                refinement.global_work_items = pixels;
                refinement.device_index = preparation.device_index;
                refinement.cuda_workspace_handoff = std::exchange(next_producer_handoff, 0U);
                ...
                if (!run_recovered_patchmatch_refinement_cuda_movable(std::move(refinement), refined, error) ||
                    !cost_and_wta(std::move(refined), iteration, false, 0U, 0U, pixels, 3U, false))
                    return false;
                ...
                for (std::uint32_t step = 0U; step < 2U; ++step)
                {
                    ...
                    propagation.reference_to_neighbor_rotation = preparation.propagation_rotation;
                    ...
                    propagation.deviation_threshold_multiplier = preparation.deviation_threshold_multiplier;
                    ...
                    propagation.defer_cuda_host_output =
                        !capture_diagnostic_checkpoints || run_final_refinement || iteration != 5U || step != 1U;
                    PatchMatchCandidateOutput propagated;
                    if (!run_recovered_patchmatch_propagation_cuda_movable(std::move(propagation), propagated, error) ||
                        !cost_and_wta(std::move(propagated),
                                      iteration,
                                      false,
                                      1U,
                                      step,
            ...
            if (run_final_refinement)
            {
                ...
                final_refinement.deviation_threshold_multiplier = preparation.deviation_threshold_multiplier;
                final_refinement.depth_view = state.depth;
                final_refinement.normal_view = state.normal;
                final_refinement.cost_view = state.cost;
                final_refinement.initial_candidate_depth = std::move(state.candidates.depth);
                final_refinement.initial_candidate_normal = std::move(state.candidates.normal);
                final_refinement.global_work_items = pixels;
                final_refinement.device_index = preparation.device_index;
                final_refinement.cuda_workspace_handoff = std::exchange(next_producer_handoff, 0U);
                final_refinement.defer_cuda_host_output = !capture_diagnostic_checkpoints;
            ...
            {
                if (!state.candidates.cuda_host_output_materialized || !state.cuda_cost_scratch_materialized ||
                    !state.cuda_main_state_materialized || !state.cuda_inlier_masks_materialized)
                {
            ...
            boundary.filter.estimated_normal_allocation.assign(estimated_pixels * 3U, std::uint8_t{0});
            boundary.filter.filtered_mask_allocation.assign(finest_pixels, std::uint8_t{0});
            boundary.filter.estimate_normal_map = true;
            boundary.filter.device_index = preparation.device_index;
            if (!capture_diagnostic_checkpoints)
            {
                boundary.filter.cuda_workspace_handoff = std::exchange(next_producer_handoff, 0U);
                boundary.filter.cuda_main_state_materialized = state.cuda_main_state_materialized;
            ...
            if (!run_recovered_patchmatch_level_boundary_cuda(boundary, output.boundary, error))
                return false;
            if (!capture_diagnostic_checkpoints)
            {
                ...
                output.state_before_filter.cuda_workspace_handoff = output.boundary.filter.cuda_workspace_handoff;
            }
            return true;
        }
    ...
    bool run_recovered_patchmatch_x16_level_cuda(const Camera& camera,
                                                 ...
                                                 RecoveredPatchMatchLevelState* consumable_previous_state)
    {
        try
        {
            ...
            const std::size_t width = (patch_camera.width_original + downscale - 1U) / downscale;
            const std::size_t height = (patch_camera.height_original + downscale - 1U) / downscale;
            const std::size_t pixels = width * height;
            const std::size_t previous_width = (patch_camera.width_original + 31U) / 32U;
            const std::size_t previous_height = (patch_camera.height_original + 31U) / 32U;
            const std::size_t previous_pixels = previous_width * previous_height;
            const std::size_t neighbor_capacity = preparation.neighbor_resources.ranked_neighbors.size();
            const std::size_t inlier_groups = (neighbor_capacity + 7U) / 8U;
            const std::size_t finest_width =
                (patch_camera.width_original + preparation.target_downscale - 1U) / preparation.target_downscale;
            const std::size_t finest_height =
                (patch_camera.height_original + preparation.target_downscale - 1U) / preparation.target_downscale;
            const std::size_t finest_pixels = finest_width * finest_height;
            const std::uint32_t estimated_downscale = preparation.target_downscale * 2U;
            const std::size_t estimated_width =
                (patch_camera.width_original + estimated_downscale - 1U) / estimated_downscale;
            const std::size_t estimated_height =
                (patch_camera.height_original + estimated_downscale - 1U) / estimated_downscale;
            const std::size_t estimated_pixels = estimated_width * estimated_height;
            if (pixels == 0U || pixels > capacity || previous_pixels == 0U || bilateral_image.size() != pixels ||
                coarsest.boundary.bilateral.depth.size() != previous_pixels ||
                coarsest.boundary.bilateral.normal.size() != previous_pixels * 3U ||
                coarsest.state_before_filter.candidates.depth.size() != hypotheses * capacity ||
                coarsest.state_before_filter.candidates.normal.size() != hypotheses * capacity * 3U ||
                coarsest.state_before_filter.winner.size() != capacity ||
                coarsest.state_before_filter.neighbor_cost.size() != neighbor_capacity * hypotheses * capacity ||
                coarsest.state_before_filter.average_cost.size() != hypotheses * capacity ||
                coarsest.state_before_filter.auxiliary.size() != inlier_groups * hypotheses * capacity ||
                (!coarsest.state_before_filter.cuda_cost_scratch_materialized &&
                 coarsest.state_before_filter.cuda_workspace_handoff == 0U) ||
                coarsest.state_before_filter.neighbor_inlier_masks.size() != inlier_groups * finest_pixels)
            {
            ...
            if (!make_recovered_patchmatch_cross_level_state(camera,
                                                             downscale,
                                                             ...
                                                             result.cross_level_initial,
                                                             error))
                return false;
            if (result.cross_level_initial.width != width || result.cross_level_initial.height != height)
            {
            ...
            if (consumable_previous_state != nullptr && consumable_previous_state != &coarsest.state_before_filter)
            {
                error = "PatchMatch x16 consumable state does not match its input";
                return false;
            }
            const bool consume_previous = !capture_diagnostic_checkpoints && consumable_previous_state != nullptr;
            if (consume_previous)
            {
                state.winner = std::move(consumable_previous_state->winner);
                state.candidates = std::move(consumable_previous_state->candidates);
                state.neighbor_cost = std::move(consumable_previous_state->neighbor_cost);
                state.average_cost = std::move(consumable_previous_state->average_cost);
            ...
            state.cuda_cost_scratch_materialized = coarsest.state_before_filter.cuda_cost_scratch_materialized;
            state.cuda_main_state_materialized = coarsest.state_before_filter.cuda_main_state_materialized;
            state.cuda_inlier_masks_materialized = coarsest.state_before_filter.cuda_inlier_masks_materialized;
            state.cuda_workspace_handoff = coarsest.state_before_filter.cuda_workspace_handoff;
            state.neighbor_inlier_masks = coarsest.state_before_filter.neighbor_inlier_masks;
            const std::vector<float>& coarse_depth = result.cross_level_initial.coarse_depth;
            const std::vector<float>& coarse_radius = result.cross_level_initial.coarse_radius;
            const auto reference_image = make_recovered_patchmatch_reference_image_allocation(
                preparation.reference, preparation.target_downscale, downscale);
            std::uint64_t next_producer_handoff = std::exchange(state.cuda_workspace_handoff, 0U);
            const bool run_final_refinement = downscale <= preparation.target_downscale * 4U;
            ...
            const auto cost_and_wta = [&state,
                                       ...
                                       &error](PatchMatchCandidateOutput candidates,
                                               std::uint32_t iteration,
                                               bool all_neighbors_state,
            ...
            const std::size_t fourth_items = ((width + 1U) / 2U) * ((height + 1U) / 2U);
            const std::size_t fourth_checker_items = ((width + 3U) / 4U) * ((height + 1U) / 2U);
            ...
            for (std::uint32_t iteration = 0U; iteration < 6U; ++iteration)
            {
                ...
                PatchMatchCandidateOutput refined;
                if (!run_recovered_patchmatch_refinement_cuda_movable(std::move(refinement), refined, error) ||
                    !cost_and_wta(std::move(refined), iteration, false, 8U, 3U, 0U, 0U, 1U, fourth_items, false))
                    return false;
                ...
                for (std::uint32_t step = 0U; step < 2U; ++step)
                {
                    ...
                    propagation.cuda_workspace_handoff = std::exchange(next_producer_handoff, 0U);
                    propagation.defer_cuda_host_output =
                        !capture_diagnostic_checkpoints || iteration != 5U || step != 1U;
                    PatchMatchCandidateOutput propagated;
                    if (!run_recovered_patchmatch_propagation_cuda_movable(std::move(propagation), propagated, error) ||
                        !cost_and_wta(std::move(propagated),
                                      iteration,
            ...
            c2p.cuda_workspace_handoff = std::exchange(next_producer_handoff, 0U);
            c2p.cuda_main_state_materialized = state.cuda_main_state_materialized;
            c2p.defer_cuda_host_output = c2p.cuda_workspace_handoff != 0U;
            PatchMatchCandidateOutput c2p_candidates;
            if (!run_recovered_patchmatch_coarse_to_precise_cuda_source_movable(std::move(c2p), c2p_candidates, error))
                return false;
            if (!cost_and_wta(
                    std::move(c2p_candidates), 6U, false, 2U, 3U, 0U, 0U, 0U, pixels, capture_diagnostic_checkpoints))
                return false;
            if (capture_diagnostic_checkpoints)
            ...
            for (const std::uint32_t iteration : {7U, 8U})
            {
                ...
                PatchMatchCandidateOutput refined;
                if (!run_recovered_patchmatch_refinement_cuda_movable(std::move(refinement), refined, error) ||
                    !cost_and_wta(std::move(refined), iteration, false, 8U, 3U, 0U, 0U, 0U, pixels, false))
                    return false;
                ...
                for (std::uint32_t step = 0U; step < 2U; ++step)
                {
                    ...
                    propagation.cuda_workspace_handoff = std::exchange(next_producer_handoff, 0U);
                    propagation.defer_cuda_host_output =
                        !capture_diagnostic_checkpoints || run_final_refinement || iteration != 8U || step != 1U;
                    PatchMatchCandidateOutput propagated;
                    if (!run_recovered_patchmatch_propagation_cuda_movable(std::move(propagation), propagated, error) ||
                        !cost_and_wta(std::move(propagated),
                                      iteration,
                                      ...
                                      0U,
                                      checker_items,
                                      capture_diagnostic_checkpoints && !run_final_refinement && iteration == 8U &&
                                          step == 1U))
                        return false;
                }
            }
            ...
            boundary.filter.estimated_normal_allocation.assign(estimated_pixels * 3U, std::uint8_t{0});
            boundary.filter.filtered_mask_allocation.assign(finest_pixels, std::uint8_t{0});
            const auto& previous_mask = coarsest.boundary.filter.filtered_mask_allocation;
            ...
            if (!run_recovered_patchmatch_level_boundary_cuda(boundary, result.boundary, error))
                return false;
            if (!capture_diagnostic_checkpoints)
            {
                ...
                result.state_before_filter.cuda_workspace_handoff = result.boundary.filter.cuda_workspace_handoff;
            }
    ...
    bool run_recovered_patchmatch_finer_level_cuda(const Camera& camera,
                                                   const RecoveredPatchMatchHostPreparation& preparation,
                                                   ...
                                                   RecoveredPatchMatchLevelState* consumable_previous_state)
    {
        try
        {
            ...
            const std::size_t previous_width =
                (patch_camera.width_original + previous_downscale - 1U) / previous_downscale;
            const std::size_t previous_height =
                (patch_camera.height_original + previous_downscale - 1U) / previous_downscale;
            const std::size_t previous_pixels = previous_width * previous_height;
            const std::size_t finest_width =
                (patch_camera.width_original + preparation.target_downscale - 1U) / preparation.target_downscale;
            const std::size_t finest_height =
            ...
            if (pixels == 0U || previous_pixels == 0U || previous_boundary.bilateral.depth.size() != previous_pixels ||
                previous_boundary.bilateral.normal.size() != previous_pixels * 3U ||
                (!target_level && bilateral_image.size() != pixels) || (target_level && !bilateral_image.empty()) ||
                previous_state_before_filter.candidates.depth.size() != hypotheses * capacity ||
                previous_state_before_filter.candidates.normal.size() != hypotheses * capacity * 3U ||
                previous_state_before_filter.winner.size() != capacity ||
                previous_state_before_filter.neighbor_cost.size() != neighbor_capacity * hypotheses * capacity ||
                previous_state_before_filter.average_cost.size() != hypotheses * capacity ||
                previous_state_before_filter.auxiliary.size() != inlier_groups * hypotheses * capacity ||
                (!previous_state_before_filter.cuda_cost_scratch_materialized &&
                 previous_state_before_filter.cuda_workspace_handoff == 0U) ||
                previous_state_before_filter.neighbor_inlier_masks.size() != inlier_groups * finest_pixels ||
                previous_boundary.filter.filtered_mask_allocation.size() != finest_pixels ||
                previous_boundary.filter.estimated_normal_allocation.size() != estimated_pixels * 3U)
            {
            ...
            state.normal = result.cross_level_initial.normal;
            state.cost = result.cross_level_initial.cost;
            if (consumable_previous_state != nullptr && consumable_previous_state != &previous_state_before_filter)
            {
            ...
            state.cuda_cost_scratch_materialized = previous_state_before_filter.cuda_cost_scratch_materialized;
            state.cuda_main_state_materialized = previous_state_before_filter.cuda_main_state_materialized;
            state.cuda_inlier_masks_materialized = previous_state_before_filter.cuda_inlier_masks_materialized;
            state.cuda_workspace_handoff = previous_state_before_filter.cuda_workspace_handoff;
            state.neighbor_inlier_masks = previous_state_before_filter.neighbor_inlier_masks;
            const std::vector<float>& coarse_depth = result.cross_level_initial.coarse_depth;
            const std::vector<float>& coarse_radius = result.cross_level_initial.coarse_radius;
            const auto reference_image = make_recovered_patchmatch_reference_image_allocation(
                preparation.reference, preparation.target_downscale, downscale);
            ...
            const auto for_each_batch = [&error](std::size_t items, auto&& callback)
            {
                ...
                const auto span = patchmatch_balanced_batch_span(static_cast<std::uint32_t>(items));
                for (std::size_t offset = 0; offset < items; offset += span)
                {
                    const std::size_t work_items = std::min<std::size_t>(span, items - offset);
            ...
            const auto run_iteration = [&](std::uint32_t iteration, bool only_fourth, bool materialize_last_propagation)
            {
                const std::size_t refinement_items = only_fourth ? fourth_items : full_items;
                if (!for_each_batch(refinement_items,
                                    [&](std::uint32_t offset, std::size_t work_items)
                                    {
                                        ...
                                        refinement.initial_candidate_normal = std::move(state.candidates.normal);
                                        refinement.iteration = iteration;
                                        refinement.only_each_fourth_pixel = only_fourth ? 1U : 0U;
                ...
                const std::size_t propagation_items = only_fourth ? fourth_checker_items : checker_items;
                for (std::uint32_t step = 0U; step < 2U; ++step)
                {
                    if (!for_each_batch(
                            propagation_items,
                            [&](std::uint32_t offset, std::size_t work_items)
                            {
                                ...
                                propagation.initial_candidate_normal = std::move(state.candidates.normal);
                                propagation.checkboard_step = step;
                                propagation.only_each_fourth_pixel = only_fourth ? 1U : 0U;
                                propagation.pixel_offset = offset;
                                propagation.global_work_items = work_items;
                                propagation.device_index = preparation.device_index;
                                propagation.cuda_workspace_handoff = std::exchange(next_producer_handoff, 0U);
                                propagation.defer_cuda_host_output =
                                    !(materialize_last_propagation && step == 1U &&
                                      static_cast<std::size_t>(offset) + work_items == propagation_items);
                                PatchMatchCandidateOutput propagated;
            ...
            for (std::uint32_t iteration = 0U; iteration < inherited_iterations; ++iteration)
            {
                if (!run_iteration(
                        iteration, true, capture_diagnostic_checkpoints && iteration + 1U == inherited_iterations))
                    return false;
            }
            if (capture_diagnostic_checkpoints)
            {
            ...
            if (!for_each_batch(full_items,
                                [&](std::uint32_t offset, std::size_t work_items)
                                {
                                    ...
                                    c2p.defer_cuda_host_output = c2p.cuda_workspace_handoff != 0U;
                                    PatchMatchCandidateOutput c2p_candidates;
                                    return run_recovered_patchmatch_coarse_to_precise_cuda_source_movable(
                                               std::move(c2p), c2p_candidates, error) &&
                                           cost_and_wta(std::move(c2p_candidates),
                                                        c2p_iteration,
                                                        false,
                                                        2U,
                                                        3U,
            ...
            if (target_level)
            {
                if (!run_iteration(2U, false, false))
                    return false;
            }
            else
            {
                if (!run_iteration(7U, false, false) || !run_iteration(8U, false, false))
                    return false;
            }
            ...
            if (!for_each_batch(
                    ...
                    [&](std::uint32_t offset, std::size_t work_items)
                    {
                        ...
                        final_refinement.defer_cuda_host_output =
                            !capture_diagnostic_checkpoints ||
                            static_cast<std::size_t>(offset) + work_items != full_items;
                        PatchMatchFinalRefinementOutput final_output;
                        if (!run_recovered_patchmatch_final_refinement_state_cuda_movable(
                                std::move(final_refinement), final_output, error))
                            return false;
                        state.cost = std::move(final_output.cost);
                        return cost_and_wta(std::move(final_output.candidates),
                                            final_iteration,
                                            target_level,
                                            8U,
                                            2U,
            ...
            if (target_level)
            {
                boundary.filter.estimated_normal_allocation = previous_boundary.filter.estimated_normal_allocation;
            }
            else
            {
                boundary.filter.estimated_normal_allocation.assign(estimated_pixels * 3U, std::uint8_t{0});
            }
            boundary.filter.filtered_mask_allocation = previous_boundary.filter.filtered_mask_allocation;
            ...
            if (target_level)
            {
                ...
                if (!run_recovered_patchmatch_filter_chain_cuda(boundary.filter, filtered, error) ||
                    !filter_recovered_patchmatch_cuda_speckle_components(static_cast<std::uint32_t>(width),
            ...
            result.level_product_depth.assign(result.boundary.filter.depth_allocation.begin(),
                                              result.boundary.filter.depth_allocation.begin() + pixels);
            const std::size_t current_inlier_bytes = inlier_groups * pixels;
            result.level_product_inlier_masks.assign(result.state_before_filter.neighbor_inlier_masks.begin(),
                                                     result.state_before_filter.neighbor_inlier_masks.begin() +
                                                         current_inlier_bytes);
            result.ranked_neighbor_camera_indices.reserve(preparation.ranked_neighbor_camera_indices.size());
            result.ranked_neighbor_camera_indices = preparation.ranked_neighbor_camera_indices;
                                                 ...
                                                 std::string& error)
    {
        try
        {
            if (pixels == 0U || neighbor_count == 0U || ranked_neighbor_index >= neighbor_count)
            {
                error = "PatchMatch packed-mask extraction indices are invalid";
                return false;
            }
            const std::size_t groups = (neighbor_count + 7U) / 8U;
            if (groups > std::numeric_limits<std::size_t>::max() / pixels || packed_masks.size() != groups * pixels)
            {
                error = "PatchMatch packed-mask extraction backing is inconsistent";
                return false;
            }
            const std::size_t group = ranked_neighbor_index / 8U;
            const std::uint8_t bit = static_cast<std::uint8_t>(std::uint8_t{1} << (ranked_neighbor_index % 8U));
            const std::size_t group_offset = group * pixels;
            std::vector<std::uint8_t> result(pixels);
            for (std::size_t pixel = 0; pixel < pixels; ++pixel)
            {
                result[pixel] = static_cast<std::uint8_t>((packed_masks[group_offset + pixel] & bit) != 0U);
            }
            output = std::move(result);
            error.clear();
            return true;
                                                ...
                                                std::string& error)
    {
        try
        {
            ...
            for (std::size_t level = 0; level < 3U; ++level)
            {
                ...
                if (widths[level] > std::numeric_limits<std::size_t>::max() / heights[level] ||
                    persisted_depth_levels[level].size() != widths[level] * heights[level])
                {
            ...
            constexpr float normal_threshold_squared = normal_threshold * normal_threshold;
            const double principal_x_over_focal = camera.model.cx / camera.model.f;
            const double principal_y_over_focal = camera.model.cy / camera.model.f;
            ...
            {
                if (source_width < 2U || source_height < 2U || target_width != 2U * source_width ||
                ...
                const double source_scale = static_cast<double>(source_downscale) / camera.model.f;
                const auto make_point = [&](const Sample& sample)
                {
                    const double depth = static_cast<double>(sample.depth);
                    return Point3{(sample.u * source_scale - principal_x_over_focal) * depth,
                ...
                {
                    const double source_y = (static_cast<double>(y) + 0.5) * static_cast<double>(source_height) /
                                                static_cast<double>(target_height) -
                                            0.5;
                    const auto iy = static_cast<std::ptrdiff_t>(source_y);
                    if (iy < 0 || iy >= static_cast<std::ptrdiff_t>(source_height - 1U))
                        continue;
                    const double fy = source_y - static_cast<double>(iy);
                    for (std::size_t x = 0; x < target_width; ++x)
                    {
                        const double source_x = (static_cast<double>(x) + 0.5) * static_cast<double>(source_width) /
                                                    static_cast<double>(target_width) -
                                                0.5;
                        const auto ix = static_cast<std::ptrdiff_t>(source_x);
                        if (ix < 0 || ix >= static_cast<std::ptrdiff_t>(source_width - 1U))
                            continue;
                        const double fx = source_x - static_cast<double>(ix);
                        const std::size_t top_left =
                            static_cast<std::size_t>(iy) * source_width + static_cast<std::size_t>(ix);
                        ...
                        if (diagonal_main < diagonal_other)
                        {
                            if (fx <= fy && d01 != 0.0F)
                            {
                                triangle = {{{static_cast<double>(ix) + 0.5, static_cast<double>(iy) + 0.5, d00},
                                             {static_cast<double>(ix) + 0.5, static_cast<double>(iy) + 1.5, d01},
                                             {static_cast<double>(ix) + 1.5, static_cast<double>(iy) + 1.5, d11}}};
                                has_triangle = true;
                            }
                            else if (fx > fy && d10 != 0.0F)
                            {
                                triangle = {{{static_cast<double>(ix) + 1.5, static_cast<double>(iy) + 0.5, d10},
                                             {static_cast<double>(ix) + 0.5, static_cast<double>(iy) + 0.5, d00},
                                             {static_cast<double>(ix) + 1.5, static_cast<double>(iy) + 1.5, d11}}};
                                has_triangle = true;
                            }
                        }
                        else if (diagonal_other < diagonal_main)
                        {
                            if (fx + fy <= 1.0 && d00 != 0.0F)
                            {
                                triangle = {{{static_cast<double>(ix) + 0.5, static_cast<double>(iy) + 0.5, d00},
                                             {static_cast<double>(ix) + 0.5, static_cast<double>(iy) + 1.5, d01},
                                             {static_cast<double>(ix) + 1.5, static_cast<double>(iy) + 0.5, d10}}};
                                has_triangle = true;
                            }
                        ...
                        if (!has_triangle || triangle[0].depth == 0.0F || triangle[1].depth == 0.0F ||
                        ...
                        const Point3 normal{a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x};
                        const Point3 center{
                            (p0.x + p1.x + p2.x) / 3.0, (p0.y + p1.y + p2.y) / 3.0, (p0.z + p1.z + p2.z) / 3.0};
                        const double facing = -(center.x * normal.x + center.y * normal.y + center.z * normal.z);
                        const double normal2 = normal.x * normal.x + normal.y * normal.y + normal.z * normal.z;
                        const double center2 = center.x * center.x + center.y * center.y + center.z * center.z;
                        if (facing < 0.0 || normal2 * (center2 * normal_threshold_squared) > facing * facing)
                            continue;

                        double depth = 0.0;
                        if (diagonal_main < diagonal_other)
                        {
                            depth = fx <= fy ? static_cast<double>(d00) * (1.0 - fy) +
                                                   static_cast<double>(d01) * (fy - fx) + static_cast<double>(d11) * fx
                                             : static_cast<double>(d00) * (1.0 - fx) +
                                                   static_cast<double>(d10) * (fx - fy) + static_cast<double>(d11) * fy;
                        }
                        else if (fx + fy <= 1.0)
                        {
                            depth = static_cast<double>(d00) * (1.0 - fx - fy) + static_cast<double>(d10) * fx +
                                    static_cast<double>(d01) * fy;
                        }
                        else
                        {
                            depth = static_cast<double>(d10) * (1.0 - fy) + static_cast<double>(d01) * (1.0 - fx) +
                                    static_cast<double>(d11) * (fx + fy - 1.0);
                        }
                        if (std::isfinite(depth) && depth > 0.0)
                            target[y * target_width + x] = static_cast<float>(depth);
                    }
                }
                return true;
            };
                                                        ...
                                                        std::string& error)
    {
        try
        {
            ...
            const std::size_t groups = (neighbor_count + 7U) / 8U;
            if (pixels == 0U || neighbor_count == 0U || groups > std::numeric_limits<std::size_t>::max() / pixels ||
            ...
            std::vector<float> depth(component_filtered_depth.begin(), component_filtered_depth.end());
            std::vector<std::uint8_t> masks(raw_packed_inlier_masks.begin(), raw_packed_inlier_masks.end());
            for (std::size_t pixel = 0; pixel < pixels; ++pixel)
            {
    ...
    bool
    run_recovered_patchmatch_d4_pyramid_cuda(const Scene& scene,
                                             ...
                                             double prebuilt_host_preparation_seconds)
    {
        try
        {
            ...
            {
                error = "recovered d4 PatchMatch pyramid requires one reference and 1..16 ranked neighbors";
                return false;
            }
            ...
            if (!run_recovered_patchmatch_coarsest_level_cuda(
                ...
                return false;
            const double x32_seconds = std::chrono::duration<double>(Clock::now() - x32_started).count();
            ...
            if (!run_recovered_patchmatch_x16_level_cuda(camera,
                ...
                return false;
            const double x16_seconds = std::chrono::duration<double>(Clock::now() - x16_started).count();
            ...
            if (x16.boundary.filter.depth_allocation.size() < x16_pixels ||
                x16.state_before_filter.neighbor_inlier_masks.size() < x16_packed_bytes)
            {
                error = "recovered d4 PatchMatch x16 level product is incomplete";
                return false;
            }
            std::vector<float> x16_level_product_depth(x16.boundary.filter.depth_allocation.begin(),
            ...
            std::vector<std::uint8_t> x16_level_product_inlier_masks(
                ...
                x16.state_before_filter.neighbor_inlier_masks.begin() + x16_packed_bytes);
            // x8 has consumed the x16 state/boundary. Preserve only the compact
            // persisted products that are needed after x4.
            x16 = {};
            std::vector<float> x8_level_product_depth = std::move(x8.level_product_depth);
            std::vector<std::uint8_t> x8_level_product_inlier_masks = std::move(x8.level_product_inlier_masks);
            std::vector<std::size_t> x8_ranked_neighbor_camera_indices = std::move(x8.ranked_neighbor_camera_indices);
            ...
            if (!run_recovered_patchmatch_finer_level_cuda(camera,
                                                           ...
                                                           capture_diagnostic_checkpoints ? nullptr
                                                                                          : &x8.state_before_filter))
                return false;
            const double x4_seconds = std::chrono::duration<double>(Clock::now() - x4_started).count();
            ...
            if (x8_ranked_neighbor_camera_indices.size() != neighbor_count ||
                x4.ranked_neighbor_camera_indices != x8_ranked_neighbor_camera_indices)
            {
            ...
            if (camera.image.width == 3072U && camera.image.height == 2304U && neighbor_count == 16U &&
            ...
            result.packed_inlier_masks[0] = std::move(x4.level_product_inlier_masks);
            result.packed_inlier_masks[1] = std::move(x8_level_product_inlier_masks);
            result.packed_inlier_masks[2] = std::move(x16_level_product_inlier_masks);
            result.ranked_neighbor_camera_indices = std::move(x4.ranked_neighbor_camera_indices);
            ...
            for (std::size_t level = 0; level < result.depth_levels.size(); ++level)
            {
                std::vector<float> persisted_depth;
                std::vector<std::uint8_t> persisted_masks;
                if (!make_recovered_patchmatch_voting_level_product(result.depth_levels[level],
                                                                    ...
                                                                    error))
                {
                    error = "recovered d4 PatchMatch level " + std::to_string(level) + " voting product: " + error;
                    return false;
                }
                                                ...
                                                std::size_t voting_batch_size)
    {
        try
        {
            ...
            std::vector<RecoveredPatchMatchPreparedCamera> prepared_camera_cache(scene.cameras.size());
            const std::size_t preparation_worker_count = std::min<std::size_t>(2U, reference_camera_indices.size());
            ...
            const auto run_preparation_worker = [&]()
            {
                ...
                {
                    const std::size_t ordinal = next_preparation_ordinal.fetch_add(1U, std::memory_order_relaxed);
                    if (ordinal >= reference_camera_indices.size())
                        return;
                    const std::size_t camera_index = reference_camera_indices[ordinal];
            ...
            for (std::thread& worker : preparation_workers)
                worker.join();
            ...
            std::uint64_t prepared_camera_cache_live_bytes = result.prepared_camera_cache_bytes;
            ...
            constexpr std::uint64_t prepared_queue_byte_budget = 64U * 1024U * 1024U;
            ...
            const auto update_prepared_host_peaks_locked = [&]()
            {
                result.resource_preparation_inflight_peak_bytes =
                    std::max(result.resource_preparation_inflight_peak_bytes, resource_preparation_inflight_bytes);
                result.prepared_queue_peak_bytes = std::max(result.prepared_queue_peak_bytes, prepared_queue_bytes);
                result.active_prepared_jobs_peak_bytes =
                    std::max(result.active_prepared_jobs_peak_bytes, active_prepared_jobs_bytes);
                result.prepared_queue_peak_jobs = std::max(result.prepared_queue_peak_jobs, prepared_jobs.size());
                result.resource_preparation_inflight_peak_count =
                    std::max(result.resource_preparation_inflight_peak_count, resource_preparation_inflight);
                result.active_prepared_jobs_peak_count =
                    std::max(result.active_prepared_jobs_peak_count, active_prepared_jobs);
                ...
                result.prepared_host_live_peak_bytes =
                    std::max(result.prepared_host_live_peak_bytes, result.prepared_camera_cache_bytes);
                result.prepared_host_live_peak_bytes = std::max(
                    result.prepared_host_live_peak_bytes,
                    saturating_add_bytes(
                        prepared_camera_cache_live_bytes,
                        saturating_add_bytes(resource_preparation_inflight_bytes,
                                             saturating_add_bytes(prepared_queue_bytes, active_prepared_jobs_bytes))));
            };
            ...
            const auto run_resource_preparation_worker = [&]()
            {
                ...
                {
                    const std::size_t ordinal = next_resource_ordinal.fetch_add(1U, std::memory_order_relaxed);
                    if (ordinal >= reference_camera_indices.size())
                        break;
                    const std::size_t camera_index = reference_camera_indices[ordinal];
                    PreparedJob job;
                    ...
                    job.dynamic_bytes = host_preparation_dynamic_bytes(job.preparation);
                    std::unique_lock lock(prepared_jobs_mutex);
                    bool consumer_accounting_valid = true;
                    const auto release_prepared_camera_consumer = [&](std::size_t consumed_camera_index)
                    {
                        if (!consumer_accounting_valid)
                            return;
                        std::size_t& remaining = prepared_camera_consumers[consumed_camera_index];
                        if (remaining == 0U)
                        {
                        ...
                        const std::uint64_t released_bytes =
                            prepared_camera_dynamic_bytes(prepared_camera_cache[consumed_camera_index]);
                        if (released_bytes > prepared_camera_cache_live_bytes)
                        {
                            consumer_accounting_valid = false;
                            return;
                        }
                    ...
                    resource_preparation_inflight_bytes =
                        saturating_add_bytes(resource_preparation_inflight_bytes, job.dynamic_bytes);
                    ++resource_preparation_inflight;
                    result.prepared_job_peak_bytes = std::max(result.prepared_job_peak_bytes, job.dynamic_bytes);
                    update_prepared_host_peaks_locked();
                    ...
                    prepared_queue_bytes = saturating_add_bytes(prepared_queue_bytes, job.dynamic_bytes);
                ...
                while (!worker_failed.load(std::memory_order_relaxed))
                {
                    PreparedJob job;
                    {
                        ...
                        active_prepared_jobs_bytes =
                            saturating_add_bytes(active_prepared_jobs_bytes, job.dynamic_bytes);
                    ...
                    try
                    {
                        ...
                        if (local_error.empty() && !patchmatch_store_root.empty())
                        {
                            ...
                            {
                                for (auto& level : result.cameras[ordinal].patchmatch.depth_levels)
                                    std::vector<float>().swap(level);
                                for (auto& level : result.cameras[ordinal].patchmatch.packed_inlier_masks)
            ...
            for (std::size_t worker = 0; worker < result.patchmatch_worker_count; ++worker)
                workers.emplace_back(run_worker);
            std::vector<std::thread> resource_preparation_workers;
            resource_preparation_workers.reserve(resource_preparation_worker_count);
            for (std::size_t worker = 0; worker < resource_preparation_worker_count; ++worker)
                resource_preparation_workers.emplace_back(run_resource_preparation_worker);
            ...
            if (patchmatch_store_root.empty())
            {
                ...
                batch.references.assign(reference_camera_indices.begin(), reference_camera_indices.end());
                batch.closure = batch.references;
                voting_batches.push_back(std::move(batch));
            }
            else if (!plan_recovered_patchmatch_store_batches(reference_camera_indices,
                                                              ranked_neighbors_by_camera,
                                                              voting_batch_size,
                                                              voting_batches,
                                                              error))
            {
            ...
            {
                std::vector<RecoveredPatchMatchD4PyramidOutput> loaded_products(scene.cameras.size());
                std::vector<const RecoveredPatchMatchD4PyramidOutput*> voting_products(scene.cameras.size(), nullptr);
                if (patchmatch_store_root.empty())
                {
                    for (const std::size_t camera_index : voting_batch.closure)
                        voting_products[camera_index] = &result.cameras[product_for_camera[camera_index]].patchmatch;
                }
                else
                {
                    for (const std::size_t camera_index : voting_batch.closure)
                    {
                        ...
                        voting_products[camera_index] = &loaded_products[camera_index];
                    }
                }
                ...
                const auto run_voting_worker = [&]()
                {
                    while (!worker_failed.load(std::memory_order_relaxed))
                    {
                        const std::size_t position = next_voting_position.fetch_add(1U, std::memory_order_relaxed);
                        if (position >= voting_batch.references.size())
                            return;
                        const std::size_t camera_index = voting_batch.references[position];
                        ...
                        try
                        {
                            ...
                            voting_neighbors.reserve(patchmatch.ranked_neighbor_camera_indices.size());
                            for (std::size_t rank = 0; rank < patchmatch.ranked_neighbor_camera_indices.size(); ++rank)
                            {
                                const std::size_t neighbor_camera = patchmatch.ranked_neighbor_camera_indices[rank];
                                ...
                                for (std::size_t level = 0; local_error.empty() && level < 3U; ++level)
                                {
                                    ...
                                    if (!unpack_recovered_patchmatch_inlier_mask(
                                            patchmatch.packed_inlier_masks[level],
                                            pixels,
                                    ...
                                    neighbor.inlier_mask_views[level] = neighbor.inlier_masks[level];
                                }
                                if (!local_error.empty())
                                    break;
                            ...
                            if (local_error.empty())
                            {
                                ...
                                if (local_error.empty() && !retain_voting_diagnostics)
                                {
                                    ...
                                    for (auto& values : camera_output.voting.depth_before_components)
