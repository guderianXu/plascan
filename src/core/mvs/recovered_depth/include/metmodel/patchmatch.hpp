...
namespace metmodel
{
    ...
    // CUDA C++ reconstruction used by the production perspective voting path.
    // Unsupported CalibrationCu models fail closed; production never silently
    // falls back to the recovered PTX oracle.
    bool run_recovered_depth_radius_estimate_cuda_source(const DepthRadiusEstimateInput& input,
                                                         std::vector<float>& radius_output,
                                                         std::string& error);

    // Explicit recovered-PTX oracle retained only for differential validation.
    ...
    // CUDA C++ reconstruction of the recovered final-voting PTX kernel.  This
    // remains a separately callable differential-validation path until its real
    // captures and complete voting-chain regressions pass bit-for-bit.
    ...
    std::uint32_t patchmatch_balanced_batch_span(std::uint32_t items,
    ...
    enum class PatchMatchUndistortSample : std::uint32_t
    {
        u8 = 0,
        u16 = 1,
        u32_to_f32 = 2,
    ...
    struct PatchMatchUndistortInput
    {
        std::uint32_t width = 0;
        std::uint32_t height = 0;
        std::uint32_t channels = 1;
        PatchMatchUndistortSample sample = PatchMatchUndistortSample::u8;
    ...
    struct PatchMatchUndistortOutput
    {
        std::uint32_t width = 0;
        std::uint32_t height = 0;
    ...
    // Independent CUDA-source implementation used for PTX differential testing
    // and by the production level-boundary path after bit-exact acceptance.
    bool run_recovered_patchmatch_bilateral_u8_cuda_source(const PatchMatchBilateralU8Input& input,
                                                           PatchMatchBilateralU8Output& output,
                                                           std::string& error);

    struct PatchMatchBilateralU16Input
    {
        std::uint32_t width = 0;
        std::uint32_t height = 0;
        std::vector<float> depth;
    ...
    struct PatchMatchBilateralF32Input
    {
        std::uint32_t width = 0;
        std::uint32_t height = 0;
        std::vector<float> depth;
    ...
    struct PatchMatchRefinementInput
    {
        ...
        // Target template specialization: 1=u8, 2=u16, 4=f32.
        std::uint32_t reference_image_sample_bytes = 1;
    ...
    struct PatchMatchFinalRefinementInput
    {
        PatchMatchCamera camera;
        std::uint32_t depth_downscale = 1;
        // Target template specialization: 1=u8, 2=u16, 4=f32. The backing is
        // byte-addressed so all specializations share the persistent workspace.
        std::uint32_t reference_image_sample_bytes = 1;
        std::vector<std::uint8_t> reference_image;
        std::uint32_t image_one_step_more_detailed = 0;
        float deviation_threshold_multiplier = 1.0F;
    ...
    struct PatchMatchPropagationInput
    {
        ...
        std::uint32_t depth_downscale = 1;
        // Target template specialization: 1=u8, 2=u16, 4=f32. The backing remains
        // byte-addressed so persistent workspace transport is shared by all three.
        std::uint32_t reference_image_sample_bytes = 1;
        std::vector<std::uint8_t> reference_image;
        std::uint32_t image_one_step_more_detailed = 0;
        float deviation_threshold_multiplier = 1.0F;
    ...
    public:
        ...
        RecoveredCudaPinnedNeighborTextures(const RecoveredCudaPinnedNeighborTextures&) = delete;
        RecoveredCudaPinnedNeighborTextures& operator=(const RecoveredCudaPinnedNeighborTextures&) = delete;
        RecoveredCudaPinnedNeighborTextures(RecoveredCudaPinnedNeighborTextures&& other) noexcept;
        RecoveredCudaPinnedNeighborTextures& operator=(RecoveredCudaPinnedNeighborTextures&& other) noexcept;

        void reset() noexcept;

    private:
        friend bool pin_recovered_patchmatch_neighbor_textures_cuda(const RecoveredPatchMatchNeighborResources&,
                                                                    std::size_t,
                                                                    RecoveredCudaPinnedNeighborTextures&,
                                                                    std::string&);
    ...
    struct PatchMatchRotateNormalsInput
    {
        PatchMatchCamera camera_transform;
        std::vector<float> normal_allocation;
        std::size_t normal_count = 0;
        std::size_t device_index = 0;
    };

    struct PatchMatchRotateNormalsOutput
    {
        std::vector<float> normal_allocation;
    };
    ...
    // Standalone differential boundary for pm_avg_all_neighbs_costs.  The target
    // uses fixed 128K planes; work_items counts logical pixels and the CUDA launch
    // contains work_items*hypotheses_per_pixel threads.
    struct PatchMatchAverageCostsInput
    {
        PatchMatchCamera camera;
        std::uint32_t depth_downscale = 1;
        std::uint32_t image_one_step_more_detailed = 0;
    ...
    struct PatchMatchAverageCostsOutput
    {
        std::vector<float> average_cost;
        std::vector<std::uint8_t> auxiliary;
    };
    ...
    struct PatchMatchCostInput
    {
        ...
        std::vector<std::uint8_t> reference_image_allocation;
        std::span<const std::uint8_t> reference_image_view;
        // Target template sample type shared by the linear reference image and
        // neighbour CUDA array: 1=U8, 2=U16, 4=F32. Storage remains byte-addressed
        // so captured allocations and persistent resource ownership stay stable.
        std::uint32_t image_sample_bytes = 1;
        std::uint32_t neighbor_texture_width = 0;
        std::uint32_t neighbor_texture_height = 0;
    ...
    bool run_recovered_patchmatch_filter_speckles_edges_cuda(const PatchMatchFilterSpecklesEdgesInput& input,
                                                             ...
                                                             std::string& error);
    // CUDA-source reconstruction currently validated for perspective cameras
    // (type 0). Other camera models remain fail-closed until matching captures
    // prove their inlined transcendental/RPC paths.
    ...
    // Complete evidence-backed d4 product boundary for one reference camera with
    // a 1..16-neighbor closure.  The strict-CUDA target RPC stereo capture proves
    // one selected neighbor; the recovered selector, packed-mask formulas, CUDA
    // cost path and depth-voting chain cover every count through the 16-neighbor
    // maximum.  Zero and counts above sixteen remain fail-closed.
    // Levels are ordered
