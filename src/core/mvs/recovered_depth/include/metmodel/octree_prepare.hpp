...
namespace metmodel
{
    ...
    build_ooc_histogram_voxels_mode0(const OocHistogramVoxelBuildInput& input);
    ...
    select_ooc_pyramid_maximum_level(const std::array<std::uint64_t, 33>& level_counts) noexcept;
    ...
    [[nodiscard]] OocAdaptiveRootMode0
    derive_ooc_adaptive_root_mode0(std::span<const OocPyramidRegistryLevelMetadata> metadata,
    ...
    [[nodiscard]] OocPyramidReductionOutput reduce_ooc_pyramid_level(const OocPyramidReductionInput& input);
    ...
    [[nodiscard]] OocPyramidPayloadLayout parse_ooc_pyramid_payload_layout(std::span<const std::byte> segment,
    ...
    [[nodiscard]] std::vector<std::byte> serialize_ooc_pyramid_payload(const OocSampleScalePyramidOutput& pyramid);

    [[nodiscard]] std::vector<std::byte>
    serialize_ooc_pyramid_registry(std::span<const OocPyramidRegistryRawRecord> records);
    ...
    [[nodiscard]] OocPyramidMetadataCodes quantize_ooc_pyramid_metadata_codes(float value, float base_scale);
    ...
    prepare_ooc_histogram_pyramid_selection_mode0(const OocHistogramVoxel& voxel,
    ...
    [[nodiscard]] OocHistogramAcceptedVote
    evaluate_ooc_histogram_sample(const OocHistogramAccumulatedSample& sample) noexcept;
    ...
    accumulate_ooc_histogram_camera_vote_mode0(OocHistogramVoxel& voxel,
                                               const OocHistogramProjectionInput& input,
                                               float threshold_b,
    ...
    [[nodiscard]] std::uint16_t ooc_weight_sum(const OocWeightedNodeRecord& record) noexcept;
    ...
    [[nodiscard]] std::uint8_t quantize_ooc_initial_weight(float cell_scale, float sample_scale) noexcept;
    ...
    [[nodiscard]] OocInitialWeightSelection select_ooc_initial_weight(float root_scale,
    ...
    [[nodiscard]] OocWeightedNodeRecord make_initial_ooc_weighted_node(const OocDepthCandidate& candidate,
                                                                       float root_scale,
                                                                       float alternate_scale,
                                                                       std::uint32_t maximum_level,
    ...
    [[nodiscard]] OocPyramidFinerLevelOutput build_ooc_pyramid_finer_level(const OocPyramidFinerLevelInput& input);
    ...
    [[nodiscard]] std::vector<float> expand_ooc_depth_roi(const OocDepthRoiExpansionInput& input);
    ...
    [[nodiscard]] std::vector<float> extract_ooc_depth_roi(std::size_t full_width,
                                                           std::size_t full_height,
    ...
    struct OocDepthRoiBounds
    {
        ...
        [[nodiscard]] bool valid() const noexcept
        {
            return x_begin >= 0 && y_begin >= 0 && x_begin < x_end && y_begin < y_end;
        }
    };
    ...
    [[nodiscard]] std::array<double, 16> condition_ooc_camera_transform_mode0(const std::array<double, 16>& matrix);
    ...
    [[nodiscard]] std::array<double, 16> invert_ooc_projective_matrix4(const std::array<double, 16>& matrix);

    [[nodiscard]] OocDepthRoiBoundsOutput compute_ooc_depth_roi_bounds_mode0(const OocDepthRoiBoundsMode0Input& input);

    [[nodiscard]] OocDepthRoiBoundsOutput
    compute_ooc_depth_roi_bounds_from_project_mode0(const OocDepthRoiProjectMode0Input& input);
    ...
    [[nodiscard]] float derive_ooc_maximum_sample_scale(const std::array<double, 3>& region_size) noexcept;
    ...
    [[nodiscard]] OocDepthRoiBounds downsample_ooc_depth_roi_bounds(const OocDepthRoiBounds& bounds);
    ...
    make_ooc_pyramid_registry_record_mode0(const OocPyramidRegistryMode0Input& input,
    ...
    [[nodiscard]] std::vector<OocPyramidCameraGroupRange> partition_ooc_pyramid_camera_groups(
        std::size_t item_count, std::size_t workitem_size_cameras, std::size_t max_workgroup_size);
    ...
    [[nodiscard]] OocPyramidWorkerPlan plan_ooc_pyramid_workers(std::uint64_t available_memory_bytes,
                                                                std::uint32_t available_openmp_threads,
                                                                std::size_t max_depthmap_pixels,
                                                                std::size_t camera_task_count);
    ...
    serialize_ooc_pyramid_bundle_mode0(std::span<const OocPyramidBundleMode0Group> groups);
    ...
    [[nodiscard]] OocSampleScalePyramidOutput build_ooc_sample_scale_pyramid_from_scene_voted_depths_mode0(
        const Scene& scene,
    ...
    [[nodiscard]] OocSampleScalePyramidOutput
    build_ooc_sample_scale_pyramid_mode0(const OocSampleScalePyramidInput& input);
    ...
    [[nodiscard]] OocPyramidRegistryLevelMetadata
    make_ooc_pyramid_registry_level_metadata(const OocSampleScaleWorkerOutput& output);
    ...
    [[nodiscard]] OocSampleScaleWorkerOutput build_ooc_sample_scale_workspace(const OocSampleScaleWorkerInput& input);
    ...
    // all-zero records.
    [[nodiscard]] std::vector<OocDepthCandidate>
    build_ooc_depth_candidate_workspace(const OocDepthCandidateInput& input);
    ...
    [[nodiscard]] std::vector<OocDepthCandidate>
    compact_ooc_depth_candidates(std::span<const OocDepthCandidate> workspace);
    ...
    [[nodiscard]] float aggregate_ooc_marching_scalar(std::span<const float> scalars, std::span<const float> weights);

    // Exact sub_1EA3C00 quotient, including the denominator-zero representation.
    [[nodiscard]] std::uint8_t normalized_ooc_weight(const OocWeightedNodeRecord& record);
    ...
    [[nodiscard]] bool ooc_weighted_node_less(const OocWeightedNodeRecord& left,
    ...
    [[nodiscard]] std::vector<OocWeightedNodeRecord>
    filter_ooc_multi_camera_nodes(std::span<const OocWeightedNodeRecord> records);
    ...
    [[nodiscard]] std::vector<OocWeightedNodeRecord>
    balance_ooc_weighted_nodes(std::span<const OocWeightedNodeRecord> records);
    ...
    initialize_ooc_octree_records(std::span<const OocWeightedNodeRecord> balanced_records);
    ...
    select_ooc_octree_nodes_breadth_first(std::span<const OocOctreeRecord> records);
    ...
    [[nodiscard]] std::vector<std::uint8_t> make_ooc_leaf_active_mask(std::span<const OocOctreeRecord> records,
    ...
    struct OocMarchingActiveCellEntry
    {
        ...
        friend bool operator==(const OocMarchingActiveCellEntry&, const OocMarchingActiveCellEntry&) = default;
    };
    ...
    plan_ooc_marching_capacity_frontier(std::span<const OocMarchingExtractNode> nodes,
    ...
    [[nodiscard]] OocMarchingActiveCells
    build_ooc_marching_active_cells(std::span<const OocMarchingExtractNode> nodes,
    ...
    struct OocMarchingTriangle
    {
        ...
        friend bool operator==(const OocMarchingTriangle&, const OocMarchingTriangle&) = default;
    };
    ...
    [[nodiscard]] OocMarchingEdgeOutput make_ooc_marching_edge_vertex(const OocMarchingEdgeInput& input);
    ...
    [[nodiscard]] OocMarchingEdgeMesh build_ooc_marching_edge_mesh(std::vector<OocMarchingCellState> cell_states,
    ...
    // center fallbacks.
    [[nodiscard]] std::vector<OocMarchingCellState>
    complete_ooc_marching_cell_states(std::vector<OocMarchingCellState> initial_states,
                                      const OocMarchingActiveCells& active_cells,
    ...
    [[nodiscard]] OocMarchingContourTrace trace_ooc_marching_local_contour(const OocMarchingCellState& cell,
    ...
    [[nodiscard]] OocMarchingAdaptiveGridStep trace_ooc_marching_adaptive_grid_step(
        const OocMarchingCellState& cell, std::uint32_t edge, const std::array<std::int32_t, 25>& grid);
    ...
    [[nodiscard]] OocMarchingRawOutput build_ooc_marching_raw_mesh(const OocMarchingEdgeMesh& edges,
                                                                   std::uint64_t root_state_index,
                                                                   std::uint32_t maximum_level,
    ...
    [[nodiscard]] OocMarchingPolygonDpResult
    triangulate_ooc_marching_polygon(std::span<const OocMarchingVertex> polygon_vertices,
    ...
    [[nodiscard]] OocFusionState prepare_ooc_fusion_state(const std::vector<OocOctreeRecord>& records,
    ...
    [[nodiscard]] std::vector<std::uint16_t> pack_ooc_fused_scalars(const std::vector<float>& fused_u);
    ...
    [[nodiscard]] std::vector<OocMarchingNode>
    make_ooc_marching_nodes(const std::vector<OocOctreeRecord>& records,
                            const std::vector<std::uint32_t>& selected_indices,
    ...
    [[nodiscard]] std::vector<OocMarchingExtractNode>
    build_ooc_marching_extract(const std::vector<OocMarchingNode>& nodes);
    ...
    {
        struct MarchingMiniPart
        {
            OocMarchingWorkCell root;
            std::size_t inner_cell_begin{};
            std::size_t inner_cell_end{};
        ...
        std::vector<MarchingMiniPart> marching_mini_parts;
        OocMarchingRawOutput raw_mesh;
        std::vector<std::uint8_t> raw_vertex_mini_boundary_mask;
        ...
        double marching_seam_seconds{};
        double marching_mini_boundary_seconds{};
    };

    // Exact default build-22956 OOC model-stage selector. The maximum support
    // level covers 99% of the balanced nodes. The first level is selected from
    // the first part's 0x20000-node cap and aligned to the two-level cadence.
    struct RecoveredOocSupportSchedule
    {
        std::uint32_t first_level{};
        std::uint32_t maximum_level{};
        std::uint32_t level_step{2U};
        std::uint32_t first_part_voxel_cap{0x20000U};
        std::uint64_t total_voxels{};
        std::uint64_t maximum_quantile_threshold{};
        std::vector<std::uint32_t> levels;
    };

    [[nodiscard]] RecoveredOocSupportSchedule
    ...
    // Recovered one-part mode-0 executor used by the South-building target path.
    // Production callers obtain the default stage list from the selector above;
    // the explicit list remains available for differential replay and overrides.
    ...
    run_recovered_ooc_multilevel_model_cpu(std::span<const OocOctreeRecord> histogram_records,
                                           ...
                                           const OocFusionParameters& fusion_parameters = {},
                                           bool partition_for_preset_qem = false);
    ...
    // update rounds and uses the recovered CUDA source kernels/launch partitioning.
    [[nodiscard]] RecoveredOocMultilevelModelOutput
