...
struct OocFusionParameters {
    ...
    std::function<void(std::uint32_t)> stage_progress;
};
...
// Persistent-device CUDA C++ implementation of the recovered
// functional-type-2 schedule.  All twelve SoA arrays stay resident for the
// complete iteration chain; each update_u/update_p launch is followed by the
// target's stream synchronization boundary.
...
// Compatibility alias for callers that selected the source implementation
// explicitly before it became the production backend.
...
// Test-only oracle retaining the extracted PTX implementation for strict
// source/PTX differential validation.  Production code must use
// run_ooc_fusion_cuda().
