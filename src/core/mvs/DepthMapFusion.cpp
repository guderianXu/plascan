...
namespace xjw
{
    namespace mvs
    {
        ...
        namespace
        {
            ...
            {
            public:
                ColorImageCache(const std::vector<FusionFrameInput>& frames, bool useColor, int capacity)
                    : m_frames(frames), m_useColor(useColor), m_capacity(std::max(0, capacity))
                {
                }
                ...
                cv::Mat get(int frameIdx)
                {
                    ...
                    if (!image.empty() && m_frames[frameIdx].imgW > 0 && m_frames[frameIdx].imgH > 0 &&
        ...
        // =============================================================================
        DepthMapFusion::DepthMapFusion(const StereoFusionConfig& config) : _config(config)
        {
        }
        ...
        bool
        DepthMapFusion::isPixelEligible(const FusionFrameInput& frame, const FrameGeometry& geometry, int row, int col)
        {
            ...
            {
                const int observation_count = static_cast<int>(frame.geometrySupportCount.at<std::uint16_t>(row, col));
            ...
            {
                const int offsets[4][2] = {{-local_gradient_radius, 0},
                ...
                for (const auto& offset : offsets)
                {
                    ...
                    if (neighbor_row < 0 || neighbor_row >= frame.depthMap.rows || neighbor_col < 0 ||
                        neighbor_col >= frame.depthMap.cols)
                    {
                        continue;
                    }
                    if (!frame.validMask.empty() && frame.validMask.at<std::uint8_t>(neighbor_row, neighbor_col) == 0)
                    {
                        continue;
                    }
                ...
                if (difference_count >= 2)
                {
                    ...
                    if (relative_differences[static_cast<std::size_t>(lower_middle)] > _config.maxLocalDepthGradient)
                    {
        ...
        // 预计算投影几何
        // =============================================================================
        void DepthMapFusion::prepareGeometry(const std::vector<FusionFrameInput>& frames,
                                             std::vector<FrameGeometry>& geom)
        {
            ...
            for (int fi = 0; fi < NF; ++fi)
            {
                ...
                if (_config.pixelParametersUsePreparedRaster && pixel_scale.usesReducedGrid())
                {
                    LOG_DEBUG("[MVS][深度融合][像素域] frame=%d raster=%dx%d grid=%dx%d "
        ...
        // 计算重叠图像：基于相机中心距离和朝向
        // =============================================================================
        void DepthMapFusion::computeOverlappingImages(const std::vector<FusionFrameInput>& frames,
                                                      ...
                                                      std::vector<std::vector<int>>& overlapping)
        {
            ...
            for (int fi = 0; fi < NF; ++fi)
            {
                ...
                {
                    overlapping[fi].reserve(
                        std::min(_config.checkNumImages, static_cast<int>(frames[fi].sourceImageIndices.size())));
                    for (int sourceIdx : frames[fi].sourceImageIndices)
                    {
                        ...
                        if (std::find(overlapping[fi].begin(), overlapping[fi].end(), sourceIdx) !=
                            overlapping[fi].end())
                        {
                            continue;
                        }
                ...
                std::sort(candidates.begin(),
                          candidates.end(),
                          [](const OverlapInfo& a, const OverlapInfo& b) { return a.dist < b.dist; });
        ...
        // =============================================================================
        bool DepthMapFusion::fusePixel(int imageIdx,
                                       int row,
                                       int col,
                                       ...
                                       FusedPoint& outPoint)
        {
            ...
            while (!bfsQueue.empty() && (int)xs.size() < _config.maxNumPixels)
            {
                ...
                if (td > 0 && hasRef)
                {
                    ...
                    if (!frames[fi].normalMap.empty() && !frames[imageIdx].normalMap.empty())
                    {
                        ...
                        for (int k = 0; k < 3; ++k)
                        {
                            nw_curr[k] = invR_curr[k * 3 + 0] * nCurr[0] + invR_curr[k * 3 + 1] * nCurr[1] +
                                         invR_curr[k * 3 + 2] * nCurr[2];
                            nw_ref[k] = invR_ref[k * 3 + 0] * nRef[0] + invR_ref[k * 3 + 1] * nRef[1] +
                                        invR_ref[k * 3 + 2] * nRef[2];
                        }

                        float cosAngle = nw_curr[0] * nw_ref[0] + nw_curr[1] * nw_ref[1] + nw_curr[2] * nw_ref[2];
                ...
                {
                    if (Xw < _config.bboxMin[0] || Xw > _config.bboxMax[0] || Yw < _config.bboxMin[1] ||
                        Yw > _config.bboxMax[1] || Zw < _config.bboxMin[2] || Zw > _config.bboxMax[2])
                    {
                        continue;
                    }
                }
            ...
            if (!nxs.empty())
            {
                ...
                float nLen =
                    std::sqrt(outPoint.nx * outPoint.nx + outPoint.ny * outPoint.ny + outPoint.nz * outPoint.nz);
                if (nLen > 1e-6f)
                {
                    outPoint.nx /= nLen;
                    outPoint.ny /= nLen;
        ...
        bool DepthMapFusion::fuseTwoViewSingleObservationFast(const std::vector<FusionFrameInput>& frames,
                                                              const std::vector<FrameGeometry>& geom,
                                                              const std::function<cv::Mat(int)>& colorProvider,
                                                              ...
                                                              MvsProgressCallback progressCb)
        {
            ...
            LOG_INFO("[MVS][深度融合] 快速双视图融合: workers=%d min_pixels=%d", workerCount, _config.minNumPixels);
            ...
            for (int fi = 0; fi < kFrameCount; ++fi)
            {
                const std::size_t pixelCount =
                    static_cast<std::size_t>(geom[fi].W) * static_cast<std::size_t>(geom[fi].H);
            ...
            auto inBoundingBox = [&](float x, float y, float z)
            {
                ...
                return x >= _config.bboxMin[0] && x <= _config.bboxMax[0] && y >= _config.bboxMin[1] &&
                       y <= _config.bboxMax[1] && z >= _config.bboxMin[2] && z <= _config.bboxMax[2];
            };
                    ...
                    [&](std::size_t worker, std::stop_token stopToken)
                    {
                        ...
                        for (;;)
                        {
                            ...
                            for (int col = 0; col < W; ++col)
                            {
                                ...
                                if (depth <= 0.f || !isPixelEligible(frames[fi], geom[fi], row, col))
                                {
                                    continue;
                                }
                                ...
                                if (projectWorldPoint(geom[other].cameraModel, x0, y0, z0, uOther, vOther))
                                {
                                    ...
                                    if (inBounds(other, otherRow, otherCol))
                                    {
                                        ...
                                        bool consistent =
                                            ...
                                            zExpected > 0.f;
                                        if (consistent && du * du + dv * dv > geom[other].maxReprojectionErrorSquared)
                                        {
                                            _reprojectionRejected.fetch_add(1, std::memory_order_relaxed);
                                            consistent = false;
                                        }
            ...
            LOG_INFO("[MVS][深度融合] 快速双视图融合完成: points=%d", static_cast<int>(fusedPoints.size()));
            if (progressCb)
            {
        ...
        bool DepthMapFusion::fuseFirstFrameObservationsFast(const std::vector<FusionFrameInput>& frames,
                                                            const std::vector<FrameGeometry>& geom,
                                                            const std::function<cv::Mat(int)>& colorProvider,
                                                            std::vector<FusedPoint>& fusedPoints,
                                                            MvsProgressCallback progressCb)
        {
            ...
            auto readWorldNormal = [&](int frameIdx, int row, int col, float& nx, float& ny, float& nz)
            {
                if (frameIdx < 0 || frameIdx >= static_cast<int>(frames.size()) || frames[frameIdx].normalMap.empty())
                {
                    return false;
                }
            ...
            {
                if (frameIdx < 0 || frameIdx >= static_cast<int>(frames.size()) || frames[frameIdx].depthMap.empty())
                {
                    return false;
                }
                const FrameGeometry& frameGeom = geom[frameIdx];
                if (frameGeom.W < 2 || frameGeom.H < 2 || row < 0 || row >= frameGeom.H || col < 0 ||
                    col >= frameGeom.W)
                {
                    return false;
                }
                ...
                if (!unprojectSample(row, leftCol, xl, yl, zl) || !unprojectSample(row, rightCol, xr, yr, zr) ||
                    !unprojectSample(upRow, col, xu, yu, zu) || !unprojectSample(downRow, col, xd, yd, zd))
                {
                    return false;
                }
            ...
            cv::Mat worldPointMap;
            bool hasPrecomputedWorldPoints = false;
            const bool explicitAccelerator = _config.computeBackend == DenseCloudComputeBackend::Cuda ||
                                             _config.computeBackend == DenseCloudComputeBackend::OpenCl;
            if (explicitAccelerator)
            {
                cv::Mat acceleratorMask = cv::Mat::zeros(H, W, CV_8U);
                std::size_t acceleratorPointCount = 0;
                for (int row = 0; row < H; ++row)
                {
                    const float* depthRow = frames[fi].depthMap.ptr<float>(row);
                    const std::uint8_t* validRow =
                        frames[fi].validMask.empty() ? nullptr : frames[fi].validMask.ptr<std::uint8_t>(row);
                    std::uint8_t* maskRow = acceleratorMask.ptr<std::uint8_t>(row);
                    for (int col = 0; col < W; ++col)
                    {
                        const float depth = depthRow[col];
                        if (std::isfinite(depth) && depth >= std::numeric_limits<float>::denorm_min() &&
                            (!validRow || validRow[col] != 0))
                        {
                            maskRow[col] = 255;
                            ++acceleratorPointCount;
                        }
                    }
                }
                ...
                options.useGPU = true;
                options.computeBackend = _config.computeBackend;
                options.cudaDeviceIndex = _config.cudaDeviceIndex;
                options.openClDeviceIndex = _config.openClDeviceIndex;
                options.minDepth = std::numeric_limits<float>::denorm_min();
                options.maxDepth = std::numeric_limits<float>::max();

                std::vector<DensePoint> acceleratedPoints;
                ...
                if (!DenseCloudBuilder::unprojectWithReport(frames[fi].depthMap,
                                                            ...
                                                            &acceleratorError))
                {
                    _unprojectionError = "深度融合反投影失败: " + acceleratorError;
                    return false;
                }
                if (acceleratedPoints.size() != acceleratorPointCount)
                {
                    _unprojectionError = "深度融合反投影结果与有效像素数量不一致";
                    return false;
                }

                worldPointMap = cv::Mat::zeros(H, W, CV_32FC3);
                std::size_t pointIndex = 0;
                for (int row = 0; row < H; ++row)
                {
                    const std::uint8_t* maskRow = acceleratorMask.ptr<std::uint8_t>(row);
                    cv::Vec3f* worldRow = worldPointMap.ptr<cv::Vec3f>(row);
                    for (int col = 0; col < W; ++col)
                    {
                        if (maskRow[col] == 0)
                        {
                        ...
                        const DensePoint& point = acceleratedPoints[pointIndex++];
                        worldRow[col] = cv::Vec3f(point.x, point.y, point.z);
                    }
                }
                _unprojectionExecutionReport = std::move(report);
                hasPrecomputedWorldPoints = true;
                LOG_INFO("[MVS][深度融合] 反投影预计算后端: requested=%s actual=%s device=%s；一致性/聚合=CPU",
                         denseCloudComputeBackendId(_unprojectionExecutionReport.requestedBackend),
                         denseCloudComputeBackendId(_unprojectionExecutionReport.actualBackend),
                         _unprojectionExecutionReport.deviceName.empty()
                             ? "CPU"
                             : _unprojectionExecutionReport.deviceName.c_str());
            }
            ...
            xjw::common::concurrency::runWorkerGroup(
                ...
                [&](std::size_t worker, std::stop_token stopToken)
                {
                    ...
                    for (;;)
                    {
                        ...
                        {
                            const float depth = depthRow[col];
                            if (depth <= 0.0f || !isPixelEligible(frames[fi], geom[fi], row, col))
                            {
                                continue;
                            }

                            FusedPoint point;
                            if (hasPrecomputedWorldPoints)
                            {
                                const cv::Vec3f& worldPoint = worldPointMap.at<cv::Vec3f>(row, col);
                            ...
                            {
                                hasPointNormal =
                                    estimateWorldNormalFromDepth(fi, row, col, point.nx, point.ny, point.nz);
                            }
                            ...
                            if (requireNeighborAgreement)
                            {
                                ...
                                for (int otherFrame : neighborFrames)
                                {
                                    ...
                                    const int otherCol = static_cast<int>(std::round(uOther));
                                    const int otherRow = static_cast<int>(std::round(vOther));
                                    if (otherRow < 0 || otherRow >= otherGeom.H || otherCol < 0 ||
                                        otherCol >= otherGeom.W)
                                    {
                                        continue;
                                    }
                                    const float du = uOther - static_cast<float>(otherCol);
                                    const float dv = vOther - static_cast<float>(otherRow);
                                    if (du * du + dv * dv > otherGeom.maxReprojectionErrorSquared)
                                    {
                                        _reprojectionRejected.fetch_add(1, std::memory_order_relaxed);
                                        continue;
                                    }
                                    ...
                                    if (std::fabs(otherDepth - expectedDepth) / expectedDepth > _config.maxDepthError)
                                    {
                                        _depthConsistencyRejected.fetch_add(1, std::memory_order_relaxed);
                                        continue;
                                    }
                                    ...
                                    if (!hasOtherNormal)
                                    {
                                        hasOtherNormal = estimateWorldNormalFromDepth(
                                            otherFrame, otherRow, otherCol, otherNx, otherNy, otherNz);
                                    }
            ...
            const float yieldRatio =
                validCount > 0 ? static_cast<float>(fusedPoints.size()) / static_cast<float>(validCount) : 1.0f;
            if (_config.enableLowYieldFallback && requireNeighborAgreement && requiredObservations > 2 &&
                validCount > 0 && yieldRatio < std::max(0.0f, _config.lowYieldFallbackMinRatio) &&
                !isFusionCancelled(_config))
            {
                ...
                {
                    fallbackOk =
                        fuseFirstFrameObservationsFast(frames, geom, colorProvider, fallbackPoints, progressCb);
                }
                catch (...)
                {
                    _config = savedConfig;
                ...
                if (fallbackPoints.size() > fusedPoints.size())
                {
                    ...
                    LOG_WARN("[MVS][深度融合] 已采用双视一致性回退: points=%d", static_cast<int>(fusedPoints.size()));
                }
            }
        ...
        // =============================================================================
        bool DepthMapFusion::fuse(const std::vector<FusionFrameInput>& frames,
                                  std::vector<FusedPoint>& fusedPoints,
                                  MvsProgressCallback progressCb,
                                  std::string* errorMsg)
        {
            ...
            resetRejectionStats();
            _unprojectionError.clear();
            _unprojectionExecutionReport = {};
            _unprojectionExecutionReport.requestedBackend = _config.computeBackend;
            _unprojectionExecutionReport.actualBackend = DenseCloudComputeBackend::Cpu;
            _unprojectionExecutionReport.deviceIndex = -1;
            _unprojectionExecutionReport.deviceName = "CPU";
            _unprojectionExecutionReport.workSubmitted = false;
            ...
            for (int fi = 0; fi < NF; ++fi)
            {
                ...
                if (!frames[fi].validMask.empty() &&
                    (frames[fi].validMask.type() != CV_8U || frames[fi].validMask.size() != frames[fi].depthMap.size()))
                {
                    if (errorMsg)
                    {
                ...
                if (_config.minGeometryObservationCount > 0 && frames[fi].geometrySupportCount.empty())
                {
                    if (errorMsg)
                    {
                     ...
                     frames[fi].geometrySupportCount.type() != CV_16UC1))
                {
                    if (errorMsg)
                    {
                        *errorMsg = "帧 " + std::to_string(fi) + " 跨视几何支持计数必须为与深度图同尺寸的 CV_16UC1";
                    }
                    return false;
                }
            }
            ...
            {
                if (_config.computeBackend == DenseCloudComputeBackend::Auto ||
                    _config.computeBackend == DenseCloudComputeBackend::Cpu)
                {
                    _unprojectionExecutionReport.workSubmitted = true;
                }
                ColorImageCache colorCache(frames, _config.useColor, _config.colorCacheCapacity);
                auto colorProvider = [&colorCache](int frameIdx) { return colorCache.get(frameIdx); };
                bool fastOk = false;
                try
                {
                    fastOk = fuseFirstFrameObservationsFast(frames, geom, colorProvider, fusedPoints, progressCb);
                }
                ...
                if (!fastOk && errorMsg)
                {
                    ...
                    else if (!_unprojectionError.empty())
                    {
                        *errorMsg = _unprojectionError;
                    }
                }
                return fastOk;
            }

            if (_config.computeBackend == DenseCloudComputeBackend::Cuda ||
                _config.computeBackend == DenseCloudComputeBackend::OpenCl)
            {
                if (errorMsg)
                {
                    *errorMsg = "显式 CUDA/OpenCL 深度融合仅支持 fuseOnlyFirstFrame 流式窗口模式";
                }
                return false;
            }
            _unprojectionExecutionReport.workSubmitted = true;
            ...
            if (!_config.fuseOnlyFirstFrame && NF == 2 && _config.minNumPixels <= 1)
            {
                ...
                try
                {
                    fastOk = fuseTwoViewSingleObservationFast(frames, geom, colorProvider, fusedPoints, progressCb);
                }
                catch (const std::exception& error)
                {
                    if (errorMsg)
            ...
            LOG_INFO("[MVS][深度融合] 完成: points=%d", static_cast<int>(fusedPoints.size()));
