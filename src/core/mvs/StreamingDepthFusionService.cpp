...
namespace xjw::mvs
{
    ...
    std::vector<int> streamingFusionWindowIndices(int referenceIndex, int frameCount, int neighborCount)
    {
                                ...
                                const FusedCloudReducer& cloudReducer)
    {
        ...
        result->unprojectionExecution = {};
        result->unprojectionExecution.requestedBackend = config.computeBackend;
        result->mixedUnprojectionBackends = false;
        ...
        for (int referenceIndex = 0; referenceIndex < frameCount; ++referenceIndex)
        {
            ...
            const std::vector<int> indices = streamingFusionWindowIndices(referenceIndex, frameCount, neighborCount);
            ...
            fusionConfig.computeBackend = config.computeBackend;
            fusionConfig.cudaDeviceIndex = config.cudaDeviceIndex;
            fusionConfig.openClDeviceIndex = config.openClDeviceIndex;
            ...
            if (frames.size() <= 2)
            {
                ...
                configured_reprojection_pixels = std::max(configured_reprojection_pixels, 3.0f);
            }
            ...
            const bool ok = fusion.fuse(
                ...
                [referenceIndex, frameCount, &progress](const std::string& stage, float ratio)
                {
                    if (progress)
                    {
                        const int percent = 70 + static_cast<int>(((static_cast<float>(referenceIndex) + ratio) /
                                                                   static_cast<float>(std::max(1, frameCount))) *
                                                                  20.0f);
            ...
            const DenseCloudExecutionReport& batchExecution = fusion.unprojectionExecutionReport();
            if (referenceIndex == 0)
            {
                result->unprojectionExecution = batchExecution;
            }
            else if (result->unprojectionExecution.actualBackend != batchExecution.actualBackend ||
                     result->unprojectionExecution.deviceIndex != batchExecution.deviceIndex)
            {
                result->mixedUnprojectionBackends = true;
                result->unprojectionExecution.fallbackUsed = true;
                result->unprojectionExecution.fallbackReason = "不同流式融合批次使用了不同反投影后端或设备";
            }
            ...
            if (progress)
            {
                ...
                progress("batch=" + std::to_string(referenceIndex + 1) + "/" + std::to_string(frameCount) + " window=" +
                             std::to_string(frames.size()) + " points=" + std::to_string(result->points.size()) +
