...
TEST(DepthProvenanceTest, TracksMeasuredAndInterpolatedSourcesSeparately)
{
    ...
    cv::Mat residual(depth.size(), CV_8UC1, cv::Scalar(0));
    residual.at<std::uint8_t>(2, 0) = 255;

    xjw::mvs::updateDepthProvenance(
        provenance, depth, targeted, cross_view, interpolated, residual);
    const auto summary = xjw::mvs::summarizeDepthProvenance(
        provenance, depth);

    EXPECT_TRUE(summary.available);
    EXPECT_EQ(summary.validPixelCount, 11);
    EXPECT_EQ(summary.nativePatchMatchPixelCount, 7);
    ...
    EXPECT_EQ(summary.residualPatchMatchPixelCount, 1);
