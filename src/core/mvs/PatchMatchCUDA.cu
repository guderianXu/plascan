...
#define CUDA_CHECK(x)                                                                                                  \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            {                                                                                                          \
                *errorMsg =                                                                                            \
                    std::string("CUDA error at ") + __FILE__ + ":" + std::to_string(__LINE__) + " " + _cuda_msg;       \
            }                                                                                                          \
            return false;                                                                                              \
        }                                                                                                              \
    } while (0)
...
template <typename T> class ReusableCudaBuffer
{
public:
    T* ptr = nullptr;
    ...
    cudaError_t reserve(std::size_t count)
    {
        ...
        const cudaError_t allocation_error = cudaMalloc(reinterpret_cast<void**>(&replacement), count * sizeof(T));
...
template <typename T> class ReusablePinnedBuffer
{
public:
    T* ptr = nullptr;
    std::size_t capacity = 0;
    ...
    cudaError_t reserve(std::size_t count)
    {
        ...
        const cudaError_t allocation_error =
            cudaHostAlloc(reinterpret_cast<void**>(&replacement), count * sizeof(T), cudaHostAllocPortable);
        if (allocation_error != cudaSuccess)
        {
            return allocation_error;
        }
...
struct PatchMatchGpuWorkspace
{
    ...
    cudaError_t initialize()
    {
        ...
        for (cudaEvent_t* event : {&uploadsReady, &cancellationCheckpoint, &computeReady, &downloadsReady})
        {
...
std::unordered_map<int, std::unique_ptr<PatchMatchGpuDeviceState>> g_patchMatchGpuDeviceStates;
...
{
public:
    explicit CudaDeviceRestoreGuard(int previousDevice) : _previousDevice(previousDevice)
    {
    }
...
struct ImageUploadLane
{
    ...
    ReusablePinnedBuffer<std::uint8_t>& host(std::size_t index)
    {
        ...
        {
            hosts.push_back(std::make_unique<ReusablePinnedBuffer<std::uint8_t>>());
        }
        return *hosts[index];
    }
...
{
public:
    explicit PatchMatchGpuRunGuard(PatchMatchGpuWorkspace& workspace) : _workspace(workspace)
    {
    }
                                  ...
                                  cudaStream_t stream)
{
    ...
    return cudaMemcpyAsync(destination, hostBuffer.ptr, bytes, cudaMemcpyHostToDevice, stream);
}
...
struct SrcImageCacheKey
{
    ...
    {
        return deviceIndex == other.deviceIndex && hostData == other.hostData && rows == other.rows &&
               cols == other.cols && step == other.step && ds == other.ds;
    }
};
...
void evictSrcImageGpuCacheIfNeeded(size_t needBytes, int deviceIndex, cudaStream_t releaseStream = nullptr)
{
    ...
    while (device_cache_bytes() + needBytes > cacheLimitBytes && !g_srcImageGpuCache.empty())
    {
        ...
        {
            if (it->first.deviceIndex == deviceIndex && it->second.pinCount == 0 &&
                (lruIt == g_srcImageGpuCache.end() || it->second.lastUseTick < lruIt->second.lastUseTick))
            {
                lruIt = it;
            }
        }
        ...
        if (lruIt->second.devicePtr)
        {
            if (lruIt->second.readyEvent)
            {
                if (releaseStream)
                {
                    cudaStreamWaitEvent(releaseStream, lruIt->second.readyEvent, 0);
                }
                else
                {
...
bool getOrUploadGrayImageGpu(const cv::Mat& srcGray,
                             int deviceIndex,
                             int scaledW,
                             int scaledH,
                             int ds,
                             ...
                             std::string* errorMsg)
{
    ...
    cudaError_t allocErr = cudaMallocAsync(reinterpret_cast<void**>(&newDevicePtr), bytes, transferStream);
    ...
    if (hostReserveError != cudaSuccess)
    {
        ...
        if (errorMsg)
        {
            *errorMsg = std::string("cudaHostAlloc failed: ") + cudaGetErrorString(hostReserveError);
        }
        return false;
    }
    std::memcpy(uploadHost.ptr, srcFloat.ptr<float>(), bytes);
    cudaError_t copyErr = cudaMemcpyAsync(newDevicePtr, uploadHost.ptr, bytes, cudaMemcpyHostToDevice, transferStream);
    ...
    cudaError_t eventError = cudaEventCreateWithFlags(&newReadyEvent, cudaEventDisableTiming);
    ...
    if (eventError != cudaSuccess)
    {
        ...
        if (errorMsg)
        {
            *errorMsg = std::string("CUDA upload event failed: ") + cudaGetErrorString(eventError);
        }
        return false;
    }

    {
        ...
        if (existing != g_srcImageGpuCache.end() && existing->second.scaledW == scaledW &&
            existing->second.scaledH == scaledH)
        {
...
__device__ bool bilinearMaskValid(const std::uint8_t* mask, int width, int height, float u, float v)
{
    if (mask == nullptr)
    {
        return true;
    ...
    return mask[row * width + column] != 0 && mask[row * width + column + 1] != 0 &&
           mask[(row + 1) * width + column] != 0 && mask[(row + 1) * width + column + 1] != 0;
}
                                  ...
                                  float& gradientY)
{
    ...
    if (mask != nullptr && (mask[y * width + x - 1] == 0 || mask[y * width + x + 1] == 0 ||
                            mask[(y - 1) * width + x] == 0 || mask[(y + 1) * width + x] == 0))
    {
        return false;
    }
    gradientX = 0.5f * (image[y * width + x + 1] - image[y * width + x - 1]);
    gradientY = 0.5f * (image[(y + 1) * width + x] - image[(y - 1) * width + x]);
    return true;
}
...
__device__ bool sourceGradient(const float* image,
                               ...
                               float& gradientY)
{
    if (!bilinearMaskValid(mask, width, height, x - 1.0f, y) || !bilinearMaskValid(mask, width, height, x + 1.0f, y) ||
        !bilinearMaskValid(mask, width, height, x, y - 1.0f) || !bilinearMaskValid(mask, width, height, x, y + 1.0f))
    {
        return false;
    }
...
__device__ inline void faceNormalTowardCamera(int row, int col, float normal[3])
{
    ...
    const float ray[3] = {c_ref_inv_K[0] * col + c_ref_inv_K[1], c_ref_inv_K[2] * row + c_ref_inv_K[3], 1.0f};
...
__device__ inline void alignedRandomFacingNormal(int row, int col, uint32_t& state, float normal[3])
{
...
alignedPerturbFacingNormal(int row, int col, const float normal[3], float amount, uint32_t& state, float output[3])
{
    output[0] = normal[0] + amount * (alignedPatchMatchRandom(state) * 2.0f - 1.0f);
    output[1] = normal[1] + amount * (alignedPatchMatchRandom(state) * 2.0f - 1.0f);
    output[2] = normal[2] + amount * (alignedPatchMatchRandom(state) * 2.0f - 1.0f);
...
__device__ void generateRandomNormal(int row, int col, curandState* rs, float normal[3])
{
    float v1, v2, s;
    do
    {
    ...
    float ray[3] = {c_ref_inv_K[0] * col + c_ref_inv_K[1], c_ref_inv_K[2] * row + c_ref_inv_K[3], 1.f};
...
/// 扰动法向量（小角度随机旋转）
__device__ void
perturbNormal(int row, int col, float perturbation, const float n[3], curandState* rs, float out[3], int trial = 0)
{
    ...
    R[1] = -ca2 * sa3;
    R[2] = sa2;
    R[3] = ca1 * sa3 + ca3 * sa1 * sa2;
    R[4] = ca1 * ca3 - sa1 * sa2 * sa3;
    R[5] = -ca2 * sa1;
    R[6] = sa1 * sa3 - ca1 * ca3 * sa2;
    R[7] = ca3 * sa1 + ca1 * sa2 * sa3;
    R[8] = ca1 * ca2;
    ...
    if (dot3(out, ray) >= 0.f)
    {
        if (trial < 3)
        {
            perturbNormal(row, col, 0.5f * perturbation, n, rs, out, trial + 1);
            return;
        }
        out[0] = n[0];
        out[1] = n[1];
...
// =============================================================================
__device__ float propagateDepth(float d1, const float n1[3], float row1, float col1, float row2, float col2)
{
    float X1[3] = {d1 * (c_ref_inv_K[0] * col1 + c_ref_inv_K[1]), d1 * (c_ref_inv_K[2] * row1 + c_ref_inv_K[3]), d1};
    float ray2[3] = {c_ref_inv_K[0] * col2 + c_ref_inv_K[1], c_ref_inv_K[2] * row2 + c_ref_inv_K[3], 1.f};
...
__device__ bool localSurfaceNormal(const float* depthMap, int width, int height, int row, int col, float normal[3])
{
    if (row <= 0 || row + 1 >= height || col <= 0 || col + 1 >= width)
    {
        return false;
    }
    const float depths[4] = {depthMap[row * width + col - 1],
                             depthMap[row * width + col + 1],
                             depthMap[(row - 1) * width + col],
                             depthMap[(row + 1) * width + col]};
    if (!(depths[0] > 0.0f) || !(depths[1] > 0.0f) || !(depths[2] > 0.0f) || !(depths[3] > 0.0f))
    {
        return false;
    }
    float points[4][3];
    ...
    {
        points[pointIndex][0] =
            depths[pointIndex] * (c_ref_inv_K[0] * static_cast<float>(pointCols[pointIndex]) + c_ref_inv_K[1]);
        points[pointIndex][1] =
            depths[pointIndex] * (c_ref_inv_K[2] * static_cast<float>(pointRows[pointIndex]) + c_ref_inv_K[3]);
        points[pointIndex][2] = depths[pointIndex];
    }
    const float horizontal[3] = {points[1][0] - points[0][0], points[1][1] - points[0][1], points[1][2] - points[0][2]};
    const float vertical[3] = {points[3][0] - points[2][0], points[3][1] - points[2][1], points[3][2] - points[2][2]};
    ...
    normal[1] *= inverseLength;
    normal[2] *= inverseLength;
    const float ray[3] = {c_ref_inv_K[0] * static_cast<float>(col) + c_ref_inv_K[1],
...
// =============================================================================
__device__ void
composeHomography(int row, int col, float depth, const float normal[3], const float* srcData, float H[9])
{
    ...
    float dist = depth * (normal[0] * (c_ref_inv_K[0] * col + c_ref_inv_K[1]) +
                          normal[1] * (c_ref_inv_K[2] * row + c_ref_inv_K[3]) + normal[2]);
    if (fabsf(dist) < 1e-6f)
        dist = 1e-6f;
    ...
    H[8] = R[8] + c_ref_inv_K[1] * (R[6] + iN0 * T[2]) + c_ref_inv_K[3] * (R[7] + iN1 * T[2]) + iN2 * T[2];
}

// =============================================================================
// 平面单应 NCC：用 H 映射 patch，计算 NCC（范围 [0,1]）
// =============================================================================
__device__ float computeHomographyNCC(int u_r,
                                      int v_r,
                                      float depth,
                                      ...
                                      float minimumMaskedSupportRatio)
{
    ...
    const float centerU = fabsf(centerZ) > 1.0e-6f ? (H[0] * u_r + H[1] * v_r + H[2]) / centerZ : -1.0f;
    const float centerV = fabsf(centerZ) > 1.0e-6f ? (H[3] * u_r + H[4] * v_r + H[5]) / centerZ : -1.0f;
    const bool centerValid =
        centerU >= 0.0f && centerV >= 0.0f && bilinearMaskValid(srcMask, srcW, srcH, centerU, centerV);
    const float referenceCenter = refImg[v_r * refW + u_r];
    const float sourceCenter = centerValid ? bilinear(srcImg, srcW, srcH, centerU, centerV) : -1.0f;
    ...
    for (int dv = -r; dv <= r; ++dv)
    {
        for (int du = -r; du <= r; ++du)
        {
            ...
            const bool gradientValid =
                referenceGradient(refImg, refMask, refW, refH, pu, pv, referenceGradientX, referenceGradientY) &&
                sourceGradient(srcImg, srcMask, srcW, srcH, u_s, v_s, sourceGradientX, sourceGradientY);
            accumulator.addGradientCandidate(
                gradientValid, referenceGradientX, referenceGradientY, sourceGradientX, sourceGradientY);
            accumulator.addCensusCandidate(
                centerValid && sourceCenter >= 0.0f, valRef - referenceCenter, valSrc - sourceCenter);
        }
    }
...
__device__ inline void pixelDepthSearchBounds(int idx,
                                              ...
                                              float& localFar)
{
    ...
    {
        radius = fmaxf(center * kDefaultPatchMatchHintRadiusRatio, (zFar - zNear) * 0.01f);
    }
...
// =============================================================================
__device__ float roundTripReprojectionError(
    int col, int row, float depth, const float* sourceDepth, int width, int height, const float sourceData[16])
{
    if (sourceDepth == nullptr || !(depth > 0.0f))
    {
        return -1.0f;
    }
    const float referenceX = (c_ref_inv_K[0] * static_cast<float>(col) + c_ref_inv_K[1]) * depth;
    const float referenceY = (c_ref_inv_K[2] * static_cast<float>(row) + c_ref_inv_K[3]) * depth;
    const float sourceX =
        sourceData[4] * referenceX + sourceData[5] * referenceY + sourceData[6] * depth + sourceData[13];
    const float sourceY =
        sourceData[7] * referenceX + sourceData[8] * referenceY + sourceData[9] * depth + sourceData[14];
    const float sourceZ =
        sourceData[10] * referenceX + sourceData[11] * referenceY + sourceData[12] * depth + sourceData[15];
    if (!(sourceZ > 0.0f))
    {
        return -1.0f;
    }
    ...
    const float roundTripX = sourceData[4] * translatedX + sourceData[7] * translatedY + sourceData[10] * translatedZ;
    const float roundTripY = sourceData[5] * translatedX + sourceData[8] * translatedY + sourceData[11] * translatedZ;
    const float roundTripZ = sourceData[6] * translatedX + sourceData[9] * translatedY + sourceData[12] * translatedZ;
    if (!(roundTripZ > 0.0f))
    {
        return -1.0f;
    }
...
__device__ float evalHypCost(int col,
                             int row,
                             float depth,
                             const float normal[3],
                             const float* refImg,
                             ...
                             float geometricMaxErrorPixels = 1.5f)
{
    ...
    const int source_count = numSrc < kMaxPatchMatchSourceViews ? numSrc : kMaxPatchMatchSourceViews;
    for (int si = 0; si < source_count; ++si)
    {
        const float* srcImg = srcImgs[si];
        float ncc = computeHomographyNCC(col,
                                         row,
                                         depth,
                                         normal,
                                         refImg,
    ...
    const JointViewSelection selection = selectJointSourceViews(scores, source_count, neighborMask, neighborBonus);
    if (selectedMask)
    {
    ...
    if (sourceDepths != nullptr && geometricWeight > 0.0f && geometricMaxErrorPixels > 0.0f)
    {
        ...
        for (int sourceIndex = 0; sourceIndex < source_count; ++sourceIndex)
        {
            ...
            const float error = roundTripReprojectionError(
                col, row, depth, sourceDepths + sourceIndex * srcW * srcH, srcW, srcH, srcDatas + sourceIndex * 16);
            if (error < 0.0f)
            {
                continue;
            }
        ...
        {
            objective += geometricWeight * normalizedErrorSum / static_cast<float>(geometricSupport);
        }
    }
    return objective;
}
...
__global__ void kernelInitializePlanes(float* depthMap,
                                       float* normalMap,
                                       float* confMap,
                                       float* objectiveMap,
                                       ...
                                       int rowOffset)
{
    ...
    const int row = rowOffset + blockIdx.y * blockDim.y + threadIdx.y;
    if (col >= W || row >= H)
    {
        return;
    }
    ...
    float localFar = zFar;
    pixelDepthSearchBounds(idx, hintDepth, hintRadius, zNear, zFar, localNear, localFar);
    ...
    const float inverseStep = (inverseNear - inverseFar) / static_cast<float>(coarseSamples - 1);
    ...
    {
        const float inverseDepth = inverseFar + inverseStep * static_cast<float>(sampleIndex);
        const float depth = 1.0f / inverseDepth;
        std::uint32_t candidateSelection = 0u;
        float candidateNcc = 0.0f;
        const float cost = evalHypCost(col,
                                       row,
                                       depth,
                                       normal,
                                       refImg,
    ...
    if (bestDepth > 0.0f)
    {
        ...
        for (int refineIndex = -6; refineIndex <= 6; ++refineIndex)
        {
            const float inverseDepth =
                fmaxf(inverseFar, fminf(inverseNear, bestInverse + refineStep * static_cast<float>(refineIndex)));
            const float depth = 1.0f / inverseDepth;
            std::uint32_t candidateSelection = 0u;
            float candidateNcc = 0.0f;
            const float cost = evalHypCost(col,
...
// =============================================================================
__global__ void kernelSweepTB(float* depthMap,
                              float* normalMap,
                              float* confMap,
                              const float* refImg,
                              int W,
                              ...
                              unsigned long long seed)
{
    ...
    for (int row = 0; row < H; ++row)
    {
        ...
        float propDepth =
            (row > 0) ? propagateDepth(prevDepth, prevNormal, (float)(row - 1), (float)col, (float)row, (float)col)
                      : currDepth;
        ...
        float nor[5][3] = {{currNormal[0], currNormal[1], currNormal[2]},
...
// Sweep 核（自下而上）
// =============================================================================
__global__ void kernelSweepBT(float* depthMap,
                              float* normalMap,
                              float* confMap,
                              const float* refImg,
                              int W,
                              ...
                              unsigned long long seed)
{
    ...
    for (int row = H - 1; row >= 0; --row)
    {
        ...
        float propDepth =
            (row < H - 1) ? propagateDepth(prevDepth, prevNormal, (float)(row + 1), (float)col, (float)row, (float)col)
                          : currDepth;
        propDepth = fmaxf(localNear, fminf(localFar, propDepth));
...
// =============================================================================
__global__ void kernelSweepLR(float* depthMap,
                              float* normalMap,
                              float* confMap,
                              const float* refImg,
                              int W,
                              ...
                              unsigned long long seed)
{
    ...
    for (int col = 0; col < W; ++col)
    {
        ...
        float propDepth =
            (col > 0) ? propagateDepth(prevDepth, prevNormal, (float)row, (float)(col - 1), (float)row, (float)col)
                      : currDepth;
        propDepth = fmaxf(localNear, fminf(localFar, propDepth));
...
// Sweep 核（自右向左）
// =============================================================================
__global__ void kernelSweepRL(float* depthMap,
                              float* normalMap,
                              float* confMap,
                              const float* refImg,
                              int W,
                              ...
                              unsigned long long seed)
{
    ...
    for (int col = W - 1; col >= 0; --col)
    {
        ...
        float propDepth =
            (col < W - 1) ? propagateDepth(prevDepth, prevNormal, (float)row, (float)(col + 1), (float)row, (float)col)
                          : currDepth;
        propDepth = fmaxf(localNear, fminf(localFar, propDepth));
...
__device__ inline void considerHypothesis(int col,
                                          int row,
                                          float candidateDepth,
                                          const float candidateNormal[3],
                                          const float* refImg,
                                          int W,
                                          int H,
                                          const float* const* srcImgs,
                                          ...
                                          float bestNormal[3])
{
    ...
    if (candidateDepth == *bestDepth && candidateNormal[0] == bestNormal[0] && candidateNormal[1] == bestNormal[1] &&
...
// =============================================================================
__global__ void kernelCheckerboardSweep(float* depthMap,
                                        float* normalMap,
                                        float* confMap,
                                        float* objectiveMap,
                                        std::uint32_t* sourceSelection,
                                        ...
                                        int rowOffset)
{
    ...
    for (int ni = 0; ni < 4; ++ni)
    {
        for (int distanceIndex = 0; distanceIndex < distanceCount; ++distanceIndex)
        {
            ...
            float neighborNormal[3] = {
                normalMap[neighborIdx * 3], normalMap[neighborIdx * 3 + 1], normalMap[neighborIdx * 3 + 2]};
            const std::uint32_t neighborMask = bestSourceSelection | sourceSelection[neighborIdx];
    ...
    if (perturbation >= 0.0f)
    {
        ...
        const float refinedNormal[3] = {bestNormal[0], bestNormal[1], bestNormal[2]};
        uint32_t rngState = alignedPatchMatchHash(
            static_cast<uint32_t>(idx) ^ (static_cast<uint32_t>(iteration + 1) * 0x85ebca6bu) ^
        ...
        {
            alignedPerturbFacingNormal(row, col, refinedNormal, fmaxf(0.02f, perturbation), rngState, randNormal);
        }
        const float randomDepth = fmaxf(localNear,
                                        fminf(localFar,
...
__global__ void kernelApplyPhotometricUniqueness(const float* depthMap,
                                                 ...
                                                 int rowOffset)
{
    ...
    const float normal[3] = {normalMap[idx * 3], normalMap[idx * 3 + 1], normalMap[idx * 3 + 2]};
    ...
    confMap[idx] =
        bestNcc * photometricUniquenessConfidenceScale(bestNcc, competingNcc, minimumMargin, minimumConfidenceScale);
}

/// 置信度阈值过滤
__global__ void kernelFinalizeDepth(
    float* depthMap, float* confMap, std::uint32_t* sourceSelection, int W, int H, float confThresh, int rowOffset)
{
    int u = blockIdx.x * blockDim.x + threadIdx.x;
    int v = rowOffset + blockIdx.y * blockDim.y + threadIdx.y;
    if (u >= W || v >= H)
        return;
    int idx = v * W + u;
                                           ...
                                           PatchMatchAuxiliaryOutput* auxiliaryOutput)
{
    ...
    const int device_index = config.cudaDeviceIndex >= 0 ? config.cudaDeviceIndex : previous_device;
    ...
    std::shared_lock<std::shared_mutex> cache_lifetime_lock(device_state.cacheLifetimeMutex);
    ...
    if (srcValidMasks)
    {
        for (int source_index = 0; source_index < N; ++source_index)
        {
            source_masks_scaled[static_cast<std::size_t>(source_index)] =
                resizedBinaryMask(&(*srcValidMasks)[static_cast<std::size_t>(source_index)], scaled_size);
            has_source_masks = has_source_masks || !source_masks_scaled[static_cast<std::size_t>(source_index)].empty();
        }
    }
    ...
    {
        packed_source_depths.assign(static_cast<std::size_t>(N) * static_cast<std::size_t>(sW * sH), 0.0f);
        for (int source_index = 0; source_index < N; ++source_index)
        {
            const cv::Mat& source_depth = (*auxiliaryInput->sourceDepthMaps)[static_cast<std::size_t>(source_index)];
            ...
            {
                source_depth_scaled =
                    source_depth_float.isContinuous() ? source_depth_float : source_depth_float.clone();
            }
            else
            {
                cv::resize(source_depth_float, source_depth_scaled, scaled_size, 0.0, 0.0, cv::INTER_NEAREST);
            }
            std::memcpy(packed_source_depths.data() +
    ...
    if (!hintScaled.empty() && hintRadius && !hintRadius->empty())
    {
        ...
        {
            cv::resize(*hintRadius, hintRadiusScaled, cv::Size(sW, sH), 0, 0, cv::INTER_NEAREST);
        }
    }

    // ── 设置常量内存 c_ref_inv_K ─────────────────────────────────
    float h_inv_K[4] = {1.f / refCamS.focalX,
                        -refCamS.principalX / refCamS.focalX,
                        1.f / refCamS.focalY,
                        -refCamS.principalY / refCamS.focalY};
    ...
    for (int si = 0; si < N; ++si)
    {
        ...
        const std::array<float, 16> source_data = buildPatchMatchSourceCameraData(refCam, srcCams[si], ds);
    ...
    // completes so another upload lane cannot invalidate those pointers.
    {
        ...
        const std::size_t conservative_batch_bytes =
            (static_cast<std::size_t>(N) + 1) * static_cast<std::size_t>(refPx) * sizeof(float);
        evictSrcImageGpuCacheIfNeeded(conservative_batch_bytes, device_index, image_upload_lane.stream);
    }
    ...
    std::vector<cudaEvent_t> image_ready_events(static_cast<std::size_t>(N) + 1, nullptr);
    ...
    for (int source_index = 0; source_index < N; ++source_index)
    {
        ...
        if (!getOrUploadGrayImageGpu(srcGrays[static_cast<std::size_t>(source_index)],
                                     device_index,
                                     sW,
                                     sH,
                                     ds,
    ...
    CUDA_CHECK(cudaMemcpyToSymbolAsync(
        c_ref_inv_K, h_inv_K, sizeof(h_inv_K), 0, cudaMemcpyHostToDevice, workspace.computeStream));
    ...
    CUDA_CHECK(cudaMemsetAsync(d_conf, 0, static_cast<std::size_t>(refPx) * sizeof(float), workspace.computeStream));
    CUDA_CHECK(cudaMemsetAsync(
        d_sourceSelection, 0, static_cast<std::size_t>(refPx) * sizeof(std::uint32_t), workspace.computeStream));
    ...
    if (has_source_masks)
    {
        std::vector<std::uint8_t> packed_masks(static_cast<std::size_t>(N) * static_cast<std::size_t>(refPx), 255);
        for (int source_index = 0; source_index < N; ++source_index)
        {
            const cv::Mat& source_mask = source_masks_scaled[static_cast<std::size_t>(source_index)];
            if (source_mask.empty())
            {
                continue;
            }
            std::memcpy(packed_masks.data() + static_cast<std::size_t>(source_index) * static_cast<std::size_t>(refPx),
        ...
        CUDA_CHECK(stageHostToDeviceCopy(
            workspace.sourceMasksHost, packed_masks.data(), source_mask_bytes, d_srcMasks, workspace.transferStream));
    }
    ...
    for (const cudaEvent_t ready_event : image_ready_events)
    {
        if (ready_event)
        {
            CUDA_CHECK(cudaStreamWaitEvent(workspace.computeStream, ready_event, 0));
        }
    }
    ...
    const int propagationIterations = patchMatchPropagationIterationCount(config.numIterations);
    const int sweepIterations = config.cudaUseParallelSweep ? propagationIterations : config.numIterations;
    const int scheduledSweepIterations = config.cudaUseParallelSweep
        ? patchMatchScheduledPropagationPassCount(
              config.numIterations, config.enableFinalPropagationPass)
        : sweepIterations;
    constexpr int kCudaCancellationTileRows = 32;
    const int cuda_tile_rows = config.cancelFlag ? kCudaCancellationTileRows : sH;
    const auto synchronizeCancellationCheckpoint = [&]()
    {
        if (!config.cancelFlag)
        {
            return true;
        }
        CUDA_CHECK(cudaEventRecord(workspace.cancellationCheckpoint, workspace.computeStream));
    ...
    for (int row_offset = 0; row_offset < sH; row_offset += cuda_tile_rows)
    {
        const int row_count = std::min(cuda_tile_rows, sH - row_offset);
        const dim3 tile_grid((sW + bW - 1) / bW, (row_count + bH - 1) / bH);
        kernelInitializePlanes<<<tile_grid, block, 0, workspace.computeStream>>>(
            d_depth,
            d_normal,
            d_conf,
            d_objective,
        ...
        if (!synchronizeCancellationCheckpoint())
        {
            return false;
        }
    }
    ...
    for (int iter = 0; iter < scheduledSweepIterations; ++iter)
    {
        ...
        {
            const bool final_propagation = iter >= propagationIterations;
            const float pass_perturbation = final_propagation ? -1.0f : perturbation;
            for (int parity = 0; parity < 2; ++parity)
            {
                for (int row_offset = 0; row_offset < sH; row_offset += cuda_tile_rows)
                {
                    const int row_count = std::min(cuda_tile_rows, sH - row_offset);
                    const dim3 tile_grid_pixel((checkerboardWidth + bW - 1) / bW, (row_count + bH - 1) / bH);
                    kernelCheckerboardSweep<<<tile_grid_pixel, blockPixel, 0, workspace.computeStream>>>(
                        d_depth,
                        d_normal,
                        d_conf,
                        d_objective,
    ...
    if (config.enablePhotometricUniqueness)
    {
        for (int row_offset = 0; row_offset < sH; row_offset += cuda_tile_rows)
        {
            const int row_count = std::min(cuda_tile_rows, sH - row_offset);
            const dim3 tile_grid((sW + bW - 1) / bW, (row_count + bH - 1) / bH);
            kernelApplyPhotometricUniqueness<<<tile_grid, block, 0, workspace.computeStream>>>(
                d_depth,
                d_normal,
                d_conf,
                d_ref,
    ...
    for (int row_offset = 0; row_offset < sH; row_offset += cuda_tile_rows)
    {
        const int row_count = std::min(cuda_tile_rows, sH - row_offset);
        const dim3 tile_grid((sW + bW - 1) / bW, (row_count + bH - 1) / bH);
        kernelFinalizeDepth<<<tile_grid, block, 0, workspace.computeStream>>>(
            d_depth, d_conf, d_sourceSelection, sW, sH, config.confidenceThresh, row_offset);
        CUDA_CHECK(cudaGetLastError());
        if (!synchronizeCancellationCheckpoint())
        {
            return false;
    ...
    CUDA_CHECK(cudaStreamWaitEvent(workspace.transferStream, workspace.computeReady, 0));
    ...
    CUDA_CHECK(cudaMemcpyAsync(workspace.sourceSelectionHost.ptr,
                               ...
                               cudaMemcpyDeviceToHost,
                               workspace.transferStream));
    CUDA_CHECK(cudaEventRecord(workspace.downloadsReady, workspace.transferStream));
    ...
    std::memcpy(depthS.ptr<float>(), workspace.depthHost.ptr, static_cast<std::size_t>(refPx) * sizeof(float));
    std::memcpy(confS.ptr<float>(), workspace.confidenceHost.ptr, static_cast<std::size_t>(refPx) * sizeof(float));
    ...
    // ── 后处理结果统计诊断 ────────────────────────────────────────
    {
        ...
        LOG_DEBUG("[MVS][PatchMatch][GPU] result valid=%d/%d (%.1f%%) depth=[%.4f,%.4f] mean=%.4f confidence=%.4f",
                  validCnt,
                  totalPx,
                  100.f * validCnt / totalPx,
                  static_cast<float>(dMin),
                  static_cast<float>(dMax),
                  static_cast<float>(dMean),
                  static_cast<float>(cMean));
    ...
    postprocessPatchMatchDepth(depthS, config, refGray);
    ...
    if (ds > 1 && !config.returnNativeResolution)
    {
        ...
        cv::resize(sourceSelectionS, sourceSelectionFull, cv::Size(refW, refH), 0, 0, cv::INTER_NEAREST);
    }
    ...
    LOG_DEBUG("[MVS][PatchMatch][GPU] scaled=%dx%d output=%dx%d range=[%.2f,%.2f] valid=%d/%d mean=%.2f "
              ...
              outputPx,
              mean,
              config.confidenceThresh,
              hasHint ? 1 : 0,
              depthSampleCount,
              scheduledSweepIterations,
              config.cudaUseParallelSweep ? 1 : 0);
    LOG_DEBUG("[MVS][PatchMatch][GPU] gray_cache ref_hit=%d source_hit=%d source_miss=%d usage=%.1f/%.1f MiB",
              refCacheHit ? 1 : 0,
              cacheHits,
              cacheMisses,
    ...
    const double gpu_slot_ms = std::chrono::duration<double, std::milli>(gpu_slot_finished - slot_acquired).count();
    const double postprocess_ms =
        std::chrono::duration<double, std::milli>(estimate_finished - gpu_slot_finished).count();
...
std::string PatchMatchDepthEstimator::cudaDeviceName(int deviceIndex)
{
    ...
    return cudaGetDeviceProperties(&properties, deviceIndex) == cudaSuccess ? std::string(properties.name)
                                                                            : std::string();
}
...
bool PatchMatchDepthEstimator::reserveGpuWorkspace(std::size_t referencePixelCount,
                                                   int sourceCount,
                                                   ...
                                                   int deviceIndex)
{
    ...
    CudaDeviceRestoreGuard device_restore_guard(previous_device);
    PatchMatchGpuDeviceState& device_state = patchMatchGpuDeviceState(device_index);
    std::shared_lock<std::shared_mutex> lifetime_lock(device_state.cacheLifetimeMutex);
    std::unique_lock<std::mutex> execution_lock(device_state.executionMutex, std::try_to_lock);
    ...
    PatchMatchGpuWorkspace& workspace = device_state.workspace;
    CUDA_CHECK(workspace.initialize());
    CUDA_CHECK(workspace.srcData.reserve(static_cast<std::size_t>(sourceCount) * 16));
    CUDA_CHECK(workspace.srcPointers.reserve(static_cast<std::size_t>(sourceCount)));
    ...
    {
        CUDA_CHECK(workspace.sourceMasks.reserve(referencePixelCount * static_cast<std::size_t>(sourceCount)));
    }
    return true;
}
