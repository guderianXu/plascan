    ...
    namespace mvs
    {
        ...
        namespace
        {
            ...
            bool populateOpenClUsability(cl_device_id device, OpenClDeviceInfo* info)
            {
                ...
                const bool compiler_query_succeeded =
                    openClBoolean(device, CL_DEVICE_COMPILER_AVAILABLE, &info->compilerAvailable);
            ...
            std::vector<EnumeratedOpenClDevice> enumerateOpenClGpuDevicesUncached()
            {
                ...
                for (cl_platform_id platform : platforms)
                {
                    ...
                    for (cl_device_id device : devices)
                    {
                        ...
                        if (clGetDeviceInfo(device, kOpenClDevicePciBusInfo, sizeof(pci_info), &pci_info, nullptr) ==
                            CL_SUCCESS)
                        {
                            char identity[64]{};
            ...
            struct OpenClRuntime
            {
                ...
                ~OpenClRuntime()
                {
                    for (ExecutionLane& lane : lanes)
                    {
                        ...
                        for (cl_kernel kernel : lane.kernels)
                        {
                            if (kernel)
                            {
            ...
            std::optional<OpenClExecutionLaneLease>
            acquireOpenClExecutionLane(const std::shared_ptr<OpenClRuntime>& runtime,
            ...
            public:
                ...
                cl_event first() const
                {
                    return _events.empty() ? nullptr : _events.front();
                }

                cl_event last() const
                {
                    return _events.empty() ? nullptr : _events.back();
                }
                ...
                cl_ulong activeNanoseconds() const
                {
                    cl_ulong total = 0;
                    for (cl_event event : _events)
                    {
                        cl_ulong start = 0;
                        cl_ulong end = 0;
                        if (eventProfileRange(
                                event, CL_PROFILING_COMMAND_START, event, CL_PROFILING_COMMAND_END, start, end))
                        {
                            total += end - start;
                        }
                    }
                                   ...
                                   cl_ulong& endNanoseconds)
            {
                return first && last &&
                       clGetEventProfilingInfo(
                           first, firstParameter, sizeof(startNanoseconds), &startNanoseconds, nullptr) == CL_SUCCESS &&
                       clGetEventProfilingInfo(last, lastParameter, sizeof(endNanoseconds), &endNanoseconds, nullptr) ==
                           CL_SUCCESS &&
                       endNanoseconds >= startNanoseconds;
            }

            std::shared_ptr<OpenClRuntime> openClRuntimeForDevice(const EnumeratedOpenClDevice& device,
                                                                  std::string* errorMsg)
            {
                ...
                runtime->imageCacheLimitBytes =
                    std::clamp<std::size_t>(static_cast<std::size_t>(device.info.globalMemoryBytes / 8),
                ...
                error = clBuildProgram(
                    runtime->program, 1, &device.device, detail::kPatchMatchOpenClBuildOptions, nullptr, nullptr);
                if (error != CL_SUCCESS)
                {
                ...
                for (std::size_t lane_index = 0; lane_index < runtime->activeLaneCount; ++lane_index)
                {
                    ...
                    lane.queue =
                        clCreateCommandQueue(runtime->context, device.device, CL_QUEUE_PROFILING_ENABLE, &error);
                    ...
                    {
                        lane.kernels[kernel_index] =
                            clCreateKernel(runtime->program, kernel_names[kernel_index], &error);
                        if (error != CL_SUCCESS || !lane.kernels[kernel_index])
                        {
                            if (errorMsg)
                            {
                                *errorMsg = openClError(kernel_names[kernel_index], error);
                            }
                            return nullptr;
                        }
                    }
            ...
            std::shared_ptr<OpenClRuntime::CachedScaledImage>
            cachedScaledGrayFloat(const std::shared_ptr<OpenClRuntime>& runtime,
                                  const cv::Mat& image,
                                  const cv::Size& size,
                                  bool* cacheHit)
            {
                ...
                {
                    return entry.sourceData == image.data && entry.sourceStep == image.step[0] &&
                           entry.sourceRows == image.rows && entry.sourceColumns == image.cols &&
                           entry.sourceType == image.type() && entry.workingSize == size;
                };
                {
                    std::lock_guard<std::mutex> lock(runtime->imageCacheMutex);
                    const auto found =
                        std::find_if(runtime->imageCache.begin(),
                ...
                while (runtime->imageCacheBytes + candidate_bytes > runtime->imageCacheLimitBytes)
                {
                    const auto oldest = std::min_element(
                           ...
                           const std::shared_ptr<OpenClRuntime::CachedScaledImage>& right)
                        {
                            ...
                            const std::uint64_t right_use =
                                right.use_count() == 1 ? right->lastUse : std::numeric_limits<std::uint64_t>::max();
        ...
        std::vector<OpenClExecutionStats> PatchMatchDepthEstimator::openClExecutionStats()
        {
            ...
            for (const auto& [device_index, runtime] : g_openClRuntimes)
            {
                ...
                stats.wallSpanMilliseconds =
                    std::chrono::duration<double, std::milli>(runtime->lastQueueFinish - runtime->firstQueueStart)
                        .count();
                ...
                stats.queueNonKernelMilliseconds =
                    std::max(0.0, stats.queueMilliseconds - stats.kernelActiveMilliseconds);
                if (stats.wallSpanMilliseconds > 0.0)
                {
                    stats.queueOccupancyRatio =
                        std::clamp(stats.queueMilliseconds / stats.wallSpanMilliseconds, 0.0, 1.0);
                    stats.kernelDutyRatio =
                        std::clamp(stats.kernelActiveMilliseconds / stats.wallSpanMilliseconds, 0.0, 1.0);
                }
                                                      ...
                                                      PatchMatchAuxiliaryOutput* auxiliaryOutput)
        {
            ...
            if (has_reference_mask)
            {
                ...
                std::memcpy(
                    packed_reference_mask.data(), reference_mask.ptr<std::uint8_t>(), packed_reference_mask.size());
            }
            ...
            const std::array<const void*, 7> input_data = {reference_float.ptr<float>(),
                                                           packed_sources.data(),
                                                           camera_data.data(),
                                                           has_reference_mask ? packed_reference_mask.data()
                                                                              : &dummy_mask,
            ...
            const int scheduled_propagation_passes =
                patchMatchScheduledPropagationPassCount(config.numIterations, config.enableFinalPropagationPass);
            ...
            const auto set_common_arguments = [&](cl_kernel kernel)
            {
                cl_uint argument_index = 0;
                for (cl_mem argument : memory_arguments)
                {
                    if (!setKernelArgument(kernel, argument_index++, argument, errorMsg))
                    {
                        return false;
                    }
                }
                ...
                return set_argument(width) && set_argument(height) && set_argument(source_count) &&
                       set_argument(patch_half) && set_argument(depth_sample_count) &&
                       set_argument(minimum_mask_ratio) && set_argument(zNear) && set_argument(zFar) &&
                       set_argument(confidence_threshold) && set_argument(uniqueness_relative_step) &&
                       set_argument(uniqueness_margin) && set_argument(uniqueness_scale) &&
                       set_argument(reference_mask_flag) && set_argument(source_mask_flag) && set_argument(hint_flag) &&
                       set_argument(hint_radius_flag) && set_argument(inv_fx) && set_argument(inv_fy) &&
                       set_argument(cx) && set_argument(cy);
            };
            for (cl_kernel kernel : execution_lane.kernels)
            {
                if (!set_common_arguments(kernel))
                {
                    return false;
                }
            }

            const std::size_t local_size[2] = {16, config.cancelFlag ? 1U : 16U};
            ...
            const auto wait_for_checkpoint = [&](cl_event event, const char* name)
            {
                ...
                if (wait_error != CL_SUCCESS)
                {
                    if (errorMsg)
                    {
                        *errorMsg = config.cancelFlag && config.cancelFlag->load(std::memory_order_relaxed)
                                        ? "PatchMatch cancelled"
                                        : openClError(name, wait_error);
                    }
                    return false;
                }
            ...
            const cl_event input_ready_event =
                last_write_event.get() ? last_write_event.get() : first_write_event.get();
            if (!wait_for_checkpoint(input_ready_event, "OpenCL input upload"))
            {
                return false;
            }

            constexpr cl_uint dispatch_row_offset_argument = 31;
            const auto enqueue_kernel =
                [&](cl_kernel kernel, const char* name, int dispatch_row_offset, const std::size_t* kernel_global_size)
            {
                if (!setKernelArgument(kernel, dispatch_row_offset_argument, dispatch_row_offset, errorMsg))
                {
                    return false;
                }
                ...
                if (enqueue_error != CL_SUCCESS)
                {
                    if (errorMsg)
                    {
                        *errorMsg = config.cancelFlag && config.cancelFlag->load(std::memory_order_relaxed)
                                        ? "PatchMatch cancelled"
                                        : openClError(name, enqueue_error);
                    }
                    return false;
                }
                queue_drain.markPending();
            ...
            constexpr std::size_t kOpenClCancellationTileRows = 1;
            ...
            {
                for (std::size_t row_offset = 0; row_offset < static_cast<std::size_t>(height);
                     row_offset += opencl_tile_rows)
                {
                    const std::size_t row_count =
                        std::min(opencl_tile_rows, static_cast<std::size_t>(height) - row_offset);
                    const std::size_t tile_global_size[2] = {
                        global_width, (row_count + local_size[1] - 1) / local_size[1] * local_size[1]};
                    if (!enqueue_kernel(kernel, name, static_cast<int>(row_offset), tile_global_size))
                    {
                        return false;
                    }
                }
            ...
            constexpr cl_uint propagation_argument_start = 32;
            for (int iteration = 0; iteration < scheduled_propagation_passes; ++iteration)
            {
                ...
                const float perturbation =
                    final_propagation ? -1.0f : 0.35f * std::pow(0.5f, static_cast<float>(iteration));
                for (int checkerboard = 0; checkerboard < 2; ++checkerboard)
                {
                    ...
                    if (!setKernelArgument(
                            execution_lane.kernels[1], propagation_argument_start, neighbor_bonus, errorMsg) ||
                        !setKernelArgument(execution_lane.kernels[1],
                                           propagation_argument_start + 1,
                                           asymmetric_propagation,
                                           errorMsg) ||
                        !setKernelArgument(
                            execution_lane.kernels[1], propagation_argument_start + 2, checkerboard, errorMsg) ||
                        !setKernelArgument(
                            execution_lane.kernels[1], propagation_argument_start + 3, iteration, errorMsg) ||
                        !setKernelArgument(
                            execution_lane.kernels[1], propagation_argument_start + 4, perturbation, errorMsg) ||
                        !enqueue_tiled_kernel(
                            execution_lane.kernels[1], "propagate_planes", checkerboard_global_size[0]))
                    {
                        return false;
                    }
                }
            ...
            cl_ulong kernel_chain_start_nanoseconds = 0;
            ...
            const double host_prepare_ms =
                std::chrono::duration<double, std::milli>(slot_wait_start - estimate_start).count();
            const double slot_wait_ms =
                std::chrono::duration<double, std::milli>(slot_acquired - slot_wait_start).count();
            const double setup_ms = std::chrono::duration<double, std::milli>(queue_wall_start - slot_acquired).count();
            const double queue_ms =
                std::chrono::duration<double, std::milli>(queue_wall_finished - queue_wall_start).count();
            ...
            const double kernel_chain_ms =
                has_kernel_profile
                    ? static_cast<double>(kernel_chain_end_nanoseconds - kernel_chain_start_nanoseconds) / 1.0e6
                    : queue_ms;
            ...
            const double total_ms =
                std::chrono::duration<double, std::milli>(estimate_finished - estimate_start).count();
            {
                std::lock_guard<std::mutex> stats_lock(runtime->executionStatsMutex);
