...
namespace xjw::detail::plamatrix_ba
{
    namespace
    {
        ...
        pixelByCameraPointJacobian(const FramePinholeCamera& camera, const double camera_point[3], double jacobian[6])
        {
            ...
            jacobian[2] = -u_scale * (distorted_x_by_x * x + distorted_x_by_y * y) * inverse_depth;
            jacobian[3] = v_scale * distorted_y_by_x * inverse_depth;
            jacobian[4] = v_scale * distorted_y_by_y * inverse_depth;
            jacobian[5] = -v_scale * (distorted_y_by_x * x + distorted_y_by_y * y) * inverse_depth;
    ...
    {
        if (!linearization || !observationDataIsUsable(observation) || !std::isfinite(huber_delta) || huber_delta < 0.0)
        {
            return false;
        }
                                                  ...
                                                  bool use_reference_point_parameterization)
    {
        ...
        const double radial = 1.0 + distortion.radialK1 * r2 + distortion.radialK2 * r4 + distortion.radialK3 * r6;
        ...
        set(BAIntrinsicParameter::FocalLength,
            u_scale * distorted_x / effective.focalX(),
            v_scale * distorted_y / effective.focalX());
        ...
        set(BAIntrinsicParameter::TangentialP1, u_scale * 2.0 * x * y, v_scale * (r2 + 2.0 * y * y));
        set(BAIntrinsicParameter::TangentialP2, u_scale * (r2 + 2.0 * x * x), v_scale * 2.0 * x * y);
