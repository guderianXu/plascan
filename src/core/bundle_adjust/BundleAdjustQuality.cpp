...
namespace xjw::detail
{
    namespace
    {
        ...
        std::array<double, 3> pointForConstraintStats(const std::vector<BATrack>& tracks,
        ...
        bool trackIsEligibleForConstraintStats(const std::vector<BARefinedPoint>* eligiblePoints, std::size_t index)
        {
            return !eligiblePoints || (index < eligiblePoints->size() && (*eligiblePoints)[index].valid);
        }

        ConstraintStats computeLaserStats(const std::vector<BATrack>& tracks,
                                          const std::vector<BARefinedPoint>* points,
                                          const std::vector<BARefinedPoint>* eligiblePoints)
        {
            ...
            for (std::size_t trackIndex = 0; trackIndex < tracks.size(); ++trackIndex)
            {
                ...
                const auto point = pointForConstraintStats(tracks, points, trackIndex);
                for (const BALaserPlaneConstraint& constraint : tracks[trackIndex].laserPlaneConstraints)
                {
                    const double distance = (point[0] - constraint.point[0]) * constraint.normal[0] +
                                  ...
                                  const std::array<double, 3>& point)
        {
            ...
            for (int row = 0; row < 3; ++row)
            {
                ...
                {
                    emitter[static_cast<size_t>(row)] += rotation[static_cast<size_t>(row * 3 + column)] *
                                   ...
                                   BAResult* result)
        {
            ...
            for (size_t shotIndex = 0; shotIndex < constraints.size(); ++shotIndex)
            {
                ...
                const double beforeRange = computedLaserRange(inputCameras[static_cast<size_t>(constraint.cameraIndex)],
                                                              constraint.leverArmCameraMeters,
                                                              constraint.initialPoint);
                const double beforeResidual = beforeRange - constraint.observedRangeMeters;
                ...
                const double afterResidual = afterRange - constraint.observedRangeMeters;
                shot.computedRangeAfterMeters = afterRange;
                shot.residualAfterMeters = afterResidual;
                shot.normalizedResidualAfter = afterResidual / constraint.sigmaRangeMeters;
                shot.valid = validPoint && std::isfinite(afterResidual) && std::isfinite(shot.normalizedResidualAfter);
            ...
            result->laserRangeConstraintCount = static_cast<int>(constraints.size());
            ...
            result->laserRangeRmsBeforeMeters =
                finiteBefore ? std::sqrt(sumSquaredBefore / static_cast<double>(constraints.size()))
                             : std::numeric_limits<double>::infinity();
            result->laserRangeRmsAfterMeters =
                finiteAfter ? std::sqrt(sumSquaredAfter / static_cast<double>(constraints.size()))
                            : std::numeric_limits<double>::infinity();
        }

        ConstraintStats computeControlPointStats(const std::vector<BATrack>& tracks,
                                                 const std::vector<BARefinedPoint>* points,
                                                 const std::vector<BARefinedPoint>* eligiblePoints)
        {
            double sumSquared = 0.0;
            ...
            for (std::size_t trackIndex = 0; trackIndex < tracks.size(); ++trackIndex)
            {
                ...
                const auto point = pointForConstraintStats(tracks, points, trackIndex);
                for (const BAControlPointConstraint& constraint : tracks[trackIndex].controlPointConstraints)
                {
            ...
            ConstraintStats stats;
            stats.count = count;
            stats.rms = count > 0 ? std::sqrt(sumSquared / static_cast<double>(count)) : 0.0;
            return stats;
        }

        bool scaleBarIsUsable(const BAScaleBarConstraint& constraint, std::size_t trackCount)
        {
            return constraint.trackIndexA >= 0 && constraint.trackIndexB >= 0 &&
                   ...
                   std::isfinite(constraint.measuredDistanceMeters) && constraint.measuredDistanceMeters > 0.0;
        }

        ConstraintStats computeScaleBarStats(const std::vector<BATrack>& tracks,
                                             const std::vector<BARefinedPoint>* points,
                                             const std::vector<BARefinedPoint>* eligiblePoints,
                                             const std::vector<BAScaleBarConstraint>& constraints)
        {
            ...
            for (const BAScaleBarConstraint& constraint : constraints)
            {
                ...
                if (!trackIsEligibleForConstraintStats(eligiblePoints,
                                                       static_cast<std::size_t>(constraint.trackIndexA)) ||
                    !trackIsEligibleForConstraintStats(eligiblePoints,
                ...
                const auto pointA =
                    pointForConstraintStats(tracks, points, static_cast<std::size_t>(constraint.trackIndexA));
                const auto pointB =
                    pointForConstraintStats(tracks, points, static_cast<std::size_t>(constraint.trackIndexB));
                const double dx = pointA[0] - pointB[0];
                const double dy = pointA[1] - pointB[1];
                const double dz = pointA[2] - pointB[2];
                const double residual = std::sqrt(dx * dx + dy * dy + dz * dz) - constraint.measuredDistanceMeters;
                                   ...
                                   BAResult* result)
        {
            ...
            {
                const ConstraintStats before = computeLaserStats(tracks, nullptr, &result->points);
                const ConstraintStats after = computeLaserStats(tracks, &result->points, &result->points);
            ...
            {
                const ConstraintStats before = computeControlPointStats(tracks, nullptr, &result->points);
                const ConstraintStats after = computeControlPointStats(tracks, &result->points, &result->points);
            ...
            if (options.enableScaleBarConstraints)
            {
                const ConstraintStats before =
                    computeScaleBarStats(tracks, nullptr, &result->points, options.scaleBarConstraints);
                const ConstraintStats after =
                    computeScaleBarStats(tracks, &result->points, &result->points, options.scaleBarConstraints);
                              ...
                              bool whitenByMeasurementScale)
        {
            ...
            {
                double weight = sanitizedObservationWeight(observation);
                if (whitenByMeasurementScale)
                {
                    const double scale = sanitizedMeasurementScale(observation);
                    weight /= scale * scale;
                }
                if (!(weight > 0.0))
                {
    ...
    double adaptivePointFilterThreshold(const std::vector<double>& pointRms, double absoluteFloor, double medianFactor)
    {
        ...
        double threshold = std::isfinite(absoluteFloor) && absoluteFloor >= 0.0 ? absoluteFloor : 0.0;
        if (finiteValues.empty() || !std::isfinite(medianFactor) || !(medianFactor > 0.0))
        {
            return threshold;
        }
                                    ...
                                    BAResult* result)
    {
        ...
        std::vector<double> candidateRmsByTrack(tracks.size(), std::numeric_limits<double>::infinity());
        const std::size_t qualityWorkerCount = static_cast<std::size_t>(
            options.numThreads > 0 ? options.numThreads : std::max(1u, std::thread::hardware_concurrency()));
        common::concurrency::parallelForIndices(
            ...
            [&](std::size_t index)
            {
                ...
                point.rmsBefore =
                    strictTrackRms(inputCameras, tracks[index], tracks[index].initialPoint, useMeasurementScale);
                if (!point.valid)
                {
        ...
        const double filterThreshold =
            options.enablePointFilter
                ? adaptivePointFilterThreshold(candidateRms, options.filterMaxReprojError, options.filterSigmaFactor)
                : std::numeric_limits<double>::infinity();
        ...
        result->meanRmsBefore = result->optimizedTracks > 0 ? sumBefore / static_cast<double>(result->optimizedTracks)
                                                            : std::numeric_limits<double>::infinity();
        result->meanRmsAfter = result->optimizedTracks > 0 ? sumAfter / static_cast<double>(result->optimizedTracks)
                                                           : std::numeric_limits<double>::infinity();
        ...
        if (result->optimizedTracks == 0 && result->laserRangeConstraintCount == 0 &&
            (result->solveStatus == BASolveStatus::Success || result->solveStatus == BASolveStatus::NoConvergence))
        {
