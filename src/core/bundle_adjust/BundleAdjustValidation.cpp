...
namespace xjw::detail
{
    namespace
    {
        ...
        bool isUsableLaserPointSqrtInformation(const std::array<double, 9>& sqrtInformation)
        {
            ...
            const double determinant =
                sqrtInformation[0] *
                    (sqrtInformation[4] * sqrtInformation[8] - sqrtInformation[5] * sqrtInformation[7]) -
                sqrtInformation[1] *
                    (sqrtInformation[3] * sqrtInformation[8] - sqrtInformation[5] * sqrtInformation[6]) +
                sqrtInformation[2] *
                    (sqrtInformation[3] * sqrtInformation[7] - sqrtInformation[4] * sqrtInformation[6]);
            const double relativeRankThreshold = 1.0e-12 * maxAbsoluteValue * maxAbsoluteValue * maxAbsoluteValue;
            return std::isfinite(determinant) && std::abs(determinant) > relativeRankThreshold;
        }
        ...
        bool hasUsableTrackObservations(const BATrack& track)
        {
            ...
            {
                if (observation.cameraIndex >= 0 && std::isfinite(observation.u) && std::isfinite(observation.v) &&
                    std::isfinite(observation.weight) && observation.weight > 0.0)
                {
                                               ...
                                               const std::set<int>& fixedCameras)
        {
            ...
            for (size_t prior_index = 0; prior_index < prior_count; ++prior_index)
            {
                ...
                for (const int fixed_index : fixedCameras)
                {
                    ...
                    const std::array<double, 3> center = cameras[static_cast<size_t>(fixed_index)].cameraCenter();
                    if (isFinitePoint(center) && squaredDistance(prior.cameraCenter, center) > 1.0e-16)
                    {
                        return true;
                    }
                }
                                                  ...
                                                  const std::set<int>& fixedCameras)
        {
            ...
            for (const BATrack& track : tracks)
            {
                ...
                for (const BAControlPointConstraint& constraint : track.controlPointConstraints)
                {
                    ...
                    {
                        const std::array<double, 3> center = cameras[static_cast<size_t>(fixed_index)].cameraCenter();
                        if (isFinitePoint(center) && squaredDistance(constraint.point, center) > 1.0e-16)
                        {
                            return true;
                        }
                    }
        ...
        bool hasCompleteControlPointGauge(const std::vector<BATrack>& tracks, const BAOptions& options)
        {
            if (!options.enableControlPointConstraints)
            {
                return false;
            ...
            for (size_t first = 0; first < points.size(); ++first)
            {
                for (size_t second = first + 1; second < points.size(); ++second)
                {
                    ...
                    for (size_t third = second + 1; third < points.size(); ++third)
                    {
                        ...
                        if (ac_squared > 1.0e-16 && cross_squared > 1.0e-12 * ab_squared * ac_squared)
                        {
                            return true;
                        }
                    }
        ...
        bool hasUsableScaleBar(const std::vector<BATrack>& tracks, const BAOptions& options)
        {
            if (!options.enableScaleBarConstraints)
            {
                return false;
            }
            return std::any_of(options.scaleBarConstraints.begin(),
                               options.scaleBarConstraints.end(),
                               [&tracks](const BAScaleBarConstraint& constraint)
                               {
                                   return constraint.trackIndexA >= 0 && constraint.trackIndexB >= 0 &&
    ...
    {
        return std::isfinite(observation.u) && std::isfinite(observation.v) &&
                                          ...
                                          const std::vector<BATrack>& tracks)
    {
        ...
        for (const BATrack& track : tracks)
        {
            if (!std::isfinite(track.initialPoint[0]) || !std::isfinite(track.initialPoint[1]) ||
            ...
            for (const BAObservation& observation : track.observations)
            {
                ...
                ++observationCount;
                const std::size_t cameraIndex = static_cast<std::size_t>(observation.cameraIndex);
                                                                         ...
                                                                         BAOptions* normalizedOptions)
    {
        ...
        *normalizedOptions = requestedOptions;
        ...
        const bool sharedFocalEnabled = parameterEnabled(BAIntrinsicParameter::FocalLength);
        if (sharedFocalEnabled && (!(requestedOptions.minSharedFocalScale > 0.0) ||
        ...
        if (!std::isfinite(requestedOptions.minSharedFocalAspectScale) ||
            ...
            requestedOptions.maxSharedFocalAspectScale < requestedOptions.minSharedFocalAspectScale)
        {
            return invalid(BASolveStatus::InvalidInput, "BA 输入验证失败: 共享焦距宽高比范围非法");
        }
        ...
        {
            return invalid(BASolveStatus::InvalidInput, "BA 输入验证失败: 扩展共享内参边界或先验非法");
        }
        ...
        {
            return invalid(BASolveStatus::InvalidInput, "BA 输入验证失败: 相机标定分组数量必须与相机数量一致");
        }
        if (!requestedOptions.sharedIntrinsicReferenceCameras.empty() &&
            requestedOptions.sharedIntrinsicReferenceCameras.size() != cameras.size())
        {
        ...
        if (std::any_of(requestedOptions.cameraCalibrationGroupIds.begin(),
        ...
        {
            return invalid(BASolveStatus::InvalidInput, "BA 输入验证失败: 初始 track RMS 粗差阈值必须有限且非负");
        }
        if (requestedOptions.enableLaserPlaneConstraints &&
            (!std::isfinite(requestedOptions.laserPlaneWeight) || requestedOptions.laserPlaneWeight <= 0.0 ||
             !std::isfinite(requestedOptions.laserHuberDeltaMeters) || requestedOptions.laserHuberDeltaMeters < 0.0))
        {
            return invalid(BASolveStatus::InvalidInput,
                           "BA 输入验证失败: LiDAR 统计权重必须为正，Huber 阈值必须有限且非负");
        }
        if (!requestedOptions.enableLaserRangeConstraints && !requestedOptions.laserRangeConstraints.empty())
        {
            return invalid(BASolveStatus::InvalidInput, "BA 输入验证失败: 已提供激光测距 shot，但未启用激光测距约束");
        }
        if (requestedOptions.enableLaserRangeConstraints)
        {
            if (requestedOptions.laserRangeConstraints.empty())
            {
                return invalid(BASolveStatus::InvalidInput, "BA 输入验证失败: 已启用激光测距约束但 shot 列表为空");
            }
            if (!std::isfinite(requestedOptions.laserRangeWeight) || requestedOptions.laserRangeWeight <= 0.0 ||
                !std::isfinite(requestedOptions.laserRangeHuberDelta) || requestedOptions.laserRangeHuberDelta < 0.0)
            {
                return invalid(BASolveStatus::InvalidInput,
                               "BA 输入验证失败: 激光测距全局权重必须为正，Huber 阈值必须有限且非负");
            }

            for (size_t shotIndex = 0; shotIndex < requestedOptions.laserRangeConstraints.size(); ++shotIndex)
            {
                const BALaserRangeConstraint& constraint = requestedOptions.laserRangeConstraints[shotIndex];
                const std::string prefix = "BA 输入验证失败: 激光测距 shot[" + std::to_string(shotIndex) + "] ";
                if (constraint.cameraIndex < 0 || constraint.cameraIndex >= static_cast<int>(cameras.size()))
                {
                    return invalid(BASolveStatus::InvalidInput, prefix + "cameraIndex 越界");
                }
                if (!isFinitePoint(constraint.initialPoint) || !isFinitePoint(constraint.leverArmCameraMeters) ||
                    !std::isfinite(constraint.observedRangeMeters) || constraint.observedRangeMeters <= 0.0 ||
                    !std::isfinite(constraint.sigmaRangeMeters) || constraint.sigmaRangeMeters <= 0.0 ||
                ...
                const double effectiveWeight = requestedOptions.laserRangeWeight * constraint.weight;
                const double normalizedScale = std::sqrt(effectiveWeight) / constraint.sigmaRangeMeters;
                const double scaledHuberDelta = requestedOptions.laserRangeHuberDelta * std::sqrt(effectiveWeight);
                if (!std::isfinite(effectiveWeight) || !std::isfinite(normalizedScale) ||
                ...
                for (const BAObservation& observation : constraint.measuredImageObservations)
                {
                    if (!observationIsUsable(observation, cameras.size()))
                    {
                        return invalid(BASolveStatus::InvalidInput, prefix + "包含非法真实 measured 像点");
                    }
                ...
                case BALaserPointMode::Constrained:
                    if (!isFinitePoint(constraint.pointPrior) ||
                        !isUsableLaserPointSqrtInformation(constraint.pointPriorSqrtInformation))
                    {
                        return invalid(BASolveStatus::InvalidInput,
                ...
                case BALaserPointMode::Free:
                {
                         ...
                         ++left)
                    {
                        ...
                        {
                            const std::array<double, 3> leftCenter = cameras[static_cast<size_t>(*left)].cameraCenter();
                            const std::array<double, 3> rightCenter =
                                cameras[static_cast<size_t>(*right)].cameraCenter();
                            if (isFinitePoint(leftCenter) && isFinitePoint(rightCenter) &&
                    ...
                    if (constraint.measuredImageObservations.size() < 2 || measuredCameras.size() < 2 ||
                ...
                default:
                    return invalid(BASolveStatus::InvalidInput, prefix + "pointMode 非法");
                }
            }
        }
        ...
        {
            return invalid(BASolveStatus::InvalidInput, "BA 输入验证失败: 物方约束 RMS 增长倍率不能小于 1");
        }
        if (requestedOptions.cameraPlaneConstraint.enabled)
        {
            const auto& constraint = requestedOptions.cameraPlaneConstraint;
            const double normalNormSquared = constraint.normal[0] * constraint.normal[0] +
            ...
            const bool finiteNormal = std::all_of(
                constraint.normal.begin(), constraint.normal.end(), [](double value) { return std::isfinite(value); });
            const bool validReferenceDistances = constraint.referenceSignedDistances.empty() ||
            ...
            if (!finitePoint || !finiteNormal || !std::isfinite(normalNormSquared) ||
                std::abs(normalNormSquared - 1.0) > 1.0e-6 || !validReferenceDistances ||
                !std::isfinite(constraint.sigmaMeters) || constraint.sigmaMeters <= 0.0 ||
                !std::isfinite(constraint.weight) || constraint.weight <= 0.0 ||
                !std::isfinite(requestedOptions.cameraPlaneHuberDelta) || requestedOptions.cameraPlaneHuberDelta < 0.0)
            {
                return invalid(BASolveStatus::InvalidInput,
        ...
        for (const int index : requestedOptions.fixedCameraIndices)
        {
            ...
            {
                return invalid(BASolveStatus::InvalidInput, "BA 输入验证失败: 固定相机索引越界");
            }
        ...
        {
            return invalid(BASolveStatus::InvalidInput,
                           "BA 输入验证失败: PlaMatrix cluster-Jacobi 大小必须位于 [1, 16]");
        }
        ...
        for (const int index : requestedOptions.fixedTrackIndices)
        {
            ...
            {
                return invalid(BASolveStatus::InvalidInput, "BA 输入验证失败: 固定轨迹索引越界");
            }
            if (!fixedTracks.insert(index).second)
            {
                return invalid(BASolveStatus::InvalidInput, "BA 输入验证失败: 固定轨迹索引重复");
            }
        }
        std::sort(normalizedOptions->fixedTrackIndices.begin(), normalizedOptions->fixedTrackIndices.end());

        if (!requestedOptions.refineCameraPose || requestedOptions.gaugePolicy == BAGaugePolicy::CallerManaged ||
            cameras.empty())
        {
            return {};
        }
        ...
        const bool completeControlPointGauge = hasCompleteControlPointGauge(tracks, requestedOptions);
        const bool absolutePose = hasEnabledPosePrior(requestedOptions) || completeControlPointGauge;
        bool absoluteScale = hasMetricPosePriorBaseline(requestedOptions) || completeControlPointGauge ||
        ...
        auto refreshScaleFromFixedCameras = [&]()
        {
            absoluteScale = absoluteScale ||
                            ...
                            hasControlPointBaselineToFixedCamera(cameras, tracks, requestedOptions, fixedCameras);
        };
