                             ...
                             plamatrix::BlockNormalEquations<double>* equations)
{
         ...
         ++group_index)
    {
        ...
        for (std::size_t parameter = 0; parameter < 9; ++parameter)
        {
            ...
            residual[parameter] = group.inverseSigma[parameter] *
                (group.parameters[parameter] - group.prior[parameter]);
        ...
        if (equations)
        {
            equations->addPrimaryResidualBlock(
                active.cameraBlockCount + static_cast<int>(group_index),
                jacobian.data(),
                residual.data(),
                9);
        }
    ...
    for (std::size_t camera_index = 0; camera_index < state.cameras.size(); ++camera_index)
    {
        ...
        if (camera_index < options.cameraPosePriors.size() &&
            linearizePosePrior(state.cameras[camera_index],
                               options.cameraPosePriors[camera_index],
                               options,
                               &linearization))
        {
        ...
        if (linearizeCameraPlane(
                state.cameras[camera_index], camera_index, options, &linearization))
        {
            cost += linearization.robustCost;
            if (equations)
            {
                         ...
                         plamatrix::BlockNormalEquations<double>* equations)
{
    ...
    for (const auto& scale : options.scaleBarConstraints)
    {
        ...
        const int block_a = active.trackPrimaryBlock[
            static_cast<std::size_t>(scale.trackIndexA)];
        const int block_b = active.trackPrimaryBlock[
            static_cast<std::size_t>(scale.trackIndexB)];
        if (block_a >= 0)
        {
        ...
        if (!blocks.empty())
        {
            equations->addPrimaryResidualBlocks(blocks,
                                                jacobians,
                                                linearization.residual.data(),
                                                1,
                                                linearization.normalWeight);
                           ...
                           plamatrix::BlockNormalEquations<double>* equations)
{
         ...
         ++shot_index)
    {
        ...
        if (linearizeLaserRange(state.cameras[static_cast<std::size_t>(shot.cameraIndex)],
                                shot,
                                point,
                                options,
                                &constraint))
        {
            ...
            const int camera_block = active.cameraBlock[
