                                 ...
                                 const std::filesystem::path &cameraList)
{
    ...
    for (qsizetype pointIndex = 0; pointIndex < points.size(); ++pointIndex)
    {
        ...
        {
            const QJsonObject observation = observations[observationIndex].toObject();
            ...
            const QJsonArray xy = observation.value(QStringLiteral("xy")).toArray();
            const std::string observationPrefix = prefix + ".observations[" +
                std::to_string(observationIndex) + "].xy";
