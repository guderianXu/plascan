...
namespace
{
                                         ...
                                         int imageHeight = 768)
    {
        ...
        for (const Vec3& point : points)
        {
            ...
            for (std::size_t cameraIndex = 0; cameraIndex < cameras.size(); ++cameraIndex)
            {
                ...
                if (!cameras[cameraIndex].projectWorldPoint(world, pixel) || pixel[0] < 16.0 ||
                    pixel[0] > imageWidth - 16.0 || pixel[1] < 16.0 || pixel[1] > imageHeight - 16.0)
                {
                    continue;
                }
                track.observations.push_back(
                    xjw::BAObservation{static_cast<int>(cameraIndex), pixel[0], pixel[1], 1.0});
            }
    ...
    std::vector<Vec3> aerialPoints()
    {
        ...
        for (int row = -8; row <= 8; ++row)
        {
            ...
            {
                points.push_back({{static_cast<double>(column) * 0.65,
    ...
    std::vector<xjw::FramePinholeCamera> weaklyParallelAerialCameras()
    {
        ...
        for (int row = -2; row <= 2; ++row)
        {
            for (int column = -2; column <= 2; ++column)
            {
                ...
                cameras.push_back(makeLookAtCamera(center, {{center[0] + offset, center[1], 0.0}}));
            }
        }
        return cameras;
    }
    ...
    std::vector<Vec3> narrowAerialPoints()
    {
        ...
        for (int row = -8; row <= 8; ++row)
        {
            for (int column = -10; column <= 10; ++column)
            {
                points.push_back({{static_cast<double>(column) * 0.035,
    ...
    std::vector<Vec3> orbitalPoints()
    {
        ...
        for (int depth = -2; depth <= 2; ++depth)
        {
            for (int row = -6; row <= 6; ++row)
            {
                ...
                {
                    points.push_back({{column * 0.78, row * 0.68, depth * 0.70 + 0.10 * std::sin(column + row)}});
                }
            }
        }
        return points;
    ...
    std::vector<Vec3> narrowOrbitalPoints()
    {
        ...
        for (int depth = -2; depth <= 2; ++depth)
        {
            for (int row = -6; row <= 6; ++row)
            {
                for (int column = -6; column <= 6; ++column)
                {
                    points.push_back({{column * 0.06, row * 0.055, depth * 0.08 + 0.01 * std::sin(column + row)}});
                }
            }
        }
        return points;
    ...
    bool enabled(const xjw::BAAdaptiveCameraModelAssessment& assessment, xjw::BAIntrinsicParameter parameter)
    {
...
TEST(BundleAdjustAdaptiveCameraModelTest, UnanchoredParallelAerialBlockKeepsIntrinsicsFixed)
{
    ...
    EXPECT_EQ(assessment.reason, "unanchored_parallel_aerial_fixed_intrinsics_doming_guard");
}
...
TEST(BundleAdjustAdaptiveCameraModelTest, WeaklyParallelUnanchoredAerialBlockKeepsIntrinsicsFixedFromFirstRound)
{
    ...
    EXPECT_EQ(xjw::enabledIntrinsicParameterCount(options.sharedIntrinsicParameterMask), 0);
}
...
TEST(BundleAdjustAdaptiveCameraModelTest, TrustedFocalWeaklyParallelAerialBlockEstimatesOnlyRadialK1)
{
    ...
    const xjw::BAAdaptiveCameraModelAssessment assessment = xjw::assessAdaptiveCameraModel(cameras, tracks, &options);
    ...
    EXPECT_EQ(assessment.reason, "unanchored_parallel_aerial_trusted_focal_k1_doming_correction");
    ...
    EXPECT_EQ(xjw::enabledIntrinsicParameterCount(options.sharedIntrinsicParameterMask), 1);
}
...
TEST(BundleAdjustAdaptiveCameraModelTest, InactiveObliqueCamerasDoNotChangeGeometry)
{
    ...
    ASSERT_TRUE(assessment.valid);
    EXPECT_EQ(assessment.activeCameraCount, static_cast<int>(activeCameraCount));
    EXPECT_GT(assessment.opticalAxisConcentration, 0.99);
    EXPECT_TRUE(assessment.unanchoredParallelAerialGuardApplied);
    EXPECT_EQ(assessment.modelName, "fixed");
}
...
TEST(BundleAdjustAdaptiveCameraModelTest, ConvergentMultiHeightOrbitReleasesMoreParameters)
{
    ...
    EXPECT_TRUE(enabled(assessment, xjw::BAIntrinsicParameter::RadialK2))
        << "reliability=" << assessment.reliability[static_cast<std::size_t>(xjw::BAIntrinsicParameter::RadialK2)]
        << " independence="
        << assessment.incrementalInformationScore[static_cast<std::size_t>(xjw::BAIntrinsicParameter::RadialK2)]
        << " sensitivity=" << assessment.sensitivity[static_cast<std::size_t>(xjw::BAIntrinsicParameter::RadialK2)]
        << " radiusP90=" << assessment.normalizedRadiusP90 << " sectors=" << assessment.occupiedPeripheralSectors;
    EXPECT_GT(xjw::enabledIntrinsicParameterCount(assessment.enabled), 2);
...
TEST(BundleAdjustAdaptiveCameraModelTest, NarrowConvergentBlockCanReleaseTangentialDistortion)
{
    ...
    EXPECT_TRUE(enabled(assessment, xjw::BAIntrinsicParameter::TangentialP1))
        << "reliability=" << assessment.reliability[static_cast<std::size_t>(xjw::BAIntrinsicParameter::TangentialP1)]
        << " sensitivity=" << assessment.sensitivity[static_cast<std::size_t>(xjw::BAIntrinsicParameter::TangentialP1)]
        << " axisBalance=" << assessment.imageAxisBalance;
    EXPECT_TRUE(enabled(assessment, xjw::BAIntrinsicParameter::TangentialP2))
        << "reliability=" << assessment.reliability[static_cast<std::size_t>(xjw::BAIntrinsicParameter::TangentialP2)]
        << " sensitivity=" << assessment.sensitivity[static_cast<std::size_t>(xjw::BAIntrinsicParameter::TangentialP2)]
        << " axisBalance=" << assessment.imageAxisBalance;
}

TEST(BundleAdjustAdaptiveCameraModelTest, NarrowFieldPlaMatrixCanEstimateLargeK1Coefficient)
{
    ...
    std::vector<xjw::BATrack> tracks = makeTracks(truthCameras, narrowAerialPoints(), 3878, 2888);
    ASSERT_GT(tracks.size(), 100u);
    for (xjw::BATrack& track : tracks)
    {
        track.controlPointConstraints.push_back({track.initialPoint, 1.0e-4, 1.0, 0});
    }
    ...
    ASSERT_TRUE(xjw::applyAdaptiveCameraModel(assessment, &options));
    ASSERT_GT(options.maxSharedRadialK1Abs * options.sharedLowOrderDistortionScale, 1.05);

    const xjw::BAResult result = xjw::BundleAdjust::optimizePoints(referenceCameras, tracks, options);

    ASSERT_TRUE(result.solutionUsable) << result.backendMessage;
    ASSERT_FALSE(result.refinedCameras.empty());
    EXPECT_NEAR(result.refinedCameras.front().distortion().radialK1, 1.05, 0.08);
}
...
TEST(BundleAdjustAdaptiveCameraModelTest, UnsupportedCalibrationGroupFreezesModel)
{
    ...
    ASSERT_TRUE(assessment.valid);
    EXPECT_EQ(assessment.reason, "calibration_group_conservative_intersection");
    EXPECT_EQ(xjw::enabledIntrinsicParameterCount(assessment.enabled), 0);
    EXPECT_EQ(assessment.modelName, "fixed");
}
...
TEST(BundleAdjustAdaptiveCameraModelTest, CalibrationGroupsUseConservativeIntersection)
{
    ...
    std::fill_n(options.cameraCalibrationGroupIds.begin(), aerialCameraCount, 0);
...
TEST(BundleAdjustAdaptiveCameraModelTest, RespectsCallerIntrinsicParameterMask)
{
    ...
    options.sharedIntrinsicParameterMask[static_cast<std::size_t>(xjw::BAIntrinsicParameter::FocalLength)] = true;
    options.sharedIntrinsicParameterMask[static_cast<std::size_t>(xjw::BAIntrinsicParameter::RadialK1)] = true;

    ASSERT_TRUE(xjw::applyAdaptiveCameraModel(assessment, &options));
    EXPECT_EQ(xjw::enabledIntrinsicParameterCount(options.sharedIntrinsicParameterMask), 2);
    EXPECT_TRUE(options.sharedIntrinsicParameterMask[static_cast<std::size_t>(xjw::BAIntrinsicParameter::FocalLength)]);
    EXPECT_TRUE(options.sharedIntrinsicParameterMask[static_cast<std::size_t>(xjw::BAIntrinsicParameter::RadialK1)]);
    EXPECT_FALSE(options.sharedIntrinsicParameterMask[static_cast<std::size_t>(xjw::BAIntrinsicParameter::RadialK2)]);
    ...
    focalFrozen.sharedIntrinsicParameterMask[static_cast<std::size_t>(xjw::BAIntrinsicParameter::FocalLength)] = false;

    EXPECT_FALSE(xjw::applyAdaptiveCameraModel(assessment, &focalFrozen));
    EXPECT_EQ(xjw::enabledIntrinsicParameterCount(focalFrozen.sharedIntrinsicParameterMask), 0);
}
...
TEST(BundleAdjustAdaptiveCameraModelTest, RestoresParametersDisabledAfterAnEarlierAdaptiveRound)
{
    ...
    active[static_cast<std::size_t>(xjw::BAIntrinsicParameter::FocalLength)] = true;

    ASSERT_TRUE(xjw::restoreInactiveAdaptiveIntrinsics(&current, references, active));
...
TEST(BundleAdjustAdaptiveCameraModelTest, PlaMatrixHonorsIndividualIntrinsicMask)
{
    ...
    const xjw::BAResult result =
        xjw::BundleAdjust::optimizePoints(cameras, makeTracks(cameras, orbitalPoints()), options);
    ...
    for (std::size_t index = 0; index < cameras.size(); ++index)
    {
        ...
        EXPECT_DOUBLE_EQ(refined_distortion.tangentialP1, source_distortion.tangentialP1);
        EXPECT_DOUBLE_EQ(refined_distortion.tangentialP2, source_distortion.tangentialP2);
    }
}

TEST(BundleAdjustAdaptiveCameraModelTest, FocalOnlyPlaMatrixUsesStableReferencePrior)
{
    if (!xjw::BundleAdjust::isBackendAvailable(xjw::BABackend::PlaMatrixCpu))
    {
        GTEST_SKIP() << "PlaMatrix CPU backend is not available";
    ...
    std::vector<xjw::BATrack> tracks = makeTracks(truthCameras, aerialPoints());
    ASSERT_GT(tracks.size(), 100u);
    for (xjw::BATrack& track : tracks)
    {
        track.controlPointConstraints.push_back({track.initialPoint, 1.0e-4, 1.0, 0});
...
TEST(BundleAdjustAdaptiveCameraModelTest, DistortionPriorRemainsAnchoredAcrossRounds)
{
    if (!xjw::BundleAdjust::isBackendAvailable(xjw::BABackend::PlaMatrixCpu))
    {
        GTEST_SKIP() << "PlaMatrix CPU backend is not available";
    ...
    const xjw::BAResult first = xjw::BundleAdjust::optimizePoints(warmCameras, tracks, options);
    ...
    const xjw::BAResult second = xjw::BundleAdjust::optimizePoints(first.refinedCameras, tracks, options);
...
TEST(BundleAdjustAdaptiveCameraModelTest, StableIntrinsicReferencePreventsBoundsFromCompoundingAcrossRounds)
{
    if (!xjw::BundleAdjust::isBackendAvailable(xjw::BABackend::PlaMatrixCpu))
    {
        GTEST_SKIP() << "PlaMatrix CPU backend is not available";
    ...
    {
        const double referenceFocal = index < referenceCameras.size() / 2 ? 800.0 : 1100.0;
        calibrationGroups[index] = index < referenceCameras.size() / 2 ? 0 : 1;
        referenceCameras[index].setIntrinsics(referenceFocal, referenceFocal, 512.0, 384.0);
        truthCameras[index] = referenceCameras[index];
        truthCameras[index].setIntrinsics(referenceFocal * 1.15, referenceFocal * 1.15 * 1.08, 537.0, 362.0);
        warmCameras[index] = referenceCameras[index];
        warmCameras[index].setIntrinsics(referenceFocal * 1.04375, referenceFocal * 1.04375 * 1.018, 519.0, 377.0);
    }

    std::vector<xjw::BATrack> tracks = makeTracks(truthCameras, orbitalPoints());
    ...
    options.sharedIntrinsicParameterMask.fill(false);
    options.sharedIntrinsicParameterMask[static_cast<std::size_t>(xjw::BAIntrinsicParameter::FocalLength)] = true;
    options.sharedIntrinsicParameterMask[static_cast<std::size_t>(xjw::BAIntrinsicParameter::FocalAspectRatio)] = true;
    options.sharedIntrinsicParameterMask[static_cast<std::size_t>(xjw::BAIntrinsicParameter::PrincipalPointX)] = true;
    options.sharedIntrinsicParameterMask[static_cast<std::size_t>(xjw::BAIntrinsicParameter::PrincipalPointY)] = true;
    options.sharedIntrinsicReferenceCameras = referenceCameras;
    ...
    const xjw::BAResult first = xjw::BundleAdjust::optimizePoints(warmCameras, tracks, options);
    ASSERT_TRUE(first.solutionUsable) << first.backendMessage;
    ASSERT_GT(first.refinedCameras.front().focalX(), referenceCameras.front().focalX() * 1.049);
    const xjw::BAResult second = xjw::BundleAdjust::optimizePoints(first.refinedCameras, tracks, options);
    ASSERT_TRUE(second.solutionUsable) << second.backendMessage;
    ...
    for (std::size_t index = 0; index < second.refinedCameras.size(); ++index)
    {
        ...
        EXPECT_GE(refined.principalX(), reference.principalX() - maxPrincipalOffset - 1.0e-8);
        EXPECT_LE(refined.principalX(), reference.principalX() + maxPrincipalOffset + 1.0e-8);
        EXPECT_GE(refined.principalY(), reference.principalY() - maxPrincipalOffset - 1.0e-8);
        EXPECT_LE(refined.principalY(), reference.principalY() + maxPrincipalOffset + 1.0e-8);
    }

    options.sharedIntrinsicParameterMask.fill(false);
    options.sharedIntrinsicParameterMask[static_cast<std::size_t>(xjw::BAIntrinsicParameter::FocalLength)] = true;
    const xjw::BAResult principalFrozen = xjw::BundleAdjust::optimizePoints(second.refinedCameras, tracks, options);
    ASSERT_TRUE(principalFrozen.solutionUsable) << principalFrozen.backendMessage;
    double expectedPrincipalOffsetX = 0.0;
    double expectedPrincipalOffsetY = 0.0;
    for (std::size_t index = 0; index < principalFrozen.refinedCameras.size(); ++index)
    {
        expectedPrincipalOffsetX +=
            principalFrozen.refinedCameras[index].principalX() - referenceCameras[index].principalX();
        expectedPrincipalOffsetY +=
            principalFrozen.refinedCameras[index].principalY() - referenceCameras[index].principalY();
    }
    const double cameraCount = static_cast<double>(principalFrozen.refinedCameras.size());
    EXPECT_NEAR(principalFrozen.refinedSharedPrincipalOffsetX, expectedPrincipalOffsetX / cameraCount, 1.0e-10);
    EXPECT_NEAR(principalFrozen.refinedSharedPrincipalOffsetY, expectedPrincipalOffsetY / cameraCount, 1.0e-10);
    EXPECT_GT(std::abs(principalFrozen.refinedSharedPrincipalOffsetX) +
