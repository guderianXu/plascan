...
namespace xjw::dense_match
{
    ...
    public:
        ...
        [[nodiscard]] const DenseMatchExecutionReport& executionReport() const;

        static bool saveDisparity(const DisparityResult& result, const std::string& filepath);

    private:
        DisparityResult processImpl(const cv::Mat& left, const cv::Mat& right);
        DisparityResult fail(std::string message);
        DisparityResult
        computeRawDisparity(const cv::Mat& left, const cv::Mat& right, const DenseMatchConfig& config) const;

        DenseMatchConfig _config;
        cv::Mat _left, _right;
        std::string _lastError;
        DenseMatchExecutionReport _executionReport;
    };
