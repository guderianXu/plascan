...
namespace xjw::dense_match
{
    ...
    DisparityResult BlockMatcher::compute(const cv::Mat& left, const cv::Mat& right)
    {
        ...
        {
            return computeBlockMatchCUDA(left,
                                         right,
        ...
        {
            return computeBlockMatchOpenCL(left,
                                           right,
                                           _config.minDisparity,
                                           _config.maxDisparity,
                                           _config.corrKernelW,
        ...
        CostVolume volume = computeCostVolume(left,
                                              right,
                                              _config.minDisparity,
                                              _config.maxDisparity,
                                              _config.corrKernelW,
...
#pragma omp parallel for num_threads(threadCount)
        ...
        if (_config.subpixel != SubpixelMode::None)
        {
            ...
            result.disparity =
                refiner.refine(result.disparity, volume, _config.minDisparity, _config.maxDisparity, result.validMask);
        }
        return result;
    }
