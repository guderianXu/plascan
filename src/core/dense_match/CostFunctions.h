...
namespace xjw::dense_match
{
    ...
    [[nodiscard]] CostVolumeBufferLayout
    checkedCostVolumeBufferLayout(int imageWidth, int imageHeight, int minDisparity, int maxDisparity);

    // Returns the valid half-open range of disparity *indices* for a left-image
    // column.  Index zero corresponds to minDisparity.
    DisparityIndexRange validDisparityIndexRangeForLeftX(int leftX, int imageWidth, int minDisparity, int maxDisparity);
    ...
    float computeCost(const uchar* left,
                      const uchar* right,
                      int x,
                      int y,
                      int d,
    ...
    CostVolume computeCostVolume(const cv::Mat& left,
                                 const cv::Mat& right,
                                 int minDisp,
                                 int maxDisp,
                                 int kernelW,
    ...
    // Runs WTA, confidence and optional parabolic subpixel refinement on CUDA.
    // The input is uploaded once and only the three final result planes return to
    // the host.  This is used after CPU SGM aggregation.
    DisparityResult selectCostVolumeCUDA(const CostVolume& volume, SubpixelMode subpixel, int cudaDevice = 0);

    // Keeps the block-matching cost volume resident through CUDA WTA,
    // confidence and parabolic subpixel refinement.
    ...
    bool isCostVolumeOpenCLAvailable(int openClDevice = 0);
    ...
    DisparityResult selectCostVolumeOpenCL(const CostVolume& volume, SubpixelMode subpixel, int openClDevice = 0);
