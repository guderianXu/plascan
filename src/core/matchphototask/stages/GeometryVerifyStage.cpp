...
namespace xjw::matchphotos
{
    ...
    namespace
    {
        ...
        double
        gridCoverage(const image_matching::PairMatchData& pair, bool firstImage, const MatchPhotosOptions& options)
        {
            ...
            for (const auto& correspondence : pair.correspondences)
            {
                ...
                const auto& observation = firstImage ? correspondence.observation0 : correspondence.observation1;
                const int column =
                    std::clamp(static_cast<int>(observation.x * columns / std::max(1, width)), 0, columns - 1);
                const int row = std::clamp(static_cast<int>(observation.y * rows / std::max(1, height)), 0, rows - 1);
                             ...
                             const QString& image1Path)
    {
        ...
        return distance == 1 || (options.pairPolicy.closeSequenceLoop && context.pairInput.images.size() > 2 &&
                                                  ...
                                                  bool adjacentImages)
    {
        ...
        metrics.inlierCount = static_cast<int>(std::count_if(
            ...
            [](const auto& correspondence) {
                return image_matching::hasFlag(correspondence.flags, image_matching::MatchRecordFlag::GeometryInlier);
            }));
        metrics.inlierRatio = metrics.rawMatchCount > 0 ? static_cast<double>(metrics.inlierCount) /
                                                              static_cast<double>(metrics.rawMatchCount)
                                                        : 0.0;
                                                    ...
                                                    const MatchPhotosOptions& options)
    {
        ...
        const double support =
            1.0 - std::exp(-static_cast<double>(metrics.inlierCount - options.geometryMinInliers + 1) / supportScale);
        ...
        {
            decision.requiredInlierRatio = std::max(minimumRatio * 0.8, decision.requiredInlierRatio - 0.06);
            decision.requiredGridCoverage = std::max(minimumCoverage * 0.8, decision.requiredGridCoverage - 0.04);
        }
        ...
        const double countScore = std::min(1.0,
                                           static_cast<double>(metrics.inlierCount) /
                                               static_cast<double>(std::max(1, options.geometryMinInliers * 2)));
        decision.score = 0.45 * std::min(1.5, ratioScore) + 0.35 * std::min(1.5, coverageScore) + 0.20 * countScore;
        decision.passed =
            metrics.inlierRatio >= minimumRatio * 0.8 && coverage >= minimumCoverage * 0.8 && decision.score >= 0.92;
        return decision;
    }
    ...
    QByteArray geometryVerificationFingerprint(const MatchPhotosOptions& options)
    {
        ...
        QJsonObject object;
        object[QStringLiteral("schema_version")] = 3;
        object[QStringLiteral("enabled")] = options.enableGeometryVerification;
        object[QStringLiteral("model")] = QStringLiteral("fundamental");
        object[QStringLiteral("reprojection_threshold_pixels")] = options.geometryReprojThreshold;
        ...
        object[QStringLiteral("maximum_iterations")] = std::max(100, options.geometryMaxIterations);
        object[QStringLiteral("confidence")] = 0.9999;
        object[QStringLiteral("random_seed")] = 0;
        object[QStringLiteral("quality_gate_version")] = 2;
        object[QStringLiteral("plamatch_non_guided_geometry")] =
            QStringLiteral("reference_local_consistency_minimum_8");
    ...
    namespace
    {
        ...
        makeGeometryReport(MatchPhotosStageStatus status, const QString& message, int itemCount = 0)
        {
        ...
        void updateGeometryRecordMetadata(MatchPhotosMatchRecord* record,
                                          bool verified,
                                          const GeometryQualityMetrics* quality,
                                          const GeometryQualityDecision* decision);

        bool usesReferencePlaMatchGeometry(const MatchPhotosMatchRecord& record, const MatchPhotosOptions& options)
        {
            return record.algorithmId == QLatin1String(image_matching::kPlaMatchHctAlgorithmId) &&
                   options.guidedMatchingMode == GuidedMatchingMode::Disabled;
        }

        void applyReferencePlaMatchGeometry(MatchPhotosMatchRecord* record,
                                            ...
                                            const MatchPhotosOptions& options)
        {
            ...
            const int match_count = static_cast<int>(pair.correspondences.size());
            pair.rawMatchCount = static_cast<std::uint32_t>(match_count);
            pair.geometryInlierCount = pair.rawMatchCount;
            pair.geometryPassed = match_count >= 8;
            ...
            updateGeometryRecordMetadata(record, true, &metrics, nullptr);
            record->settings[QStringLiteral("geometry_mode")] = QStringLiteral("plamatch_local_consistency");
        }
        ...
        VerificationInput makeVerificationInput(const image_matching::PairMatchData& pair)
        {
            ...
            for (int index = 0; index < count; ++index)
            {
                ...
                input.features0.keypoints.emplace_back(correspondence.observation0.x,
                ...
                input.features1.keypoints.emplace_back(correspondence.observation1.x,
                ...
                input.matches.cvMatches.emplace_back(index, index, 1.0f - correspondence.confidence);
            }
        ...
        {
            if (!record.pairData || !record.settings.value(QStringLiteral("geometry_cache_reusable")).toBool(false) ||
            ...
            const int flaggedInliers = static_cast<int>(
            ...
            const bool adjacent = areSequenceAdjacent(context, options, record.image0Path, record.image1Path);
            const GeometryQualityMetrics metrics = measureGeometryQuality(pair, options, adjacent);
            const bool referencePlaMatchGeometry = usesReferencePlaMatchGeometry(record, options);
            const bool expectedPassed = referencePlaMatchGeometry
                                            ? rawCount >= 8
                                            : pair.geometryModel != image_matching::GeometryModel::None &&
                                                  evaluateGeometryQuality(metrics, options).passed;
            return flaggedInliers == static_cast<int>(pair.geometryInlierCount) &&
                   (!referencePlaMatchGeometry || (pair.geometryInlierCount == pair.rawMatchCount &&
                                                   pair.geometryModel == image_matching::GeometryModel::None)) &&
                                          ...
                                          const GeometryQualityDecision* decision = nullptr)
        {
            ...
            record->settings[QStringLiteral("geometry_inlier_ratio")] =
                rawCount > 0 ? static_cast<double>(inlierCount) / static_cast<double>(rawCount) : 0.0;
            if (quality)
            {
            ...
            if (decision)
            {
                ...
                record->settings[QStringLiteral("geometry_required_inlier_ratio")] = decision->requiredInlierRatio;
                record->settings[QStringLiteral("geometry_required_grid_coverage")] = decision->requiredGridCoverage;
            }
        }
                                                    ...
                                                    std::vector<MatchPhotosMatchRecord>* matchRecords) const
    {
        if (options.planOnly)
        {
            return makeGeometryReport(MatchPhotosStageStatus::Skipped, QStringLiteral("plan-only 模式，跳过几何验证"));
        }
        if (!matchRecords || matchRecords->empty())
        {
            return makeGeometryReport(MatchPhotosStageStatus::Skipped, QStringLiteral("没有可用于几何验证的匹配结果"));
        }
        ...
        int reusedInliers = 0;
        int referenceLocalPairs = 0;
        for (int index = 0; index < totalPairs; ++index)
        {
            MatchPhotosMatchRecord& record = (*matchRecords)[static_cast<std::size_t>(index)];
            if (usesReferencePlaMatchGeometry(record, options))
            {
                applyReferencePlaMatchGeometry(&record, context, options);
                ++referenceLocalPairs;
            ...
            ++reusedPairs;
            const bool adjacent = areSequenceAdjacent(context, options, record.image0Path, record.image1Path);
            const GeometryQualityMetrics metrics = measureGeometryQuality(*record.pairData, options, adjacent);
            const GeometryQualityDecision decision = evaluateGeometryQuality(metrics, options);
            updateGeometryRecordMetadata(&record, true, &metrics, &decision);
            reusedPassedPairs += record.passedGeometry ? 1 : 0;
            reusedInliers += record.geometricInlierCount;
        ...
        if (workIndices.empty())
        {
            return makeGeometryReport(
                ...
                QStringLiteral("参考兼容几何完成（缓存全命中）：局部一致性 %1 对，缓存复用 %2 对，通过 %3 对，"
                               "内点 %4，未运行 USAC")
                    .arg(referenceLocalPairs)
                    .arg(reusedPairs)
        ...
        const int workerCount = resolveGeometryVerificationWorkers(static_cast<int>(workIndices.size()),
                                                                   std::thread::hardware_concurrency());
        std::atomic_int nextIndex{0};
        std::atomic_int completed{reusedPairs + referenceLocalPairs};
        ...
        catch (const std::exception& error)
        {
            return makeGeometryReport(MatchPhotosStageStatus::Failed,
                                      QStringLiteral("几何验证 worker 异常：%1").arg(QString::fromUtf8(error.what())),
                                      passedPairs.load());
        }
        catch (...)
        {
            return makeGeometryReport(
                MatchPhotosStageStatus::Failed, QStringLiteral("几何验证 worker 发生未知异常"), passedPairs.load());
        }

        if (shouldCancelMatchPhotos(context))
        {
            return makeGeometryReport(
                MatchPhotosStageStatus::Failed, QStringLiteral("用户取消几何验证"), passedPairs.load());
        }
        ...
        return makeGeometryReport(MatchPhotosStageStatus::Completed,
                                  QStringLiteral("几何验证完成：通过 %1 对，失败 %2 对，内点 %3，复用 %4 对，"
                                                 "其中局部一致性 %5 对，CPU worker %6，总计 %7 ms")
                                      .arg(passedPairs.load())
                                      .arg(failedPairs)
