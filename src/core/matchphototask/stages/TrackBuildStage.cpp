...
image_matching::ImageMatchWriteResult persistMatches(
                               ...
                               MatchPhotosResult *result)
{
    ...
    const auto appendPairSide = [&](const QString &ownerPath,
                                    const QString &peerPath,
                                    const QJsonObject &pairSettings)
    {
    ...
    for (PendingRecord &item : pending)
    {
        ...
        item.record.settings[QStringLiteral("neighbor_count")] =
            item.record.neighborVariantCount;
...
MatchPhotosStageReport TrackBuildStage::run(
    const MatchPhotosContext &context,
    ...
    MatchPhotosResult *result) const
{
    ...
    const image_matching::ImageMatchWriteResult writeResult =
        persistMatches(context, *matchRecords);
    if (!writeResult.success)
    {
        return makeTrackReport(MatchPhotosStageStatus::Failed,
                               QStringLiteral("写入每影像匹配分片失败：%1")
