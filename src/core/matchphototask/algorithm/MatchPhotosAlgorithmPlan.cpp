...
namespace xjw
{
    namespace matchphotos
    {
        ...
        {
            QString summary = plan.displayName.isEmpty() ? plan.algorithmId : plan.displayName;
            ...
            summary += QStringLiteral("，精度 %1/downscale=%2")
                           .arg(alignmentAccuracyName(static_cast<AlignmentAccuracy>(plan.alignmentDownscale)))
                           .arg(plan.alignmentDownscale);
            if (plan.preferCuda)
            {
            ...
            {
                summary += plan.guidedMatchingMode == GuidedMatchingMode::Automatic ? QStringLiteral("，自动引导匹配")
