...
namespace xjw::image_matching
{
    ...
    namespace
    {
...
#if !defined(PLASCAN_HAS_TENSORRT)
        void registerUnavailableTensorRtAlgorithm(const ImageMatchingAlgorithmDescriptor& descriptor)
        {
            QString ignored_error;
            ImageMatchingRegistry::registerAlgorithm(
                descriptor,
                [id = descriptor.id](const ImageMatchingRuntimeConfig&) -> std::unique_ptr<IImageMatchingAlgorithm> {
                    throw std::runtime_error(
                        QStringLiteral("算法 %1 不可用：PlaScan 构建时未启用 TensorRT").arg(id).toStdString());
                },
                &ignored_error);
        }
        ...
        registerUnavailableTensorRtAlgorithms();
                               ...
                               registerPlaMatchHctAlgorithm();
                           });
        }

    } // namespace

    bool ImageMatchingRegistry::registerAlgorithm(const ImageMatchingAlgorithmDescriptor& descriptor,
                                                  ImageMatchingAlgorithmFactory factory,
    ...
    std::unique_ptr<IImageMatchingAlgorithm> ImageMatchingRegistry::create(const QString& algorithmId,
                                                                           ...
                                                                           QString* errorMessage)
    {
        ...
        catch (const std::exception& error)
        {
            if (errorMessage)
            {
                *errorMessage = QStringLiteral("创建影像匹配算法失败: %1").arg(QString::fromUtf8(error.what()));
            }
            return nullptr;
        }
    }
