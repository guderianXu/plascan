...
namespace xjw::image_matching
{
    ...
    struct ImageMatchingAlgorithmDescriptor
    {
        ...
        bool suppliesCoarsePairPreselection = false; ///< 算法提供专用粗像对筛选能力。
        bool supportsBatchFeatureMatching = false;   ///< 可跨像对复用设备端特征驻留。
    };
    ...
    struct ImageMatchingRuntimeConfig
    {
        ...
        /// Align Photos Accuracy API 值：Highest/High/Medium/Low/Lowest = 0/1/2/4/8。
    ...
    struct ImageFeatureInput
    {
        ...
        double coordinateOffsetX = 0.0; ///< 输入坐标恢复到原图后的像素中心偏移。
        double coordinateOffsetY = 0.0;
    };

    struct FeaturePairInput
    {
        const FeatureSet* features0 = nullptr;
        const FeatureSet* features1 = nullptr;
    };

    using BatchMatchProgressCallback = std::function<void(std::size_t completed, std::size_t total)>;
    ...
    public:
        ...
        virtual MatchResult matchFeatures(const FeatureSet& features0, const FeatureSet& features1);
        virtual std::vector<MatchResult> matchFeatureBatch(std::span<const FeaturePairInput> pairs,
