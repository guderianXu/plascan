...
namespace xjw
{
    ...
    public:
         ...
         * @return 分配的 Point3DId
         */
        Point3DId addPoint3DWithTrack(const std::array<double, 3>& xyz, const Track& track);
        ...
        /// 判断参考点槽位是否存在但当前被过滤为无效。
        bool hasInactivePoint3D(Point3DId id) const;

        /// 将活动点标记为参考流程的无效槽位，保留 point ID 和二维观测绑定。
        bool deactivatePoint3D(Point3DId id);

        /// 用新的几何原位恢复参考点槽位。
        bool restorePoint3DWithTrack(Point3DId id, const std::array<double, 3>& xyz, const Track& track, double error);

        /// 删除三维点，并清理关联观测
        void deletePoint3D(Point3DId id);

        /// 硬清全部活动/无效点槽位和二维绑定，用于显式重建结构的流程边界。
        void clearPoints3D();
    ...
    private:
        ...
        /// 参考流程中过滤失效但仍保留稳定 track-point 身份的点槽位。
        std::unordered_map<Point3DId, ScenePoint3D> inactivePoint3DMap;
