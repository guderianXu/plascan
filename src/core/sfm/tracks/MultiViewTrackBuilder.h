...
namespace xjw
{
    ...
    struct MultiViewTrackBuildResult
    {
        ...
        int rejectedInconsistentBridgeEdges = 0;   ///< 因缺少闭环/交叉支持而拒绝的组件桥接边数。
        int acceptedSupportedBridgeEdges = 0;      ///< 由闭环或多条跨组件边支持的组件合并数。
    ...
    public:
        struct BuildOptions
        {
            bool enableBridgeConsistencyCheck = true;     ///< 阻断两个已成形组件之间缺少多视支持的单边桥接。
            int bridgeCheckMinComponentSize = 2;          ///< 两侧组件至少达到该观测数才检查桥接一致性。
            int matureBridgeComponentSize = 3;            ///< 两侧均达到该规模时必须具有闭环或多边支持。
            int minimumBridgeCrossEdges = 2;              ///< 合并两个组件至少需要的独立跨组件边数（含当前边）。
            int minimumBridgeCycleSupport = 1;            ///< 当前边至少需要的共同第三观测数。
            float unsupportedBridgeMinScoreRatio = 0.92f; ///< 小组件无闭环时，桥接分数相对内部均值的下限。
