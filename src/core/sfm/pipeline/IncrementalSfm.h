...
namespace xjw
{
    ...
    /// “对齐照片”在不同 SfM 阶段使用不同的 BA 迭代和点网刷新策略。
    enum class SfmBundleAdjustmentStage
    {
        InitialPairTrial,
        Incremental,
        FinalVariance
    ...
    struct IncrementalSfmOptions
    {
        ...
        bool useReferenceInitialPairTrials = true;
        ...
        int initialPairLookaheadMaxImages = 3;
        /// 仅在关闭有限前瞻时生效：完整多初始模型评估的最大图像数。
        int multiInitialPairMaxImages = 32;

        // --- 注册选项 ---
        PnpOptions pnpOptions;
        /// 每轮先在同一三维模型上评估全部后方交会候选，再按参考支持度阈值批量接纳。
        bool useReferenceBatchResection = true;
        /// 大型网络按参考相机图分成互斥核心块，独立重建后以共享轨迹稳健 Sim3 合并。
        bool enableIndependentCameraBlocks = true;
        /// 参考分块只在大型网络启用；较小或含孤立影像的项目保持单模型增长语义。
        int independentBlockMinImages = 101;
        /// 参考相机图收缩的核心块规模参数，默认与“对齐照片”的 50/50 一致。
        int independentBlockGroupSize = 50;
        /// 参考收缩拒绝超大合并时较小一侧的规模门槛，默认 50。
        int independentBlockMinimumLargeSide = 50;
        /// 直接共享轨迹不足时，用于 Sim3 producer 的跨块边界相机上限。
        int independentBlockProducerOverlapImages = 16;
        ...
        /// 非参考扩展：最终方差精化后再次尝试注册影像。默认关闭，避免重复完整五轮 BA。
        bool retryUnregisteredAfterFinalBA = false;
        /// 最终 BA 后最多执行的缺口注册轮数。
    ...
    struct IncrementalSfmResult
    {
        ...
        int independentCameraBlocks = 1;              ///< 独立重建后参与合并的核心块数。
        int independentBlockMergeInliers = 0;         ///< 各次稳健 Sim3 合并的内点总数。
        ...
        int baRefinedCalibrationGroupCount = 0; ///< 最终全局 BA 实际参与内参优化的标定组数
        int baSelfCalibrationStagesRun = 0;     ///< 最终全局 BA 实际执行的自标定阶段数
    ...
    public:
        ...
        /// 提供上游已经按“对齐照片”策略构建并选定的完整轨迹，避免 SfM 入口二次抽稀。
        void setInputMultiViewTracks(const std::vector<Track>& tracks);
    ...
    private:
        ...
        friend class IndependentBlockReconstructor;
        ...
        int _lastGlobalBARefinedCalibrationGroupCount = 0;
        int _lastGlobalBASelfCalibrationStagesRun = 0;
        ...
        std::vector<Track> _providedInputMultiViewTracks;
        /// 参考 selected track 的逐影像反向索引；PnP 直接消费完整轨迹而非仅遍历 pairwise 邻边。
        std::unordered_map<ImageId, std::vector<std::size_t>> _inputTrackIndicesByImage;
        ...
        struct ImageRegistrationEvaluation
        {
            ...
            int minimumTrackLength = 0;
            double inlierRatio = 0.0;
            bool usedSequenceRecovery = false;
            bool usedPosePrefilter = false;
            int prefilterCandidateCount = 0;
            bool strictSmallSupport = false;
            int occupiedGridCells = 0;
            bool sequenceConsistencyRejected = false;
            std::string error;
        };

        /// 在冻结的重建上只读求解一张影像；调用方负责串行提交相机和输出日志。
        ImageRegistrationEvaluation
        evaluateImageRegistration(ImageId imageId, bool preferSequencePrior, bool forceSequenceConsistency) const;

        /// 在冻结的当前三维模型上评估全部候选，并按参考支持度调度一次性注册一批相机。
        std::vector<ImageId> registerImageBatch(int remainingCapacity);
         ...
         */
        bool findRegisteredSequenceNeighbor(ImageId imageId, int direction, ImageId* neighborOut, int* stepsOut) const;
         ...
         *
         * 初始试算使用 20 次/净增量 100，正式增量使用 10 次/正净增量，
         * 最终阶段固定执行五轮 10 次 BA 和 3σ 点网刷新。
         */
        void iterativeGlobalBA(SfmBundleAdjustmentStage stage);
