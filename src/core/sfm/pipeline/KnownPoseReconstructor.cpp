...
namespace xjw
{
    ...
    KnownPoseReconstructor::KnownPoseReconstructor(IncrementalSfm& owner) : _owner(owner)
    {
    }
    ...
    IncrementalSfmResult IncrementalSfm::runKnownCameraPoseReconstruction(SfmProgressCallback progressCb)
    {
        ...
        // 影像序号假设相机必然成圆来伪造几何。
        const auto triangulationPolicy =
            resolveKnownPoseTriangulationPolicy(_reconstruction, _correspondenceGraph, imageIds, _sfmOptions);
        if (triangulationPolicy.adapted)
        {
            Logger::instance()->infof("[SFM] Known-pose narrow-baseline adaptation: minTriAngle %.3f -> %.3f deg "
        ...
        ReferenceTrackBuilder trackBuilder;
        for (ImageId imageId : imageIds)
        {
        ...
        for (ImageId imageId : imageIds)
        {
            ...
            for (ImageId otherImageId : connected)
            {
                ...
                for (const FeatureMatch& match : matches)
                {
                    ...
                    if (!knownPoseMatchPassesGeometry(
                            *_reconstruction, imageId, otherImageId, match, triangulationPolicy.triangulatorOptions))
                    {
        ...
        ReferenceTrackBuildOptions trackBuildOptions;
        trackBuildOptions.tiePointLimit = static_cast<std::size_t>(std::max(0, _sfmOptions.maxTracksPerImage));
        ReferenceTrackBuildResult multiViewTracks = trackBuilder.build(trackBuildOptions);

        // 人工先验轨迹拥有独立身份。自动组件中若包含同一人工观测，先删除该自动轨迹，
        // 再追加 materializedPriorTracks，避免一个 2D 特征同时归属两个三维点。
        multiViewTracks.tracks.erase(
            std::remove_if(
                multiViewTracks.tracks.begin(),
                multiViewTracks.tracks.end(),
                [this](const Track& track)
                {
                    return std::any_of(
                        track.elements.begin(),
                        track.elements.end(),
                        [this](const TrackElement& element)
                        { return !_correspondenceGraph.priorTrackId(element.imageId, element.featureIdx).empty(); });
                }),
            multiViewTracks.tracks.end());
        multiViewTracks.tracks.insert(
            multiViewTracks.tracks.end(), _materializedPriorTracks.begin(), _materializedPriorTracks.end());
        for (Track& track : multiViewTracks.tracks)
            tagPriorTrackSource(&track);
        if (!multiViewTracks.tracks.empty())
        {
            ...
            const auto trackStats =
                triangulator.triangulateTracks(multiViewTracks.tracks, triangulationPolicy.triangulatorOptions);
            ...
            Logger::instance()->infof("[SFM] Known-pose reference tracks: pairs=%d matches=%d rawMatches=%d "
                                      "geometryRejected=%d generated=%d selected=%zu duplicateObservations=%d "
                                      "shortTracks=%d spatialPruned=%d hist=%s created=%d addedObservations=%d",
                                      indexedPairCount,
                                      indexedMatchCount,
                                      rawIndexedMatchCount,
                                      geometryRejectedMatchCount,
                                      multiViewTracks.generatedTrackCount,
                                      multiViewTracks.tracks.size(),
                                      multiViewTracks.removedDuplicateObservations,
                                      multiViewTracks.removedShortTracks,
                                      multiViewTracks.prunedBySpatialSelection,
                                      trackLengthHistogram.str().c_str(),
                                      trackStats.numCreated,
                                      trackStats.numContinued);
            Logger::instance()->infof("[SFM] Known-pose triangulation stats: inputTracks=%d inputLong=%d unusable=%d "
                                      "noCandidate=%d createdTwoView=%d createdLong=%d deferredPureTwoView=%d "
                                      "depthRejected=%d reprojRejected=%d",
        ...
        const int filteredPoints =
            triangulator.filterPoints(_sfmOptions.filterMaxReprojError, triangulationPolicy.filterMinTriAngle);
        ...
        if (_reconstruction->numRegisteredImages() >= 2 && _reconstruction->numPoints3D() > 0)
        {
            ...
            baFilteredPoints =
                baTriangulator.filterPoints(_sfmOptions.filterMaxReprojError, triangulationPolicy.filterMinTriAngle);
        ...
        const double finalTwoViewTrackRatio =
            finalTrackCount > 0 ? static_cast<double>(finalTwoViewTrackCount) / static_cast<double>(finalTrackCount)
                                : 0.0;
        ...
        const bool hasOnlyTwoViewOutputFromMultiViewInput = result.numRegisteredImages >= 3 &&
                                                            inputMultiViewTrackCount > 0 && result.numPoints3D > 0 &&
                                                            finalLongTrackCount == 0;
        const bool hasTooLittleMultiViewOutputFromMultiViewInput =
            result.numRegisteredImages >= 3 && hasEnoughMultiViewInputForQualityGate &&
            ((inputMultiViewTrackCount >= kKnownPoseMinPointsForTrackRatioGate &&
        ...
        result.success = result.numRegisteredImages >= 2 && result.numPoints3D > 0 &&
                         !hasOnlyTwoViewOutputFromMultiViewInput && !hasTooLittleMultiViewOutputFromMultiViewInput;
        ...
        if (hasOnlyTwoViewOutputFromMultiViewInput)
        {
            Logger::instance()->warnf(
                ...
                "createdLongTracks=%d finalTwoViewTracks=%d",
        ...
        else if (hasTooLittleMultiViewOutputFromMultiViewInput)
        {
            Logger::instance()->warnf(
                ...
                "createdLongTracks=%d finalLongTracks=%d finalTwoViewTracks=%d "
                                                              ...
                                                              std::vector<FramePinholeCamera>* baCameras)
    {
        ...
        const double directRmse = std::sqrt(directSquaredError / static_cast<double>(currentCenters.size()));
        const double inputExtent = centerExtent(inputCenters);
        const double noOpTolerance = std::max(1e-8, inputExtent * 1e-6);
        if (directRmse <= noOpTolerance)
        {
            Logger::instance()->infof("[SFM] Known-pose Sim3 alignment skipped: camera centers already aligned "
            ...
            for (size_t j = i + 1; j < inputCenters.size(); ++j)
            {
                ...
                for (size_t k = j + 1; k < inputCenters.size(); ++k)
                {
                    ...
                    maxTriangleArea2 = std::max(
                        maxTriangleArea2, std::sqrt(cross[0] * cross[0] + cross[1] * cross[1] + cross[2] * cross[2]));
                }
            }
        }
        const double layoutAreaTolerance = std::max(1e-12, inputExtent * inputExtent * 1e-6);
        if (maxTriangleArea2 <= layoutAreaTolerance)
        {
            Logger::instance()->warnf("[SFM] Known-pose Sim3 alignment skipped: camera-center layout is collinear "
        ...
        {
            Logger::instance()->warnf(
                "[SFM] Known-pose Sim3 alignment skipped: insufficient robust camera-center inliers");
            return;
        }
        if (!(transform.rmse + noOpTolerance < directRmse))
        {
            Logger::instance()->infof("[SFM] Known-pose Sim3 alignment skipped: no meaningful center improvement "
        ...
        for (Point3DId pointId : _reconstruction->allPoint3DIds())
        {
            ...
            _reconstruction->point3D(pointId).xyz = transformPoint(transform, _reconstruction->point3D(pointId).xyz);
        }
    ...
    {
        if (!_sfmOptions.useKnownCameraPoses || !_sfmOptions.refineKnownCameraPoseWithSoftPrior || !_reconstruction)
        {
            return;
        }
        ...
        for (Point3DId pointId : _reconstruction->allPoint3DIds())
        {
            ...
            for (std::size_t elementIndex = 0; elementIndex < point.track.elements.size(); ++elementIndex)
            {
                ...
                const bool alreadyIndexed = std::any_of(
                    ...
                    { return previous.imageId == element.imageId && previous.featureIdx < image.keypoints.size(); });
