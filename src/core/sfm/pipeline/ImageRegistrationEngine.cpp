...
namespace xjw
{
    ...
    ImageRegistrationEngine::ImageRegistrationEngine(IncrementalSfm& owner) : _owner(owner)
    {
    }

    std::vector<ImageId> ImageRegistrationEngine::findParallelAerialPoseOutliers(
        const SfmReconstruction& reconstruction, double minimumAxisConcentration, double minimumAngularDeviationDegrees)
    {
        ...
        const double mean_norm =
            std::sqrt(mean_axis[0] * mean_axis[0] + mean_axis[1] * mean_axis[1] + mean_axis[2] * mean_axis[2]);
        const double concentration = mean_norm / static_cast<double>(axes.size());
        if (!std::isfinite(concentration) || concentration < std::clamp(minimumAxisConcentration, 0.0, 1.0) ||
            !(mean_norm > 1.0e-12))
        {
            return {};
        }
        ...
        {
            const double dot =
                std::clamp(axis[0] * mean_axis[0] + axis[1] * mean_axis[1] + axis[2] * mean_axis[2], -1.0, 1.0);
        ...
        const double threshold =
            std::max(std::max(1.0, minimumAngularDeviationDegrees), median_deviation + 6.0 * std::max(0.5, mad));
        ...
        const double branch_expansion_threshold = std::max(12.0, median_deviation + 3.0 * std::max(0.5, mad));
        ...
        for (const ImageId seed_id : strong_outliers)
        {
            ...
            while (candidate_id > 0)
            {
                ...
                if (deviation_it == deviation_by_id.end() || deviation_it->second <= branch_expansion_threshold)
                {
                    break;
                }
    ...
    IncrementalSfmResult ImageRegistrationEngine::run(int totalImages,
                                                      ...
                                                      bool initialPairTrial,
                                                      bool stabilizeModel,
                                                      bool runInitialGrowthRefinement)
    {
        return _owner.runRegistrationFromCurrentInitialization(totalImages,
                                                               std::move(progressCb),
                                                               registrationLimit,
                                                               runFinalRefinement,
                                                               initialPairTrial,
                                                               stabilizeModel,
                                                               runInitialGrowthRefinement);
    }

    IncrementalSfmResult IncrementalSfm::runRegistrationFromCurrentInitialization(int totalImages,
                                                                                  SfmProgressCallback progressCb,
                                                                                  int registrationLimit,
                                                                                  bool runFinalRefinement,
                                                                                  bool initialPairTrial,
                                                                                  ...
                                                                                  bool runInitialGrowthRefinement)
    {
        ...
        const int configuredRegistrationLimit =
            registrationLimit > 0 ? registrationLimit : _sfmOptions.maxRegisteredImages;
        const int registrationTarget =
            configuredRegistrationLimit > 0 ? std::clamp(configuredRegistrationLimit, 2, totalImages) : totalImages;
        int iterSinceLastLocalBA = 0;
        int iterSinceLastGlobalBA = 0;
        const bool useReferenceBaSchedule =
            stabilizeModel && _sfmOptions.executionProfile == SfmExecutionProfile::FullRefinement;
        const SfmBundleAdjustmentStage growthStage =
            initialPairTrial ? SfmBundleAdjustmentStage::InitialPairTrial : SfmBundleAdjustmentStage::Incremental;

        // 参考状态机在初始二视图形成后先稳定当前模型，再进行第一批后方交会。
        // 初始像对 evaluator 使用 20 次/净增量 100；正式增长使用 10 次/正净增量。
        if (useReferenceBaSchedule && runInitialGrowthRefinement)
        {
            SfmBundleAdjustCoordinator(*this).iterative(growthStage);
            invalidateVisibilityCache();
        }

        while (regCount < registrationTarget && !_isAborted)
        {
            std::vector<ImageId> registeredBatch;
            if (_sfmOptions.useReferenceBatchResection)
            {
                registeredBatch = registerImageBatch(registrationTarget - regCount);
            }
            else if (stabilizeModel)
            {
                const ImageId nextId = selectNextImage();
                if (nextId != kInvalidImageId && registerImage(nextId))
                {
                    _registerFailCount.erase(nextId);
                    _deferredFailedImages.clear();
                    registeredBatch.push_back(nextId);
                }
                else if (nextId != kInvalidImageId)
                {
            ...
            if (registeredBatch.empty())
            {
                ...
                Logger::instance()->warnf("[SFM] Registration stopped: no acceptable resection candidate "
                                          "(registered=%d/%d, points=%zu, permanentlyFailed=%zu)",
                                          regCount,
                                          totalImages,
            ...
            if (useReferenceBaSchedule)
            {
                ...
                // 如果只三角化新影像，首轮 BA 后的全轨刷新必然继续加点，造成每批固定多跑一轮 BA。
                TriangulatorOptions refreshOptions = _sfmOptions.triangulatorOptions;
                refreshOptions.bindCompleteInputTrack = true;
                refreshOptions.deferPureTwoViewTracks = false;
            ...
            {
                for (ImageId imageId : registeredBatch)
                {
            ...
            iterSinceLastLocalBA += static_cast<int>(registeredBatch.size());
            iterSinceLastGlobalBA += static_cast<int>(registeredBatch.size());

            if (useReferenceBaSchedule)
            {
                // 每批相机接纳后立即稳定当前完整块；下一批 PnP 必须消费刷新后的点网。
                SfmBundleAdjustCoordinator(*this).iterative(growthStage);
                invalidateVisibilityCache();
            }
            else
            {
                // 粗评估保留节流后的局部/周期 BA，避免每个焦距候选执行完整参考调度。
                if (iterSinceLastLocalBA >= _sfmOptions.localBAInterval)
                {
                    SfmBundleAdjustCoordinator(*this).run(true, registeredBatch);
                ...
                if (iterSinceLastGlobalBA >= _sfmOptions.globalBAInterval)
                {
                    if (SfmBundleAdjustCoordinator::shouldRunPeriodicGlobalBa(
                            regCount, registrationTarget, iterSinceLastGlobalBA, _sfmOptions.globalBAInterval))
                    {
                    ...
                    {
                        Logger::instance()->infof("[SFM] Deferred periodic global BA to final refinement: "
        ...
        if (stabilizeModel && runFinalRefinement && !_isAborted && _reconstruction->numRegisteredImages() >= 2)
        {
            ...
            if (_sfmOptions.filterMinTrackLen > 1)
            {
                ...
                Logger::instance()->infof(
                    "[SFM] Filtered %d short-track points (minTrackLen=%d)", nShort, _sfmOptions.filterMinTrackLen);
            }
        }
        ...
        result.baRefinedCalibrationGroupCount = _lastGlobalBARefinedCalibrationGroupCount;
        result.baSelfCalibrationStagesRun = _lastGlobalBASelfCalibrationStagesRun;
        ...
        result.baAdaptiveCameraModelFittingEvaluated = _lastGlobalBAAdaptiveCameraModelFittingEvaluated;
        result.baAdaptiveCameraModelFittingApplied = _lastGlobalBAAdaptiveCameraModelFittingApplied;
        result.baIntrinsicParameterMask = _lastGlobalBAIntrinsicParameterMask;
        result.baIntrinsicParameterReliability = _lastGlobalBAIntrinsicParameterReliability;
        result.baIntrinsicParameterIncrementalInformationScore =
            _lastGlobalBAIntrinsicParameterIncrementalInformationScore;
        result.baIntrinsicParameterSensitivity = _lastGlobalBAIntrinsicParameterSensitivity;
        ...
        result.baCameraModelGeometryStrength = _lastGlobalBACameraModelGeometryStrength;
        result.baCameraModelOpticalAxisConcentration = _lastGlobalBACameraModelOpticalAxisConcentration;
        result.baCameraModelMedianTriangulationAngle = _lastGlobalBACameraModelMedianTriangulationAngle;
        result.baCameraModelNormalizedRadiusP90 = _lastGlobalBACameraModelNormalizedRadiusP90;
        result.baCameraModelOccupiedPeripheralSectors = _lastGlobalBACameraModelOccupiedPeripheralSectors;
        result.baCameraModelObservationCount = _lastGlobalBACameraModelObservationCount;
        result.baCameraModelMultiViewTrackRatio = _lastGlobalBACameraModelMultiViewTrackRatio;
        result.baCameraModelObservationSupport = _lastGlobalBACameraModelObservationSupport;
        result.baCameraModelPeripheralCoverage = _lastGlobalBACameraModelPeripheralCoverage;
        result.baCameraModelSectorCoverage = _lastGlobalBACameraModelSectorCoverage;
        result.baCameraModelImageAxisBalance = _lastGlobalBACameraModelImageAxisBalance;
    ...
    {
        if (!_sfmOptions.retryUnregisteredAfterFinalBA || !_reconstruction || _isAborted ||
        ...
        for (int pass = 0; pass < maxPasses && !_isAborted; ++pass)
        {
            ...
            std::stable_sort(candidates.begin(),
                             candidates.end(),
                             [&](ImageId lhs, ImageId rhs) { return _visibilityCache[lhs] > _visibilityCache[rhs]; });
            ...
            for (ImageId imageId : candidates)
            {
                ...
                {
                    Logger::instance()->warnf("[SFM] Final-BA retry rejected image %u: %s",
                                              imageId,
                ...
                ++registeredThisPass;
                ++registeredByRetry;
                Logger::instance()->infof("[SFM] Final-BA retry registered image %u (%zu/%d)",
                                          imageId,
    ...
    int IncrementalSfm::repairParallelAerialPoseOutliersAfterFinalBA()
    {
        ...
        if (!_sfmOptions.repairParallelAerialPoseOutliers || !_sfmOptions.useSequencePoseRecovery ||
            _sfmOptions.sequenceLoopClosure || !_reconstruction || _isAborted || _controlNetworkApplied ||
            _reconstruction->numRegisteredImages() < 32)
        {
            return 0;
        }

        const std::vector<ImageId> outliers = ImageRegistrationEngine::findParallelAerialPoseOutliers(*_reconstruction);
        const std::size_t maximumOutliers = std::max<std::size_t>(
            2, static_cast<std::size_t>(std::ceil(static_cast<double>(_reconstruction->numRegisteredImages()) * 0.05)));
        if (outliers.empty())
        {
            return 0;
        }
        ...
        {
            Logger::instance()->warnf("[SFM] Parallel-aerial pose audit found %zu/%zu outliers; "
        ...
        Logger::instance()->warnf(
            "[SFM] Parallel-aerial pose audit isolated %zu camera(s): [%s]", outliers.size(), imageList.str().c_str());
        ...
        for (std::size_t pass = 0; pass < outliers.size() && !pending.empty() && !_isAborted; ++pass)
        {
            ...
            auto directNeighborCount = [&](ImageId imageId)
            {
                ...
                if (imageId + 1 < _reconstruction->numImages() && _reconstruction->isRegistered(imageId + 1))
                {
                    ++count;
                }
                return count;
            ...
            while (!_isAborted)
            {
                ...
                {
                    if (attemptedThisPass.count(imageId) == 0 && directNeighborCount(imageId) > 0)
                    {
                        candidates.push_back(imageId);
                    }
                }
                ...
                ++registeredThisPass;
                Logger::instance()->infof(
                    "[SFM] Parallel-aerial pose repair registered image %u (%zu remaining)", imageId, pending.size());
            }
        ...
        _lastAerialPoseOutliersRepaired = _lastAerialPoseOutliersDetected - static_cast<int>(pending.size());
        Logger::instance()->infof("[SFM] Parallel-aerial pose repair completed: detected=%d repaired=%d rejected=%zu",
    ...
    ImageId IncrementalSfm::selectNextImage() const
    {
        ...
        for (const auto& [id, count] : _visibilityCache)
        {
            ...
            if (_permanentlyFailedImages.count(id))
                continue;
            if (_sfmOptions.enforceSequencePoseConsistency && _reconstruction->numRegisteredImages() >= 3 &&
    ...
    std::vector<ImageId> IncrementalSfm::registerImageBatch(int remainingCapacity)
    {
        if (_visibilityCacheDirty)
        {
            rebuildVisibilityCache();
        ...
        std::vector<std::pair<ImageId, std::size_t>> candidates;
        for (const auto& [imageId, visibleCount] : _visibilityCache)
        {
            if (_reconstruction->isRegistered(imageId) || _deferredFailedImages.contains(imageId) ||
                _permanentlyFailedImages.contains(imageId) || visibleCount < 4)
            {
                continue;
            }
            if (_sfmOptions.enforceSequencePoseConsistency && _reconstruction->numRegisteredImages() >= 3 &&
                !hasRegisteredSequenceNeighbor(imageId))
            {
                continue;
            }
            candidates.emplace_back(imageId, visibleCount);
        }
        ...
        common::concurrency::parallelForIndices(candidates.size(),
                                                threadCount,
                                                [&](std::size_t candidateIndex) {
                                                    evaluations[candidateIndex] = evaluateImageRegistration(
                                                        candidates[candidateIndex].first, false, false);
                                                });

        std::vector<SuccessfulCandidate> successful;
        constexpr int maxRetries = 5;
        for (std::size_t candidateIndex = 0; candidateIndex < candidates.size(); ++candidateIndex)
        {
            const auto& [imageId, visibleCount] = candidates[candidateIndex];
            static_cast<void>(visibleCount);
            ...
            {
                Logger::instance()->warnf("[SFM] Image %u reference resection rejected: %s",
                                          imageId,
                                          evaluation.error.empty() ? "unknown error" : evaluation.error.c_str());
                continue;
            }

            int& failCount = _registerFailCount[imageId];
            ++failCount;
            Logger::instance()->warnf("[SFM] Image %u batch resection failed (%d/%d): %s",
                                      imageId,
                                      failCount,
                                      maxRetries,
                                      evaluation.error.empty() ? "unknown error" : evaluation.error.c_str());
            ...
            {
                _permanentlyFailedImages.insert(imageId);
                _deferredFailedImages.erase(imageId);
            }
            else
            {
                _deferredFailedImages.insert(imageId);
            }
        }
        ...
        const std::size_t cutoff =
            referenceResectionSupportCutoff(static_cast<std::size_t>(std::max(0, successful.front().support)));
        const std::size_t capacity =
            remainingCapacity > 0 ? static_cast<std::size_t>(remainingCapacity) : successful.size();
        std::vector<ImageId> accepted;
        accepted.reserve(std::min(capacity, successful.size()));
        for (const SuccessfulCandidate& candidate : successful)
        {
            if (accepted.size() >= capacity || static_cast<std::size_t>(std::max(0, candidate.support)) < cutoff)
            {
                break;
            }
            _reconstruction->registerImage(candidate.imageId, candidate.camera);
            _registerFailCount.erase(candidate.imageId);
            accepted.push_back(candidate.imageId);
        }
        if (!accepted.empty())
        {
            _deferredFailedImages.clear();
        ...
        Logger::instance()->infof(
            "[SFM] Reference batch resection: evaluated=%zu solved=%zu accepted=%zu strongest=%d cutoff=%zu "
            "threads=%zu seconds=%.3f",
            candidates.size(),
            successful.size(),
            accepted.size(),
            successful.front().support,
            cutoff,
            std::min(threadCount, std::max<std::size_t>(1, candidates.size())),
            std::chrono::duration<double>(std::chrono::steady_clock::now() - evaluationStarted).count());
        return accepted;
    }
                                       ...
                                       int* supportingInliers)
    {
        ...
        ImageRegistrationEvaluation evaluation =
            evaluateImageRegistration(imageId, preferSequencePrior, forceSequenceConsistency);
        if (!evaluation.success)
        {
            _lastErrorMessage = evaluation.error;
            if (evaluation.sequenceConsistencyRejected)
            {
                Logger::instance()->warnf(
                    "[SFM] Image %u sequence distance rejected: %s", imageId, evaluation.error.c_str());
            }
            return false;
        }
        ...
        Logger::instance()->infof("[SFM] Image %u PnP success: observations=%zu, inliers=%d, inlierRatio=%.3f, "
                                  ...
                                  "prefilterCandidates=%d, rawProposals=%zu, strictSmallSupport=%s, gridCells=%d",
                                  imageId,
                                  evaluation.observations,
                                  evaluation.supportingInliers,
                                  evaluation.inlierRatio,
                                  evaluation.minimumTrackLength,
                                  evaluation.usedSequenceRecovery ? "true" : "false",
                                  evaluation.usedPosePrefilter ? "true" : "false",
                                  evaluation.prefilterCandidateCount,
    ...
    IncrementalSfm::ImageRegistrationEvaluation IncrementalSfm::evaluateImageRegistration(
        ImageId imageId, bool preferSequencePrior, bool forceSequenceConsistency) const
    {
        ImageRegistrationEvaluation evaluation;
        // 加载相机内参
        ...
        const bool reference_resection = _sfmOptions.executionProfile == SfmExecutionProfile::FullRefinement &&
                                         _sfmOptions.useReferenceBatchResection;
        const int minTrackLengthForPnp =
            reference_resection ? 2 : effectivePnpMinTrackLength(_sfmOptions, _reconstruction->numRegisteredImages());
        std::vector<PnpCorrespondenceProposal> proposals;

        const auto inputTrackIndices = _inputTrackIndicesByImage.find(imageId);
        if (reference_resection && inputTrackIndices != _inputTrackIndicesByImage.end())
        {
            // “对齐照片”以后方交会影像在完整 selected track 中的观测直接查找已绑定三维点，
            // 不要求候选影像与持点影像之间还保留一条 direct pairwise edge。
            for (const std::size_t trackIndex : inputTrackIndices->second)
            {
                if (trackIndex >= _inputMultiViewTracks.size())
                {
                    continue;
                }
                const Track& inputTrack = _inputMultiViewTracks[trackIndex];
                FeatureIdx candidateFeature = kInvalidFeatureIdx;
                for (const TrackElement& element : inputTrack.elements)
                {
                    if (element.imageId == imageId)
                    {
                        candidateFeature = element.featureIdx;
                        break;
                    }
                }
                if (candidateFeature == kInvalidFeatureIdx || candidateFeature >= img.keypoints.size())
                {
                    continue;
                }

                // 参考点网为完整 track 的每个 observation 预绑定 point id，
                // 后方交会可直接读取 track.point，无需再扫描其它影像投票。
                if (candidateFeature < img.point3DIds.size())
                {
                    const Point3DId directPointId = img.point3DIds[candidateFeature];
                    if (pointUsableForPnp(*_reconstruction, directPointId, minTrackLengthForPnp))
                    {
                        int supportingViews = 0;
                        for (const TrackElement& element : inputTrack.elements)
                        {
                            if (element.imageId != imageId && _reconstruction->isRegistered(element.imageId))
                            {
                                ++supportingViews;
                            }
                        }
                        const ScenePoint3D& point = _reconstruction->point3D(directPointId);
                        proposals.push_back(
                            {candidateFeature,
                             directPointId,
                             std::max(1, supportingViews),
                             point.track.length(),
                             std::isfinite(inputTrack.confidence) ? inputTrack.confidence : 0.0,
                             std::isfinite(point.error) ? point.error : std::numeric_limits<double>::infinity()});
                        continue;
                    }
                }

                // 兼容旧工程中尚未转换为完整轨迹绑定的 partial point。
                std::vector<std::pair<Point3DId, int>> pointVotes;
                pointVotes.reserve(inputTrack.elements.size());
                for (const TrackElement& element : inputTrack.elements)
                {
                    if (element.imageId == imageId)
                    {
                    ...
                    const ImageData& registeredImage = _reconstruction->image(element.imageId);
                    if (element.featureIdx >= registeredImage.point3DIds.size())
                    {
                        continue;
                    }
                    const Point3DId pointId = registeredImage.point3DIds[element.featureIdx];
                    if (!pointUsableForPnp(*_reconstruction, pointId, minTrackLengthForPnp))
                    {
                        continue;
                    }
                    const auto vote = std::find_if(pointVotes.begin(),
                                                   pointVotes.end(),
                                                   [pointId](const auto& entry) { return entry.first == pointId; });
                    if (vote == pointVotes.end())
                    {
                        pointVotes.emplace_back(pointId, 1);
                    }
                    else
                    {
                        ++vote->second;
                ...
                const auto bestVote =
                    std::max_element(pointVotes.begin(),
                                     pointVotes.end(),
                                     [&](const auto& left, const auto& right)
                                     {
                                         ...
                                         const ScenePoint3D& leftPoint = _reconstruction->point3D(left.first);
                                         const ScenePoint3D& rightPoint = _reconstruction->point3D(right.first);
                                         if (leftPoint.track.length() != rightPoint.track.length())
                                         {
                                             return leftPoint.track.length() < rightPoint.track.length();
                                         }
                                         const double leftError = std::isfinite(leftPoint.error)
                                                                      ? leftPoint.error
                                                                      : std::numeric_limits<double>::infinity();
                                         const double rightError = std::isfinite(rightPoint.error)
                                                                       ? rightPoint.error
                                                                       : std::numeric_limits<double>::infinity();
                ...
                const ScenePoint3D& point = _reconstruction->point3D(bestVote->first);
                proposals.push_back(
                    {candidateFeature,
                     bestVote->first,
                     bestVote->second,
        ...
        const std::vector<PnpCorrespondenceProposal> selected_proposals = selectUniquePnpCorrespondences(proposals);
        ...
        const int configured_min_inliers = reference_resection ? 4 : _sfmOptions.pnpOptions.minNumInliers;
        const int strict_min_inliers =
            std::clamp(_sfmOptions.pnpOptions.strictSmallSupportMinInliers, 4, std::max(4, configured_min_inliers));
        const bool strict_small_support = _sfmOptions.pnpOptions.allowStrictSmallSupportRecovery &&
                                          static_cast<int>(worldPts.size()) < configured_min_inliers &&
                                          static_cast<int>(worldPts.size()) >= strict_min_inliers;
        if (static_cast<int>(worldPts.size()) < configured_min_inliers && !strict_small_support)
        {
            std::ostringstream oss;
            oss << "not enough 2D-3D observations for PnP: observations=" << worldPts.size()
                << ", minPnPInliers=" << configured_min_inliers << ", strictRecoveryMin=" << strict_min_inliers
                << ", minTrackLength=" << minTrackLengthForPnp << ", connectedNeighbors=" << neighbors.size()
                << ", rawProposals=" << proposals.size();
            evaluation.error = oss.str();
            return evaluation;
        }
        ...
        {
            pnpOptions.minNumInliers = 4;
            pnpOptions.minInlierRatio = 0.0;
            pnpOptions.smallSampleThreshold = 0;
            pnpOptions.allowRelaxedInlierRatio = false;
            pnpOptions.allowStrictSmallSupportRecovery = false;
        ...
        if (strict_small_support)
        {
            ...
            pnpOptions.minInlierRatio =
                std::max(pnpOptions.minInlierRatio, std::clamp(pnpOptions.strictSmallSupportMinInlierRatio, 0.0, 1.0));
            pnpOptions.smallSampleMinInlierRatio =
                std::max(pnpOptions.smallSampleMinInlierRatio, pnpOptions.minInlierRatio);
            pnpOptions.allowRelaxedInlierRatio = false;
        }
        pnpOptions.ransacSeed =
            opencv_utils::stableRansacSeed(imageId,
        ...
        auto solveSequenceRecovery = [&]()
        {
            ...
            int previousSteps = 0;
            int nextSteps = 0;
            const bool hasPrevious = findRegisteredSequenceNeighbor(imageId, -1, &previousImageId, &previousSteps);
            const bool hasNext = findRegisteredSequenceNeighbor(imageId, 1, &nextImageId, &nextSteps);
            ...
            const bool useOneSidedRecovery =
                _sfmOptions.allowOneSidedSequencePoseRecovery && (hasDirectPrevious != hasDirectNext);
            const int sequenceMinInliers = isDirectlyBracketed ? _sfmOptions.bracketedSequencePnpMinInliers
                                                               : _sfmOptions.oneSidedSequencePnpMinInliers;
            const double sequenceMinInlierRatio = isDirectlyBracketed ? _sfmOptions.bracketedSequencePnpMinInlierRatio
            ...
            if (isDirectlyBracketed || useOneSidedRecovery)
            {
                ...
                recoveryOptions.initialPosePrefilterMaxReprojError =
                    std::max(recoveryOptions.initialPosePrefilterMaxReprojError,
                             isDirectlyBracketed ? recoveryOptions.maxReprojError * 12.0
                                                 : _sfmOptions.oneSidedSequencePosePrefilterMaxReprojError);
                recoveryOptions.initialPosePrefilterMinCandidates =
                    std::max(recoveryOptions.minNumInliers, sequenceMinInliers);
            }
        ...
        if (pnpResult.success && strict_small_support)
        {
            ...
            strict_spatial_support_accepted =
                ...
                spatial_support.occupiedRows >= 2 && spatial_support.occupiedColumns >= 2;
        }
        if (!pnpResult.success || !strict_spatial_support_accepted)
        {
            std::ostringstream oss;
            oss << "PnP failed: observations=" << worldPts.size() << ", inliers=" << pnpResult.numInliers
                ...
                << ", gridCells=" << spatial_support.occupiedCells << ", gridRows=" << spatial_support.occupiedRows
        ...
        {
            evaluation.sequenceConsistencyRejected = true;
            evaluation.error = "sequence distance check failed: " + sequenceConsistencyReason;
            return evaluation;
        }

        evaluation.success = true;
        evaluation.camera = std::move(cam);
        evaluation.supportingInliers = pnpResult.numInliers;
        evaluation.observations = worldPts.size();
        evaluation.rawProposals = proposals.size();
        evaluation.minimumTrackLength = minTrackLengthForPnp;
        evaluation.inlierRatio = pnpResult.inlierRatio;
        evaluation.usedSequenceRecovery = usedSequenceRecovery;
        evaluation.usedPosePrefilter = pnpResult.usedInitialPosePrefilter;
        evaluation.prefilterCandidateCount = pnpResult.prefilterCandidateCount;
        evaluation.strictSmallSupport = strict_small_support;
        evaluation.occupiedGridCells = spatial_support.occupiedCells;
        return evaluation;
    }
                                                        ...
                                                        int* stepsOut) const
    {
        ...
        *stepsOut = 0;
        ...
        auto hasRegisteredCamera = [&](ImageId candidate) {
            return candidate < imageCount && _reconstruction->isRegistered(candidate) &&
    ...
    double IncrementalSfm::registeredSequenceAdjacentDistanceMedian(ImageId excludedImageId) const
    {
        ...
        {
            if (a == excludedImageId || b == excludedImageId || !_reconstruction->isRegistered(a) ||
                !_reconstruction->isRegistered(b) || !_reconstruction->hasCamera(a) || !_reconstruction->hasCamera(b))
            {
                return;
            }
                                                         ...
                                                         bool force) const
    {
        ...
        if (force)
        {
            ...
            const double main_axis_norm =
                std::sqrt(main_axis[0] * main_axis[0] + main_axis[1] * main_axis[1] + main_axis[2] * main_axis[2]);
            if (main_axis_samples >= 12 && main_axis_norm > 1.0e-12 && std::isfinite(main_axis_norm))
            {
                for (double& component : main_axis)
                {
                    component /= main_axis_norm;
                }
                const double dot = std::clamp(candidateAxis[0] * main_axis[0] + candidateAxis[1] * main_axis[1] +
                ...
                if (!std::isfinite(angle) || angle > maximumNeighborAxisAngleDegrees)
                {
                    ...
                    {
                        std::ostringstream oss;
                        oss << "image=" << imageId << ", mainOpticalAxisAngle=" << angle
            ...
            for (const ImageId neighborId : registeredNeighbors)
            {
                ...
                if (!std::isfinite(angle) || angle > maximumNeighborAxisAngleDegrees)
                {
                    ...
                    {
                        std::ostringstream oss;
                        oss << "image=" << imageId << ", neighbor=" << neighborId << ", opticalAxisAngle=" << angle
                            << ", maximum=" << maximumNeighborAxisAngleDegrees;
                        *reason = oss.str();
                    }
                    return false;
        ...
        for (ImageId neighborId : registeredNeighbors)
        {
            ...
            if (!std::isfinite(distance) || distance < minDistance || distance > maxDistance)
            {
                ...
                {
                    std::ostringstream oss;
                    oss << "image=" << imageId << ", neighbor=" << neighborId << ", distance=" << distance
                        << ", median=" << medianDistance << ", allowed=[" << minDistance << "," << maxDistance << "]";
                    *reason = oss.str();
                }
                return false;
            }
    ...
    void IncrementalSfm::rebuildVisibilityCache()
    {
        ...
        common::concurrency::parallelForIndices(
            ...
            [&](std::size_t pendingIndex)
            {
                ...
                size_t numVisible = 0;
                if (_sfmOptions.executionProfile == SfmExecutionProfile::FullRefinement &&
                    _sfmOptions.useReferenceBatchResection && _reconstruction->hasImage(id))
                {
                    const ImageData& image = _reconstruction->image(id);
                    for (const Point3DId pointId : image.point3DIds)
                    {
                        if (pointUsableForPnp(*_reconstruction, pointId, minTrackLengthForPnp))
                        {
                            ++numVisible;
        ...
        for (std::size_t pendingIndex = 0; pendingIndex < pendingIds.size(); ++pendingIndex)
        {
            _visibilityCache[pendingIds[pendingIndex]] = visibleCounts[pendingIndex];
        }
        _visibilityCacheDirty = false;
        if (pendingIds.size() >= 32)
        {
            const double elapsedSeconds =
                std::chrono::duration<double>(std::chrono::steady_clock::now() - visibilityStarted).count();
            Logger::instance()->infof("[SFM] visibility_cache rebuilt pending=%zu threads=%d seconds=%.3f",
                                      pendingIds.size(),
                                      threadCount,
                                      elapsedSeconds);
        }
        ...
        for (size_t featureIndex = 0; featureIndex < numFeatures; ++featureIndex)
        {
            ...
            for (const auto& trackElem : point.track.elements)
            {
                auto correspondences =
                    _correspondenceGraph.findCorrespondences(trackElem.imageId, trackElem.featureIdx);
