...
namespace xjw
{
    ...
    namespace
    {
        ...
        GlobalReprojectionState evaluateGlobalReprojection(const SfmReconstruction& reconstruction)
        {
            ...
            {
                state.rms =
                    std::sqrt(static_cast<double>(squared_error_sum / static_cast<long double>(state.observations)));
            }
            return state;
        }
    ...
    HierarchicalBundleAdjuster::HierarchicalBundleAdjuster(IncrementalSfm& owner) : _owner(owner)
    {
    }
    ...
    {
        return enabled && refineCameraPose && minimumImageCount >= 2 && registeredImageCount >= minimumImageCount;
    }
                                                       ...
                                                       bool concurrentBackendAvailable)
    {
        ...
        const int configured_limit = configuredMaximum > 0 ? configuredMaximum : total_threads;
        return std::max(1, std::min({blockCount, total_threads, configured_limit}));
    }

    int
    HierarchicalBundleAdjuster::resolveWorkerThreadCount(int totalThreadCount, int activeWorkerCount, int workerIndex)
    {
        const int total_threads = std::max(1, totalThreadCount);
        const int active_workers = std::clamp(activeWorkerCount, 1, total_threads);
        const int normalized_index = std::clamp(workerIndex, 0, active_workers - 1);
    ...
    bool HierarchicalBundleAdjuster::shouldWriteBackPoint(std::size_t blockObservationCount,
                                                          std::size_t totalRegisteredObservationCount)
    {
        return totalRegisteredObservationCount >= 2 && blockObservationCount == totalRegisteredObservationCount;
    }

    bool HierarchicalBundleAdjuster::isGlobalWriteBackConsistent(double rmsBefore,
                                                                 std::size_t observationsBefore,
                                                                 double rmsAfter,
                                                                 std::size_t observationsAfter)
    {
        if (!std::isfinite(rmsBefore) || !std::isfinite(rmsAfter) || observationsBefore == 0 || observationsAfter == 0)
        {
            return false;
        }
        const double coverage_ratio = static_cast<double>(observationsAfter) / static_cast<double>(observationsBefore);
    ...
    HierarchicalBaRunSummary HierarchicalBundleAdjuster::run()
    {
        ...
        {
            Logger::instance()->info("[BA] hierarchical skipped reason=absolute_or_manual_constraints_present");
            return summary;
        }

        const int repeat_threshold = std::max(2, _owner._sfmOptions.hierarchicalBATargetBlockSize / 2);
        const int previous_attempt_count =
            std::max(_owner._lastHierarchicalBAImageCount, _owner._lastHierarchicalBAAttemptImageCount);
        if (previous_attempt_count > 0 && registered_count - previous_attempt_count < repeat_threshold)
        {
        ...
        partition_options.targetCoreSize =
            static_cast<std::size_t>(std::max(2, _owner._sfmOptions.hierarchicalBATargetBlockSize));
        partition_options.overlapSize =
            static_cast<std::size_t>(std::max(2, _owner._sfmOptions.hierarchicalBAOverlapImages));
        const std::vector<CovisibilityBlock> blocks = CovisibilityPartitioner::partition(
            _owner._reconstruction->registeredImageIds(), _owner._correspondenceGraph, partition_options);
        if (blocks.size() < 2)
        {
            return summary;
        }
        ...
        std::vector<std::vector<Point3DId>> candidate_points_by_block(blocks.size());
        std::vector<int> observations_per_block(blocks.size(), 0);
        std::vector<std::size_t> touched_blocks;
        touched_blocks.reserve(blocks.size());
        for (Point3DId point_id : _owner._reconstruction->allPoint3DIds())
        {
            ...
            for (const TrackElement& element : point.track.elements)
            {
                ...
                {
                    if (observations_per_block[owner->second]++ == 0)
                    {
                        touched_blocks.push_back(owner->second);
                    }
                }
            }
            std::size_t best_block = 0;
            int best_count = 0;
            for (const std::size_t block_index : touched_blocks)
            {
                const int count = observations_per_block[block_index];
            ...
            for (const std::size_t block_index : touched_blocks)
            {
                candidate_points_by_block[block_index].push_back(point_id);
                observations_per_block[block_index] = 0;
            }
        }
        ...
        for (std::size_t first = 0; first < blocks.size(); first += worker_count)
        {
            ...
            for (std::size_t block_index = first; block_index < end; ++block_index)
            {
                ...
                const int block_thread_count =
                    resolveWorkerThreadCount(total_threads, active_worker_count, worker_index);
                                             ...
                                             [&, block_index, block_thread_count]()
                                             {
                                                 return solveBlock(block_index,
                                                                   ...
                                                                   *_owner._reconstruction,
                                                                   candidate_points_by_block[block_index],
                                                                   block_options,
                                                                   block_thread_count);
                                             }));
            }
            for (std::future<BlockOutcome>& future : futures)
            {
                ...
                {
                    Logger::instance()->warnf("[BA] hierarchical block failed exception=%s", error.what());
                }
            }
        ...
        for (const BlockOutcome& outcome : outcomes)
        {
            ...
            const std::unordered_set<ImageId> active_image_ids(outcome.cameraIds.begin(), outcome.cameraIds.end());
            for (std::size_t index = 0; index < outcome.pointIds.size() && index < outcome.result.points.size();
                 ++index)
            {
        ...
        const GlobalReprojectionState global_after = evaluateGlobalReprojection(*_owner._reconstruction);
        const bool globally_consistent =
            candidate_applied_blocks > 0 &&
            isGlobalWriteBackConsistent(
                global_before.rms, global_before.observations, global_after.rms, global_after.observations);
        Logger::instance()->infof("[BA] hierarchical global_consistency accepted=%s rms=%.6f->%.6f "
        ...
        else
        {
            ...
            {
                Logger::instance()->warn("[BA] hierarchical merge rejected reason=global_reprojection_inconsistent; "
                                         "restored pre-block cameras and points");
            }
        }

        summary.totalSeconds = std::chrono::duration<double>(std::chrono::steady_clock::now() - started).count();
        if (summary.applied())
        {
        ...
        Logger::instance()->infof("[BA] hierarchical result appliedBlocks=%d/%d cameras=%d points=%d totalSeconds=%.3f",
