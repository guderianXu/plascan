...
namespace xjw
{
    ...
    void IncrementalSfm::setInputMultiViewTracks(const std::vector<Track>& tracks)
    {
        _providedInputMultiViewTracks = tracks;
    }

    void IncrementalSfm::rebuildInputTrackObservationIndex()
    {
        _inputTrackIndicesByImage.clear();
        _inputTrackIndicesByImage.reserve(_reconstruction ? _reconstruction->numImages() : 0);
        for (std::size_t trackIndex = 0; trackIndex < _inputMultiViewTracks.size(); ++trackIndex)
        {
            for (const TrackElement& element : _inputMultiViewTracks[trackIndex].elements)
            {
                _inputTrackIndicesByImage[element.imageId].push_back(trackIndex);
            }
        }
    }
    ...
    void IncrementalSfm::materializePriorTracks()
    {
        ...
        for (const control_points::PriorTrack& priorTrack : _pendingPriorTracks)
        {
            ...
            for (const control_points::PriorObservation& observation : priorTrack.observations)
            {
                ...
                if (!usable_state || observation.stale || !finite || !_reconstruction->hasImage(observation.imageId))
                {
            ...
            {
                rejectTrack(duplicate_image ? "duplicate image observation" : "fewer than two usable observations");
                continue;
            }
            ...
            for (const control_points::PriorObservation* observation : accepted_observations)
            {
                ...
                const bool conflicts_with_feature =
                    std::any_of(image.keypoints.begin(),
                                image.keypoints.end(),
                                [observation](const FeatureKeypoint& keypoint)
                                {
            ...
            if (!_correspondenceGraph.addPriorTrack(priorTrack.markerId, graph_observations, confidence))
            {
            ...
            _priorTrackDiagnostics.observationsAccepted += static_cast<int>(accepted_observations.size());
    ...
    void IncrementalSfm::applyControlNetworkDiagnostics(IncrementalSfmResult* result) const
    {
        ...
        result->checkPointRms =
            _controlNetworkResult.checkPointResiduals.empty()
                ? 0.0
                : std::sqrt(check_sum_squared / static_cast<double>(_controlNetworkResult.checkPointResiduals.size()));
        ...
        const auto marker_point = [this](const std::string& marker_id, std::array<double, 3>* point)
        {
            if (!point)
                return false;
            for (Point3DId point_id : _reconstruction->allPoint3DIds())
            {
                ...
                if (candidate.track.source == TrackSource::PriorMarker && candidate.track.sourceId == marker_id)
                {
                    *point = candidate.xyz;
                    return true;
                }
        ...
        {
            if (!scale_bar.enabled || !std::isfinite(scale_bar.measuredDistance) || scale_bar.measuredDistance <= 0.0)
            {
                continue;
            }
            std::array<double, 3> first{};
            std::array<double, 3> second{};
            if (!marker_point(scale_bar.firstMarkerId, &first) || !marker_point(scale_bar.secondMarkerId, &second))
            {
                continue;
            }
            ...
            const double residual = std::sqrt(dx * dx + dy * dy + dz * dz) - scale_bar.measuredDistance;
        ...
        result->controlScaleBarRms =
            control_scale_count > 0 ? std::sqrt(control_scale_sum_squared / static_cast<double>(control_scale_count))
                                    : 0.0;
        result->checkScaleBarRms =
            check_scale_count > 0 ? std::sqrt(check_scale_sum_squared / static_cast<double>(check_scale_count)) : 0.0;
    ...
    {
        const auto it =
            std::find_if(_pendingPriorTracks.cbegin(),
                         _pendingPriorTracks.cend(),
                         [&markerId](const control_points::PriorTrack& track) { return track.markerId == markerId; });
                                                ...
                                                std::vector<FramePinholeCamera>* baCameras)
    {
        ...
        for (Point3DId point_id : _reconstruction->allPoint3DIds())
        {
            ...
            if (point.track.source != TrackSource::PriorMarker)
                continue;
            const control_points::PriorTrack* prior = priorTrack(point.track.sourceId);
            if (!prior || !prior->hasReference || !prior->referenceUsable)
                continue;
            ...
            if (prior->role == control_points::MarkerRole::ControlPoint)
                ++usable_control_count;
        }
        if (usable_control_count < 3)
            return false;
    ...
    {
        if (!track || track->elements.empty())
            return;
        std::string source_id;
        for (const TrackElement& element : track->elements)
        {
            const std::string current = _correspondenceGraph.priorTrackId(element.imageId, element.featureIdx);
            if (current.empty() || (!source_id.empty() && current != source_id))
                return;
            source_id = current;
        }
        if (!source_id.empty())
    ...
    IncrementalSfmResult IncrementalSfm::run(SfmProgressCallback progressCb)
    {
        ...
        _inputMultiViewTracks.swap(_providedInputMultiViewTracks);
        _inputTrackIndicesByImage.clear();
        _finalTrackConsolidationAttempted = false;
        _stableIntrinsicReferenceByImageId.clear();
        _referenceCommittedIntrinsicParameterMask = {};
        _isAborted = false;
        ...
        // ---- 步骤 0：复用前端参考轨迹，或从原始对应图按相同语义构建一次 ----
        if (!_sfmOptions.useKnownCameraPoses && _inputMultiViewTracks.empty())
        {
            ...
            Logger::instance()->infof("[SFM] Align-photos-compatible tracks: beforeSpatial=%d selected=%d pruned=%d "
                                      "removedMatches=%zu retainedMatches=%zu tiePointLimit=%d",
            ...
            Logger::instance()->infof("[SFM] Retained %zu complete input tracks for final point-network consolidation",
                                      _inputMultiViewTracks.size());
        }
        else if (!_sfmOptions.useKnownCameraPoses)
        {
            Logger::instance()->infof("[SFM] Reusing %zu align-photos-compatible tracks selected by MatchPhotos",
                                      _inputMultiViewTracks.size());
        }
        if (!_sfmOptions.useKnownCameraPoses)
        {
            rebuildInputTrackObservationIndex();
        }
        ...
        // 大型共视网络先按参考策略独立重建互斥核心块，再通过稳定轨迹 Sim3 合并。
        // 子块会关闭该入口，因此不会递归分组。
        IndependentBlockReconstructor blockReconstructor(*this);
        if (std::optional<IncrementalSfmResult> blockResult = blockReconstructor.runIfNeeded(progressCb))
        {
            applyPriorTrackDiagnostics(&*blockResult);
            applyControlNetworkDiagnostics(&*blockResult);
            return std::move(*blockResult);
        }
        ...
        if (_sfmOptions.autoSelectInitPair)
        {
            ...
            const int trialTargetImages = initialPairTrialTargetImages(_sfmOptions, totalImages);
            const SfmReconstruction baseReconstruction = *_reconstruction;
            IncrementalSfmResult bestTrialResult;
            ReferenceInitialPairScore bestScore;
            ...
            for (size_t ci = 0; ci < candidates.size(); ++ci)
            {
                ...
                Logger::instance()->infof(
                    "[SFM] Trying init pair candidate %zu/%zu: (%u, %u)", ci + 1, candidates.size(), initId1, initId2);
                if (!initializer.initialize(initId1, initId2))
                {
                    Logger::instance()->warnf(
                        "[SFM] Candidate (%u, %u) failed: %s", initId1, initId2, _lastErrorMessage.c_str());
                    continue;
                }
                ...
                IncrementalSfmResult trialResult =
                    registration_engine.run(totalImages,
                                            progressCb,
                ...
                const ReferenceInitialPairScore trialScore =
                    evaluateReferenceInitialPairScore(*trialResult.reconstruction, initId1, initId2);
                ++evaluatedCandidates;
                Logger::instance()->infof("[SFM] Init pair trial (%u, %u): registered=%d/%d, points=%d, "
                                          "maxCameraRms=%.4f, accuracy=%.6f",
                                          initId1,
                                          initId2,
                                          trialResult.numRegisteredImages,
                                          trialScoreTarget,
                                          trialResult.numPoints3D,
                                          trialScore.maximumCameraReprojectionRms,
                                          trialScore.geometryAccuracy);
                ...
                if (!hasBestScore || referenceInitialPairScoreBetter(trialScore, bestScore))
                {
                    ...
                    bestInitId1 = initId1;
                    bestInitId2 = initId2;
                    bestSecondCamera = _reconstruction->camera(initId2);
                }
                if (_sfmOptions.useReferenceInitialPairTrials &&
                    referenceInitialPairScoreStable(trialScore, trialScoreTarget))
                {
                    stableTrialFound = true;
                    break;
                }
            }
            ...
            // 三相机试算仍不稳定时，对胜出像对的确定性五点姿态假设逐一扩展到五台相机。
            if (_sfmOptions.useReferenceInitialPairTrials && !stableTrialFound && totalImages > trialTargetImages)
            {
                ...
                for (const FramePinholeCamera& hypothesis : hypotheses)
                {
                    ...
                    IncrementalSfmResult extendedTrial =
                        registration_engine.run(totalImages, progressCb, extendedTarget, false, true);
                    if (_isAborted)
                    {
                        return result;
                    }
                    const ReferenceInitialPairScore extendedScore =
                        evaluateReferenceInitialPairScore(*extendedTrial.reconstruction, bestInitId1, bestInitId2);
                    if (extendedScore.maximumCameraReprojectionRms > 2.0)
                    {
                        continue;
                    }
                    if (!hasBestScore || !referenceInitialPairScoreBetter(bestScore, extendedScore))
                    {
                        bestScore = extendedScore;
                        hasBestScore = true;
                        bestTrialResult = std::move(extendedTrial);
                        bestSecondCamera = hypothesis;
                        selectedTrialTargetImages = extendedTarget;
                    }
                }
            }

            Logger::instance()->infof("[SFM] Selected bounded initial-pair trial (%u, %u): "
                                      "registered=%d/%d, points=%d, maxCameraRms=%.4f, accuracy=%.6f",
                                      bestInitId1,
                                      bestInitId2,
                                      bestTrialResult.numRegisteredImages,
                                      selectedTrialTargetImages,
                                      bestTrialResult.numPoints3D,
                                      bestScore.maximumCameraReprojectionRms,
                                      bestScore.geometryAccuracy);

            initializer.resetTrial(baseReconstruction);
            const bool restarted = bestSecondCamera
                                       ? initializer.initializeWithPose(bestInitId1, bestInitId2, *bestSecondCamera)
                                       : initializer.initialize(bestInitId1, bestInitId2);
            if (!restarted)
            {
            ...
            finalResult.initialPairLookaheadTargetImages =
                _sfmOptions.initialPairLookaheadMaxImages > 0 ? selectedTrialTargetImages : 0;
            finalResult.selectedInitialPairLookaheadRegisteredImages = bestTrialResult.numRegisteredImages;
        ...
        IncrementalSfmResult explicitPairResult = registration_engine.run(totalImages, progressCb);
    ...
    bool
    IncrementalSfm::reportProgress(int numRegistered, int numTotal, const std::string& msg, SfmProgressCallback& cb)
    {
        if (!cb)
            return true;
