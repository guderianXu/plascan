...
- 自动选种子默认对排名靠前的初始像对做有限前瞻：每个 seed 最多注册 8 张影像、跳过最终全局精化，
  按注册数、点数和 RMS 选出胜者后，再从干净输入只完整重建一次。这使大工程也能比较多个 seed，
...
小型固定焦距 BA 保持使用 Legacy CPU/OpenMP；共享焦距由 PlaMatrix 在同一个非线性问题中与相机、三维点联合优化。
只有相机数和观测数达到 Auto 门槛时才选择 PlaMatrix CUDA/OpenCL。正式 BA 日志会记录相机、轨迹、观测、线程、状态、
...
- `MultiViewTrackBuilder` 在合并两个已成形组件时要求三观测闭环、第二条独立跨组件边或小组件的相对高置信支持；
  两侧均达到三观测的成熟组件不能再由单条边直接桥接。原生双视轨迹和三视链式增长不受该门控影响。
