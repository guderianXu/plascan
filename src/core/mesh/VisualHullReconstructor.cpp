    ...
    namespace
    {
        ...
        std::vector<int> edgeConnectedFaceRoots(const TriMesh& mesh)
        {
            ...
            for (int face_index = 0; face_index < static_cast<int>(mesh.faces.size()); ++face_index)
            {
                ...
                const bool valid = face.v[0] >= 0 && face.v[1] >= 0 && face.v[2] >= 0 &&
        ...
        void recomputeNormalsAndColors(TriMesh* mesh, const std::vector<VisualHullView>& views)
        {
            ...
            const float visibilityTolerance = std::max({maxX - minX, maxY - minY, maxZ - minZ}) / 80.0f;
                ...
                for (const MeshVertex& vertex : mesh->vertices)
                {
                    ...
                    if (row >= 0 && column >= 0 && row < depth.rows && column < depth.cols)
                    {
                        depth.at<float>(row, column) =
                            std::min(depth.at<float>(row, column), static_cast<float>(cameraDepth));
                    }
                }
            ...
            for (MeshVertex& vertex : mesh->vertices)
            {
                ...
                const float outwardScore = vertex.nx * (vertex.x - centroidX) + vertex.ny * (vertex.y - centroidY) +
                ...
                for (std::size_t viewIndex = 0; viewIndex < views.size(); ++viewIndex)
                {
                    ...
                    if (!view.silhouetteMask.empty() && view.silhouetteMask.at<std::uint8_t>(row, column) == 0)
                    {
                        continue;
                    }
                    ...
                    const float score = vertex.nx * directionX + vertex.ny * directionY + vertex.nz * directionZ;
                    if (score <= 0.10f)
                    {
                        continue;
                    }
                ...
                {
                    vertex.r = static_cast<std::uint8_t>(std::clamp(accumulatedRed / accumulatedWeight, 0.0f, 255.0f));
                    vertex.g =
                        static_cast<std::uint8_t>(std::clamp(accumulatedGreen / accumulatedWeight, 0.0f, 255.0f));
                    vertex.b = static_cast<std::uint8_t>(std::clamp(accumulatedBlue / accumulatedWeight, 0.0f, 255.0f));
                }
            }
        }

        void closeOccupiedField(std::vector<float>* field, int gridSize, int iterations, int workers)
        {
            if (field == nullptr || gridSize < 3 || iterations <= 0)
            {
                return;
            }
            const std::size_t layer_size = static_cast<std::size_t>(gridSize) * gridSize;
            const auto offset = [gridSize, layer_size](int x, int y, int z)
            {
                return static_cast<std::size_t>(z) * layer_size + static_cast<std::size_t>(y) * gridSize +
                ...
                for (int z = 1; z < gridSize - 1; ++z)
                {
                    for (int y = 1; y < gridSize - 1; ++y)
                    {
                        for (int x = 1; x < gridSize - 1; ++x)
                        {
                            ...
                            updated[center] = occupied[center] || occupied[offset(x - 1, y, z)] ||
                                              occupied[offset(x + 1, y, z)] || occupied[offset(x, y - 1, z)] ||
                                              occupied[offset(x, y + 1, z)] || occupied[offset(x, y, z - 1)] ||
...
#pragma omp parallel for schedule(static) num_threads(workers)
            ...
            for (std::size_t index = 0; index < field->size(); ++index)
            {
                const float magnitude = std::max(1.0e-6f, std::abs((*field)[index]));
                (*field)[index] = occupied[index] ? -magnitude : magnitude;
            }
        }
    ...
    MeshConnectivityStats VisualHullReconstructor::analyzeConnectivity(const TriMesh& mesh)
    {
        ...
        {
            (void)root;
            stats.largestComponentFaceCount = std::max(stats.largestComponentFaceCount, component.faceCount);
        ...
        std::sort(stats.components.begin(),
                  stats.components.end(),
                  [](const MeshConnectivityStats::Component& left, const MeshConnectivityStats::Component& right)
                  { return left.faceCount > right.faceCount; });
        stats.largestComponentFaceRatio = mesh.faces.empty() ? 0.0
                                                             : static_cast<double>(stats.largestComponentFaceCount) /
                                                                   static_cast<double>(mesh.faces.size());
        return stats;
    }

    bool VisualHullReconstructor::requiresSilhouetteOnlyRetry(const MeshConnectivityStats& stats,
                                                              double minimumLargestComponentFaceRatio,
                                                              int maximumConnectedComponents)
    {
        return stats.componentCount <= 0 || stats.componentCount > std::max(1, maximumConnectedComponents) ||
               stats.largestComponentFaceRatio < std::clamp(minimumLargestComponentFaceRatio, 0.0, 1.0);
    }
    ...
    bool VisualHullReconstructor::retainLargestConnectedComponent(TriMesh* mesh)
    {
        ...
        const auto largest =
            std::max_element(face_counts.begin(),
                             face_counts.end(),
                                               ...
                                               VisualHullExecutionInfo* executionInfo)
    {
        if (executionInfo)
        {
            *executionInfo = VisualHullExecutionInfo{};
            executionInfo->requestedBackend = config.computeBackend;
            executionInfo->requestedDeviceIndex = config.computeDeviceIndex;
        }
        if (!mesh)
        {
            if (errorMessage)
        ...
        *mesh = TriMesh{};
        ...
        const int validViewCount = static_cast<int>(std::count_if(views.begin(),
                                                                  views.end(),
        ...
        const detail::RegularGrid3D field_grid{
            config.boundsMin, config.boundsMax, {resolution, resolution, resolution}};
        std::string field_error;
        if (!detail::evaluateVisualHullFieldGrid(
                views, config, field_grid, &field, nullptr, &field_error, executionInfo))
        {
            if (errorMessage)
            {
        ...
        closeOccupiedField(&field, gridSize, std::clamp(config.topologyClosingIterations, 0, 3), workers);
        ...
        if (Mc33IsoSurfaceExtractor::isAvailable())
        {
            ...
            Mc33IsoSurfaceResult extraction = Mc33IsoSurfaceExtractor::extract(config.boundsMin,
                                                                               config.boundsMax,
        ...
        try
        {
            if (!extracted)
            {
                ...
                auto [vertices, faces] = marchingCubes.extract(
                    ...
                    {
                        const int xIndex =
                            std::clamp(static_cast<int>(std::lround((x - config.boundsMin[0]) / stepX)), 0, resolution);
                        const int yIndex =
                            std::clamp(static_cast<int>(std::lround((y - config.boundsMin[1]) / stepY)), 0, resolution);
                        const int zIndex =
                            std::clamp(static_cast<int>(std::lround((z - config.boundsMin[2]) / stepZ)), 0, resolution);
        ...
        catch (const std::exception& exception)
        {
            if (errorMessage)
            {
                *errorMessage = topology_extraction_error.empty()
                                    ? exception.what()
                                    : topology_extraction_error + "; fallback extraction failed: " + exception.what();
            }
            return false;
        }
        ...
        detail::taubinSmooth(
            mesh, std::clamp(config.smoothingIterations, 0, 20), std::clamp(config.smoothingLambda, 0.0f, 0.49f));
