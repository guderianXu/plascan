...
namespace xjw::mesh
{
    ...
    namespace
    {
        ...
        bool
        meshVerticesInsidePaddedLayout(const TriMesh& mesh, const DepthTsdfLayout& layout, float paddingVoxels = 2.0f)
        {
                                                                       ...
                                                                       float minimumConfidence)
        {
            ...
            for (std::size_t vertex_index = baselineVertexCount; vertex_index < candidate.vertices.size();
                 ++vertex_index)
            {
                ...
                for (const int first_neighbor : boundary_neighbors[static_cast<std::size_t>(start)])
                {
                    ...
                    for (int frame_index = 0; frame_index < frames.size(); ++frame_index)
                    {
                        ...
                        for (const MeshVertex& sample : samples)
                        {
                            ...
                            const int column = static_cast<int>(std::lround(pixel[0]));
                            const int row = static_cast<int>(std::lround(pixel[1]));
                            if (row < 0 || column < 0 || row >= frame.supportMask.rows ||
        ...
        QVector<DepthFrameArtifact> selectMemoryBoundedDepthFrameArtifacts(const QVector<DepthFrameArtifact>& artifacts)
        {
            ...
            const DepthAuxiliaryBridgeSelectionResult bridge_selection =
                DepthAuxiliaryBridgeSelector::select(bridge_nodes);
            ...
            if (orbital_selection)
            {
                ...
                {
                    fixed_views.push_back(
                        {index, selected[index].refIndex, selected[index].cameraModel.cameraCenter()});
                }
        ...
        bool
        loadMask(const QString& path, const cv::Size& size, cv::Mat* mask, QString* reason, bool allow_resize = false)
        {
            if (!mask)
            {
                return false;
            ...
            for (int pass = 0; pass < maximum_passes; ++pass)
            {
                ...
                for (std::size_t index = 0; index < mesh->vertices.size(); ++index)
                {
                    ...
                    const int x = std::clamp(
                        static_cast<int>(std::lround((vertex.x - layout.boundsMin[0]) / layout.voxelSize[0])),
                        0,
                        layout.cells[0]);
                    const int y = std::clamp(
                        static_cast<int>(std::lround((vertex.y - layout.boundsMin[1]) / layout.voxelSize[1])),
                        0,
                        layout.cells[1]);
                    const int z = std::clamp(
                        static_cast<int>(std::lround((vertex.z - layout.boundsMin[2]) / layout.voxelSize[2])),
                        0,
                        layout.cells[2]);
...
#pragma omp parallel for schedule(static)                                                                              \
                                                   ...
                                                   const DepthTsdfOptions& options)
    {
        ...
        if (!options.enableAuxiliarySurfaceOnlyIntegration)
        {
            ...
            result = build(geometry_frames, geometry_options);
            result.statistics.inputFrameCount = frames.size();
            result.statistics.geometryInputFrameCount = geometry_frames.size();
            result.statistics.effectiveAuxiliarySurfaceOnlyIntegration = false;
            result.statistics.auxiliarySurfaceOnlyFrameCount = auxiliary_ref_indices.size();
        ...
        if (robust_frame_quality_weighting_enabled)
        {
            effective_frame_quality_weights =
                robustFrameQualityWeights(raw_frame_quality_weights,
        ...
        const DepthTsdfBoundsResult bounds =
            estimateBounds(retained_frames, options.useEvidenceAwareBounds, options.preservePerFrameCoverageBounds);
        if (!bounds.ok)
        {
...
#pragma omp parallel for schedule(static) num_threads(workerCount)                                                     \
    ...
    reduction(+ : geometrySourceNonzeroLocalMaskObservationCount,                                                      \
            ...
            for (int y = 0; y <= result.layout.cells[1]; ++y)
            {
                ...
                for (int x = 0; x <= result.layout.cells[0]; ++x)
                {
                    ...
                    for (int frame_index = 0; frame_index < frames.size(); ++frame_index)
                    {
                        ...
                        if (observationUsesSurfaceOnlyIntegration(observation, options) &&
                        ...
                        const float spread_support_weight_multiplier =
                            observationInverseDepthSpreadSupportWeightMultiplier(observation, options);
        ...
        for (std::size_t index = 0; index < supported.size(); ++index)
        {
            ...
            const bool sample_supported = isSampleSupported(evidenceSupportWeight[index],
                                                            support[index],
                                                            maximumEvidenceSupportObservationWeight[index],
                                                            options,
            ...
            const bool field_weight_supported = isSampleSupported(weight[index],
                                                                  support[index],
        ...
        if (result.statistics.effectiveOrbitalGapBoundaryRecovery && !orbital_gap_boundary_recovery_candidates.empty())
        {
            const int accepted_count =
                growGeometryVerifiedSingleViewSamples(result.layout,
                                                      tsdf,
        ...
        if (options.enableMeasuredSupportConnectivity && !geometrySourceMask.empty())
        {
            ...
            if (connectivity.recoveredSampleCount > 0)
            {
                ...
                finalizeMeasuredSupportTopologyTransaction(baseline_quality,
                                                           candidate_quality,
                                                           measured_support_baseline,
            ...
            if (options.enableCrossViewAnchoredSurfaceRecovery && !crossViewRepairedSurfaceWeight.empty())
            {
                ...
                for (int pass = 0; pass < growth_passes; ++pass)
                {
                    ...
                    const DepthTsdfZeroCrossingRecoveryStatistics recovery =
                        recoverGeometryVerifiedZeroCrossingSamples(result.layout,
                                                                   surface_candidate_tsdf,
                                                                   weight,
                                                                   geometrySourceMask,
                                                                   repaired_eligible,
        ...
        if (options.enableVisibilityOccupancyCompletion)
        {
            ...
            while (true)
            {
                ...
                const bool accepted_cut =
                    shouldApplyVisibilityOccupancyCut(occupancy.ok,
                                                      empty_cut,
        ...
        if (options.collectZeroCrossingDiagnostics || options.collectAcquisitionGapReport)
        {
            const DepthTsdfZeroCrossingStatistics zero_crossings =
                analyzeZeroCrossings(result.layout, tsdf, weight, supported);
        ...
        try
        {
                ...
                result.statistics.visualHullCompletionRecoveredSampleCount > 0)
            {
                ...
                const DepthTsdfVisualHullTopologyGuardEvaluation evaluation = evaluateVisualHullCompletionTopologyGuard(
        ...
        const float minimum_degenerate_face_area = options.visibilityOccupancyCellBoundaryExtraction ||
                                                           options.enableConsistentIsoSurfaceExtraction ||
                                                           options.enableMc33IsoSurfaceExtraction
                                                       ? 0.0f
                                                       : 5.0e-9f;
        ...
        detail::removeDegenerateFaces(&result.mesh, minimum_degenerate_face_area);
        ...
        const bool catastrophic_component_loss = isCatastrophicComponentFilterLoss(
        ...
        const auto pre_denoising_orientation = repair_face_orientation_safely(&result.mesh);
            ...
            result.mesh.faceCount() > options.simplifyTargetFaces)
        {
            ...
            { return after <= before + std::max(8, static_cast<int>(std::ceil(before * 0.10f))); };
            const bool accept_simplification =
                shouldAcceptQuadricSimplification(input_face_count,
        ...
        if (options.enableQuadricSimplification && options.enableVoxelFallbackSimplification &&
            shouldAttemptVoxelFallbackSimplification(
                result.mesh.faceCount(), options.simplifyTargetFaces, result.statistics.quadricSimplifyReachedTarget))
        {
            ...
            detail::removeDegenerateFaces(&candidate, minimum_degenerate_face_area);
            detail::removeSmallConnectedComponents(
            ...
            if (options.enableVoxelFallbackQemPolish && candidate.faceCount() > polish_target_faces)
            {
                ...
                const bool accept_polish =
                    shouldAcceptQuadricSimplification(result.statistics.voxelFallbackQemPolishInputFaceCount,
                                                      candidate.faceCount(),
        ...
        if (options.enableSilhouetteAwareFinalHoleFill && options.fillSmallBoundaryHoles)
        {
                ...
                options.enableVisibilityConstrainedFinalHoleFill)
            {
                    ...
                    candidate.faceCount() > options.simplifyTargetFaces)
                {
                    ...
                    post_fill_options.minimumFaceArea = minimum_degenerate_face_area;
        ...
        const auto final_orientation = repair_face_orientation_safely(&result.mesh);
