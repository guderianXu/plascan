...
namespace xjw::mesh::detail
{
    namespace
    {
        ...
        cv::Mat signedDistanceFromMask(const cv::Mat& mask)
        {
            ...
            cv::distanceTransform(foreground, inside_distance, cv::DIST_L2, cv::DIST_MASK_5);
            cv::distanceTransform(background, outside_distance, cv::DIST_L2, cv::DIST_MASK_5);
            return inside_distance - outside_distance;
        }

        bool bilinearSample(const cv::Mat& image, const double pixel[2], float* value)
        {
            if (value == nullptr || image.type() != CV_32FC1 || image.empty() || pixel[0] < 0.0 || pixel[1] < 0.0 ||
                pixel[0] > static_cast<double>(image.cols - 1) || pixel[1] > static_cast<double>(image.rows - 1))
            {
                return false;
            }
            ...
            const float top = image.at<float>(y0, x0) * (1.0f - tx) + image.at<float>(y0, x1) * tx;
            const float bottom = image.at<float>(y1, x0) * (1.0f - tx) + image.at<float>(y1, x1) * tx;
    ...
    std::vector<PreparedVisualHullView> prepareVisualHullFieldViews(const std::vector<VisualHullView>& views)
    {
        ...
        for (const VisualHullView& view : views)
        {
            ...
            entry.view = &view;
            entry.signedSilhouetteDistance = signedDistanceFromMask(view.silhouetteMask);
                                            ...
                                            const VisualHullConfig& config)
    {
        ...
        {
            if (prepared.view == nullptr || prepared.signedSilhouetteDistance.empty())
            {
                continue;
            }
            ...
            margins.push_back(static_cast<float>(signed_pixel_distance * camera_depth / mean_focal));

            if (config.enableDepthFreeSpaceCarving && !view.depthMap.empty() && view.depthMap.type() == CV_32FC1)
            {
                const int column = static_cast<int>(std::lround(pixel[0]));
                const int row = static_cast<int>(std::lround(pixel[1]));
                if (row >= 0 && column >= 0 && row < view.depthMap.rows && column < view.depthMap.cols)
                {
                    const float measured_depth = view.depthMap.at<float>(row, column);
                    if (std::isfinite(measured_depth) && measured_depth > 0.0f)
                    {
                        const float tolerance = std::max(1.0e-6f, measured_depth * config.relativeDepthTolerance);
                        if (camera_depth < measured_depth - tolerance)
                        {
        ...
        if (margins.empty() || static_cast<int>(margins.size()) < config.minimumVisibleViews)
        {
            return 1.0f;
        }
        if (config.enableDepthFreeSpaceCarving && free_space_violations >= config.minimumDepthFreeSpaceViolations)
        {
            return 1.0f;
        }

        const std::size_t allowed_violations = static_cast<std::size_t>(
            std::clamp(config.allowedSilhouetteViolations, 0, static_cast<int>(margins.size()) - 1));
        std::nth_element(
            margins.begin(), margins.begin() + static_cast<std::ptrdiff_t>(allowed_violations), margins.end());
        const float support_margin = margins[allowed_violations];
                                       ...
                                       const VisualHullConfig& config)
    {
        ...
        for (const VisualHullView& view : views)
        {
            ...
            const int column = static_cast<int>(std::lround(pixel[0]));
            const int row = static_cast<int>(std::lround(pixel[1]));
            if (row < 0 || column < 0 || row >= view.silhouetteMask.rows || column >= view.silhouetteMask.cols)
            {
                continue;
            }
            ...
            if (view.silhouetteMask.at<std::uint8_t>(row, column) == 0)
            {
                ...
                if (silhouette_violations > config.allowedSilhouetteViolations)
                {
                    return false;
                }
                continue;
            ...
            if (config.enableDepthFreeSpaceCarving && view.depthMap.type() == CV_32FC1 && !view.depthMap.empty() &&
                row < view.depthMap.rows && column < view.depthMap.cols)
            {
                ...
                if (std::isfinite(measured_depth) && measured_depth > 0.0f)
                {
                    ...
                    {
                        ++free_space_violations;
                        if (free_space_violations >= config.minimumDepthFreeSpaceViolations)
                        {
                            return false;
                        }
                    }
        ...
        return visible_views >= config.minimumVisibleViews &&
               silhouette_violations <= config.allowedSilhouetteViolations;
    }
