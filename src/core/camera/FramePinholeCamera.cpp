...
FramePinholeCamera::FramePinholeCamera(Camera camera,
                                       std::optional<CameraImageSize> imageSize,
...
bool FramePinholeCamera::isValid() const noexcept
{
                                     ...
                                     CameraImagingRay *ray) const
{
    ...
    if (!_camera.unprojectPixel(source_pixel, 1.0, point_at_unit_depth))
    {
        return false;
    }

    CameraImagingRay result;
    result.originMeters = _camera.cameraCenter();
...
bool FramePinholeCamera::groundToImage(const std::array<double, 3> &groundMeters,
                                       CameraGroundProjection *projection) const
{
    ...
    if (!_camera.projectWorldPointWithDepth(groundMeters.data(), pixel, positive_depth))
    {
        return false;
    }
