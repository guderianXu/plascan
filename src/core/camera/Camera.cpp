...
void Camera::setPose(const std::array<double, 9> &R, const std::array<double, 3> &C)
{
...
void Camera::setCameraCenter(const std::array<double, 3> &cameraCenter)
{
    _pose.cameraCenter = cameraCenter;
}

void Camera::setIntrinsics(double fu, double fv, double cu, double cv)
{
...
void Camera::setIntrinsicsMillimeters(double fuMm,
                                      double fvMm,
                                      double cuMm,
                                      double cvMm,
...
void Camera::setPixelPitch(double pixelPitch)
{
    if (pixelPitch > 0.0)
    {
...
void Camera::setAxisDirections(int uDir, int vDir)
{
...
void Camera::setDepthAxisFlipped(bool depthAxisFlipped)
{
    _pose.depthAxisFlipped = depthAxisFlipped;
    _isLoaded = true;
}

void Camera::setDistortion(double k1, double k2, double k3, double p1, double p2)
{
...
Camera Camera::normalizedForPositiveDepth() const
{
    Camera normalized = *this;
    if (!_isLoaded)
    {
        return normalized;
    }
...
bool Camera::loadFromFile(const std::string &path)
{
    ...
    {
        fprintf(stderr,
                "[Camera] loadFromFile 失败：fu=%.6g fv=%.6g 必须 > 0\n",
                _intrinsics.focalX,
                _intrinsics.focalY);
        return false;
    }
...
bool Camera::projectWorldPointWithDepth(const double world[3],
                                        double pixel[2],
...
bool Camera::unprojectPixel(const double pixel[2], double positiveDepth, double world[3]) const
{
