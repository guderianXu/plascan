// ============================================================
// 文件：CameraFormatConverter_tests.cpp
// 功能：在临时目录构造最小 Middlebury、EPFL、COLMAP 和 Metashape 数据集，
//       验证格式识别、内外参转换、影像列表排序、warning 与 summary 输出。
//
// 测试写入的影像内容只是占位文本；转换器只要求影像路径存在，不负责解码。
// 每个场景使用独立固定目录并在结束时清理，便于失败后人工复现产物结构。
// ============================================================
...
{
    // summary/list 均为小型文本产物，整体读取便于断言关键字段而不绑定行号。
...
{
    // Middlebury 行提供 K、R_wc、t_wc；预期中心按 C=-R_wc^T*t_wc 恢复。
...
{
    // EPFL 直接提供 R_cw/C；非零 skew 无法写入 PlaScan Tsai，必须进入 warning。
    const std::filesystem::path root =
...
{
    // images.txt 的 IMAGE_ID/记录顺序不应泄漏到输出；列表按影像名确定性排序。
    const std::filesystem::path root =
...
{
    // 模拟 Project.files/0/doc.xml 与同级 Depthimages，覆盖 adjusted 标定、
    // camera label 数字后缀剥离、畸变保留和 4×4 camera-to-world 变换。
    const std::filesystem::path root =
