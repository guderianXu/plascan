// ============================================================
// 文件：test_tsai_loader.cpp — 基于仓库 Tsai 数据的相机模型回归测试
//
// 测试内容：
//   1. 从 .tsai 文件加载相机参数
//   2. 世界到相机坐标和轴向点投影的一致性
//   3. 旋转矩阵正交性与行列式
//   4. 批量加载仓库 testData/tsai 下的全部测试相机
//
// 注意：本文件依赖仓库级 `testData/tsai`，不是 camera 模块内的小型
// `testdata/` 示例。CMake 通过 TEST_DATA_DIR 注入绝对路径，避免测试运行
// 目录影响数据定位。
// ============================================================
...
{
#ifdef TEST_DATA_DIR
    // 正常 CTest 路径由 CMake 定义，源码树和构建树可位于不同目录。
    return TEST_DATA_DIR;
#else
    // 仅供未经过 CMake 定义、从常见构建目录手工运行测试二进制时兜底。
    return "../testData";
#endif
}
...
{
    // 保留为几何断言的通用辅助；当前测试主要直接展开距离公式以输出上下文。
