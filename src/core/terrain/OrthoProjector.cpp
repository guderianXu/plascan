...
bool sampleDemElevation(const DemGridData &demGrid,
                        double worldX,
                        double worldY,
                        double *elevation)
{
    ...
    if (grid_x < -0.5 || grid_y < -0.5
        || grid_x > static_cast<double>(demGrid.width) - 0.5
    ...
    const int nearest_x =
        std::clamp(static_cast<int>(std::lround(grid_x)), 0, demGrid.width - 1);
    const int nearest_y =
        std::clamp(static_cast<int>(std::lround(grid_y)), 0, demGrid.height - 1);
    ...
    const bool bilinear_valid =
        demGrid.validMask.at<uchar>(y0, x0) != 0
        && demGrid.validMask.at<uchar>(y0, x1) != 0
        && demGrid.validMask.at<uchar>(y1, x0) != 0
        && demGrid.validMask.at<uchar>(y1, x1) != 0;
    if (bilinear_valid)
    {
...
{
    return cv::Vec3b(
        static_cast<uchar>(std::clamp(color[0], 0.0f, 255.0f)),
                     ...
                     std::vector<int> *contributingFrames)
{
    ...
    *color = cv::Vec3f(
        static_cast<float>(averaged[0]),
        static_cast<float>(averaged[1]),
        static_cast<float>(averaged[2]));
    return true;
}
                             ...
                             const ProgressCallback &progressCallback)
{
    ...
    if (!ortho_internal::loadFrames(
            inputs, options, &frames, cancelFlag, errorMsg))
    {
        return false;
    }
    ...
    for (int row = 0; row < height; ++row)
    {
        ...
        for (int col = 0; col < width; ++col)
        {
            const double world_x = projected.outputGrid.minEdgeX
                + (static_cast<double>(col) + 0.5)
                    * projected.outputGrid.reference.stepX;
            const double world_y = projected.outputGrid.minEdgeY
                + (static_cast<double>(row) + 0.5)
                    * projected.outputGrid.reference.stepY;
            ...
            const double world[3]{
                world_x,
                world_y,
                elevation + demElevationOffset};
            candidates.clear();
            for (int frame_index = 0;
                 frame_index < static_cast<int>(frames.size());
                 ++frame_index)
            {
            ...
            if (options.ghostFilter
                && options.blendMode != OrthoBlendMode::FirstValid)
            {
            ...
            cv::Vec3f color;
            if (!blendCandidates(
                    candidates, options.blendMode, &color, &contributing_frames))
            {
                continue;
            }
        ...
        {
            const int percent = 5
                + static_cast<int>(85.0 * static_cast<double>(row + 1)
    ...
    projected.holeFilledPixelCount =
        ortho_internal::fillSmallInteriorHoles(
            &projected.imageBgr,
    ...
    projected.coverageRatio = surface_pixels > 0
        ? static_cast<double>(filled_pixels) / static_cast<double>(surface_pixels)
        : 0.0;
    ...
    cv::bitwise_or(
        projected.coverageMask,
        projected.holeFilledMask,
        projected.outputGrid.reference.validMask);
