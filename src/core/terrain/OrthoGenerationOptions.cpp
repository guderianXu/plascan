...
namespace xjw
{
    ...
    namespace
    {
        ...
        bool parseProjectionType(const QString& token, OrthoProjectionType* type, QString* errorMsg)
        {
            ...
            if (token == QLatin1String("cylindrical") || token == QLatin1String("simple_cylindrical"))
            {
        ...
        bool parseSurfaceType(const QString& token, OrthoSurfaceType* type, QString* errorMsg)
        {
        ...
        bool parseColorSource(const QString& token, OrthoColorSource* source, QString* errorMsg)
        {
        ...
        bool parseBlendMode(const QString& token, OrthoBlendMode* mode, QString* errorMsg)
        {
        ...
        bool parseSizingMode(const QString& token, OrthoSizingMode* mode, QString* errorMsg)
        {
    ...
    {
        return mode == OrthoSizingMode::MaximumDimension ? QStringLiteral("maximum_dimension")
    ...
    {
        return type == OrthoSurfaceType::PointCloud ? QStringLiteral("point_cloud") : QStringLiteral("dem");
    }

    QString orthoColorSourceToken(OrthoColorSource source)
    {
        return source == OrthoColorSource::PointColors ? QStringLiteral("point_colors") : QStringLiteral("images");
    }

    bool
    OrthoGenerationOptions::fromJson(const QJsonObject& settings, OrthoGenerationOptions* options, QString* errorMsg)
    {
        if (!options)
        {
            if (errorMsg)
        ...
        QString sizing_mode = settings.value(QStringLiteral("sizing_mode")).toString().trimmed();
        ...
        if (!parseProjectionType(settings.value(QStringLiteral("projection_type")).toString(QStringLiteral("dem_grid")),
                                 &parsed.projectionType,
                                 errorMsg) ||
            !parseSurfaceType(settings.value(QStringLiteral("surface_type")).toString(QStringLiteral("dem")),
                              &parsed.surfaceType,
                              errorMsg) ||
            !parseColorSource(settings.value(QStringLiteral("color_source")).toString(QStringLiteral("images")),
                              &parsed.colorSource,
                              errorMsg) ||
            !parseBlendMode(settings.value(QStringLiteral("blend_mode")).toString(QStringLiteral("mosaic")),
                            &parsed.blendMode,
                            errorMsg) ||
            !parseSizingMode(sizing_mode, &parsed.sizingMode, errorMsg))
        {
            return false;
        }
        ...
        parsed.maximumDimension = settings.value(QStringLiteral("maximum_dimension")).toInt(parsed.maximumDimension);
        ...
        parsed.sharpnessWeighting = settings.value(QStringLiteral("sharpness_weighting")).toBool(false);
        ...
        parsed.holeFillMaxArea = settings.value(QStringLiteral("hole_fill_max_area")).toInt(parsed.holeFillMaxArea);
        parsed.holeFillRadius = settings.value(QStringLiteral("hole_fill_radius")).toDouble(parsed.holeFillRadius);
        parsed.useProjectMasks = settings.value(QStringLiteral("use_project_masks")).toBool(false);
        parsed.maximumPixelCount = static_cast<qint64>(settings.value(QStringLiteral("maximum_pixel_count"))
                                                           .toDouble(static_cast<double>(parsed.maximumPixelCount)));
        parsed.bodyReferenceAuto = settings.value(QStringLiteral("body_reference_auto")).toBool(true);
        ...
        parsed.centralMeridian = settings.value(QStringLiteral("central_meridian")).toDouble();
        if (!parseTerrainComputeBackend(
                settings.value(QStringLiteral("compute_backend")).toString(QStringLiteral("auto")),
                &parsed.computeBackend,
                errorMsg))
        {
            return false;
        }
        parsed.computeDeviceIndex =
            settings.value(QStringLiteral("compute_device_index")).toInt(parsed.computeDeviceIndex);
    ...
    bool OrthoGenerationOptions::validate(QString* errorMsg) const
    {
        ...
        const bool demCombination = projectionType == OrthoProjectionType::DemGrid &&
                                    surfaceType == OrthoSurfaceType::Dem && colorSource == OrthoColorSource::Images;
        ...
        else if (!finiteValue(pixelSizeX) || !finiteValue(pixelSizeY) || pixelSizeX < 0.0 || pixelSizeY < 0.0)
        {
            message = QStringLiteral("正射像元大小必须为有限的非负数");
        }
        else if (sizingMode == OrthoSizingMode::MaximumDimension && maximumDimension <= 0)
        {
            message = QStringLiteral("正射最大尺寸必须大于 0");
        }
        else if (bounds.enabled &&
                 (!finiteValue(bounds.minX) || !finiteValue(bounds.minY) || !finiteValue(bounds.maxX) ||
                  !finiteValue(bounds.maxY) || bounds.minX >= bounds.maxX || bounds.minY >= bounds.maxY))
        {
            message = QStringLiteral("正射区域边界无效，必须满足 min_x < max_x 且 min_y < max_y");
        }
        else if (fillHoles && (holeFillMaxArea <= 0 || !finiteValue(holeFillRadius) || holeFillRadius <= 0.0))
        {
        ...
        else if (!finiteValue(bodyCenterX) || !finiteValue(bodyCenterY) || !finiteValue(bodyCenterZ) ||
                 !finiteValue(referenceRadius) || !finiteValue(centralMeridian))
        {
            message = QStringLiteral("小天体参考参数必须为有限数值");
        }
        else if (computeDeviceIndex < -1)
        {
            message = QStringLiteral("地形计算设备索引必须为 -1 或非负整数");
        }
        else if (projectionType == OrthoProjectionType::SimpleCylindrical && !bodyReferenceAuto &&
                 referenceRadius <= 0.0)
        {
    ...
    QJsonObject OrthoGenerationOptions::toResolvedJson() const
    {
        ...
        object[QStringLiteral("maximum_pixel_count")] = static_cast<double>(maximumPixelCount);
        ...
        object[QStringLiteral("compute_backend")] = terrainComputeBackendToken(computeBackend);
        object[QStringLiteral("compute_device_index")] = computeDeviceIndex;
        return object;
    }
