...
namespace
{
    ...
    bool resolveDemVerticalOffsetForOrtho(const QJsonObject& projectMeta, const QString& demPath, double* zOffset)
    {
        if (zOffset)
        {
            *zOffset = 0.0;
        ...
        for (int index = demResults.size() - 1; index >= 0; --index)
        {
            ...
            if (QFileInfo(recordDemPath)
                    .fileName()
                    .compare(QFileInfo(demPath).fileName(),
#ifdef Q_OS_WIN
                             Qt::CaseInsensitive
#else
                             Qt::CaseSensitive
        ...
        return fileNameMatchCount == 1 && applyRelativeDemOffset(uniqueFileNameMatch, zOffset);
    }

    std::shared_ptr<xjw::PlaPointCloud> readTerrainPointCloud(const QString& pointCloudPath, QString* errorMsg)
    {
        ...
        catch (const std::exception& exception)
        {
            if (errorMsg)
            {
                *errorMsg = QStringLiteral("读取点云失败: %1").arg(QString::fromUtf8(exception.what()));
            }
            return {};
        }
        if (errorMsg)
        {
            *errorMsg = QStringLiteral("不支持的点云格式: %1；请选择 PLY、OBJ 或 XYZ").arg(pointCloudPath);
        }
        return {};
    }

    bool resolveCpuOnlyOrthoExecution(const xjw::OrthoGenerationOptions& options,
                                      const QString& operation,
                                      xjw::TerrainComputeExecution* execution,
                                      QString* errorMsg)
    {
        if (options.computeBackend == xjw::TerrainComputeBackend::Cuda ||
            options.computeBackend == xjw::TerrainComputeBackend::OpenCl)
        {
            if (errorMsg)
            {
                *errorMsg = QStringLiteral("%1 当前仅支持 CPU；显式指定 %2 时不允许回退")
                                .arg(operation, xjw::terrainComputeBackendDisplayName(options.computeBackend));
            }
            return false;
        }

        execution->backend = xjw::TerrainComputeBackend::Cpu;
        execution->deviceIndex = -1;
        execution->deviceName = QStringLiteral("CPU");
        if (options.computeBackend == xjw::TerrainComputeBackend::Auto)
        {
            execution->fallbackReason = QStringLiteral("%1 尚未实现 GPU 路径，Auto 已回退 CPU").arg(operation);
        }
        return true;
    }
    ...
    {
        xjw::TerrainComputeExecution compute_execution;
        if (!resolveCpuOnlyOrthoExecution(options, QStringLiteral("点云着色 DOM"), &compute_execution, errorMsg))
        {
            return false;
        }
        if (progressCallback)
        ...
        const std::shared_ptr<xjw::PlaPointCloud> cloud = readTerrainPointCloud(pointCloudPath, errorMsg);
        ...
        if (!xjw::PointCloudDomGenerator::generate(*cloud, options, &projected, errorMsg, cancelFlag, progressCallback))
        {
            return false;
        }
        if (progressCallback)
        ...
        if (outputGeoTiff)
        {
            if (!xjw::DemDomIO::writeDomGeoTiff(
                    projected.imageBgr, projected.validMask, projected.reference, outputPath, errorMsg))
            {
                return false;
            }
        }
        else
        {
            ...
            if (!xjw::DemDomIO::writeDomImage(northUp, outputPath, xjw::DomImageFormat::Png, errorMsg))
            {
                return false;
            }
        }
        if (result)
        {
            ...
            output[QStringLiteral("projection_wkt_present")] = !projected.reference.projection.projectionWkt.isEmpty();
            output[QStringLiteral("dom_georeferenced")] = outputGeoTiff;
            output[QStringLiteral("has_coverage_alpha")] = true;
            output[QStringLiteral("input_point_count")] = static_cast<double>(projected.inputPointCount);
            output[QStringLiteral("projected_point_count")] = static_cast<double>(projected.projectedPointCount);
            output[QStringLiteral("valid_pixel_count")] = static_cast<double>(projected.validPixelCount);
            output[QStringLiteral("coverage_ratio")] = projected.coverageRatio;
            output[QStringLiteral("compute_backend")] = xjw::terrainComputeBackendToken(compute_execution.backend);
            output[QStringLiteral("compute_device_index")] = compute_execution.deviceIndex;
            output[QStringLiteral("compute_device_name")] = compute_execution.deviceName;
            output[QStringLiteral("compute_fallback_reason")] = compute_execution.fallbackReason;
...
namespace xjw
{
                                              ...
                                              QString* errorMsg)
    {
        ...
        {
            if (errorMsg)
                *errorMsg = QStringLiteral("读取点云失败: %1").arg(QString::fromStdString(e.what()));
            return false;
        }
        ...
        {
            if (errorMsg)
                *errorMsg = QStringLiteral("读取点云失败: 文件为空或格式不支持");
            return false;
        }
        ...
        if (result)
        {
            ...
            output[QStringLiteral("dem_reference")] =
                relativeDem ? QStringLiteral("relative") : QStringLiteral("absolute");
                                               ...
                                               QString* errorMsg)
    {
        ...
        if (result)
        {
            ...
            const double dem_max_x = dem_min_x + static_cast<double>(metadata.width) * metadata.stepX;
            const double dem_max_y = dem_min_y + static_cast<double>(metadata.height) * metadata.stepY;
            QJsonObject output;
            output[QStringLiteral("resolved_settings")] = planned.resolvedOptions.toResolvedJson();
            output[QStringLiteral("coordinate_system")] = metadata.projection.coordinateSystem.isEmpty()
                                                              ? QStringLiteral("Local Coordinates")
                                                              : metadata.projection.coordinateSystem;
            output[QStringLiteral("projection_wkt_present")] = !metadata.projection.projectionWkt.isEmpty();
            ...
            output[QStringLiteral("estimated_memory_bytes")] = static_cast<double>(planned.estimatedMemoryBytes);
            *result = output;
        }
        return true;
    }
                                               ...
                                               const OrthoProgressCallback& progressCallback)
    {
        ...
        if (options.surfaceType == OrthoSurfaceType::PointCloud)
        {
            return generatePointCloudOrtho(
                demPath, outputPath, options, result, errorMsg, cancelFlag, progressCallback);
        }
        if (images.isEmpty())
        {
            if (errorMsg)
        ...
        TerrainComputeExecution compute_execution;
        const bool camera_projected = !projectMeta.isEmpty();
        if (!camera_projected && !resolveCpuOnlyOrthoExecution(
                                     options, QStringLiteral("无相机参数的兼容 DOM"), &compute_execution, errorMsg))
        {
            return false;
        }
        ...
        if (camera_projected)
        {
            ...
            const auto projector_progress = [&progressCallback](const QString& stage, int percent)
            {
                if (progressCallback)
                {
            ...
            if (!OrthoProjector::project(
                    demGrid, inputs, options, dem_z_offset, &projection, errorMsg, cancelFlag, projector_progress))
            {
                return false;
            }

            dom_image = std::move(projection.imageBgr);
            cv::bitwise_or(projection.coverageMask, projection.holeFilledMask, output_valid_mask);
            ...
            compute_execution = projection.computeExecution;
        ...
        {
            if (options.bounds.enabled || options.sizingMode == OrthoSizingMode::MaximumDimension ||
                (options.pixelSizeX > 0.0 && options.pixelSizeY > 0.0 &&
                 std::abs(options.pixelSizeX - options.pixelSizeY) > 1e-12))
            {
                if (errorMsg)
                {
                    *errorMsg = QStringLiteral("无相机参数的兼容 DOM 模式不支持自定义区域、最大尺寸或非方形像元");
                }
                return false;
            }
            DomGenerationOptions fallback_options;
            fallback_options.outputResolution = options.pixelSizeX > 0.0 ? options.pixelSizeX : options.pixelSizeY;
            ...
            if (!DomGenerator::generateFromImages(demGrid, images, fallback_options, &dom_image, errorMsg))
            {
                return false;
            }
            ...
            output_reference.stepX = demGrid.stepX * static_cast<double>(demGrid.width) / dom_image.cols;
            output_reference.stepY = demGrid.stepY * static_cast<double>(demGrid.height) / dom_image.rows;
            output_reference.minX = min_x + 0.5 * output_reference.stepX;
            output_reference.minY = min_y + 0.5 * output_reference.stepY;
            if (!demGrid.validMask.empty())
            {
                cv::resize(demGrid.validMask, output_valid_mask, dom_image.size(), 0.0, 0.0, cv::INTER_NEAREST);
            }
            ...
            coverage_ratio =
                static_cast<double>(filled_pixel_count) / static_cast<double>(demGrid.width * demGrid.height);
        }
        ...
        const DomImageFormat output_format = (outputPath.endsWith(QStringLiteral(".tif"), Qt::CaseInsensitive) ||
                                              outputPath.endsWith(QStringLiteral(".tiff"), Qt::CaseInsensitive))
                                                 ? DomImageFormat::Tiff
                                                 : DomImageFormat::Png;
        ...
        if (output_geotiff)
        {
            if (!DemDomIO::writeDomGeoTiff(dom_image, output_valid_mask, output_reference, outputPath, errorMsg))
            {
                return false;
            }
        }
        else
        {
            ...
            if (!DemDomIO::writeDomImage(north_up_image, outputPath, output_format, errorMsg))
            {
                return false;
            }
        }
        ...
        if (result)
        {
            ...
            output[QStringLiteral("resolved_settings")] = resolved_options.toResolvedJson();
            ...
            output[QStringLiteral("has_coverage_alpha")] = !output_valid_mask.empty();
            output[QStringLiteral("valid_pixel_count")] =
                output_valid_mask.empty() ? 0.0 : static_cast<double>(cv::countNonZero(output_valid_mask));
            output[QStringLiteral("projection_wkt_present")] = !output_reference.projection.projectionWkt.isEmpty();
            ...
            output[QStringLiteral("contributing_camera_count")] = contributing_camera_count;
            output[QStringLiteral("filled_pixel_count")] = static_cast<double>(filled_pixel_count);
            output[QStringLiteral("hole_filled_pixel_count")] = static_cast<double>(hole_filled_pixel_count);
            output[QStringLiteral("coverage_ratio")] = coverage_ratio;
            output[QStringLiteral("compute_backend")] = terrainComputeBackendToken(compute_execution.backend);
            output[QStringLiteral("compute_device_index")] = compute_execution.deviceIndex;
            output[QStringLiteral("compute_device_name")] = compute_execution.deviceName;
            output[QStringLiteral("compute_fallback_reason")] = compute_execution.fallbackReason;
            output[QStringLiteral("width")] = dom_image.cols;
            output[QStringLiteral("height")] = dom_image.rows;
            output[QStringLiteral("algorithm_version")] =
                camera_projected ? QStringLiteral("ortho_projector_v1") : QStringLiteral("aligned_composite_legacy_v1");
            *result = output;
        }
                                               ...
                                               QString* errorMsg)
    {
        ...
        return generateOrthoProduct(images, demPath, outputPath, settings, projectMeta, result, errorMsg, nullptr, {});
    }
    ...
    bool TerrainPipeline::generateOrthoProduct(const QStringList& images,
                                               ...
                                               QString* errorMsg)
    {
        return generateOrthoProduct(images, demPath, outputPath, resolution, QJsonObject(), result, errorMsg);
    }

    bool TerrainPipeline::generateFromObjMtl(
        const QString& objPath, const QString& outputDir, double demResolution, QJsonObject* result, QString* errorMsg)
    {
        ...
        {
            if (errorMsg)
                *errorMsg = QStringLiteral("OBJ 读取失败: %1").arg(QString::fromStdString(e.what()));
            return false;
        }
    ...
    bool TerrainPipeline::generateFromObjMtlDir(
        const QString& dirPath, const QString& outputDir, double demResolution, QJsonObject* result, QString* errorMsg)
    {
        // 1. 扫描目录下所有 OBJ 文件
        QDir dir(dirPath);
    ...
    bool TerrainPipeline::generateSmallBodyGlobalProducts(const QString& surfacePath,
                                                          const QString& outputDir,
        ...
        const QString& objPath, const QString& outputDir, double demResolution, QJsonObject* result, QString* errorMsg)
    {
        ...
        const ProjectionJob jobs[] = {
            ...
            {AsteroidProjectionType::AzimuthalEquidistant,
             ...
             {center.referenceRadius, center.referenceRadius, center.referenceRadius},
             "aeqd"},
            {AsteroidProjectionType::TriaxialEllipsoid, triaxRotMatrix, triaxEllipsoid, "triaxial"},
        };
        ...
        for (const ProjectionJob& job : jobs)
        {
            ...
            for (std::size_t i = 0; i < nPts; ++i)
            {
                ...
                AsteroidProjection::projectPoint(static_cast<double>(pt.x()),
            ...
            if (!DemGenerator::generateFromPointCloud(projCloud, demOptions, &projDemGrid, &denseCloud, &demError))
            {
                ...
                {
                    *errorMsg =
                        QStringLiteral("投影 %1 DEM 生成失败: %2").arg(QString::fromLatin1(job.suffix)).arg(demError);
                }
                return false;
            }
            ...
            const QString demTif =
                QDir(productsDir).filePath(QStringLiteral("dem_%1.tif").arg(QString::fromLatin1(job.suffix)));
            QString demMsgBuf;
            if (!DemDomIO::writeDemRaster(projDemGrid, demTif, DemRasterFormat::Float32Tiff, &demMsgBuf))
            {
                ...
                {
                    *errorMsg =
                        QStringLiteral("写出 %1 DEM 失败: %2").arg(QString::fromLatin1(job.suffix)).arg(demMsgBuf);
                }
                return false;
            }
            ...
            if (hasTexture)
            {
                ...
                if (meshInput.mesh.hasNormals())
                    projMesh.mesh.setNormals(*meshInput.mesh.normals());
                if (meshInput.mesh.hasColors())
                    projMesh.mesh.setColors(*meshInput.mesh.colors());
                if (meshInput.mesh.hasTextureCoords())
                    projMesh.mesh.setTextureCoords(*meshInput.mesh.textureCoords());
                if (meshInput.mesh.hasFaces())
                    projMesh.mesh.setFaces(*meshInput.mesh.faces());
                if (meshInput.mesh.hasFaceTextureIndices())
                    projMesh.mesh.setFaceTextureIndices(*meshInput.mesh.faceTextureIndices());
                ...
                {
                    const QString domTif =
                        QDir(productsDir).filePath(QStringLiteral("dom_%1.tif").arg(QString::fromLatin1(job.suffix)));
                    QString domMsgBuf;
