                                ...
                                const QString &note = QString())
{
    ...
    const QString cleanPath = supported ? existingCleanPath(path) : QDir::cleanPath(path.trimmed());
...
QJsonArray buildGenerateModelSourceCandidates(const QJsonObject &metadata)
{
    ...
    const QJsonArray sparseResults = metadata.value(QStringLiteral("aerial_triangulation_results")).toArray();
    for (int index = sparseResults.size() - 1; index >= 0; --index)
    {
        const QJsonObject record = sparseResults.at(index).toObject();
        const QString path = sparseCloudPathFromAtRecord(record);
        if (path.isEmpty() || seenPaths.contains(path))
        {
            continue;
        }
...
{
    auto *dlg = prepareDialog<GenerateModelDialog>(
        DialogSettingKeys::GenerateModel, _generateModelStore);
    if (!dlg)
    {
        return;
    }
