...
namespace
{
                           ...
                           const QString& targetName)
    {
        ...
        return samePathText(path, targetPath) || samePathText(path, targetAbsolutePath) ||
               samePathText(absolutePath, targetPath) || samePathText(absolutePath, targetAbsolutePath) ||
    ...
    {
        for (const QString& key : {QStringLiteral("final_model_path"),
    ...
    {
        if (source == QStringLiteral("depth_maps"))
            return QStringLiteral("深度图");
        if (source == QStringLiteral("point_cloud"))
            return QStringLiteral("点云");
        if (source == QStringLiteral("tie_points"))
            return QStringLiteral("连接点");
        if (source == QStringLiteral("model"))
    ...
    {
        if (surfaceType == QStringLiteral("arbitrary_3d"))
            return QStringLiteral("任意 (3D)");
        if (surfaceType == QStringLiteral("height_field"))
            return QStringLiteral("高度场 (2.5D)");
    ...
    QString qualityText(QString profile)
    {
        ...
        if (profile == QStringLiteral("highest"))
            return QStringLiteral("超高");
        if (profile == QStringLiteral("high"))
            return QStringLiteral("高");
        if (profile == QStringLiteral("medium"))
            return QStringLiteral("中等");
        if (profile == QStringLiteral("low"))
            return QStringLiteral("低");
        if (profile == QStringLiteral("lowest"))
            return QStringLiteral("超低");
        if (profile == QStringLiteral("detail"))
            return QStringLiteral("细节");
        if (profile == QStringLiteral("balanced"))
            return QStringLiteral("均衡");
        if (profile == QStringLiteral("lite"))
            return QStringLiteral("轻量");
    ...
    QString filterText(QString mode)
    {
        ...
        if (mode == QStringLiteral("disabled"))
            return QStringLiteral("已禁用");
        if (mode == QStringLiteral("mild"))
            return QStringLiteral("轻度");
        if (mode == QStringLiteral("moderate"))
            return QStringLiteral("中度");
        if (mode == QStringLiteral("aggressive"))
            return QStringLiteral("强度");
    ...
    RasterResolution resolutionFromRecord(const QJsonObject& record)
    {
        ...
        {
            resolution.x =
                positiveValue(rpc_result, {QStringLiteral("pixel_size_x"), QStringLiteral("grid_resolution_m")});
        }
        if (!(resolution.y > 0.0))
        {
        ...
        resolution.unit = unit == QStringLiteral("degree") || record.contains(QStringLiteral("angular_resolution_deg"))
                              ? QStringLiteral("°/px")
                              : QStringLiteral("m/px");
        return resolution;
    }
    ...
    QString resolutionText(const RasterResolution& resolution)
    {
        ...
        const auto number = [](double value) { return QLocale().toString(value, 'f', value < 1.0 ? 3 : 2); };
        if (qFuzzyCompare(resolution.x + 1.0, resolution.y + 1.0))
        {
            return QStringLiteral("%1 %2").arg(number(resolution.x), resolution.unit);
        }
        return QStringLiteral("X %1 × Y %2 %3").arg(number(resolution.x), number(resolution.y), resolution.unit);
    }
...
SelectionPropertiesWidget::SelectionPropertiesWidget(QWidget* parent) : QWidget(parent)
{
                                                       ...
                                                       const QString& resourcePath)
{
    ...
    if (section.contains(tr("3D模型")) || section.contains(tr("模型")) ||
    ...
    if (!raster_resolution.isEmpty() && (section.contains(QStringLiteral("DEM")) || section.contains(tr("正射"))))
    {
                                                          ...
                                                          const QString& resourcePath) const
{
    ...
    if (section.contains(QStringLiteral("DEM")))
        keys.push_back(QStringLiteral("dem_results"));
    if (section.contains(tr("正射")))
        keys.push_back(QStringLiteral("ortho_results"));
    if (keys.isEmpty())
    {
...
QVector<SelectionPropertiesWidget::PropertyRow> SelectionPropertiesWidget::modelPropertyRows(
    const QJsonObject& meta, const QJsonObject& record, const QString& resourcePath) const
{
    QLocale locale;
    locale.setNumberOptions(locale.numberOptions() & ~QLocale::OmitGroupSeparator);
    const QJsonObject reconstruction = record.value(QStringLiteral("reconstruction_parameters")).toObject();
    const QJsonObject depthSnapshot = record.value(QStringLiteral("depth_generation_parameters")).toObject();
    ...
    const bool hasVertexColors = record.value(QStringLiteral("has_vertex_colors"))
                                     .toBool(record.value(QStringLiteral("calculateVertexColors")).toBool(false) ||
                                             record.contains(QStringLiteral("reliably_colored_vertex_count")));
    rows.push_back({tr("顶点颜色"),
                    hasVertexColors
                        ? record.value(QStringLiteral("vertex_color_format")).toString(QStringLiteral("3波段, uint8"))
                        : (hasVertexColorField ? tr("无") : tr("不可用"))});
    rows.push_back(
        {tr("格式"),
         record.value(QStringLiteral("final_model_format")).toString(QFileInfo(resourcePath).suffix().toUpper())});
    ...
    qint64 depthArtifactBytes =
        static_cast<qint64>(depthSnapshot.value(QStringLiteral("artifact_bytes")).toDouble(-1.0));
    ...
    if (record.value(QStringLiteral("source_data")).toString() == QStringLiteral("depth_maps") &&
        depthSnapshot.isEmpty() && !depthSource.isEmpty())
    {
        depthElapsedMs = 0.0;
        ...
        for (const QJsonValue& value : meta.value(QStringLiteral("depth_map_results")).toArray())
        {
                                           ...
                                           QStringLiteral("depth_provenance_path")})
            {
                ...
                if (artifactInfo.exists())
                    depthArtifactBytes += artifactInfo.size();
            }
        }
    ...
    if (record.value(QStringLiteral("source_data")).toString() == QStringLiteral("depth_maps"))
    {
        ...
        rows.push_back({tr("最大邻域数量"), maximumNeighbors >= 0 ? locale.toString(maximumNeighbors) : tr("不可用")});
        rows.push_back({tr("深度图数量"), depthFrameCount > 0 ? locale.toString(depthFrameCount) : tr("不可用")});
        rows.push_back({tr("处理时间"), elapsedText(depthElapsedMs)});
        rows.push_back({tr("深度产物大小"), depthArtifactBytes >= 0 ? fileSizeText(depthArtifactBytes) : tr("不可用")});
        if (depthSnapshot.value(QStringLiteral("missing_reason_schema_version")).toInt() > 0)
        {
            const double missing_ratio =
                depthSnapshot.value(QStringLiteral("missing_within_support_ratio")).toDouble(-1.0);
            const qint64 missing_count =
                static_cast<qint64>(depthSnapshot.value(QStringLiteral("missing_pixel_count")).toDouble(0.0));
            rows.push_back({tr("深度缺失"),
                            missing_ratio >= 0.0
                                ? tr("%1（%2 像素）")
                                      .arg(locale.toString(missing_ratio * 100.0, 'f', 2) + QStringLiteral("%"))
                                      .arg(locale.toString(missing_count))
                                : tr("不可用")});
            const QJsonObject reason_counts = depthSnapshot.value(QStringLiteral("missing_reason_counts")).toObject();
            const QVector<QPair<QString, QString>> reason_labels{
                {QStringLiteral("insufficient_geometry_support_pixel_count"), tr("跨视证据不足")},
                {QStringLiteral("geometry_contradiction_pixel_count"), tr("多视几何冲突")},
                {QStringLiteral("patchmatch_unresolved_pixel_count"), tr("PatchMatch 未求解")},
                {QStringLiteral("low_confidence_pixel_count"), tr("低置信度剔除")},
                {QStringLiteral("local_depth_outlier_pixel_count"), tr("局部离群剔除")},
                {QStringLiteral("small_component_pixel_count"), tr("小连通域剔除")},
            ...
            {
                const qint64 count = static_cast<qint64>(reason_counts.value(reason.first).toDouble(0.0));
            ...
            rows.push_back({tr("主要缺失原因"),
                            dominant_count > 0
                                ? tr("%1（%2 像素）").arg(dominant_reason).arg(locale.toString(dominant_count))
                                : dominant_reason});
            const qint64 targeted_recovered = static_cast<qint64>(
                depthSnapshot.value(QStringLiteral("targeted_gap_recovered_pixel_count")).toDouble(0.0));
            rows.push_back({tr("缺口定向恢复"), locale.toString(targeted_recovered) + tr(" 像素")});
        }
        if (depthSnapshot.value(QStringLiteral("depth_provenance_schema_version")).toInt() > 0)
        {
            rows.push_back({tr("深度来源"), {}, true});
            rows.push_back({tr("原生 PatchMatch"),
                            locale.toString(static_cast<qint64>(
                                depthSnapshot.value(QStringLiteral("native_patchmatch_pixel_count")).toDouble(0.0))) +
                                tr(" 像素")});
            rows.push_back({tr("定向 PatchMatch"),
                            locale.toString(static_cast<qint64>(
                                depthSnapshot.value(QStringLiteral("targeted_patchmatch_pixel_count")).toDouble(0.0))) +
                                tr(" 像素")});
            rows.push_back({tr("跨视测量恢复"),
                            locale.toString(static_cast<qint64>(
                                depthSnapshot.value(QStringLiteral("cross_view_measured_pixel_count")).toDouble(0.0))) +
                                tr(" 像素")});
            rows.push_back({tr("一致性后局部 PatchMatch"),
                            locale.toString(static_cast<qint64>(
                                depthSnapshot.value(QStringLiteral("residual_patchmatch_pixel_count")).toDouble(0.0))) +
                                tr(" 像素")});
            rows.push_back(
                {tr("锚定插值"),
                 locale.toString(static_cast<qint64>(
                     depthSnapshot.value(QStringLiteral("anchored_interpolation_pixel_count")).toDouble(0.0))) +
                     tr(" 像素")});
        }
    }

    rows.push_back({tr("重建参数"), {}, true});
    rows.push_back({tr("表面类型"),
                    surfaceTypeText(reconstruction.value(QStringLiteral("surface_type"))
    ...
    rows.push_back(
        {tr("插值"),
         interpolationText(reconstruction.value(QStringLiteral("interpolation"))
    ...
    rows.push_back(
        {tr("严格的体积掩模"), strictMasks.isBool() ? (strictMasks.toBool() ? tr("是") : tr("否")) : tr("不可用")});
    ...
    const int targetFaces = record.value(QStringLiteral("configured_target_faces"))
                                .toInt(reconstruction.value(QStringLiteral("target_faces")).toInt(-1));
    if (targetFaces >= 0)
    {
        rows.push_back({tr("目标面数"), locale.toString(targetFaces)});
    }
    rows.push_back({tr("处理时间"),
                    elapsedText(reconstruction.value(QStringLiteral("processing_elapsed_ms"))
                                    .toDouble(record.value(QStringLiteral("processing_elapsed_ms")).toDouble(-1.0)))});
    const qint64 requiredBytes =
        static_cast<qint64>(record.value(QStringLiteral("tsdf_required_bytes")).toDouble(-1.0));
    rows.push_back({tr("内存用量（估算）"), requiredBytes >= 0 ? fileSizeText(requiredBytes) : tr("不可用")});
    ...
    if (softwareVersion.isEmpty())
        softwareVersion = QCoreApplication::applicationVersion();
