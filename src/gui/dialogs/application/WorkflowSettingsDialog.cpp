...
namespace
{
    ...
    constexpr auto kPlaMatchHctAlgorithmId = "plamatch_hct";
    ...
    constexpr int kWorkflowSettingsVersion = 9;
    ...
    constexpr WorkflowEntry kWorkflowEntries[] = {{kAerialWorkflowId, "空中三角测量"},
    ...
    QJsonObject defaultAerialSettings()
    {
        ...
        settings[QStringLiteral("algorithm_id")] = QString::fromLatin1(kPlaMatchHctAlgorithmId);
        ...
        settings[QStringLiteral("sift_maximum_ratio")] = 0.98;
        settings[QStringLiteral("sift_minimum_adaptive_ratio")] = 0.78;
        settings[QStringLiteral("adaptive_sift_ratio")] = true;
        return settings;
    }
    ...
    {
        QJsonObject settings;
        settings[QStringLiteral("compute_mode")] = QString::fromLatin1(kHybridComputeMode);
        return settings;
    }
    ...
    QJsonObject normalizedSettings(const QJsonObject& settings)
    {
        ...
        const QString selected = settings.value(QStringLiteral("selected_workflow")).toString().trimmed().toLower();
        ...
        {
            if (it.key() != QLatin1String(kRetiredReconstructionWorkflowId) && it.value().isObject())
            {
    ...
    QWidget* makeUnavailableWorkflowPage(const QString& workflowId, const QString& title, QWidget* parent)
    {
        ...
        auto* message = new QLabel(QStringLiteral("%1 的项目级设置尚未开放。\n"
...
WorkflowSettingsDialog::WorkflowSettingsDialog(QWidget* parent) : QDialog(parent)
{
    setupUi();
...
QJsonObject WorkflowSettingsDialog::defaultSettings()
{
    ...
    workflows[QString::fromLatin1(kModelGenerationWorkflowId)] = defaultModelGenerationSettings();
...
QJsonObject WorkflowSettingsDialog::aerialTriangulationSettings(const QJsonObject& settings)
{
    ...
    const QString algorithmId = source.value(QStringLiteral("algorithm_id")).toString().trimmed().toLower();
    ...
    const int lomaRKeypointBudget = source.value(QStringLiteral("loma_r_keypoint_budget")).toInt();
    aerial[QStringLiteral("loma_r_keypoint_budget")] =
        lomaRKeypointBudget == 1024 || lomaRKeypointBudget == 2048 || lomaRKeypointBudget == 3840 ? lomaRKeypointBudget
                                                                                                  : 0;
    const double siftMaximumRatio =
        std::clamp(source.value(QStringLiteral("sift_maximum_ratio")).toDouble(0.98), 0.5, 0.999);
    aerial[QStringLiteral("sift_maximum_ratio")] = siftMaximumRatio;
    aerial[QStringLiteral("sift_minimum_adaptive_ratio")] =
        std::clamp(source.value(QStringLiteral("sift_minimum_adaptive_ratio")).toDouble(0.78), 0.0, siftMaximumRatio);
    aerial[QStringLiteral("adaptive_sift_ratio")] = source.value(QStringLiteral("adaptive_sift_ratio")).toBool(true);
    return aerial;
}

QJsonObject WorkflowSettingsDialog::modelGenerationSettings(const QJsonObject& settings)
{
    const QJsonObject workflows = settings.value(QStringLiteral("workflows")).toObject();
    ...
    model_settings[QStringLiteral("modelGenerationContractRevision")] = 2;
    const QString depth_quality =
        source.value(QStringLiteral("depthQualityProfile")).toString(QStringLiteral("medium")).trimmed().toLower();
    model_settings[QStringLiteral("depthQualityProfile")] =
        depth_quality == QStringLiteral("highest") || depth_quality == QStringLiteral("high") ||
    ...
    const QString interpolation =
        source.value(QStringLiteral("interpolation")).toString(QStringLiteral("enabled")).trimmed().toLower();
    model_settings[QStringLiteral("interpolation")] =
        interpolation == QStringLiteral("disabled") || interpolation == QStringLiteral("extrapolated")
            ? interpolation
    ...
    else if (face_count_mode == QStringLiteral("low") || face_count_mode == QStringLiteral("medium") ||
             face_count_mode == QStringLiteral("high"))
    {
        model_settings[QStringLiteral("faceCountMode")] = face_count_mode;
    }
    ...
    {
        model_settings[QStringLiteral("faceCountMode")] =
            legacy_faces <= 200000 ? QStringLiteral("high") : QStringLiteral("custom");
...
void WorkflowSettingsDialog::setupUi()
{
    ...
    for (const WorkflowEntry& entry : kWorkflowEntries)
    {
        _workflowCombo->addItem(QString::fromUtf8(entry.displayName), QString::fromLatin1(entry.id));
    }
    ...
    _aerialForm = new QFormLayout(aerialGroup);
    _aerialForm->setFieldGrowthPolicy(QFormLayout::AllNonFixedFieldsGrow);
    ...
    _aerialForm->addRow(QStringLiteral("匹配算法:"), _matchingAlgorithmCombo);

    _siftMaximumRatioSpin = new QDoubleSpinBox(aerialGroup);
    _siftMaximumRatioSpin->setObjectName(QStringLiteral("aerialSiftMaximumRatioSpin"));
    _siftMaximumRatioSpin->setDecimals(3);
    _siftMaximumRatioSpin->setRange(0.5, 0.999);
    _siftMaximumRatioSpin->setSingleStep(0.01);
    _siftMaximumRatioSpin->setValue(0.98);
    _siftMaximumRatioSpin->setToolTip(QStringLiteral("SIFT 最近邻与次近邻距离比的宽松上限；值越小越严格。"));
    xjw::gui::dialogs::configureWorkflowInputWidget(_siftMaximumRatioSpin);
    _aerialForm->addRow(QStringLiteral("SIFT 歧义比上限:"), _siftMaximumRatioSpin);

    _adaptiveSiftRatioCheck = new QCheckBox(QStringLiteral("按当前像对自适应收紧 SIFT 歧义比"), aerialGroup);
    _adaptiveSiftRatioCheck->setObjectName(QStringLiteral("aerialAdaptiveSiftRatioCheck"));
    _adaptiveSiftRatioCheck->setToolTip(QStringLiteral("候选充足时按双向互检 ratio 分布自动收紧；候选少时保留上限。"));
    xjw::gui::dialogs::configureWorkflowCheckBox(_adaptiveSiftRatioCheck);
    _aerialForm->addRow(QString(), _adaptiveSiftRatioCheck);

    _lomaRKeypointBudgetCombo = new QComboBox(aerialGroup);
    _lomaRKeypointBudgetCombo->setObjectName(QStringLiteral("aerialLoMaRKeypointBudgetCombo"));
    ...
    _aerialForm->addRow(QStringLiteral("LoMa-R 特征档位:"), _lomaRKeypointBudgetCombo);

    _matchingResourceRow = new QWidget(aerialGroup);
    _matchingResourceRow->setObjectName(QStringLiteral("aerialMatchingResourceRow"));
    auto* enginePathLayout = new QHBoxLayout(_matchingResourceRow);
    enginePathLayout->setContentsMargins(0, 0, 0, 0);
    enginePathLayout->setSpacing(6);
    _matchingResourceEdit = new QLineEdit(_matchingResourceRow);
    ...
    _matchingResourceBrowseButton = new QToolButton(_matchingResourceRow);
    _matchingResourceBrowseButton->setObjectName(QStringLiteral("aerialMatchingResourceBrowseButton"));
    _matchingResourceBrowseButton->setIcon(style()->standardIcon(QStyle::SP_DialogOpenButton));
    _matchingResourceBrowseButton->setToolTip(QStringLiteral("选择 ONNX 模型或兼容的本机 TensorRT 资源"));
    _matchingResourceBrowseButton->setFixedSize(32, 30);
    _downloadModelButton = new QPushButton(QStringLiteral("下载模型"), _matchingResourceRow);
    ...
    _aerialForm->addRow(QStringLiteral("模型资源:"), _matchingResourceRow);

    _matchingResourceStatusLabel = new QLabel(aerialGroup);
    _matchingResourceStatusLabel->setObjectName(QStringLiteral("aerialMatchingResourceStatusLabel"));
    ...
    _aerialForm->addRow(QStringLiteral("当前生效:"), _matchingResourceStatusLabel);
    ...
    _modelComputeModeCombo->setObjectName(QStringLiteral("modelComputeModeCombo"));
    xjw::gui::dialogs::configureWorkflowComboBox(_modelComputeModeCombo);
    _modelComputeModeCombo->addItem(QStringLiteral("只用 CUDA"), QString::fromLatin1(kCudaComputeMode));
    _modelComputeModeCombo->addItem(QStringLiteral("只用 OpenCL"), QString::fromLatin1(kOpenClComputeMode));
    _modelComputeModeCombo->addItem(QStringLiteral("混合使用 CUDA + OpenCL"), QString::fromLatin1(kHybridComputeMode));
    model_form->addRow(QStringLiteral("计算模式:"), _modelComputeModeCombo);

    _cudaDeviceStatusLabel = new QLabel(model_group);
    _cudaDeviceStatusLabel->setObjectName(QStringLiteral("modelCudaDeviceStatusLabel"));
    ...
    _openClDeviceStatusLabel->setObjectName(QStringLiteral("modelOpenClDeviceStatusLabel"));
    ...
    _modelComputePolicyLabel->setObjectName(QStringLiteral("modelComputePolicyLabel"));
    ...
    _detectComputeDevicesButton = new QPushButton(QStringLiteral("重新检测设备"), model_group);
    _detectComputeDevicesButton->setObjectName(QStringLiteral("modelDetectComputeDevicesButton"));
    model_form->addRow(QString(), _detectComputeDevicesButton);
    model_layout->addWidget(model_group);
    model_layout->addStretch(1);
    _workflowPages->addWidget(model_page);

    _workflowPages->addWidget(
        makeUnavailableWorkflowPage(QString::fromLatin1(kDemWorkflowId), QStringLiteral("创建 DEM"), _workflowPages));
    ...
    connect(_workflowCombo,
            qOverload<int>(&QComboBox::currentIndexChanged),
            this,
            &WorkflowSettingsDialog::setCurrentWorkflow);
    connect(_matchingAlgorithmCombo,
            qOverload<int>(&QComboBox::currentIndexChanged),
            this,
            [this]() { switchAlgorithmResource(); });
    connect(_matchingResourceEdit, &QLineEdit::textChanged, this, [this]() { refreshMatchingResourceStatus(); });
    connect(_lomaRKeypointBudgetCombo,
            qOverload<int>(&QComboBox::currentIndexChanged),
            this,
            [this]() { refreshMatchingResourceStatus(); });
    connect(_matchingResourceBrowseButton,
            &QToolButton::clicked,
            this,
            [this]()
            {
                ...
                const QString startPath = currentPath.isEmpty() ? QString() : QFileInfo(currentPath).absolutePath();
                const QString selected = QFileDialog::getOpenFileName(
                    this,
                    loMaR ? QStringLiteral("选择 LoMa-R ONNX 模型清单") : QStringLiteral("选择 LightGlue ONNX 模型"),
                    startPath,
    ...
    connect(_downloadModelButton, &QPushButton::clicked, this, &WorkflowSettingsDialog::downloadCurrentModelPackage);
    connect(_modelComputeModeCombo,
            qOverload<int>(&QComboBox::currentIndexChanged),
            this,
            [this]() { refreshModelComputePolicy(); });
    connect(
        _detectComputeDevicesButton, &QPushButton::clicked, this, &WorkflowSettingsDialog::refreshModelComputeDevices);

    auto* buttons =
        new QDialogButtonBox(QDialogButtonBox::RestoreDefaults | QDialogButtonBox::Ok | QDialogButtonBox::Cancel, this);
...
void WorkflowSettingsDialog::applySettings(const QJsonObject& requestedSettings)
{
    ...
    const QString selectedWorkflow = _appliedSettings.value(QStringLiteral("selected_workflow")).toString();
    ...
    const int defaultAlgorithmIndex = _matchingAlgorithmCombo->findData(QString::fromLatin1(kPlaMatchHctAlgorithmId));
    _matchingAlgorithmCombo->setCurrentIndex(algorithmIndex >= 0 ? algorithmIndex : std::max(0, defaultAlgorithmIndex));
    _lightGlueEnginePath = aerial.value(QStringLiteral("lightglue_tensorrt_engine")).toString();
    _lomaRPackagePath = aerial.value(QStringLiteral("loma_r_tensorrt_package")).toString();
    const int budgetIndex =
        _lomaRKeypointBudgetCombo->findData(aerial.value(QStringLiteral("loma_r_keypoint_budget")).toInt());
    _lomaRKeypointBudgetCombo->setCurrentIndex(budgetIndex >= 0 ? budgetIndex : 0);
    _siftMaximumRatioSpin->setValue(aerial.value(QStringLiteral("sift_maximum_ratio")).toDouble(0.98));
    _adaptiveSiftRatioCheck->setChecked(aerial.value(QStringLiteral("adaptive_sift_ratio")).toBool(true));

    const QJsonObject model_settings = modelGenerationSettings(_appliedSettings);
    const int compute_mode_index =
        _modelComputeModeCombo->findData(model_settings.value(QStringLiteral("compute_mode")).toString());
    _modelComputeModeCombo->setCurrentIndex(compute_mode_index >= 0 ? compute_mode_index : 2);
...
QJsonObject WorkflowSettingsDialog::collectSettings() const
{
    ...
    settings[QStringLiteral("workflow_settings_version")] = kWorkflowSettingsVersion;
    settings[QStringLiteral("selected_workflow")] = _workflowCombo->currentData().toString();

    QJsonObject workflows = settings.value(QStringLiteral("workflows")).toObject();
    QJsonObject aerial = workflows.value(QString::fromLatin1(kAerialWorkflowId)).toObject();
    const QString algorithmId = _matchingAlgorithmCombo->currentData().toString();
    aerial[QStringLiteral("algorithm_id")] =
        algorithmId.isEmpty() ? QString::fromLatin1(kPlaMatchHctAlgorithmId) : algorithmId;
    ...
    aerial[QStringLiteral("loma_r_keypoint_budget")] = _lomaRKeypointBudgetCombo->currentData().toInt();
    aerial[QStringLiteral("sift_maximum_ratio")] = _siftMaximumRatioSpin->value();
    aerial[QStringLiteral("sift_minimum_adaptive_ratio")] =
        std::clamp(aerial.value(QStringLiteral("sift_minimum_adaptive_ratio")).toDouble(0.78),
                   0.0,
                   _siftMaximumRatioSpin->value());
    aerial[QStringLiteral("adaptive_sift_ratio")] = _adaptiveSiftRatioCheck->isChecked();
    workflows[QString::fromLatin1(kAerialWorkflowId)] = aerial;
    QJsonObject model_settings = workflows.value(QString::fromLatin1(kModelGenerationWorkflowId)).toObject();
    model_settings[QStringLiteral("compute_mode")] = _modelComputeModeCombo->currentData().toString();
    workflows[QString::fromLatin1(kModelGenerationWorkflowId)] = model_settings;
    settings[QStringLiteral("workflows")] = workflows;
    return settings;
}
...
{
    const QString algorithmId = _matchingAlgorithmCombo->currentData().toString();
    const bool modelBacked =
        algorithmId == QLatin1String(kSiftLightGlueAlgorithmId) || algorithmId == QLatin1String(kLoMaRAlgorithmId);
    const bool autoSift = algorithmId == QLatin1String(kAutoSiftAlgorithmId);
    const bool plaMatchHct = algorithmId == QLatin1String(kPlaMatchHctAlgorithmId);
    const bool loMaR = algorithmId == QLatin1String(kLoMaRAlgorithmId);
    _aerialForm->setRowVisible(_siftMaximumRatioSpin, autoSift);
    _aerialForm->setRowVisible(_adaptiveSiftRatioCheck, autoSift);
    _aerialForm->setRowVisible(_lomaRKeypointBudgetCombo, loMaR);
    _aerialForm->setRowVisible(_matchingResourceRow, modelBacked);
    ...
    _lomaRKeypointBudgetCombo->setEnabled(loMaR);
    _siftMaximumRatioSpin->setEnabled(autoSift);
    _adaptiveSiftRatioCheck->setEnabled(autoSift);
    if (modelBacked)
    {
    ...
    else if (autoSift)
    {
        ...
        _matchingResourceStatusLabel->setText(QStringLiteral("内置 SIFT，无需下载模型；自动优先 CUDA，"
    ...
    else if (plaMatchHct)
    {
        ...
        palette.setColor(QPalette::WindowText, QColor(35, 110, 70));
        _matchingResourceStatusLabel->setPalette(palette);
        _matchingResourceStatusLabel->setText(QStringLiteral("内置 PlaMatch-HCT，无需下载模型；支持 CUDA/OpenCL/CPU，"
                                                             "实际后端按连接点设备设置解析"));
        _downloadModelButton->setVisible(false);
    }
    else
...
void WorkflowSettingsDialog::refreshMatchingResourceStatus()
{
    ...
    if (algorithmId == QLatin1String(kLoMaRAlgorithmId))
    {
        ...
        const auto resolved = xjw::matchphotos::resolveLoMaRTensorRtPackage(options, 40000, false);
        resolvedPath = resolved.manifestPath;
        detail = resolved.isValid() ? QStringLiteral("  [K=%1, D=%2, %3x%4]")
    ...
    else if (algorithmId == QLatin1String(kSiftLightGlueAlgorithmId))
    {
        ...
        const auto resolved = xjw::matchphotos::resolveLightGlueTensorRtEngine(options, 4096, false);
        resolvedPath = resolved.path;
        detail = resolved.bucketKeypoints > 0 ? QStringLiteral("  [K=%1]").arg(resolved.bucketKeypoints) : QString();
    }
    ...
    else if (algorithmId == QLatin1String(kPlaMatchHctAlgorithmId))
    {
        QPalette palette = _matchingResourceStatusLabel->palette();
        palette.setColor(QPalette::WindowText, QColor(35, 110, 70));
        _matchingResourceStatusLabel->setPalette(palette);
    ...
    if (resolvedPath.isEmpty())
    {
        ...
        _matchingResourceStatusLabel->setPalette(palette);
        _matchingResourceStatusLabel->setText(detail.isEmpty() ? QStringLiteral("未找到自动模型资源") : detail);
    ...
    palette.setColor(QPalette::WindowText, QColor(35, 110, 70));
    _matchingResourceStatusLabel->setPalette(palette);
    _matchingResourceStatusLabel->setText(QDir::toNativeSeparators(resolvedPath) + detail);
    _downloadModelButton->setVisible(false);
}
...
void WorkflowSettingsDialog::downloadCurrentModelPackage()
{
    ...
    if (algorithmId == QLatin1String(kLoMaRAlgorithmId))
    {
        ...
        const int effectiveBudget = xjw::matchphotos::resolveLoMaRKeypointBudget(
            40000, requestedBudget, xjw::matchphotos::queryMatchPhotosGpuMemory(0));
    ...
    const QString targetDirectory = xjw::common::model::modelPackageInstallDirectory(package, resolver);
    QString errorMessage;
    if (!ModelPackageDownloadDialog::downloadPackage(package, targetDirectory, this, &errorMessage))
    {
        return;
    }

    _matchingResourceEdit->setText(xjw::common::model::modelPackageEntryPoint(package, resolver));
    refreshMatchingResourceStatus();
}

void WorkflowSettingsDialog::refreshModelComputeDevices()
{
    if (!_modelComputeModeCombo || !_cudaDeviceStatusLabel || !_openClDeviceStatusLabel)
    {
        return;
    }
    ...
    {
        const QString name = QString::fromStdString(xjw::mvs::PatchMatchDepthEstimator::cudaDeviceName(device_index));
        if (name.isEmpty())
        {
            continue;
        }
        ...
        const QString identity =
            QString::fromStdString(xjw::mvs::PatchMatchDepthEstimator::cudaDeviceIdentity(device_index));
        if (!identity.isEmpty())
        {
    ...
    const std::vector<xjw::mvs::OpenClDeviceInfo> opencl_devices = xjw::mvs::PatchMatchDepthEstimator::openClDevices();
    for (const xjw::mvs::OpenClDeviceInfo& device : opencl_devices)
    {
        const QString identity = QString::fromStdString(device.physicalDeviceIdentity);
        const bool nvidia_opencl = xjw::mvs::isNvidiaOpenClVendor(device.vendor);
        const bool duplicates_cuda = !identity.isEmpty() && cuda_identities.contains(identity);
        ...
        const double memory_gib = static_cast<double>(device.globalMemoryBytes) / (1024.0 * 1024.0 * 1024.0);
    ...
    cuda_palette.setColor(QPalette::WindowText, _cudaAvailable ? QColor(35, 110, 70) : QColor(180, 45, 45));
    _cudaDeviceStatusLabel->setPalette(cuda_palette);
    _cudaDeviceStatusLabel->setText(
        _cudaAvailable
            ? QStringLiteral("可用，共 %1 个设备\n%2").arg(cuda_lines.size()).arg(cuda_lines.join(QLatin1Char('\n')))
            : QStringLiteral("不可用：当前构建或驱动未提供可用 CUDA 设备"));

    QPalette opencl_palette = _openClDeviceStatusLabel->palette();
    opencl_palette.setColor(QPalette::WindowText, _openClAvailable ? QColor(35, 110, 70) : QColor(180, 45, 45));
    _openClDeviceStatusLabel->setPalette(opencl_palette);
    _openClDeviceStatusLabel->setText(_openClAvailable
                                          ? QStringLiteral("可用，共 %1 个设备\n%2")
                                                .arg(eligible_opencl_devices)
                                                .arg(opencl_lines.join(QLatin1Char('\n')))
                                          : QStringLiteral("不可用：未检测到同时支持运行和在线编译的 OpenCL GPU"));

    auto set_mode_enabled = [this](const char* mode, bool enabled)
    {
        const int index = _modelComputeModeCombo->findData(QString::fromLatin1(mode));
        auto* model = qobject_cast<QStandardItemModel*>(_modelComputeModeCombo->model());
        if (model && index >= 0)
        {
    ...
    const bool selected_available = (selected_mode == QLatin1String(kCudaComputeMode) && _cudaAvailable) ||
    ...
    {
        const QString fallback_mode =
            _hybridAvailable
                ? QString::fromLatin1(kHybridComputeMode)
                : (_cudaAvailable ? QString::fromLatin1(kCudaComputeMode)
                                  : (_openClAvailable ? QString::fromLatin1(kOpenClComputeMode) : selected_mode));
...
void WorkflowSettingsDialog::refreshModelComputePolicy()
{
    ...
    if (mode == QLatin1String(kCudaComputeMode))
    {
        ...
        policy = QStringLiteral("深度图估计、点云预处理和可用的模型求解阶段仅使用 CUDA；"
                                "不会调度 OpenCL 设备。");
    }
    else if (mode == QLatin1String(kOpenClComputeMode))
    {
        available = _openClAvailable;
        policy = QStringLiteral("深度图估计、点云预处理和 Poisson 稀疏求解仅使用 OpenCL；"
                                "自动忽略 NVIDIA 设备且不会调用 CUDA。网格提取等串行阶段仍在 CPU 执行。");
    }
    else
    {
        available = _hybridAvailable;
        policy = QStringLiteral("深度图估计会并行调度 CUDA 与独立 OpenCL GPU；"
                                "同一物理显卡的重复接口会自动去重。模型预处理优先 OpenCL，"
                                "Poisson 求解使用 CUDA。");
    }

    QPalette palette = _modelComputePolicyLabel->palette();
    palette.setColor(QPalette::WindowText, available ? QColor(35, 80, 120) : QColor(180, 45, 45));
    _modelComputePolicyLabel->setPalette(palette);
    _modelComputePolicyLabel->setText(available ? policy : QStringLiteral("当前机器不满足该模式：") + policy);
}
