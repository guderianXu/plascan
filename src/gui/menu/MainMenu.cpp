...
namespace
{
    ...
    template <typename T> T* findNamedChild(QObject* root, const char* name)
    {
    ...
    QMenu* ensureSubMenu(
        QObject* root, QMenu* parent, const QString& objectName, const QString& title, QAction* before = nullptr)
    {
    ...
    QIcon makeCameraImageToolbarIcon()
    {
        ...
        imageMountain << QPointF(10.0, 46.0) << QPointF(21.0, 28.0) << QPointF(29.0, 37.0) << QPointF(34.0, 33.0)
                      << QPointF(46.0, 46.0);
    ...
    QIcon makeImageRotationToolbarIcon(bool rotateLeft)
    {
        ...
        painter.setBrush(iconColor);
        QPolygonF mountain;
        mountain << QPointF(9.0, 50.0) << QPointF(20.0, 34.0) << QPointF(28.0, 43.0) << QPointF(35.0, 37.0)
                 << QPointF(47.0, 50.0);
    ...
    QIcon makeFeaturePointsToolbarIcon()
    {
        ...
        for (const QPointF& point : {QPointF(13, 13),
                                     QPointF(28, 11),
                                     QPointF(43, 15),
                                     QPointF(10, 29),
                                     QPointF(27, 28),
    ...
    QIcon makeDepthOverlayToolbarIcon()
    {
        ...
        painter.setPen(Qt::NoPen);
        const QList<QColor> colors = {QColor(49, 54, 149),
                                      QColor(69, 117, 180),
                                      QColor(116, 173, 209),
                                      QColor(171, 217, 233),
                                      QColor(224, 243, 248),
    ...
    QIcon makeNavigationToolbarIcon()
    {
        ...
        painter.drawPolygon(QPolygonF{
            QPointF(11.0, 6.0), QPointF(44.0, 31.0), QPointF(29.0, 34.0), QPointF(22.0, 50.0), QPointF(11.0, 6.0)});
        return QIcon(pixmap);
    }
 ...
 */
MainMenu::MainMenu(QMainWindow* mainWindow) : QObject(mainWindow), _mainWindow(mainWindow)
{
    if (_mainWindow)
    {
        buildMenuSystem();
...
void MainMenu::buildMenuSystem()
{
    ...
    auto installModelDisplayMenu = [this](QMenu* modelMenu)
    {
        ...
        QMenu* displayMenu =
            ensureSubMenu(_mainWindow, modelMenu, QStringLiteral("menuModelDisplayHideItems"), tr("显示/隐藏项目"));
        if (!displayMenu)
        {
            return;
        }
        ...
        QMenu* imageMenu =
            ensureSubMenu(_mainWindow, displayMenu, QStringLiteral("menuModelDisplayImages"), tr("显示图像"));
        if (!imageMenu)
        {
            return;
        }

        _toggleCameraImagesAct = ensureCheckableAction(
            _mainWindow, imageMenu, imageMenu, QStringLiteral("actionToggleCameraImages"), tr("显示图像"), false);
        _toggleCameraImagesAct->setToolTip(tr("根据当前三维视角自动选择方向相近的 SfM 照片，并与点云或模型对齐显示"));
        ...
        _showCameraImagesInForegroundAct = ensureCheckableAction(_mainWindow,
                                                                 ...
                                                                 tr("在前景中显示"),
                                                                 false);
        _showCameraImagesInForegroundAct->setToolTip(tr("在点云或模型前方显示当前匹配照片"));
        ...
        _showCameraImagesInBackgroundAct = ensureCheckableAction(_mainWindow,
                                                                 ...
                                                                 tr("在后景中显示"),
                                                                 true);
        _showCameraImagesInBackgroundAct->setToolTip(tr("在点云或模型后方显示当前匹配照片"));
        ...
        _lockCameraImageAct = ensureCheckableAction(
            _mainWindow, imageMenu, imageMenu, QStringLiteral("actionLockCameraImage"), tr("锁定图像"), false);
        _lockCameraImageAct->setToolTip(tr("固定当前匹配照片并切换为二维平移、缩放；取消锁定后恢复三维导航"));

        QMenu* viewModeMenu =
            ensureSubMenu(_mainWindow, modelMenu, QStringLiteral("menuModelViewMode"), tr("视图模式"));
        QMenu* tiePointViewModeMenu =
            ensureSubMenu(_mainWindow, viewModeMenu, QStringLiteral("menuModelTiePointViewMode"), tr("连接点"));
        if (!tiePointViewModeMenu)
        {
            return;
        }

        _tiePointColorModeAct = ensureCheckableAction(_mainWindow,
        ...
        _tiePointElevationModeAct = ensureCheckableAction(_mainWindow,
                                                          tiePointViewModeMenu,
                                                          tiePointViewModeMenu,
        ...
        _tiePointImageCountModeAct = ensureCheckableAction(_mainWindow,
                                                           tiePointViewModeMenu,
                                                           tiePointViewModeMenu,
        ...
        auto* tiePointModeGroup =
            tiePointViewModeMenu->findChild<QActionGroup*>(QStringLiteral("actionGroupTiePointViewMode"));
        ...
        QMenu* modelViewModeMenu =
            ensureSubMenu(_mainWindow, viewModeMenu, QStringLiteral("menuModelSurfaceViewMode"), tr("模型"));
        ...
        auto addModelMode =
            [this, modelViewModeMenu](
                QAction** action, const QString& objectName, const QString& text, const QString& toolTip, bool checked)
        {
            *action =
                ensureCheckableAction(_mainWindow, modelViewModeMenu, modelViewModeMenu, objectName, text, checked);
        ...
        auto* modelModeGroup =
            modelViewModeMenu->findChild<QActionGroup*>(QStringLiteral("actionGroupModelSurfaceViewMode"));
        if (!modelModeGroup)
        {
            modelModeGroup = new QActionGroup(modelViewModeMenu);
            modelModeGroup->setObjectName(QStringLiteral("actionGroupModelSurfaceViewMode"));
        }
        modelModeGroup->setExclusive(true);
        for (QAction* action : {_modelTextureModeAct,
                                _modelShadedModeAct,
                                _modelSolidModeAct,
    ...
    auto installViewMenuLayout = [this](QMenu* viewMenu, QMenu* windowMenu)
    {
        ...
        QMenu* imageMenu = ensureSubMenu(_mainWindow, viewMenu, QStringLiteral("menuViewImageDisplay"), tr("影像显示"));
        ...
        imageMenu->addSeparator();
        for (QAction* action :
             {_showFeaturePointsAct, _showFeatureResidualsAct, _showMaskOverlayAct, _showDepthOverlayAct})
        {
            if (action)
            {
                imageMenu->addAction(action);
        ...
        setManagedWindowActions(
            {_toggleWorkspaceAct, _togglePropertiesAct, _toggleWorkAct, _togglePhotosAct, _toggleLogAct},
        ...
        if (style)
        {
            ...
            _toggleMainToolbarAct->setIcon(style->standardIcon(QStyle::SP_ToolBarHorizontalExtensionButton));
        }
    ...
    auto installCameraToolbarButton = [this]()
    {
        ...
        _toggleLocalAxesAct = ensureCheckableAction(
            _mainWindow, _mainWindow, nullptr, QStringLiteral("actionToggleLocalAxes"), tr("显示本地轴"), true);
        ...
        _cameraToolbarWidgetAct =
            xjw::gui::toolbar::createToolbarSplitButton(_toolBar,
                                                        _toggleCamerasAct,
                                                        cameraMenu,
    ...
    {
        if (!_toolBar || !_toggleCameraImagesAct || !_showCameraImagesInForegroundAct ||
            !_showCameraImagesInBackgroundAct || !_lockCameraImageAct)
        {
            return;
        }
        ...
        _toggleCameraImagesAct->setToolTip(tr("按当前三维视角自动匹配并对齐显示 SfM 原始照片"));
    ...
    auto installImageRotationToolbarButtons = [this]()
    {
        ...
        auto installButton = [this](QAction* action, const QString& objectName, QAction*& toolbarWidgetAction)
        {
            ...
            toolbarWidgetAction =
                xjw::gui::toolbar::createToolbarButton(_toolBar, action, objectName, _toolbarEditingSeparatorAct);
        };

        installButton(
            _rotateImageLeftAct, QStringLiteral("toolButtonRotateImageLeft"), _rotateImageLeftToolbarWidgetAct);
        installButton(
            _rotateImageRightAct, QStringLiteral("toolButtonRotateImageRight"), _rotateImageRightToolbarWidgetAct);
    };
    ...
    auto installMaskEditingActions = [this]()
    {
        ...
        _rectangleMaskAct = createToolAction(QStringLiteral("actionRectangleMask"),
                                             tr("矩形蒙版"),
                                             tr("拖动矩形添加或擦除照片蒙版（M）"),
                                             QKeySequence(Qt::Key_M),
                                             makeMaskEditingToolIcon(0));
        _scissorsMaskAct = createToolAction(QStringLiteral("actionScissorsMask"),
                                            tr("智能剪刀"),
                                            tr("沿边缘逐点放置选区，双击闭合（L）"),
                                            QKeySequence(Qt::Key_L),
                                            makeMaskEditingToolIcon(1));
        _smartPaintMaskAct = createToolAction(QStringLiteral("actionSmartPaintMask"),
                                              tr("智能绘画"),
                                              tr("按颜色边界绘制照片蒙版（P）"),
                                              QKeySequence(Qt::Key_P),
                                              makeMaskEditingToolIcon(2));
        _magicWandMaskAct = createToolAction(QStringLiteral("actionMagicWandMask"),
                                             tr("魔棒"),
                                             tr("选择与点击位置相连的相似颜色区域（W）"),
        ...
        for (QAction* action : {_rectangleMaskAct, _scissorsMaskAct, _smartPaintMaskAct, _magicWandMaskAct})
        {
    ...
    auto installImageOverlayToolbarButtons = [this, &installMaskEditingActions]()
    {
        ...
        if (!_toolBar || !_showFeaturePointsAct || !_showFeatureResidualsAct || !_showMaskOverlayAct ||
            !_showDepthOverlayAct || !_depthOverlayAllLevelsAct || !_depthOverlayLevel1Act || !_depthOverlayLevel2Act ||
            !_depthOverlayLevel3Act || !_showDepthIntensityAct || !_resetViewAct || !_featureVisualizationAct ||
            !_rectangleMaskAct || !_scissorsMaskAct || !_smartPaintMaskAct || !_magicWandMaskAct ||
            !_maskEditorSettingsAct || !_resetMaskSelectionAct)
        {
            return;
        }
        ...
        if (!_toolBar->findChild<QToolButton*>(QStringLiteral("toolButtonResetImageView")))
        {
            _resetImageViewToolbarWidgetAct = xjw::gui::toolbar::createToolbarButton(
                _toolBar, _resetViewAct, QStringLiteral("toolButtonResetImageView"), _toolbarEditingSeparatorAct);
        }
        if (!_toolBar->findChild<QToolButton*>(QStringLiteral("toolButtonShowFeaturePoints")))
        {
            ...
            _showFeaturePointsToolbarWidgetAct =
                xjw::gui::toolbar::createToolbarSplitButton(_toolBar,
                                                            _showFeaturePointsAct,
                                                            pointsMenu,
                                                            QStringLiteral("toolButtonShowFeaturePoints"),
                                                            _toolbarEditingSeparatorAct);
        }
        if (!_toolBar->findChild<QToolButton*>(QStringLiteral("toolButtonShowMaskOverlay")))
        {
            _showMaskOverlayToolbarWidgetAct =
                xjw::gui::toolbar::createToolbarButton(_toolBar,
                                                       _showMaskOverlayAct,
                                                       QStringLiteral("toolButtonShowMaskOverlay"),
                                                       _toolbarEditingSeparatorAct);
        }
        if (!_toolBar->findChild<QToolButton*>(QStringLiteral("toolButtonMaskEditor")))
        {
            ...
            _maskEditingToolbarWidgetAct =
                xjw::gui::toolbar::createToolbarSplitButton(_toolBar,
                                                            _rectangleMaskAct,
                                                            maskMenu,
                                                            QStringLiteral("toolButtonMaskEditor"),
                                                            _toolbarEditingSeparatorAct);
            if (auto* button = _toolBar->findChild<QToolButton*>(QStringLiteral("toolButtonMaskEditor")))
            {
        ...
        if (!_toolBar->findChild<QToolButton*>(QStringLiteral("toolButtonShowDepthOverlay")))
        {
            ...
            _showDepthOverlayToolbarWidgetAct =
                xjw::gui::toolbar::createToolbarSplitButton(_toolBar,
    ...
    auto initializeToolbar = [this, &installZoomToolbarButtons]()
    {
        ...
        if (_saveAct)
        {
            ...
            _saveToolbarWidgetAct =
                xjw::gui::toolbar::createToolbarButton(_toolBar, _saveAct, QStringLiteral("toolButtonSaveProject"));
        }
        _toolBar->addSeparator();
        installZoomToolbarButtons();
        _navigationToolbarWidgetAct = xjw::gui::toolbar::createToolbarButton(
            _toolBar, _navigationAct, QStringLiteral("toolButtonModelNavigation"));
        ...
        _pointSelectionToolbarWidgetAct = xjw::gui::toolbar::createToolbarSplitButton(
            _toolBar, _rectangleSelectionAct, selection_menu, QStringLiteral("toolButtonPointSelection"));
        auto* selection_button = _toolBar->findChild<QToolButton*>(QStringLiteral("toolButtonPointSelection"));
        if (selection_button)
        {
            for (QAction* action : {_rectangleSelectionAct, _circleSelectionAct, _freehandSelectionAct})
            {
                connect(action,
                        &QAction::triggered,
                        selection_button,
                        [selection_button, action]() { selection_button->setDefaultAction(action); });
            }
        }
    ...
    if (findNamedChild<QAction>(_mainWindow, "actionNewProject"))
    {
        ...
        _importPointCloudAct = ensurePlainAction(
            _mainWindow, importMenu, importMenu, QStringLiteral("actionImportPointCloud"), tr("导入点云..."));
        _importPointCloudAct->setToolTip(tr("导入 Metashape 导出的 OBJ、PLY 或 XYZ 点云"));
        _importModelAct = ensurePlainAction(
            _mainWindow, importMenu, importMenu, QStringLiteral("actionImportModel"), tr("导入模型..."));
        _importModelAct->setToolTip(tr("导入 Metashape 导出的 OBJ 或 PLY 模型，并复制材质和纹理"));
        _importReferenceAct = ensurePlainAction(_mainWindow,
                                                importMenu,
                                                importMenu,
                                                QStringLiteral("actionImportReference"),
                                                tr("导入参考..."),
                                                _importPointCloudAct);
        _importReferenceAct->setToolTip(tr("导入相机位置/姿态参考（Cameras_WGS84.txt，并自动读取 GNSS_offset.txt），"
                                           "或标记控制点/检查点参考（GCPs_WGS84.txt、CSV）"));
        _importCameraAct = ensurePlainAction(_mainWindow,
                                             importMenu,
                                             importMenu,
                                             QStringLiteral("actionImportCamera"),
                                             tr("导入相机..."),
                                             _importPointCloudAct);
        _importCameraAct->setToolTip(tr("打开相机校准窗口，为项目影像导入 .tsai 相机参数"));
        ...
        _exportPortableProjectAct = findNamedChild<QAction>(_mainWindow, "actionExportPortableProject");
        ...
        _toggleFullScreenAct =
            ensurePlainAction(_mainWindow, viewMenu, nullptr, QStringLiteral("actionToggleFullScreen"), tr("全屏"));
        ...
        QObject* rotationActionParent = viewMenu ? static_cast<QObject*>(viewMenu) : static_cast<QObject*>(_mainWindow);
        ...
        _toggleHenanUniversityBrandAct = findNamedChild<QAction>(_mainWindow, "actionToggleHenanUniversityBrand");
        _featureVisualizationAct = findNamedChild<QAction>(_mainWindow, "actionFeatureVisualization");
        _showFeaturePointsAct = ensureCheckableAction(_mainWindow,
                                                      _mainWindow,
                                                      viewMenu,
        ...
        _showFeatureResidualsAct = ensureCheckableAction(_mainWindow,
                                                         _mainWindow,
                                                         viewMenu,
                                                         ...
                                                         false,
                                                         _featureVisualizationAct);
        _showMaskOverlayAct = ensureCheckableAction(_mainWindow,
                                                    _mainWindow,
                                                    viewMenu,
        ...
        QObject* windowActionParent =
            windowMenu ? static_cast<QObject*>(windowMenu) : static_cast<QObject*>(_mainWindow);
        _toggleWorkspaceAct = ensureCheckableAction(
            _mainWindow, windowActionParent, nullptr, QStringLiteral("actionToggleWorkspace"), tr("工作区"), true);
        _toggleWorkspaceAct->setToolTip(tr("显示或隐藏工作区面板"));
        _togglePropertiesAct = ensureCheckableAction(
            _mainWindow, windowActionParent, nullptr, QStringLiteral("actionToggleProperties"), tr("属性"), true);
        _togglePropertiesAct->setToolTip(tr("显示或隐藏选择对象属性面板"));
        _toggleWorkAct = ensureCheckableAction(
            _mainWindow, windowActionParent, nullptr, QStringLiteral("actionToggleWork"), tr("工作"), true);
        _toggleWorkAct->setToolTip(tr("显示或隐藏后台工作面板"));
        _togglePhotosAct = ensureCheckableAction(
            _mainWindow, windowActionParent, nullptr, QStringLiteral("actionTogglePhotos"), tr("照片"), true);
        _togglePhotosAct->setToolTip(tr("显示或隐藏照片面板"));
        if (!_toggleLogAct)
        {
            _toggleLogAct = ensureCheckableAction(
                _mainWindow, windowActionParent, nullptr, QStringLiteral("actionToggleLog"), tr("控制台"), true);
        }
        _toggleLogAct->setText(tr("控制台"));
        _toggleLogAct->setCheckable(true);
        _toggleLogAct->setToolTip(tr("显示或隐藏控制台"));
        _toggleMainToolbarAct = ensureCheckableAction(
            _mainWindow, windowActionParent, nullptr, QStringLiteral("actionToggleMainToolbar"), tr("主工具栏"), true);
        _toggleMainToolbarAct->setToolTip(tr("显示或隐藏主工具栏"));
        _restoreDefaultWindowLayoutAct = ensurePlainAction(_mainWindow,
                                                           windowActionParent,
                                                           nullptr,
                                                           QStringLiteral("actionRestoreDefaultWindowLayout"),
                                                           tr("恢复默认窗口布局"));
        _restoreDefaultWindowLayoutAct->setToolTip(tr("恢复工作区、属性、工作、照片、控制台和主工具栏的默认布局"));
        QObject* viewActionParent = viewMenu ? static_cast<QObject*>(viewMenu) : static_cast<QObject*>(_mainWindow);
        _toggleGizmoAct = ensureCheckableAction(_mainWindow,
                                                ...
                                                _toggleCamerasAct);
        _toggleGizmoAct->setToolTip(tr("显示或隐藏 3D 视图中的旋转轨迹球"));
        _toggleHenanUniversityBrandAct = ensureCheckableAction(_mainWindow,
                                                               viewActionParent,
                                                               viewMenu,
        ...
        _workflowAerialTriangulationAct = findNamedChild<QAction>(_mainWindow, "actionWorkflowAerialTriangulation");
        _workflowSettingsAct = findNamedChild<QAction>(_mainWindow, "actionWorkflowSettings");
        ...
        {
            QObject* actionParent =
                workflowMenu ? static_cast<QObject*>(workflowMenu) : static_cast<QObject*>(_mainWindow);
            _workflowAerialTriangulationAct = new QAction(tr("空中三角测量..."), actionParent);
            _workflowAerialTriangulationAct->setObjectName(QStringLiteral("actionWorkflowAerialTriangulation"));
        ...
        if (!_workflowSettingsAct)
        {
            ...
            _workflowSettingsAct->setToolTip(tr("设置空中三角测量的设备、并行度和数值门限"));
            if (workflowMenu)
            {
        ...
        {
            QObject* actionParent = viewMenu ? static_cast<QObject*>(viewMenu) : static_cast<QObject*>(_mainWindow);
        ...
        if (!_importReferenceDatasetAct)
        {
            ...
            _importReferenceDatasetAct->setToolTip(
                tr("以外部引用方式导入 DEM、LAS/LAZ/COPC 或点云文件，用于精度检查和后续软约束"));
            if (toolsMenu)
            {
                if (_viewWorkflowReportAct)
                {
        ...
        if (!_detectMarkersAct)
        {
            QObject* action_parent =
                _markersMenu ? static_cast<QObject*>(_markersMenu) : static_cast<QObject*>(_mainWindow);
        ...
        if (!_reviewMarkerDetectionsAct)
        {
            ...
            _reviewMarkerDetectionsAct->setObjectName(QStringLiteral("actionReviewMarkerDetections"));
        ...
        if (!_referenceTerrainBundleAdjustAct)
        {
            ...
            _referenceTerrainBundleAdjustAct->setToolTip(
                tr("检查参考 DEM/LiDAR 是否可作为 BA 软约束，并生成前置检查报告"));
            if (toolsMenu)
            {
                if (_viewWorkflowReportAct)
                {
        ...
        const QList<QAction*> workflowActions{_workflowAerialTriangulationAct,
    ...
    _toggleHenanUniversityBrandAct = ensureCheckableAction(
        _mainWindow, viewMenu, viewMenu, QStringLiteral("actionToggleHenanUniversityBrand"), tr("河南大学校徽"), true);
    _toggleHenanUniversityBrandAct->setToolTip(tr("显示或隐藏主工具栏中的河南大学校徽"));
    viewMenu->addSeparator();
    // 特征点可视化设置对话框入口
    ...
    _toggleWorkspaceAct = ensureCheckableAction(
        _mainWindow, windowMenu, windowMenu, QStringLiteral("actionToggleWorkspace"), tr("工作区"), true);
    _toggleWorkspaceAct->setToolTip(tr("显示或隐藏工作区面板"));
    _togglePropertiesAct = ensureCheckableAction(
        _mainWindow, windowMenu, windowMenu, QStringLiteral("actionToggleProperties"), tr("属性"), true);
    _togglePropertiesAct->setToolTip(tr("显示或隐藏选择对象属性面板"));
    _toggleWorkAct = ensureCheckableAction(
        _mainWindow, windowMenu, windowMenu, QStringLiteral("actionToggleWork"), tr("工作"), true);
    _toggleWorkAct->setToolTip(tr("显示或隐藏后台工作面板"));
    _togglePhotosAct = ensureCheckableAction(
        _mainWindow, windowMenu, windowMenu, QStringLiteral("actionTogglePhotos"), tr("照片"), true);
    _togglePhotosAct->setToolTip(tr("显示或隐藏照片面板"));
    ...
    _restoreDefaultWindowLayoutAct->setObjectName(QStringLiteral("actionRestoreDefaultWindowLayout"));
    _restoreDefaultWindowLayoutAct->setToolTip(tr("恢复工作区、属性、工作、照片、控制台和主工具栏的默认布局"));
...
{
    _showDepthOverlayAct = ensureCheckableAction(_mainWindow,
                                                 actionParent,
                                                 viewMenu,
    ...
    _depthOverlayAllLevelsAct = ensureCheckableAction(
        _mainWindow, actionParent, nullptr, QStringLiteral("actionDepthOverlayAllLevels"), tr("所有级别"), true);
    _depthOverlayLevel1Act = ensureCheckableAction(
        _mainWindow, actionParent, nullptr, QStringLiteral("actionDepthOverlayLevel1"), tr("级别 1"), false);
    _depthOverlayLevel2Act = ensureCheckableAction(
        _mainWindow, actionParent, nullptr, QStringLiteral("actionDepthOverlayLevel2"), tr("级别 2"), false);
    _depthOverlayLevel3Act = ensureCheckableAction(
        _mainWindow, actionParent, nullptr, QStringLiteral("actionDepthOverlayLevel3"), tr("级别 3"), false);
    _showDepthIntensityAct = ensureCheckableAction(
        _mainWindow, actionParent, nullptr, QStringLiteral("actionShowDepthIntensity"), tr("显示强度"), false);
...
{
    QMenu* tiePointsMenu = ensureSubMenu(_mainWindow, toolsMenu, QStringLiteral("menuTiePoints"), tr("连接点"), before);
    if (!tiePointsMenu)
    {
        return;
    }

    _createTiePointsAct = ensurePlainAction(
        _mainWindow, tiePointsMenu, tiePointsMenu, QStringLiteral("actionCreateTiePoints"), tr("创建连接点..."));
    _createTiePointsAct->setToolTip(tr("打开连接点创建参数对话框"));

    _thinTiePointsAct = ensurePlainAction(
        _mainWindow, tiePointsMenu, tiePointsMenu, QStringLiteral("actionThinTiePoints"), tr("稀释连接点..."));
    ...
    _viewTiePointMatchesAct = ensurePlainAction(
        _mainWindow, tiePointsMenu, tiePointsMenu, QStringLiteral("actionViewTiePointMatches"), tr("查看匹配..."));
...
void MainMenu::setManagedWindowActions(const QList<QAction*>& dockActions, const QList<QAction*>& toolBarActions)
{
    if (!_windowMenu)
    {
        return;
    ...
    if (!dockActions.isEmpty() && (!toolBarActions.isEmpty() || _toggleHenanUniversityBrandAct))
    {
                                              ...
                                              const QString& level3Reason)
{
    ...
    const auto updateUnavailableHint =
        [](QAction* action, bool available, const QString& levelLabel, const QString& reason)
    {
        if (!action)
        {
            return;
        ...
        const QString hint =
            available
                ...
                : (reason.trimmed().isEmpty()
                       ? QStringLiteral("%1栅格未保存；请重新生成深度图并保存该级别的可视化栅格。").arg(levelLabel)
                       : reason);
        action->setToolTip(hint);
        action->setStatusTip(hint);
    };
    updateUnavailableHint(_depthOverlayAllLevelsAct, finalAvailable, QStringLiteral("最终层"), finalReason);
    updateUnavailableHint(_depthOverlayLevel1Act, level1Available, QStringLiteral("Level 1"), level1Reason);
    updateUnavailableHint(_depthOverlayLevel2Act, level2Available, QStringLiteral("Level 2"), level2Reason);
    updateUnavailableHint(_depthOverlayLevel3Act, level3Available, QStringLiteral("Level 3"), level3Reason);
    updateImageActionAvailability();
}
...
QAction* MainMenu::toggleLogAction() const
{
    return _toggleLogAct;
}
QAction* MainMenu::toggleMainToolbarAction() const
{
    return _toggleMainToolbarAct;
}
QAction* MainMenu::restoreDefaultWindowLayoutAction() const
{
    return _restoreDefaultWindowLayoutAct;
}
QToolBar* MainMenu::toolBar() const
{
    return _toolBar;
}

QAction* MainMenu::newAction() const
{
    return _newAct;
}
QAction* MainMenu::openAction() const
{
    return _openAct;
}
QAction* MainMenu::saveAction() const
{
    return _saveAct;
}
QAction* MainMenu::minimizeAction() const
{
    return _minimizeAct;
}

QAction* MainMenu::zoomInAction() const
{
    return _zoomInAct;
}
QAction* MainMenu::zoomOutAction() const
{
    return _zoomOutAct;
}
QAction* MainMenu::resetViewAction() const
{
    return _resetViewAct;
}
QAction* MainMenu::toggleFullScreenAction() const
{
    return _toggleFullScreenAct;
}
QAction* MainMenu::rotateImageLeftAction() const
{
    return _rotateImageLeftAct;
}
QAction* MainMenu::rotateImageRightAction() const
{
    return _rotateImageRightAct;
}
QAction* MainMenu::showFeaturePointsAction() const
{
    return _showFeaturePointsAct;
}
QAction* MainMenu::showFeatureResidualsAction() const
{
    return _showFeatureResidualsAct;
}
QAction* MainMenu::showMaskOverlayAction() const
{
    return _showMaskOverlayAct;
}
QAction* MainMenu::rectangleMaskAction() const
{
    return _rectangleMaskAct;
}
QAction* MainMenu::scissorsMaskAction() const
{
    return _scissorsMaskAct;
}
QAction* MainMenu::smartPaintMaskAction() const
{
    return _smartPaintMaskAct;
}
QAction* MainMenu::magicWandMaskAction() const
{
    return _magicWandMaskAct;
}
QAction* MainMenu::maskEditorSettingsAction() const
{
    return _maskEditorSettingsAct;
}
QAction* MainMenu::resetMaskSelectionAction() const
{
    return _resetMaskSelectionAct;
}
QAction* MainMenu::showDepthOverlayAction() const
{
    return _showDepthOverlayAct;
}
QAction* MainMenu::depthOverlayAllLevelsAction() const
{
    return _depthOverlayAllLevelsAct;
}
QAction* MainMenu::depthOverlayLevel1Action() const
{
    return _depthOverlayLevel1Act;
}
QAction* MainMenu::depthOverlayLevel2Action() const
{
    return _depthOverlayLevel2Act;
}
QAction* MainMenu::depthOverlayLevel3Action() const
{
    return _depthOverlayLevel3Act;
}
QAction* MainMenu::showDepthIntensityAction() const
{
    return _showDepthIntensityAct;
}
QAction* MainMenu::toggleGizmoAction() const
{
    return _toggleGizmoAct;
}
QAction* MainMenu::toggleCamerasAction() const
{
    return _toggleCamerasAct;
}
QAction* MainMenu::toggleCameraThumbnailsAction() const
{
    return _toggleCameraThumbnailsAct;
}
QAction* MainMenu::toggleLocalAxesAction() const
{
    return _toggleLocalAxesAct;
}
QAction* MainMenu::toggleCameraImagesAction() const
{
    return _toggleCameraImagesAct;
}
QAction* MainMenu::showCameraImagesInForegroundAction() const
{
    return _showCameraImagesInForegroundAct;
}
QAction* MainMenu::showCameraImagesInBackgroundAction() const
{
    return _showCameraImagesInBackgroundAct;
}
QAction* MainMenu::lockCameraImageAction() const
{
    return _lockCameraImageAct;
}
QAction* MainMenu::tiePointColorModeAction() const
{
    return _tiePointColorModeAct;
}
QAction* MainMenu::tiePointElevationModeAction() const
{
    return _tiePointElevationModeAct;
}
QAction* MainMenu::tiePointImageCountModeAction() const
{
    return _tiePointImageCountModeAct;
}
QAction* MainMenu::modelTextureModeAction() const
{
    return _modelTextureModeAct;
}
QAction* MainMenu::modelShadedModeAction() const
{
    return _modelShadedModeAct;
}
QAction* MainMenu::modelSolidModeAction() const
{
    return _modelSolidModeAct;
}
QAction* MainMenu::modelWireframeModeAction() const
{
    return _modelWireframeModeAct;
}
QAction* MainMenu::modelElevationModeAction() const
{
    return _modelElevationModeAct;
}
QAction* MainMenu::modelConfidenceModeAction() const
{
    return _modelConfidenceModeAct;
}
QAction* MainMenu::modelAssignedImageModeAction() const
{
    return _modelAssignedImageModeAct;
}
QAction* MainMenu::toggleHenanUniversityBrandAction() const
{
    return _toggleHenanUniversityBrandAct;
}

QAction* MainMenu::addPhotoAction() const
{
    return _addPhotoAct;
}
QAction* MainMenu::addFolderAction() const
{
    return _addFolderAct;
}
QAction* MainMenu::importReferenceAction() const
{
    return _importReferenceAct;
}
QAction* MainMenu::importCameraAction() const
{
    return _importCameraAct;
}
QAction* MainMenu::importPointCloudAction() const
{
    return _importPointCloudAct;
}
QAction* MainMenu::importModelAction() const
{
    return _importModelAct;
}
QAction* MainMenu::featureVisualizationAction() const
{
    return _featureVisualizationAct;
}
QAction* MainMenu::workflowAerialTriangulationAction() const
{
    return _workflowAerialTriangulationAct;
}
QAction* MainMenu::workflowSettingsAction() const
{
    return _workflowSettingsAct;
}
QAction* MainMenu::overlapAnalysisAction() const
{
    return _overlapAnalysisAct;
}
QAction* MainMenu::intersectionCheckAction() const
{
    return _intersectionCheckAct;
}
QAction* MainMenu::intersectionViewResultsAction() const
{
    return _intersectionViewResultsAct;
}
QAction* MainMenu::createDEMAction() const
{
    return _createDEMAct;
}
QAction* MainMenu::generateOrthoAction() const
{
    return _generateOrthoAct;
}
QAction* MainMenu::createPointCloudAction() const
{
    return _createPointCloudAct;
}
QAction* MainMenu::generateModelAction() const
{
    return _generateModelAct;
}
QAction* MainMenu::generateTextureAction() const
{
    return _generateTextureAct;
}

QAction* MainMenu::viewWorkflowReportAction() const
{
    return _viewWorkflowReportAct;
}
QAction* MainMenu::createTiePointsAction() const
{
    return _createTiePointsAct;
}
QAction* MainMenu::thinTiePointsAction() const
{
    return _thinTiePointsAct;
}
QAction* MainMenu::cleanTiePointsAction() const
{
    return _cleanTiePointsAct;
}
QAction* MainMenu::viewTiePointMatchesAction() const
{
    return _viewTiePointMatchesAct;
}
QAction* MainMenu::manualPointCloudPruneAction() const
{
    return _manualPointCloudPruneAct;
}
QAction* MainMenu::navigationAction() const
{
    return _navigationAct;
}
QAction* MainMenu::rectangleSelectionAction() const
{
    return _rectangleSelectionAct;
}
QAction* MainMenu::circleSelectionAction() const
{
    return _circleSelectionAct;
}
QAction* MainMenu::freehandSelectionAction() const
{
    return _freehandSelectionAct;
}
...
{
    return _generateMaskAct;
}
QAction* MainMenu::surveyControlAction() const
{
    return _surveyControlAct;
}
QAction* MainMenu::detectMarkersAction() const
{
    return _detectMarkersAct;
}
QAction* MainMenu::reviewMarkerDetectionsAction() const
{
    return _reviewMarkerDetectionsAct;
}
QAction* MainMenu::printMarkersAction() const
{
    return _printMarkersAct;
}
QAction* MainMenu::importReferenceDatasetAction() const
{
    return _importReferenceDatasetAct;
}
QAction* MainMenu::referenceQualityCheckAction() const
{
    return _referenceQualityCheckAct;
}
QAction* MainMenu::referenceTerrainBundleAdjustAction() const
{
    return _referenceTerrainBundleAdjustAct;
}

QAction* MainMenu::exportMatchedPairsAction() const
{
    return _exportMatchedPairsAct;
}
QAction* MainMenu::exportPortableProjectAction() const
{
    return _exportPortableProjectAct;
}

QAction* MainMenu::toggleWorkspaceAction() const
{
    return _toggleWorkspaceAct;
}
QAction* MainMenu::togglePropertiesAction() const
{
    return _togglePropertiesAct;
}
QAction* MainMenu::toggleWorkAction() const
{
    return _toggleWorkAct;
}
QAction* MainMenu::togglePhotosAction() const
{
    return _togglePhotosAct;
}
