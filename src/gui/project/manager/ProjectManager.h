...
public slots:
    ...
    bool replaceImageCameras(const QStringList& targetImagePaths,
                             ...
                             QString* errorMsg = nullptr);
    /// 清除指定影像的相机参数
    bool clearImageCameras(const QStringList& imagePaths, int* updatedCount = nullptr, QString* errorMsg = nullptr);
    ...
    QMap<QString, xjw::FramePinholeCamera> getCamerasForImages(const QStringList& images,
                                                               bool* hasCamerasForAll = nullptr) const;
    QMap<QString, xjw::RpcCameraModel> getRpcCamerasForImages(const QStringList& images,
                                                              bool* hasCamerasForAll = nullptr) const;
...
private:
    ...
    bool _portableExportActive = false;
    ...
    // 辅助：统一提示框（默认标题“提示”）
    void showWarning(const QString& message, const QString& title = QStringLiteral("提示")) const;
    // 展示当前 BA 关键指标，并要求用户明确保留或丢弃待提交结果。
    void presentBundleAdjustPreview();
    void startImageImport(const QStringList& imagePaths, const QString& sourceLabel);
    void importProjectAsset(bool modelAsset);
    bool rejectLifecycleChangeDuringResourceCleanup(const QString& operation) const;
};
