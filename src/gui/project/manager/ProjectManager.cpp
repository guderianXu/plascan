...
namespace
{
    ...
    QString normalizedDepthBatchDirectory(const QString& path)
    {
        ...
        return QDir::cleanPath(
            (info.exists() && info.isDir() ? info : QFileInfo(info.absolutePath())).absoluteFilePath());
    }

    QString storedDepthBatchQualityProfile(const QJsonObject& metadata, const QString& source_path)
    {
        const QString requested_directory = normalizedDepthBatchDirectory(source_path);
        const QJsonArray records = metadata.value(QStringLiteral("depth_map_results")).toArray();
        for (int index = records.size() - 1; index >= 0; --index)
        {
            const QJsonObject record = records.at(index).toObject();
            QString record_path = record.value(QStringLiteral("mvs_output_dir")).toString();
            if (record_path.isEmpty())
            {
                record_path = record.value(QStringLiteral("raw_depth_path")).toString();
            }
            if (!requested_directory.isEmpty() &&
                normalizedDepthBatchDirectory(record_path).compare(requested_directory, Qt::CaseInsensitive) != 0)
            {
                continue;
            }
    ...
    ImageFolderScan scanImageFolder(const QString& folder)
    {
        ...
        const QFileInfoList files =
            dir.entryInfoList(imageFolderNameFilters(), QDir::Files | QDir::Readable, QDir::Name | QDir::IgnoreCase);
    ...
    bool validateBaPriorImport(const QString& path, QString* errorMessage)
    {
        ...
        if (suffix == QLatin1String("json"))
        {
            ...
            if (!xjw::lidar::loadPlanetaryLaserJsonFile(xjw::common::io::toUtf8Path(path), {}, &dataset, &loadError))
            {
                if (errorMessage)
                {
                    *errorMessage =
    ...
    QString firstReferenceLaserPriorPath(const QJsonObject& meta)
    {
        ...
        for (const QJsonValue& value : references)
        {
            ...
            const QString suffix = QFileInfo(path).suffix().toLower();
            const bool laserType = type == QLatin1String("lidar") || type == QLatin1String("reference_lidar") ||
    ...
    QString firstPlanetaryLaserPriorPath(const QJsonObject& meta)
    {
        ...
        for (const QJsonValue& value : references)
        {
            const QJsonObject reference = value.toObject();
            if (!isBaPriorRole(reference.value(QStringLiteral("role")).toString()) ||
                reference.value(QStringLiteral("type")).toString().toLower() != QLatin1String("planetary_laser_shots"))
            {
                continue;
            }
            const QString path = reference.value(QStringLiteral("path")).toString().trimmed();
    ...
    void appendMetaArrayRecord(QJsonObject* meta, const QString& arrayKey, const QJsonObject& record)
    {
...
ProjectManager::ProjectManager(ProjectData* projectData, QWidget* parent)
    : QObject(parent), _parent(parent), _projectData(projectData),
      _sessionFacade(std::make_unique<xjw::gui::project::ProjectSessionFacade>(projectData)),
      _sparseReconstructionManager(new ProjectSparseReconstructionManager(this, projectData, parent, this)),
      _pointCloudWorkflowController(new ProjectPointCloudWorkflowController(this, projectData, parent, this)),
...
{
    _uiCommands->setDirectoryAccessors([this](const QString& key) { return getLastUsedDir(key); },
                                       [this](const QString& key, const QString& dir) { saveLastUsedDir(key, dir); });

    // 连接ProjectData信号
    if (_projectData)
    {
        xjw::core::project::ProjectResourceCleanupService::installAutomaticRecovery(_projectData);
        ...
        connect(_projectData, &ProjectData::projectOpened, this, advanceSessionGeneration);
        connect(_projectData, &ProjectData::projectClosed, this, advanceSessionGeneration);
        connect(_projectData,
                &ProjectData::activeChunkChanged,
                this,
                [advanceSessionGeneration](const QString&, const QString&, int) { advanceSessionGeneration(); });
        connect(_projectData, &ProjectData::projectOpened, this, &ProjectManager::projectOpened);
        connect(_projectData, &ProjectData::projectSaved, this, &ProjectManager::projectSaved);
        connect(_projectData, &ProjectData::projectClosed, this, &ProjectManager::projectClosed);
        connect(_projectData, &ProjectData::chunkListChanged, this, &ProjectManager::chunkListChanged);
        connect(_projectData,
                ...
                [this](const QJsonObject& meta)
                {
                    emit projectMetadataChanged(
                        xjw::gui::project::ProjectTiePointResultService::metadataWithCurrentOnly(meta,
                                                                                                 currentProjectPath()));
                });
    ...
    connect(_lifecycleController,
            &ProjectLifecycleController::projectOpenStarted,
            this,
    ...
    connect(_lifecycleController,
            &ProjectLifecycleController::projectOpenProgressChanged,
            this,
            &ProjectManager::projectOpenProgressChanged);
    connect(_lifecycleController,
            &ProjectLifecycleController::projectOpenFinished,
            this,
            &ProjectManager::projectOpenFinished);
    connect(_lifecycleController, &ProjectLifecycleController::saveStarted, this, &ProjectManager::saveStarted);
    connect(_lifecycleController, &ProjectLifecycleController::saveFinished, this, &ProjectManager::saveFinished);
    connect(_maskWorkflowController,
    ...
    connect(
        _maskWorkflowController, &ProjectMaskWorkflowController::masksGenerated, this, &ProjectManager::masksGenerated);
    connect(_maskWorkflowController,
    ...
    connect(_modelManager, &ProjectModelManager::meshProgressChanged, this, &ProjectManager::meshProgressChanged);
    connect(_modelManager, &ProjectModelManager::meshProgressFinished, this, &ProjectManager::meshProgressFinished);
    ...
    connect(_pointCloudWorkflowController,
            ...
            [this](bool success)
            {
                ...
                const bool missing_depth_batch = success && !_pendingAutomaticModelSettings.isEmpty();
    ...
    connect(_pointCloudWorkflowController,
            ...
            [this](const QString& output_directory, int)
            {
                ...
                model_settings[QStringLiteral("depthMapSourcePath")] = output_directory;
...
{
    if (rejectLifecycleChangeDuringResourceCleanup(QStringLiteral("新建项目")))
    {
        return;
    }
...
{
    if (rejectLifecycleChangeDuringResourceCleanup(QStringLiteral("打开项目")))
    {
        return;
    }
...
{
    if (rejectLifecycleChangeDuringResourceCleanup(QStringLiteral("保存项目")))
    {
        return;
    }
...
{
    if (rejectLifecycleChangeDuringResourceCleanup(QStringLiteral("关闭项目")))
    {
        return;
    }
    if (_modelManager)
...
void ProjectManager::addFolder()
{
    ...
    if (_imageImportActive)
    {
        QMessageBox::information(
            _parent, QStringLiteral("提示"), QStringLiteral("已有影像加载任务正在运行，请等待完成后再试。"));
        return;
    }
    ...
    xjw::gui::tasks::runGuardedWithOutcome(
        this,
        [folder]() -> ImageFolderScan { return scanImageFolder(folder); },
        [folder, session](ProjectManager* self, xjw::gui::tasks::TaskOutcome<ImageFolderScan> outcome)
        {
            ...
            if (!outcome.succeeded())
            {
                ...
                QMessageBox::critical(self->_parent,
                                      QStringLiteral("错误"),
                                      QStringLiteral("添加文件夹失败: %1").arg(outcome.errorMessage));
                return;
            }
            ...
            if (!scan.success)
            {
                ...
                QMessageBox::critical(
                    self->_parent, QStringLiteral("错误"), QStringLiteral("添加文件夹失败: %1").arg(scan.errorMessage));
                return;
            }
...
void ProjectManager::startImageImport(const QStringList& imagePaths, const QString& sourceLabel)
{
    ...
    xjw::gui::tasks::runGuardedWithOutcome(
        ...
        [session, sourceLabel, total](ProjectManager* self, xjw::gui::tasks::TaskOutcome<ImageImportBatch> outcome)
        {
            ...
            if (!batch.success)
            {
                ...
                QMessageBox::critical(
                    self->_parent, QStringLiteral("错误"), QStringLiteral("加载影像失败: %1").arg(batch.errorMessage));
                return;
            }
            ...
            if (!self->_projectData->addImagesFromSharedStore(batch.projectImagePaths, batch.skipped, &message))
            {
                ...
                QMessageBox::critical(
                    self->_parent, QStringLiteral("错误"), QStringLiteral("提交影像元数据失败: %1").arg(message));
                return;
            }
            ...
            QString finishedMessage =
                batch.skipped > 0 ? QStringLiteral("已加载 %1 张影像，跳过 %2 张重复影像").arg(added).arg(batch.skipped)
                                  : QStringLiteral("已加载 %1 张影像").arg(added);
            if (importedRpcCameras > 0)
            {
                finishedMessage += QStringLiteral("，自动读取 %1 个 RPC 相机").arg(importedRpcCameras);
            }
            LOG_INFO(QStringLiteral("%1（来源：%2，共选择 %3 张）").arg(finishedMessage, sourceLabel).arg(total));
...
void ProjectManager::importProjectAsset(bool modelAsset)
{
    ...
    if (!requireOpenProject(
            _projectData, _parent, QStringLiteral("请先打开或创建项目，再导入%1。").arg(assetName), dialogTitle))
    {
        return;
    }

    const QString dialogKey = modelAsset ? QStringLiteral("import_model") : QStringLiteral("import_point_cloud");
    const QString filter = modelAsset ? QStringLiteral("Metashape/通用模型 (*.obj *.ply);;OBJ 模型 (*.obj);;"
                                                       "PLY 模型 (*.ply);;所有文件 (*)")
                                      : QStringLiteral("Metashape/通用点云 (*.obj *.ply *.xyz);;OBJ 点云 (*.obj);;"
                                                       "PLY 点云 (*.ply);;XYZ 点云 (*.xyz);;所有文件 (*)");
    const QString selectedPath = QFileDialog::getOpenFileName(_parent, dialogTitle, getLastUsedDir(dialogKey), filter);
    ...
    request.type =
        modelAsset ? xjw::common::project::ProjectAssetType::Model : xjw::common::project::ProjectAssetType::PointCloud;
    request.sourcePath = selectedPath;
    request.projectRoot = xjw::common::project::ProjectIO::projectRootFromPlascan(currentProjectPath());
    const auto session = currentSessionContext();

    xjw::gui::tasks::runGuardedWithOutcome(
        this,
        [request]() { return xjw::common::project::ProjectAssetImporter::importAsset(request); },
        [dialogTitle, modelAsset, session](
            ProjectManager* self, xjw::gui::tasks::TaskOutcome<xjw::common::project::ProjectAssetImportResult> outcome)
        {
            if (!self || !self->_projectData)
            {
                return;
            ...
            {
                QMessageBox::critical(
                    self->_parent, dialogTitle, QStringLiteral("导入失败: %1").arg(outcome.errorMessage));
                return;
            }

            const xjw::common::project::ProjectAssetImportResult result = std::move(*outcome.value);
            if (!result.success)
            {
                QMessageBox::critical(self->_parent,
                                      dialogTitle,
                                      result.errorMessage.isEmpty() ? QStringLiteral("导入失败。")
                                                                    : result.errorMessage);
                return;
            }
                    ...
                    result.resultArrayKey, result.resultPathKey, result.projectRecord, true))
            {
                ...
                QMessageBox::critical(
                    self->_parent, dialogTitle, QStringLiteral("资源已读取，但无法写入项目成果记录。"));
                return;
            }

            QString message =
                QStringLiteral("已导入 %1\n%2\n顶点/点数: %3")
                    .arg(modelAsset ? QStringLiteral("模型") : QStringLiteral("点云"), result.importedPath)
            ...
            LOG_INFO(QStringLiteral("Metashape %1已导入: %2 -> %3")
                         .arg(modelAsset ? QStringLiteral("模型") : QStringLiteral("点云"),
                              result.sourcePath,
                              result.importedPath));
        });
}
...
{
    return _cameraSetupManager && _cameraSetupManager->initializeCamerasFromExifOrDefault(settings);
}
...
{
    if (!requireOpenProject(_projectData, _parent, QStringLiteral("请先打开项目，再导入参考 DEM/LiDAR。")))
    {
        return;
    }
    ...
    const QString purpose = QInputDialog::getItem(_parent,
    ...
    const QString role = purpose == baPurpose ? QStringLiteral("ba_prior") : QStringLiteral("validation");

    QString compatibilityError;
    if (role == QLatin1String("ba_prior") && !validateBaPriorImport(selected, &compatibilityError))
    {
    ...
    LOG_INFO(QStringLiteral("参考数据已导入: %1 role=%2").arg(QFileInfo(selected).fileName(), role));
}
...
{
    if (!requireOpenProject(_projectData, _parent, QStringLiteral("请先打开项目，再管理测绘控制点。")))
    {
        return;
    }
    ...
    dialog.setSurveyControlMetadata(xjw::gui::project::surveyControlDialogMetadata(_projectData, &metadataError));
    ...
    connect(&dialog,
            ...
            [this, &dialog]()
            {
                ...
                const auto result = xjw::gui::project::importSurveyControlCsv(_projectData, selected, QString());
                if (!result.imported)
                {
                    showWarning(result.errorMessage.isEmpty() ? QStringLiteral("导入测绘控制 CSV 失败。")
                                                              : result.errorMessage,
                                QStringLiteral("测绘控制"));
                    return;
                }

                dialog.setSurveyControlMetadata(xjw::gui::project::surveyControlDialogMetadata(_projectData));
...
{
    if (!requireOpenProject(_projectData, _parent, QStringLiteral("请先打开项目，再执行点云/DEM 精度检查。")))
    {
        return;
    }
    ...
    if (!result.saved)
    {
        showWarning(result.errorMessage.isEmpty() ? QStringLiteral("生成点云/DEM 精度检查报告失败。")
                                                  : result.errorMessage,
...
{
    if (!requireOpenProject(_projectData, _parent, QStringLiteral("请先打开项目，再使用参考地形约束重新平差。")))
    {
        return;
    }
    ...
    if (!result.saved)
    {
        showWarning(result.errorMessage.isEmpty() ? QStringLiteral("生成参考地形平差前置检查报告失败。")
                                                  : result.errorMessage,
    ...
    {
        QMessageBox::warning(
            _parent, QStringLiteral("参考地形约束重新平差"), QStringLiteral("项目影像少于 2 张，无法执行 BA。"));
        return;
    }

    const QString bundleAdjustDir = xjw::common::project::ProjectIO::projectBundleAdjustDir(currentProjectPath());
    const QString outputDir =
        QDir(bundleAdjustDir)
            .filePath(QStringLiteral("%1_%2")
                          .arg(usePlanetaryLaser ? QStringLiteral("planetary_laser_range")
                                                 : (useLaserSurface ? QStringLiteral("reference_laser_surface")
    ...
    if (usePlanetaryLaser)
    {
        ...
        if (!xjw::lidar::loadPlanetaryLaserJsonFile(
                xjw::common::io::toUtf8Path(planetaryLaserPriorPath), {}, &planetaryLaserDataset, &laserError))
        {
            showWarning(QStringLiteral("读取行星激光 SI JSON 失败: %1").arg(QString::fromStdString(laserError)),
                        QStringLiteral("行星激光测距平差"));
            return;
        }
        if (planetaryLaserDataset.sensorModel == xjw::lidar::PlanetaryLaserSensorModel::LineScan)
        {
            QMessageBox::information(
                _parent,
                QStringLiteral("行星激光测距平差"),
        ...
        if (planetaryLaserDataset.rangeType == xjw::lidar::PlanetaryLaserRangeType::RoundTrip)
        {
            showWarning(QStringLiteral("该数据 range_type=round_trip，请先按产品定义换算为单程几何距离。"),
                        QStringLiteral("行星激光测距平差"));
            return;
        }
        if (planetaryLaserDataset.sensorModel == xjw::lidar::PlanetaryLaserSensorModel::Unknown)
        {
        ...
        if (planetaryLaserDataset.rangeType == xjw::lidar::PlanetaryLaserRangeType::Unknown)
        {
        ...
        const QString datasetFrame = QString::fromStdString(planetaryLaserDataset.reference.bodyFixedFrame);
        planetaryCameraCoordinateFrame =
            QInputDialog::getText(_parent,
        ...
        {
            showWarning(QStringLiteral("相机坐标系 %1 与激光坐标系 %2 不一致；请先完成坐标转换。")
                            .arg(planetaryCameraCoordinateFrame, datasetFrame),
                        QStringLiteral("行星激光测距平差"));
            return;
        }

        const bool hasNonZeroLeverArm =
            std::any_of(planetaryLaserDataset.shots.begin(),
        ...
        planetaryCameraSensorFrame = QString::fromStdString(planetaryLaserDataset.reference.laserFrame);
        if (hasNonZeroLeverArm)
        {
            ...
            if (!sensorFrameAccepted ||
                planetaryCameraSensorFrame != QString::fromStdString(planetaryLaserDataset.reference.laserFrame))
            {
                showWarning(QStringLiteral("非零杆臂框架不一致，无法安全建立测距方程。"),
                            QStringLiteral("行星激光测距平差"));
                return;
            }
        }
    ...
    {
        bool accepted = false;
        laserAssociationDistanceMeters = QInputDialog::getDouble(_parent,
        ...
        laserSigmaMeters = QInputDialog::getDouble(_parent,
                                                   QStringLiteral("LiDAR 平差参数"),
        ...
        laserHuberDeltaMeters = QInputDialog::getDouble(_parent,
                                                        QStringLiteral("LiDAR 平差参数"),
    ...
    if (usePlanetaryLaser)
    {
        ...
        prompt =
            QStringLiteral("将使用 ISIS 风格的稀疏 laser-range shot 约束 frame-camera BA。\n\n"
                           "数据: %1\n"
                           "目标/坐标系: %2 / %3\n"
                           "shot 数量: %4\n"
                           "sensor/range: %5 / %6\n"
                ...
                .arg(QString::fromStdString(planetaryLaserDataset.reference.bodyFixedFrame))
                .arg(static_cast<int>(planetaryLaserDataset.shots.size()))
                .arg(QString::fromLatin1(xjw::lidar::planetaryLaserSensorModelName(planetaryLaserDataset.sensorModel)))
                .arg(QString::fromLatin1(xjw::lidar::planetaryLaserRangeTypeName(planetaryLaserDataset.rangeType)))
                .arg(images.size())
                .arg(outputDir);
    }
    ...
    const QMessageBox::StandardButton choice =
        QMessageBox::question(_parent, dialogTitle, prompt, QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
    ...
    if (usePlanetaryLaser)
    {
        ...
        extra[QStringLiteral("planetary_laser_camera_coordinate_frame")] = planetaryCameraCoordinateFrame;
        extra[QStringLiteral("planetary_laser_camera_sensor_frame")] = planetaryCameraSensorFrame;
        extra[QStringLiteral("planetary_laser_confirm_unknown_sensor_is_frame")] = confirmUnknownSensorIsFrame;
        extra[QStringLiteral("planetary_laser_confirm_unknown_range_is_one_way")] = confirmUnknownRangeIsOneWay;
    ...
    else if (useLaserSurface)
    {
        ...
        extra[QStringLiteral("laser_association_max_distance_m")] = laserAssociationDistanceMeters;
...
void ProjectManager::deleteGeneratedData(const QString& section, const QStringList& resourcePaths)
{
    ...
    {
        QMessageBox::information(
            _parent, QStringLiteral("删除数据"), QStringLiteral("已有资源清理任务正在运行，请等待完成后再试。"));
        return;
    }
    if (section == QStringLiteral("照片"))
    {
        QMessageBox::information(
            _parent, QStringLiteral("删除数据"), QStringLiteral("照片分组不支持删除数据，请使用移除引用。"));
        return;
    }

    const int selectedCount = resourcePaths.size();
    const bool deletingTiePoints = section == QStringLiteral("连接点");
    const QString dialogTitle = deletingTiePoints ? QStringLiteral("移除连接点") : QStringLiteral("删除数据");
    const QMessageBox::StandardButton confirm = QMessageBox::question(
        _parent,
        dialogTitle,
        deletingTiePoints ? QStringLiteral("确定移除当前连接点及其关联生成文件吗？此操作不可撤销。")
                          : (selectedCount == 1
    ...
    if (section == QStringLiteral("连接点"))
    {
        ...
        if (!result.success)
        {
            QMessageBox::warning(_parent, dialogTitle, QStringLiteral("删除失败：%1").arg(result.errorMessage));
            return;
        }

        refreshReconstructionQualityReport();
        QMessageBox::information(_parent, dialogTitle, QStringLiteral("已移除连接点及其关联生成文件。"));
        return;
    }

    const auto presentResult = [this, section](const xjw::core::project::ResourceCleanupResult& cleanupResult)
    {
        if (cleanupResult.unsupportedSection)
        {
            QMessageBox::warning(
                _parent, QStringLiteral("删除数据"), QStringLiteral("当前分组暂不支持删除数据：%1").arg(section));
            return;
        }
        ...
        {
            QMessageBox::warning(
                _parent, QStringLiteral("删除数据"), QStringLiteral("删除失败：%1").arg(cleanupResult.errorMessage));
            return;
        }
        ...
        {
            QMessageBox::information(
                _parent, QStringLiteral("删除数据"), QStringLiteral("未找到可删除的%1数据记录。").arg(section));
            return;
        }
    ...
    const auto prepared = xjw::core::project::ProjectResourceCleanupService::prepareGeneratedDataCleanup(
        _projectData, section, resourcePaths);
    ...
    auto shutdownResult = std::make_shared<xjw::core::project::ResourceCleanupResult>(prepared.preparationResult());
    const QPointer<ProjectData> cleanupProjectData(_projectData);
    _resourceCleanupShutdownFinalize = [cleanupProjectData, prepared, requestWidget, shutdownResult]()
    {
        if (cleanupProjectData)
        {
            xjw::core::project::ProjectResourceCleanupService::finalizePreparedCleanup(
                cleanupProjectData.data(), prepared, *shutdownResult);
        }
        if (requestWidget)
        {
            requestWidget->setEnabled(true);
    ...
    _resourceCleanupFuture = xjw::gui::tasks::runGuardedWithOutcome(
        ...
        {
            const auto result = xjw::core::project::ProjectResourceCleanupService::executePreparedCleanup(prepared);
        ...
        {
            xjw::core::project::ResourceCleanupResult cleanupResult = prepared.preparationResult();
            if (outcome.succeeded())
            {
            ...
            const bool finalized = xjw::core::project::ProjectResourceCleanupService::finalizePreparedCleanup(
            ...
            {
                LOG_INFO(QStringLiteral("资源清理完成时项目会话或持久化代次已变化，忽略旧任务 UI 结果"));
                return;
            }
...
{
    if (rejectLifecycleChangeDuringResourceCleanup(QStringLiteral("新建 Chunk")))
    {
        return;
    }
    ...
    const QString name =
        QInputDialog::getText(
            _parent, QStringLiteral("新建 Chunk"), QStringLiteral("名称："), QLineEdit::Normal, QString(), &accepted)
            .trimmed();
    if (!accepted)
    {
        return;
    ...
    {
        QMessageBox::critical(_parent, QStringLiteral("新建 Chunk 失败"), error);
    }
}

void ProjectManager::renameChunk(const QString& chunkId)
{
    if (rejectLifecycleChangeDuringResourceCleanup(QStringLiteral("重命名 Chunk")))
    {
        return;
    }
    ...
    for (const QJsonValue& value : _projectData->chunks())
    {
        ...
        {
            currentName = chunk.value(QStringLiteral("name")).toString();
            break;
        }
    }
    bool accepted = false;
    const QString name = QInputDialog::getText(_parent,
    ...
    {
        QMessageBox::critical(_parent, QStringLiteral("重命名 Chunk 失败"), error);
    }
}

void ProjectManager::removeChunk(const QString& chunkId)
{
    if (rejectLifecycleChangeDuringResourceCleanup(QStringLiteral("删除 Chunk")))
    {
        return;
    }
    if (!_projectData || chunkId.trimmed().isEmpty())
    ...
    {
        QMessageBox::critical(_parent, QStringLiteral("删除 Chunk 失败"), error);
    }
}

void ProjectManager::switchChunk(const QString& chunkId)
{
    if (rejectLifecycleChangeDuringResourceCleanup(QStringLiteral("切换 Chunk")))
    {
        return;
    }
    if (!_projectData || chunkId.trimmed().isEmpty())
    ...
    {
        QMessageBox::critical(_parent, QStringLiteral("切换 Chunk 失败"), error);
    }
}
...
bool ProjectManager::rejectLifecycleChangeDuringResourceCleanup(const QString& operation) const
{
    ...
    QMessageBox::information(
        _parent, QStringLiteral("资源清理进行中"), QStringLiteral("资源清理任务完成前无法%1，请稍候。").arg(operation));
    return true;
}
...
{
    return {currentProjectPath(), _sessionFacade->activeChunkId(), _projectSessionGeneration};
}

bool ProjectManager::isCurrentSession(const xjw::gui::project::ProjectSessionContext& context) const
{
...
void ProjectManager::refreshReconstructionQualityReport()
{
    ...
    const auto reportResult = xjw::gui::project::writeReconstructionQualityProjectReport(_projectData);
...
void ProjectManager::appendImageMatchResults(const QVector<ProjectImageMatchResultRecord>& records)
{
    ...
    const QStringList& images, const QString& outputDir, int threads, bool dryRun, const QJsonObject& extraSettings)
{
    ...
    opts.baOpt.maxAcceptedConstraintRmsGrowth =
        qMax(1.0,
             extraSettings.value(QStringLiteral("ba_max_accepted_constraint_rms_growth"))
                 .toDouble(opts.baOpt.maxAcceptedConstraintRmsGrowth));
    ...
    opts.baOpt.maxAcceptedRmsGrowth = qMax(
        0.0,
        extraSettings.value(QStringLiteral("ba_max_accepted_rms_growth")).toDouble(opts.baOpt.maxAcceptedRmsGrowth));
    ...
    opts.exportObservationDetails = extraSettings.value(QStringLiteral("export_observation_details")).toBool(true);
    ...
    opts.laserAssociationMaxDistanceMeters =
        qMax(0.0, extraSettings.value(QStringLiteral("laser_association_max_distance_m")).toDouble(0.05));
    opts.laserVoxelSizeMeters = qMax(0.0, extraSettings.value(QStringLiteral("laser_voxel_size_m")).toDouble(0.0));
    opts.laserMaxCurvature = qBound(0.0, extraSettings.value(QStringLiteral("laser_max_curvature")).toDouble(0.2), 1.0);
    opts.laserMaxSamples = qBound(1, extraSettings.value(QStringLiteral("laser_max_samples")).toInt(500000), 10000000);
    ...
    opts.laserWeight = qMax(0.0, extraSettings.value(QStringLiteral("laser_weight")).toDouble(0.0));
    opts.laserSigmaMeters = qMax(1e-9, extraSettings.value(QStringLiteral("laser_sigma_m")).toDouble(0.0025));
    opts.laserHuberDeltaMeters = qMax(1e-9, extraSettings.value(QStringLiteral("laser_huber_delta_m")).toDouble(0.05));
    ...
    opts.planetaryLaserDataPath = extraSettings.value(QStringLiteral("planetary_laser_data_path")).toString().trimmed();
    ...
    opts.referenceTerrainSigmaMeters =
    ...
    opts.referenceTerrainMaxAssociationDistanceMeters =
        qMax(0.0, extraSettings.value(QStringLiteral("reference_terrain_max_association_distance_m")).toDouble(2.0));
    opts.referenceTerrainHuberDeltaMeters =
    ...
    opts.baOpt.progressCallback = [baProgressSelf, cancelFlag, session](
                                      int currentIteration, int maxIterations, double avgRms, int validPoints) -> bool
    {
        ...
        const int percent =
            qBound(10, 10 + static_cast<int>(std::lround(80.0 * currentIteration / safeMaxIterations)), 90);
        ...
        QMetaObject::invokeMethod(
            ...
            [baProgressSelf, cancelFlag, session, stage, percent]()
            {
                ...
                if (baProgressSelf->_atCancelFlag != cancelFlag || !baProgressSelf->isCurrentSession(session) ||
    ...
    xjw::gui::tasks::runGuardedWithOutcome(
         ...
         session]() mutable
        {
            ...
            QMetaObject::invokeMethod(
                ...
                [self, cancelFlag, session]()
                {
                    ...
                    if (self->_atCancelFlag != cancelFlag || !self->isCurrentSession(session) ||
                        cancelFlag->load(std::memory_order_relaxed))
                    {
                        return;
                    }
            ...
            const BundleAdjustExecutionResult executionResult =
                runBundleAdjustExecution(coreData, plascanPath, images, minMatches, std::move(opts));
            ...
            LOG_INFO(QStringLiteral("BA: 启动后台平差计算 (%1 台相机, %2 条轨迹)...")
        ...
        [cancelFlag, session](ProjectManager* manager, xjw::gui::tasks::TaskOutcome<void> outcome)
        {
            ...
            QMessageBox::warning(manager->_parent, QStringLiteral("光束法平差"), outcome.errorMessage);
        });
}
...
// 空中三角测量相关
// ==============================================================================
bool ProjectManager::setImageCameras(const QMap<QString, QJsonObject>& cameras, int* updatedCount, QString* errorMsg)
{
...
bool ProjectManager::replaceImageCameras(const QStringList& targetImagePaths,
                                         ...
                                         QString* errorMsg)
{
    return _sessionFacade->replaceImageCameras(targetImagePaths, cameras, updatedCount, clearedCount, errorMsg);
}

bool ProjectManager::clearImageCameras(const QStringList& imagePaths, int* updatedCount, QString* errorMsg)
{
    return _sessionFacade->clearImageCameras(imagePaths, updatedCount, errorMsg);
}

QMap<QString, xjw::FramePinholeCamera> ProjectManager::getCamerasForImages(const QStringList& images,
...
QMap<QString, xjw::RpcCameraModel> ProjectManager::getRpcCamerasForImages(const QStringList& images,
                                                                          bool* hasCamerasForAll) const
{
    if (hasCamerasForAll)
        *hasCamerasForAll = true;
    ...
    for (const QString& imagePath : images)
    {
        ...
        const bool loadedFromProject =
            xjw::common::project::imageCameraFromEntry(imageMetaByPath.value(normalized), &camera);
...
void ProjectManager::startGenerateModelAsync(const QJsonObject& settings)
{
    ...
    const auto stored_depth_compatibility =
        source_data == QStringLiteral("depth_maps") && !depth_source.isEmpty()
            ? xjw::gui::project::assessStoredDepthBatchCompatibility(
                  ...
                  xjw::gui::project::depthBatchRequirementsForModelSettings(settings))
            : xjw::gui::project::StoredDepthBatchCompatibility{};
    const bool stored_depth_batch_incompatible = !depth_source.isEmpty() && !stored_depth_compatibility.compatible;
    ...
    {
        LOG_INFO(QStringLiteral("[模型生成] 现有深度图批次不兼容，将自动重新估计：%1")
...
void ProjectManager::startDemFromPointCloudAsync(const xjw::gui::project::DemGenerationRequest& request)
{
...
void ProjectManager::startMapProjectAsync(const xjw::gui::project::OrthoGenerationRequest& request)
{
    if (_terrainProductsManager)
    {
...
void ProjectManager::presentBundleAdjustPreview()
{
    ...
    {
        const auto presentation = buildBundleAdjustPreviewPresentation(_pendingBaResult, _pendingBaCameraMeta.size());
        ...
        message_box.setIcon(presentation.qualityWarning ? QMessageBox::Warning : QMessageBox::Question);
        ...
        QPushButton* keep_button = message_box.addButton(QStringLiteral("保留结果"), QMessageBox::AcceptRole);
        QPushButton* discard_button = message_box.addButton(QStringLiteral("丢弃结果"), QMessageBox::DestructiveRole);
...
bool ProjectManager::acceptBundleAdjustPreview(QString* errorMsg)
{
    ...
    {
        if (errorMsg)
            *errorMsg = QStringLiteral("当前没有可应用的平差预览结果");
        return false;
    }

    const QStringList allImages = _projectData ? _projectData->getAllImages() : QStringList();
    const auto commitResult = commitBundleAdjustPreview(_projectData, _pendingBaCameraMeta, _pendingBaResult);
    ...
    const auto artifactsResult =
        finalizeBundleAdjustArtifacts(assetsDir,
                                      _pendingBaResult,
                                      allImages,
    ...
    {
        LOG_WARN(QStringLiteral("写入 BA 点云 sidecar 失败: %1").arg(artifactsResult.sparseCloudExport.errorMessage));
    }
...
{
    const auto result = xjw::gui::project::replaceTiePointResult(
...
{
    if (!_pendingAutomaticModelSettings.isEmpty() && _pointCloudWorkflowController)
    {
