...
TEST(SfmQualityReportTest, SummarizesTrackErrorAngleAndCoverage)
{
    ...
    EXPECT_EQ(report.totalImageCount, 5);
    EXPECT_EQ(report.registeredImageCount, 4);
    EXPECT_EQ(report.pointCount, 3);
    EXPECT_EQ(report.twoViewTrackCount, 1);
    EXPECT_EQ(report.multiViewTrackCount, 2);
    EXPECT_EQ(report.weakTrackCount, 1);
    EXPECT_EQ(report.weakTriangulationAngleCount, 1);
    EXPECT_EQ(report.highReprojectionErrorCount, 1);
    EXPECT_NEAR(report.trackLength.mean, 3.0, 1e-9);
    EXPECT_NEAR(report.reprojectionError.p95, 4.2, 1e-9);
    EXPECT_NEAR(report.triangulationAngle.min, 1.0, 1e-9);
    EXPECT_NEAR(report.observationGridCoverageMean, 7.0 / 12.0, 1e-9);
...
TEST(SfmQualityReportTest, EmptyInputProducesStableJson)
{
    ...
    const xjw::SfmQualityReport report = xjw::analyzeSfmQuality({}, options);
    const QJsonObject json = report.toJson();

    EXPECT_EQ(report.pointCount, 0);
    EXPECT_EQ(report.trackLength.count, 0);
    EXPECT_EQ(report.observationGridCoverageMean, 0.0);
...
TEST(SfmQualityReportTest, FlagsPoorObservationSpatialCoverage)
{
    ...
    EXPECT_LT(report.observationGridCoverageMean, 0.20);
    EXPECT_FALSE(report.acceptableForMvs);
    EXPECT_TRUE(report.qualityWarnings.contains(QStringLiteral("poor_observation_spatial_coverage")));
