...
class RepoHygieneTest(unittest.TestCase):
    ...
    def test_pinned_qt_opencv_source_build_is_self_contained(self):
        ...
        apriltag_start = superbuild.index("ExternalProject_Add(plascan_apriltag_source")
        gdal_start = superbuild.index("ExternalProject_Add(plascan_gdal_source")
        apriltag_block = superbuild[apriltag_start:gdal_start]
        gdal_block = superbuild[
            gdal_start:superbuild.index("add_custom_target(plascan_source_dependencies")
        ]
        self.assertNotIn("GDAL_ENABLE_DRIVER", apriltag_block)
        self.assertNotIn("OGR_ENABLE_DRIVER", apriltag_block)
        self.assertIn('"-UGDAL_ENABLE_DRIVER_*"', gdal_block)
        self.assertIn('"-UOGR_ENABLE_DRIVER_*"', gdal_block)
        self.assertIn('"-DGDAL_BUILD_OPTIONAL_DRIVERS:BOOL=OFF"', gdal_block)
        self.assertIn('"-DOGR_BUILD_OPTIONAL_DRIVERS:BOOL=OFF"', gdal_block)
        self.assertIn('"-DGDAL_USE_HDF5:BOOL=OFF"', gdal_block)
        self.assertIn('"-DGDAL_USE_NETCDF:BOOL=OFF"', gdal_block)
        self.assertIn('"-DGDAL_ENABLE_DRIVER_HDF5:BOOL=OFF"', gdal_block)
        self.assertIn('"-DGDAL_ENABLE_DRIVER_NETCDF:BOOL=OFF"', gdal_block)
        for driver in ("GTIFF", "HFA", "JPEG", "JP2OPENJPEG", "PDS", "PNG", "VRT"):
            self.assertIn(f'"-DGDAL_ENABLE_DRIVER_{driver}:BOOL=ON"', gdal_block)
        self.assertIn('"-DOGR_ENABLE_DRIVER_GPKG:BOOL=OFF"', gdal_block)
