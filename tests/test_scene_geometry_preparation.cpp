...
namespace
{
    ...
    TEST(SceneGeometryPreparationTest, ComputesReusableCloudSummary)
    {
        const SceneRenderCloud cloud =
            makePointCloud({QVector3D(-2.0f, -1.0f, 3.0f), QVector3D(2.0f, 1.0f, 7.0f), QVector3D(0.0f, 0.0f, 5.0f)});
    ...
    TEST(SceneGeometryPreparationTest, SpatialSummaryWeightsOnlyFinitePoints)
    {
        ...
        const SceneRenderCloud cloud = makePointCloud({QVector3D(-1.0f, 0.0f, 2.0f),
    ...
    TEST(SceneGeometryPreparationTest, LargeFiniteCoordinatesKeepFiniteCenter)
    {
        ...
        const SceneRenderCloud cloud = makePointCloud({QVector3D(large, large, large),
    ...
    {
        const SceneRenderCloud cloud =
            makePointCloud({QVector3D(0.0f, 0.0f, 1.0f), QVector3D(1.0f, 0.0f, 2.0f), QVector3D(0.0f, 1.0f, 3.0f)});
    ...
    TEST(SceneGeometryPreparationTest, BuildsBoundedSpatialChunksWithStableSourceMapping)
    {
        ...
        {
            points.emplace_back(
                float(index % 16) / 8.0f - 1.0f, float((index / 16) % 16) / 8.0f - 1.0f, float(index / 256) * 0.25f);
            counts.push_back(index);
        }
        const PointRenderPreparation prepared = preparePointRenderData(makePointCloud(points), counts);
        const QVector<PointRenderChunkPreparation> chunks = preparePointRenderChunks(
            prepared.vertexData, 9 * int(sizeof(float)), prepared.scalarData, prepared.spatialSummary, 64, 128);
    ...
    TEST(SceneGeometryPreparationTest, CullsOffscreenChunksAndHonorsPointBudget)
    {
        ...
        const QVector<PointRenderChunkPreparation> chunks{makeChunk(0.0f, 1'000), makeChunk(4.0f, 1'000)};
        QMatrix4x4 clipMatrix;
        clipMatrix.setToIdentity();

        const PointRenderPlan plan = planPointRenderChunks(chunks, clipMatrix, QSize(1000, 800), 2.0f, 250);
    ...
    TEST(SceneGeometryPreparationTest, DrawsEveryVisiblePointWhenCloudFitsBudget)
    {
        EXPECT_EQ(kDefaultMaximumVisiblePointCount, 5'000'000);
        ...
        const QVector<PointRenderChunkPreparation> chunks{
            makeChunk(-0.5f, 600), makeChunk(0.5f, 800), makeChunk(4.0f, 1'000)};
        QMatrix4x4 clipMatrix;
        clipMatrix.setToIdentity();

        const PointRenderPlan plan = planPointRenderChunks(chunks, clipMatrix, QSize(1000, 800), 3.0f, 3'000);

        ASSERT_EQ(plan.drawCounts.size(), 3);
        EXPECT_EQ(plan.drawCounts.at(0), 600);
        EXPECT_EQ(plan.drawCounts.at(1), 800);
        EXPECT_EQ(plan.drawCounts.at(2), 0);
        EXPECT_EQ(plan.visibleChunkCount, 2);
        EXPECT_EQ(plan.visiblePointCount, 1'400);
    }

    TEST(SceneGeometryPreparationTest, SelectsFromImmutablePreparedVertices)
    {
        const SceneRenderCloud cloud =
            makePointCloud({QVector3D(-0.8f, 0.0f, 0.0f), QVector3D(0.0f, 0.0f, 0.0f), QVector3D(0.8f, 0.0f, 0.0f)});
        ...
        const std::vector<PointVertexIndex> selected = selectPointVertexIndices(
            prepared.vertexData, 9 * int(sizeof(float)), QRect(40, 40, 20, 20), identity, QSize(100, 100), QPointF());
    ...
    {
        const SceneRenderCloud cloud =
            makePointCloud({QVector3D(-0.2f, 0.0f, -0.5f), QVector3D(0.8f, 0.0f, 0.0f), QVector3D(0.2f, 0.0f, 0.5f)});
        const PointRenderPreparation prepared = preparePointRenderData(cloud, {7, 13, 29});
        QMatrix4x4 identity;
        identity.setToIdentity();
        constexpr int stride_bytes = 9 * int(sizeof(float));

        const PointSelectionPreparation selection = preparePointSelection(prepared.vertexData,
                                                                          stride_bytes,
                                                                          prepared.scalarData,
                                                                          QRect(35, 40, 31, 20),
                                                                          identity,
        ...
        EXPECT_EQ(std::memcmp(selection.vertexData.constData(), prepared.vertexData.constData(), stride_bytes), 0);
        ...
        std::memcpy(compact_scalars, selection.scalarData.constData(), sizeof(compact_scalars));
    ...
    {
        const SceneRenderCloud cloud =
            makePointCloud({QVector3D(-0.2f, 0.0f, -0.5f), QVector3D(0.0f, 0.0f, 0.0f), QVector3D(0.2f, 0.0f, 0.5f)});
        const PointRenderPreparation prepared = preparePointRenderData(cloud, {7, 13, 29});
        QMatrix4x4 identity;
        identity.setToIdentity();
    ...
    {
        const SceneRenderCloud cloud = makePointCloud({QVector3D(10.0f, 0.0f, 0.0f),
                                                       QVector3D(20.0f, 0.0f, 0.0f),
                                                       QVector3D(30.0f, 0.0f, 0.0f),
                                                       QVector3D(40.0f, 0.0f, 0.0f)});
        const PointRenderPreparation prepared = preparePointRenderData(cloud, {2, 4, 6, 8});
        constexpr int stride_bytes = 9 * int(sizeof(float));

        const PointSelectionPreparation selection =
            prepareIndexedPointSelection(prepared.vertexData, stride_bytes, prepared.scalarData, {1U, 3U});

        ASSERT_TRUE(selection.isValid());
        EXPECT_EQ(selection.indices, (std::vector<PointVertexIndex>{1U, 3U}));
        const float* vertices = reinterpret_cast<const float*>(selection.vertexData.constData());
        EXPECT_FLOAT_EQ(vertices[0], 20.0f);
        EXPECT_FLOAT_EQ(vertices[9], 40.0f);
        const float* scalars = reinterpret_cast<const float*>(selection.scalarData.constData());
    ...
    {
        const PointRenderPreparation prepared =
            preparePointRenderData(makePointCloud({QVector3D(), QVector3D(1.0f, 0.0f, 0.0f)}));
        constexpr int stride_bytes = 9 * int(sizeof(float));
        EXPECT_FALSE(
            prepareIndexedPointSelection(prepared.vertexData, stride_bytes, prepared.scalarData, {1U, 0U}).isValid());
        EXPECT_FALSE(
            prepareIndexedPointSelection(prepared.vertexData, stride_bytes, prepared.scalarData, {2U}).isValid());
        std::atomic_bool cancelled{true};
        EXPECT_FALSE(
            prepareIndexedPointSelection(prepared.vertexData, stride_bytes, prepared.scalarData, {0U}, &cancelled)
                .isValid());
    }

    TEST(SceneGeometryPreparationTest, IgnoresPointsOutsideClipDepthOrWithNonFiniteClip)
    {
        const SceneRenderCloud cloud = makePointCloud({QVector3D(0.0f, 0.0f, 0.0f),
        ...
        std::memcpy(vertex_data.data() + 3 * 9 * int(sizeof(float)), &infinity, sizeof(float));
        QMatrix4x4 identity;
        identity.setToIdentity();

        const std::vector<PointVertexIndex> selected = selectPointVertexIndices(
            vertex_data, 9 * int(sizeof(float)), QRect(0, 0, 100, 100), identity, QSize(100, 100), QPointF());
    ...
    {
        const SceneRenderCloud cloud = makePointCloud({QVector3D(-0.5f, 0.0f, 0.0f),
        ...
        const std::vector<PointVertexIndex> selected = selectPointVertexIndices(
            prepared.vertexData, 9 * int(sizeof(float)), QRect(25, 25, 50, 50), identity, QSize(100, 100), QPointF());
    ...
    {
        const SceneRenderCloud cloud =
            makePointCloud({QVector3D(0.0f, 0.0f, 0.0f), QVector3D(0.5f, 0.5f, 0.0f), QVector3D(-0.5f, 0.0f, 0.0f)});
        const PointRenderPreparation prepared = preparePointRenderData(cloud);
        QMatrix4x4 identity;
        identity.setToIdentity();
        ...
        const std::vector<PointVertexIndex> selected = selectPointVertexIndices(
            prepared.vertexData, 9 * int(sizeof(float)), region, identity, QSize(100, 100), QPointF());
    ...
    {
        const SceneRenderCloud cloud =
            makePointCloud({QVector3D(0.0f, 0.0f, 0.0f), QVector3D(0.7f, 0.0f, 0.0f), QVector3D(0.0f, 0.7f, 0.0f)});
        const PointRenderPreparation prepared = preparePointRenderData(cloud);
        QMatrix4x4 identity;
        identity.setToIdentity();
        ScreenSelectionRegion region;
        region.shape = ScreenSelectionShape::Polygon;
        region.polygon = QPolygonF{QPointF(20.0, 80.0), QPointF(50.0, 20.0), QPointF(80.0, 80.0)};
    ...
    {
        const SceneRenderCloud cloud =
            makePointCloud({QVector3D(-0.5f, 0.0f, 0.0f), QVector3D(0.0f, 0.0f, 0.0f), QVector3D(0.5f, 0.0f, 0.0f)});
    ...
    {
        auto cloud = std::make_shared<SceneRenderCloud>(makePointCloud({QVector3D(0.0f, 0.0f, 0.0f),
                                                                        QVector3D(1.0f, 0.0f, 0.0f),
                                                                        QVector3D(2.0f, 0.0f, 0.0f),
                                                                        QVector3D(3.0f, 0.0f, 0.0f)}));
        const QVector<int> counts{2, 4, 6, 8};

        PointCloudEditResult filtered = filterPointCloudWithDelta(cloud, {1U, 3U}, counts);
        ...
        PointCloudEditResult restored =
            restorePointCloudFromDelta(filtered.cloud, std::move(filtered.undo), filtered.imageCounts);
    ...
    {
        auto source = std::make_shared<SceneRenderCloud>(
            makePointCloud({QVector3D(0.0f, 0.0f, 0.0f), QVector3D(1.0f, 0.0f, 0.0f)}));
        std::atomic_bool cancelled{true};

        PointCloudEditResult filtered = filterPointCloudWithDelta(source, {1U}, {}, &cancelled);
        EXPECT_FALSE(filtered.isValid());

        auto filtered_cloud = std::make_shared<SceneRenderCloud>(makePointCloud({QVector3D(0.0f, 0.0f, 0.0f)}));
        PointCloudEditDelta delta;
        delta.originalPointCount = 2;
        delta.removedIndices = {1U};
        delta.removedCloud = std::make_shared<SceneRenderCloud>(makePointCloud({QVector3D(1.0f, 0.0f, 0.0f)}));
        PointCloudEditResult restored = restorePointCloudFromDelta(filtered_cloud, std::move(delta), {}, &cancelled);
    ...
    TEST(SceneGeometryPreparationTest, PruneUndoPreservesPointAlignedAttributes)
    {
        ...
        PointCloudEditResult restored = restorePointCloudFromDelta(filtered.cloud, std::move(filtered.undo));
