...
- MVS 与 PlaPoint 的自动设备选择统一为 CUDA → OpenCL → CPU。MVS 在任务开始时为整批深度帧解析一次
  后端，并把实际后端纳入续跑配置哈希；MVS 逐卡获取租约，部分 CUDA 卡忙时继续使用其余 CUDA 卡，
  全部 CUDA 不可用后才预检 OpenCL context/kernel，再失败才在首帧前选择 CPU。显式后端不可用或
  运行失败时明确报错，不再在恢复阶段静默换成 CPU。Poisson 的 OpenCL PCG 尚未实现，Auto 会记录
  原因后使用 CPU 求解器。
