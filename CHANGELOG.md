...
- 空三新增参考 MetaShape 公开工作流概念、由 PlaScan 独立实现的自适应相机模型策略：不再硬编码完整
  Brown 或按场景类型二选一，而是从粗解的摄影几何、像面覆盖和结构消元信息中按自有可靠性评分逐项启用
  `f/aspect/cx/cy/k1/k2/k3/p1/p2`；诊断报告同时记录
  请求/调度/有效/写回状态、逐参数可靠度和选择依据；完整已知位姿保持输入标定，多轮 BA 不会因末轮
  no-op 丢失前轮已经生效的自标定结果；`IncrementalSfm::run()` 内的周期性、最终和重试全局 BA 按影像 ID
  共用首次有效标定参考，边界和弱先验不再跨调用复合漂移；单次迭代的多起点只在首轮执行，Legacy 严格遵守
  逐参数掩码和稳定焦距边界，不再接受多标定组/扩展模型的不等价回退。
- PlaPoint 新增 CPU-owned OpenCL 点云后端，覆盖体素降采样、统计/半径离群点过滤、KNN 法向估计和
  高度格网聚合；CLI 新增独立的 `--point-cloud-backend auto|cuda|opencl|cpu`，GUI/CLI 的稀疏点云
  预处理、稠密点云精炼和网格预处理共用同一设备策略。
...
- Ceres 大规模联合 BA 移除重复问题扫描、逐轨观测深拷贝和 `std::set`，按相机共享投影快照与
  ...
  从 2.484 s 降至 2.135 s，RMS 保持 `0.5743401084 -> 0.1968729905`；同一网络的自标定从完整
  9 参数模型自适应收缩为 `f+k1` 后，含策略评估的热运行中位数从 10.982 s 降至 7.133 s（-35.0%），
  最终 RMS 只增加 0.000616 px，且从预算内未收敛变为稳定收敛。
...
- MVS 深度算法修订到第 28 版；Auto 深度估计由整批单后端切换为 CUDA + OpenCL 帧级异构调度：
  ...
  身份，仍不确定的 NVIDIA OpenCL 接口会保守跳过，且保持每块物理 GPU 单内核执行通道。调度器
  为每个设备保留初始标定帧，根据帧耗时移动平均动态分配后续任务，并在队列尾部阻止慢设备领取会
  拖长整批墙钟的最后帧；首个失败帧只允许跨后端重试一次并退役故障设备。流式双 GPU 任务使用两个
  受任务数和真实驻留字节双重约束的产物保存 worker，等待入队的生产者结果也计入容量，减少单保存线程反压
  造成的 GPU 空档，并汇总记录保存墙钟、生产者等待和驻留峰值。尾部收益门只把最快设备实际可领取的帧
  计入清队预测，保证被某一后端排除的重试不会永久等待。默认 CPU 总预算改为逻辑线程数减 2，显式 CLI
  预算不再被旧的 7 线程上限截断，并按实际 GPU 主机帧槽连同余数完整分摊像素线程；OpenMP 后处理和
  独占融合也遵守同一总预算。计算/保存线程新增异常边界，保存预约在成功入队后才转交计数，最终过滤后
  批量保存与随后 BFS 融合均可及时响应取消，不再因异常或取消留下永久等待。异构批次保留 `auto` 配置 token、
  逐帧记录实际设备并在项目元数据中标记为 `hybrid`；Auto 可复用兼容的单后端或异构批次，显式
  CUDA/OpenCL/CPU 仍严格只使用并复用指定的单后端。
- MVS 深度算法修订到第 27 版：粗/中层金字塔不再先放大到原图再缩回工作尺寸，同时保持原有全分辨率
  ...
  边界处唯一性竞争深度与 CPU/CUDA 不一致造成的置信度误杀；多视一致性按
  源批次复用参考反投影，残余重估在投影源深度前执行空洞阈值预检。上述优化不启用插值，也不放宽
  置信度、一致性或融合验收门限；Hyb2 AMD `gfx1036` 最终源码重放由 233.16 秒降至 170.02 秒
  （缩短 27.1%），14 帧验收分类和 2/14 融合资格保持不变；第 26 版深度缓存会重新生成。
- 3D 模型与点云视图改为复用静态 GPU 顶点/索引缓冲，通过 shader uniform 完成阴影、纯色、纹理、
  高程和观测数模式切换；点云高亮、相机方向线/局部轴和大点精灵均使用 GPU 批处理。PLY/OBJ/XYZ
  加载、空间摘要、框选、编辑、撤销和 BinaryLE 快照写盘采用 single-flight 后台任务与增量 undo，
  避免百万点/面数据在 GUI 线程重复扫描、复制或展开。
- MVS/模型 CPU 热点继续收敛：相机投影复用单次世界到相机变换，零畸变反投影走解析快路径，
  航拍一致性检查跳过未消费的连续几何指标；跨视图行处理复用 OpenMP，TSDF 帧读取和模型着色的
  并行异常按确定顺序回传，不再让异常跨越 OpenMP 边界。
- PlaPoint 的自动设备选择保持 CUDA → OpenCL → CPU；Poisson 的 OpenCL PCG 尚未实现，Auto 会记录原因后使用
  CPU 求解器。MVS 旧的整批单后端解析已由上述 CUDA + OpenCL 异构调度取代；显式后端不可用或运行失败时仍
  明确报错，不在恢复阶段静默切换。
- 模型生成的深度帧/证据文件改为最多 8 路有界并行读取，避免按全部逻辑线程同时展开全分辨率矩阵；
  顶点颜色可见性图和逐顶点多视融合使用确定性 OpenMP 并行，并新增帧发现、加载、TSDF、颜色和模型
  总耗时统计。Agisoft 航拍 100 相机固定输入在不开启插值时由 8.12 秒降至 3.07 秒（2.647 倍），
  帧加载由 3.449 秒降至 0.845 秒（4.082 倍），串并行 PLY SHA-256 完全一致。
- OpenCL MVS 默认使用单 command queue 双主机准备槽，并把一致性检查后的残余深度重估也纳入该
  双缓冲流水线；同一物理 GPU 仍由跨进程租约和唯一执行槽串行提交 kernel。新增批次级墙钟、队列
  占用、帧间空档、队列内非 kernel 时间和端到端 kernel duty 统计。Hyb2 AMD `gfx1036` 重放中
  双槽把 89 次 OpenCL 调用的端到端 kernel duty 从单槽 95.0% 提高到 97.1%，帧间空档从 12.71 秒
  降到 6.88 秒；两者墙钟均约 273.26 秒且质量逐项一致，说明该核显已由 kernel 吞吐而非 CPU 准备限制。
  Windows 任务管理器的尖峰曲线因此不再被误判为同等比例的真实设备空闲。
- MVS 深度算法修订到第 25 版：CUDA 与 OpenCL 统一采用确定性逆深度粗搜、共享采样/传播预算、
  棋盘式平面传播候选、随机法向/深度扰动、5% 默认 hint 搜索半径和全来源数光度唯一性判定；新增
  CUDA/OpenCL 同一掩膜平面的跨设备数值回归。Hyb2 独占单 GPU 重放中，CUDA 为 14/14 帧可融合、
  18.70 秒，AMD `gfx1036` OpenCL 为 13/14、269.50 秒且 kernel duty 接近 100%；两份禁用插值模型
  均为单组件闭合流形、无边界边，表面 Chamfer-L1 为包围盒对角线的 0.111%。
- OpenCL MVS 深度算法修订到第 24 版：使用持久化深度/法向/得分状态、棋盘式平面传播、确定性随机
  微调和收敛后唯一性判定；初始化、传播与读回通过同一事件链提交，核显缓冲区复用可映射主机内存，
  每个物理 GPU 仍只允许一个任务进程和一个 kernel 提交通道。Hyb2 的 AMD `gfx1036` 重放在不放宽
  90% 融合门限、不开启额外插值的情况下达到 13/14 帧可融合，kernel duty 接近 100%。
...
- TSDF 新增默认关闭的“实测支持连通性”实验路径：只提升已由真实深度积分、具备双来源和低离散证据的
  ...
  14,000 门槛，因此生产默认保持关闭；新增拒绝原因与零交叉归因用于下一阶段定位缺拍视角和表面方位。
- MVS 深度算法修订到第 21 版：在全批次深度后处理完成后冻结只读快照，以至少两个相机方向扇区的
  多视深度建立残余空洞搜索层，并运行两组独立局部 PatchMatch；只有同时通过光度、逆深度离散、
  跨扇区几何确认和自由空间门的结果才以 `residual_patchmatch` 实测来源写回，且不会级联到同批其他帧。
  Dino 严格禁用插值验收新增 10,507 个实测像素，严格缺失从 134,251（7.511%）降到 123,744
  （6.923%），中位覆盖率/IoU 提高到 0.9645/0.9392、边缘 P90 降到 17.04 px；但 80k 面模型仍有
  16,096 条开放边，因此后续转向实测深度支持下的 TSDF 连通性恢复。
- MVS 深度算法修订到第 20 版，新增带四方向锚点、逆深度平面拟合、跨层/残差/包络安全门的实验性
  曲面先验，并保证单组来源退回最近锚点严格门。Dino 验收中该先验接受 1,854 个多源候选，开放边从
  15,814 略降到 15,797、边缘 P90 从 17.62 px 降到 17.42 px，但严格缺失率从 7.511% 回升到
  7.518%，IoU 也略降，因此生产默认关闭，后续改用一致性后残余缺口局部重估。
- MVS 深度算法修订到第 19 版：轨道物体缺口定向恢复最多使用 6 个已验证来源，交错拆成两个不重叠
  来源组独立运行局部 PatchMatch；只有逆深度一致的假设才使用放宽的曲面先验门。Dino 严格禁用插值
  验收中，定向恢复从 5,507 增到 6,720，最终实测像素增加 559，缺失率从 7.543% 降到 7.511%；
  16 视角中位 IoU 从 0.9358 提高到 0.9382，边缘 P90 从 20.16 px 降到 17.62 px，但开放边仍有
  15,814，尚需继续优化曲面先验。
- MVS 深度算法修订到第 18 版：融合前低置信深度不再统一硬删除，参考帧加至少两个来源在逆深度和
  连续几何证据上均一致时保留原始实测深度，并继续按原置信度低权参与融合。Dino 16 帧严格禁用
  插值验收中实测来源增加 2,571 像素，缺失率从 7.686% 降到 7.543%；模型仍有 12,648 条开放边，
  后续多源定向恢复方案记录于 `docs/plans/2026-08-05-dino-multisource-targeted-recovery.md`。
- MVS 深度算法修订到第 17 版，最终有效深度持久化原生 PatchMatch、定向 PatchMatch、跨视测量恢复
  和锚定插值四类来源；“禁用插值”的 TSDF 只排除锚定插值观测，模型属性显示来源汇总，并提供
  批次来源完整性验证脚本。Dino 16 帧验收中 1,700,136 个有效像素全部分类，允许插值相对禁用插值
  将 80k 面模型开放边从 16,000 降到 6,560（-59.0%）。
...
- `OverlapAnalysisDialog.cpp` 拆分 DEM 文件选择对话框参数中的超长语句，并新增行宽回归测试，继续收敛重叠分析对话框格式规范。
...
- CPack 现在跨平台内置并校验 U2Net 与 LightGlue 便携 ONNX，缺失或哈希不符时停止打包；安装版首次
  匹配生成的 TensorRT engine 改写入可写用户模型目录，不再尝试修改 `Program Files` 或 `/opt` 下的
  只读模型资源。
- 修复“视图模式 → 模型 → 阴影”切换后丢失网格顶点颜色的问题；阴影模式现在保留源顶点颜色并叠加
  光照。非 8-bit GeoTIFF 显示缓存改为同源串行转换、同目录暂存和原子提交，损坏或过期缓存会重建，
  并发完整图/缩略图加载不会再读取半成品。
