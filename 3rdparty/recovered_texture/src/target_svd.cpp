...
double target_stable_hypot(double first, double second) {
    ...
    if (absolute_second == 0.0) return 0.0;
    const double ratio = absolute_first / absolute_second;
    return absolute_second * std::sqrt(1.0 + ratio * ratio);
}

double target_grouped_dot(const double* first, std::ptrdiff_t first_stride,
                          const double* second, std::ptrdiff_t second_stride,
                          std::size_t count) {
    ...
    for (; index + 4 <= count; index += 4) {
        ...
        group += first[(index + 1) * first_stride] *
                 second[(index + 1) * second_stride];
        group += first[(index + 2) * first_stride] *
                 second[(index + 2) * second_stride];
        group += first[(index + 3) * first_stride] *
                 second[(index + 3) * second_stride];
        sum += group;
    }
...
NaturalUvSvd3x3 decompose_natural_uv_covariance_exact(
    ...
    constexpr bool singular_values_only = false;
    std::vector<double> row_major(matrix.begin(), matrix.end());
    ...
    if (!singular_values_only) right.assign(cols * cols, 0.0);
    ...
    auto U = [&](std::size_t row, std::size_t column) -> double& {
        return u[row * rows + column];
    };
    auto V = [&](std::size_t row, std::size_t column) -> double& {
        return right[row * cols + column];
    };
    ...
    for (std::size_t column = 0; column < cols; ++column) {
        ...
        if (column < rows && next < cols) {
            ...
            if (scale != 0.0) {
                ...
                for (std::size_t row = next; row < rows; ++row) {
                    sum = target_grouped_dot(
                        &U(row, next), 1, &U(column, next), 1, cols - next);
                    const double multiplier = sum * inverse_factor;
                    for (std::size_t other = next; other < cols; ++other)
        ...
        norm_bound = std::max(
            norm_bound, std::abs(singular[column]) + std::abs(superdiagonal[column]));
    }

    const double qr_tolerance =
        norm_bound * std::numeric_limits<double>::epsilon();
    ...
    if (!singular_values_only) {
        ...
        for (std::size_t reverse = cols; reverse-- > 0;) {
            ...
            if (inverse_factor != 0.0) {
                for (std::size_t other = following; other < cols; ++other) {
                    const double sum = target_grouped_dot(
                        &V(following, column), static_cast<std::ptrdiff_t>(cols),
                        &V(following, other), static_cast<std::ptrdiff_t>(cols),
                        cols - following);
                    const double multiplier = sum * inverse_factor;
        ...
        for (std::size_t reverse = std::min(rows, cols); reverse-- > 0;) {
            ...
            if (g != 0.0) {
                ...
                for (std::size_t other = following; other < rows; ++other) {
                    const double sum = target_grouped_dot(
                        &U(following, column), static_cast<std::ptrdiff_t>(rows),
                        &U(following, other), static_cast<std::ptrdiff_t>(rows),
                        rows - following);
                    const double multiplier = sum * inverse_factor;
                    for (std::size_t row = column; row < rows; ++row)
                        U(row, other) += multiplier * U(row, column);
                ...
                const double column_scale =
                    inverse_factor * U(column, column);
                for (std::size_t row = column; row < rows; ++row)
    ...
    if (std::getenv("METASHAPE_TEXTURE_TRACE_TARGET_SVD_QR")) {
        std::cerr << std::setprecision(17) << "target_svd_qr_entry rows=" << rows
                  << " cols=" << cols << " singular=[";
    ...
    for (std::size_t reverse = cols; reverse-- > 0;) {
        ...
        for (std::size_t iteration = 0; iteration < 30; ++iteration) {
            ...
            for (std::size_t scan = value_index + 1; scan-- > 0;) {
                ...
                if (split == 0) break;
                previous = split - 1;
                if (std::abs(singular[previous]) <= qr_tolerance) break;
            }
            if (cancel) {
                double cosine = 0.0;
                double sine = 1.0;
                for (std::size_t row = split; row <= value_index; ++row) {
                    ...
                    if (std::abs(first) + qr_tolerance == qr_tolerance) break;
                    ...
                    if (!singular_values_only) {
                        for (std::size_t matrix_row = 0; matrix_row < rows;
                             ++matrix_row) {
                            const double left = U(matrix_row, previous);
                            const double right_value = U(matrix_row, row);
                            U(matrix_row, previous) =
                                left * cosine + right_value * sine;
                            U(matrix_row, row) =
                                right_value * cosine - left * sine;
                        }
                    }
                }
            }
            ...
            double shift =
                (((first_super + final_value) / final_super) *
                     ((first_super - final_value) / middle_value) +
                 middle_value / final_super - final_super / middle_value) *
                0.5;
            first_super = target_stable_hypot(shift, 1.0);
            if (shift < 0.0) first_super = -first_super;
            shift = first_value - final_value * (final_value / first_value) +
                    (middle_value / (shift + first_super) - final_super) *
                        (final_super / first_value);
            double cosine = 1.0;
            double sine = 1.0;
            for (std::size_t column = split; column <= penultimate; ++column) {
                ...
                if (!singular_values_only) {
                    for (std::size_t row = 0; row < cols; ++row) {
                        ...
                        V(row, column) =
                            first_value * cosine + final_value * sine;
                        V(row, following) =
                            final_value * cosine - first_value * sine;
                    }
                }
                ...
                if (!singular_values_only) {
                    for (std::size_t row = 0; row < rows; ++row) {
                        ...
                        U(row, column) =
                            middle_value * cosine + final_value * sine;
                        U(row, following) =
                            final_value * cosine - middle_value * sine;
                    }
                }
            }
    ...
    for (std::size_t destination = 0; destination < cols; ++destination) {
        ...
        for (std::size_t candidate = destination + 1; candidate < cols; ++candidate)
            if (singular[candidate] > singular[selected]) selected = candidate;
    ...
    NaturalUvSvd3x3 result;
    std::copy_n(u.begin(), 9, result.left_columns.begin());
    std::copy_n(right.begin(), 9, result.right_columns.begin());
    std::copy_n(singular.begin(), 3, result.singular_values.begin());
