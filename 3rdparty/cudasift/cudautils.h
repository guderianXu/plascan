...
inline void __safeThreadSync(const char *file, const int line)
{
  ...
  if (cudaSuccess != err) {
    throw std::runtime_error(std::string("CudaSift thread synchronize error in file <") + file + ">, line " +
                             std::to_string(line) + ": " + cudaGetErrorString(err));
  }
}
...
inline void __checkMsg(const char *errorMessage, const char *file, const int line)
{
  ...
  if (cudaSuccess != err) {
    throw std::runtime_error(std::string("CudaSift CUDA error: ") + errorMessage + " in file <" + file +
                             ">, line " + std::to_string(line) + ": " + cudaGetErrorString(err));
  }
}
