function(vcpkg_find_cuda)
    cmake_parse_arguments(
        ...
        vfc
        ""
        "OUT_CUDA_TOOLKIT_ROOT;OUT_CUDA_VERSION;OUT_CUDA_COMPILER"
        ""
    )
    ...
    if(DEFINED ENV{CUDACXX} AND NOT "$ENV{CUDACXX}" STREQUAL "")
        file(TO_CMAKE_PATH "$ENV{CUDACXX}" explicit_nvcc)
        if(NOT EXISTS "${explicit_nvcc}")
            message(FATAL_ERROR
                "CUDACXX points to a missing CUDA compiler: ${explicit_nvcc}")
        endif()
        set(NVCC "${explicit_nvcc}")
        message(STATUS "Using CUDA compiler from CUDACXX: ${NVCC}")
    endif()
    ...
    if(DEFINED vfc_OUT_CUDA_COMPILER)
        set(${vfc_OUT_CUDA_COMPILER} "${NVCC}" PARENT_SCOPE)
    endif()
endfunction()
