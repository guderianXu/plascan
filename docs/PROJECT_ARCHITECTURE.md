...
占据先验、深度/轮廓/自由空间约束以及几何验证边界恢复仍保留，用于在实测深度样本之间构造连续曲面，
而不是要求每个输出三角形都落在原生深度像素上。禁用插值也继续在高分辨率融合场中定位零交叉；72 级
占据单元边界仅能通过显式兼容/诊断开关直接输出，避免低分辨率量化造成顶点减少和轮廓锯齿。禁用插值
不绕过最终深度完整性质量门；报告会记录视觉外壳是否被抑制及占据先验是否生效。2026-08-08 的 Hyb2
