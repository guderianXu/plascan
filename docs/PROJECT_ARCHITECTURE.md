...
仍允许产生小幅数值差异，并由跨后端掩膜平面回归测试约束。自动后端的超高质量环拍若同时检测到 CUDA 与 OpenCL，
整批帧只交给 CUDA 设备；显式 OpenCL 或无 CUDA 环境仍使用 OpenCL。这样避免动态队列把任意困难
视角分给数值质量不同的核显，同时继续保证同一物理 GPU 只有一个 kernel 执行槽和跨进程租约。
