...
当模型对话框把插值设为“已禁用”时，模型工作流进入仅观测表面模式：包括低细节提前返回配置在内，
统一关闭可见性占据补全、视觉外壳有符号距离补全、环拍缺口恢复、跨视锚定恢复、边界补洞和独立的
环拍视觉外壳替换。这里仍允许 MC33 在已经融合的 TSDF 单元内定位零交叉；这属于等值面提取必需的
线性定位，不会创建新的深度、占据体或补洞表面。禁用插值路径不再要求正负角点都具有直接支持，避免
窄带边缘被误删成裂洞。质量报告会记录外壳补全是否因禁用插值而被抑制，避免界面显示“已禁用”但
实际仍在未观测区域生成表面。
...
`effective_mc33_require_supported_sign_change`。当用户禁用插值时，所有几何完成器关闭，同时关闭这个
单元级后过滤；否则它会在 hyb2 一类窄带数据上拒绝过半数已经提取的连通表面。
