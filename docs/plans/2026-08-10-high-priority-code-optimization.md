...
- Windows CPU CI 使用专用 `x64-windows-ci-release` host/target triplet，只构建 Release 依赖；同时关闭顶层
  默认 feature，跳过该门禁不需要的 Ceres SuiteSparse/LAPACK 支持，避开 GitHub Windows runner 的 VS 18 flang
  无法加载 `iso_fortran_env.mod`、进而编译 `lapack-reference` 失败的问题。生产、CUDA 与发布构建仍通过
  默认 `ceres-suitesparse` feature 保留 SuiteSparse/LAPACK。vcpkg manifest dry-run、workflow YAML
  与仓库规范测试通过。
