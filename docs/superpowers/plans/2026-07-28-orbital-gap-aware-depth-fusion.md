...
### 2026-07-29 GUI 任意视角空洞复验

- 旧验收只使用相机投影视图，不能覆盖模型背面和任意观察方向。现增加固定 Front/Side/Top
  正交视图，并以同一当前二进制、同一深度产物、仅切换一个设置的方式做 A/B。
- Hyb2 的深度不确定度 P90 为 `9.757` 体素，高于基础带 `7.5 × 1.2 = 9.0`，因此自动把
  截断带和表面支持带调整为 `11.403`。与显式关闭对照相比，边界边
  `11618 → 6165`，P10/中位/最低召回
  `0.5563/0.7898/0.4628 → 0.5975/0.8122/0.5164`；固定正交视图确认正面大洞缩小，
  侧面和顶部没有形成膨胀外壳。
- 无启用门槛的初版会错误调整 Dino。最终策略要求 P90 至少超过基础带 20%：
  Dino P90 `8.110`、Temple P90 `3.260` 均不启用。两者默认开启与显式关闭的同二进制
  输出在面数、边界边及逐视角召回统计上逐项一致，确认不会改动 Dino 尖刺/耳部或 Temple 柱间开口。
- 跨视修补像素锚定支持只使 Hyb2 边界边减少 6 条，局部单元片和单视几何恢复还会增加边界，
  因此该路径不再作为环拍默认值。
- 本轮改善了 GUI 实际模型的任意视角完整性，但 Hyb2 最低召回仍只有 `0.5164`，且仍有残余开放边；
  不能描述为达到 Metashape 或严格质量门。后续应继续改善原始深度跨视对齐，而不是无条件补洞。
- GUI 默认 OBJ 复验同时保留 PLY：两者均为 `127690` 顶点、`249651` 面，原始 OBJ 顶点与 PLY
  最大坐标差小于 `5e-9`，三角形索引逐项一致；因此剩余空洞不来自 OBJ 导出或加载时丢面。
- `test_mesh_reconstructor` 新增及相关定向测试 5/5 通过；全量 155/159，通过数增加但仍保留
  4 个既有 `ConsistentIsoSurfaceExtractorTest` 失败，没有删除或屏蔽。

### 2026-07-29 环拍缺口姿态容差

- Hyb2 的相机中心和旋转误差已经超过 11.403 体素截断带能够稳定吸收的范围。把所有环拍数据
  无条件放宽到 16 体素会损伤近邻薄表面，因此最终实现只在环向覆盖分析报告
  `significantGap=true` 时启用附加容差：不确定度系数至少为 `1.5`，最大截断带为 16 体素。
- 使用 GUI 默认设置（未在 JSON 中显式覆盖截断带）重新生成
  `mesh_gui_gap_adaptive_default_r73`，报告确认
  `configured_orbital_gap_adaptive_truncation=true`、
  `effective_orbital_gap_adaptive_truncation=true`，实际截断带为 16 体素。
- 与此前 11.403 体素 GUI 基线相比，边界边 `6165 → 4472`（减少 27.5%），聚合/中位/P10/最低
  投影召回从 `0.7729/0.8122/0.5975/0.5164` 提升到
  `0.7802/0.8201/0.6110/0.5327`，面数 `249651 → 247210`。固定 Front/Side/Top 视图显示正面
  主空洞继续缩小，侧面和顶部未出现大面积虚假外壳。
- 新增策略及相关 TSDF、帧策略、工作流设置测试共 72/72 通过；当前
  `test_mesh_reconstructor` 全量 160/160 通过，前一阶段记录的四个等值面合成用例失败在当前
  工作树中已不再复现。GUI 主程序必须重新编译并重启后才会使用新默认值；旧进程生成的模型不能
  作为此修复的验收结果。
- 该修改仍不等于深度质量已经达到 Metashape：帧 9/10 的召回只有 `0.5327/0.5891`，残余小孔
  需要从跨视深度对齐和局部位姿细化继续处理，不能再通过无条件扩大截断带掩盖。
