...
DISK/ALIKED 导出已移至 scripts/models/export_disk_aliked.py
(含 NMS 修复 + DeformableConv2d monkey-patch)。

用法：
    conda activate plascan
    python scripts/models/export_models.py --loftr --roma
    python scripts/models/export_models.py --all  # 仅 LoFTR + RoMa
...
MODELS_DIR = Path(__file__).resolve().parents[2] / "resources" / "models"
